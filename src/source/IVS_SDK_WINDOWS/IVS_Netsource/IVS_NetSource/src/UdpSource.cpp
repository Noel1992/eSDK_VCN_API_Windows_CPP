...
CUdpSource::CUdpSource(void)
    : m_bMulticast(false)
{
	eSDK_MEMSET(&m_SenderVideoRTCPSockAddr, 0, sizeof(sockaddr_in));
	eSDK_MEMSET(&m_SenderAudioRTCPSockAddr, 0, sizeof(sockaddr_in));
}
...
void CUdpSource::ProcessRecevData()
{
    ...
    for (int i = 0; i < NET_SOURCE_UDP_SOCKET_NUM; i++)
    {
        ...
        int len = recvfrom(socket, m_pRecvBuf, RTP_MEDIA_BUF_LEN - 1, 0, (SOCKADDR *)&addr, &nLen);
        if (0 >= len || RTP_MEDIA_BUF_LEN - 1 < len)
        {
		...
		else
        {
			...
			if (VIDEO_RTP_SOCKET == i)
			{
					...
					|| (addr.sin_port != htons((unsigned short)uiRemoteVideoRtpPort)))
				{
					...
					eSDK_STRNCPY(szRemoteIp, sizeof(szRemoteIp), inet_ntoa(addr.sin_addr), IVS_IP_LEN - 1);
