...
CRtspPacket::CRtspPacket()
{
    ...
    eSDK_MEMSET(&m_stMediaInfo, 0, sizeof(m_stMediaInfo));
}
...
string CRtspPacket::GetCSeq()
{
    ...
    char szCSeq[16] = {0};
    eSDK_SPRINTF(szCSeq, sizeof(szCSeq), "%lu", ulCSeq);
...
)const
{
    ...
    if (bRelativeTime)
    {
        eSDK_SPRINTF(szStartTime, sizeof(szStartTime), "%u-", ulStartTime);
    ...
    else
    {
        ...
        struct tm _tm;
		eSDK_MEMSET(&_tm, 0, sizeof(tm));
		localtime_s(&_tm, &tStartTime);
        
        eSDK_SPRINTF(szStartTime, sizeof(szStartTime), 
...
)
{
    ...
    eSDK_SPRINTF(szLocalPort, sizeof(szLocalPort), "%u", stNatReqPara.usLocalPort);
...
)
{
    ...
    //速度转换为字符串
    char szSpeed[16] = {0};
    eSDK_SPRINTF(szSpeed, sizeof(szSpeed), "%f", stVcrReqPara.fSpeed);
