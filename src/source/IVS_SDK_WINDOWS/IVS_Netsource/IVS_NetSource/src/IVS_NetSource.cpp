...
//全局变量，标识Netsource初始化次数
int g_NetsourceInitCount = 0;

IVS_NETSOURCE_API IVS_INT32 __stdcall IVS_NETSOURCE_Init(const char* pLogPath)
{
	if (g_NetsourceInitCount > 0)
	{
		g_NetsourceInitCount++;
		return IVS_SUCCEED;
	}
    // 指定目录（多进程场景）
                                                                   ...
                                                                   IVS_VOID* pUser)
{
	...
	{
		IVS_LOG(IVS_LOG_INFO, "IVS_NETSOURCE_SetDataCallBack", "NET_DATA_CALLBACK is NULL", ulChannel, cbDataCallBack, pUser);
	}
