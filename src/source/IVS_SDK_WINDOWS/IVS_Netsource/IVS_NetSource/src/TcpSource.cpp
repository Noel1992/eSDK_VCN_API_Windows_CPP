...
long CTcpSource::RecvRTSP()
{
	...
	eSDK_MEMSET(pRtspBuf, 0, RTP_MEDIA_BUF_LEN - sizeof(RTP::INTERLEAVED_HEAD));
...
void CTcpSource::PacketRtspInfo(const string strLocalIp, unsigned long ulLocalPort, const string strSourceIp, unsigned long ulSourcePort)
{
	...
	eSDK_STRNCPY(stMediaInfo.szLocalIp, sizeof(stMediaInfo.szLocalIp), strLocalIp.c_str(), RTSP::MAX_LEN_IP);
    eSDK_STRNCPY(stMediaInfo.szSessionId, sizeof(stMediaInfo.szSessionId), m_pChannelInfo->GetSessionId().c_str(), RTSP::MAX_LEN_SESSION_ID);
	eSDK_STRNCPY(stMediaInfo.szSessionUrl, sizeof(stMediaInfo.szSessionUrl), strSessionUrl.c_str(), RTSP::MAX_LEN_SESSION_URL);
