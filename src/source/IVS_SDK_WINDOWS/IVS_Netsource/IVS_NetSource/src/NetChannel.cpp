...
const ULONG G_MULTI_CAST_IP_BEGIN = ntohl(inet_addr("224.0.0.0"));  // 广播开始地址 
const ULONG G_MULTI_CAST_IP_END = ntohl(inet_addr("239.255.255.255"));  // 广播结束地址 
                               ...
                               NET_EXCEPTION_CALLBACK cbExceptionCallBack)
{
    ...
    if (PROTOCOL_RTP_OVER_UDP == m_channelInfo.GetProtocolType())
    {
        ...
        // 组播;
		ULONG ulIP = ntohl(inet_addr(strRemoteSendStreamIP.c_str()));
		if((ulIP >= G_MULTI_CAST_IP_BEGIN) && (ulIP <= G_MULTI_CAST_IP_END))
		{
