...
IVS_INT32 CStreamMgr::ReInitBusiness
	(   IN CBusiness& objBusiness)
{
	const unsigned long ulBusinessId = objBusiness.GetBusinessId();

	IVS_DBG_LOG("[ BusinessID = %lu ] ReInit Business In Stream Manager Begin.", ulBusinessId);
	...
	if (objBusiness.IsRemoteBusiness()) //lint !e1055
	{
		...
		{
			IVS_RUN_LOG_ERR(
				"[ BusinessID = %lu ] ReInit Remote Business In Stream Manager Failed. Remote Decoder Manager Has Stopped.",
				ulBusinessId);
			return IVS_TVWALL_TV_WALL_SRC_STOPED;
		}

		lResult = m_pRemoteDecoderMgr->ReInitBusiness(objBusiness);
		if (IVS_SUCCEED != lResult)
		{
			IVS_RUN_LOG_ERR( "[ BusinessID = %lu ] ReInit Remote Business In Stream Manager Failed.", ulBusinessId);
			return lResult;
		}
	}

	IVS_DBG_LOG("[ BusinessID = %lu ] ReInit Business In Stream Manager Success.", ulBusinessId);
	return IVS_SUCCEED;
}
...
IVS_INT32 CStreamMgr::ReStartBusiness
(   IN CBusiness& objBusiness)
{
    const unsigned long ulBusinessId = objBusiness.GetBusinessId();

    IVS_DBG_LOG("[ BusinessID = %lu ] ReStart Business In Stream Manager Begin.", ulBusinessId);
    ...
    if (objBusiness.IsRemoteBusiness())
    {
        ...
        {
            IVS_RUN_LOG_ERR(
                    "[ BusinessID = %lu ] ReStart Remote Business In Stream Manager Failed. Remote Decoder Manager Has Stopped.",
                    ulBusinessId);
            return IVS_TVWALL_TV_WALL_SRC_STOPED;
        }
        ...
        if(!CToolsHelp::Strncpy(objBusiness.m_szMediaDistributeIp, sizeof(objBusiness.m_szMediaDistributeIp), objBusiness.m_szServerIp,
                sizeof(objBusiness.m_szMediaDistributeIp) - 1))
		{
			IVS_RUN_LOG_ERR("ReStart Business Copy MediaDistributeIp error.");
			return IVS_ALLOC_MEMORY_ERROR;
		}

        lResult = m_pRemoteDecoderMgr->ReStartBusiness(objBusiness);
        if (IVS_SUCCEED != lResult)
        {
            IVS_RUN_LOG_ERR( "[ BusinessID = %lu ] ReStart Remote Business In Stream Manager Failed.", ulBusinessId);
            return lResult;
        }
        ...
        {
            IVS_RUN_LOG_INF(
                    "[ BusinessID = %lu ] ReStart Remote Business In Stream Manager Success. Direct Connect To Video Source.",
                    ulBusinessId);
            return IVS_SUCCEED;
        }
    }

    IVS_DBG_LOG("[ BusinessID = %lu ] ReStart Business In Stream Manager Success.", ulBusinessId);
    return IVS_SUCCEED;
}
