...

{
    ...
    //媒体服务器IP
    eSDK_MEMSET(m_szServerIp, 0, sizeof(m_szServerIp));

    //媒体本地IP
    eSDK_MEMSET(m_szLocalIp, 0, sizeof(m_szLocalIp));

    //解码器IP
    eSDK_MEMSET(m_szDecoderIp, 0, sizeof(m_szDecoderIp));

    //媒体分发IP
    eSDK_MEMSET(m_szMediaDistributeIp, 0, sizeof(m_szMediaDistributeIp));

    //解码器端口信息
    eSDK_MEMSET(&m_stDecoderPortInfo, 0, sizeof(m_stDecoderPortInfo));

    //媒体分发端口信息
    eSDK_MEMSET(&m_stMediaDistributePortInfo, 0, sizeof(m_stMediaDistributePortInfo));

    //媒体服务器端口信息
    eSDK_MEMSET(&m_stServerPortInfo, 0, sizeof(m_stServerPortInfo));

    //本地端口信息
    eSDK_MEMSET(&m_stLocalPortInfo, 0, sizeof(m_stLocalPortInfo));
