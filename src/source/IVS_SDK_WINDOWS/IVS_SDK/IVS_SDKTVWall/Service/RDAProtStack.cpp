...
{
    eSDK_MEMSET(m_szTagName, 0x00, LEN_RDA_ATTRTAG_NAME);
};
...
{
    eSDK_MEMSET(m_pszMD5Key, 0, MAX_MD5KEY_LENGTH);
    ...
    CRDAMsg&            objRDAMsg) const
{
	...
	eSDK_MEMSET(pBufferPos, 0, MD5_DIGEST_LENGTH);
								...
								CRDAMsg *pRDAMsg, IVS_ULONG &ulValidLength) const
{
    ...
    *(RDA_UINT4*)pBufferPos = VOS_htonl(stMsgHead.ulPackLength);
    ...
    eSDK_MEMSET(pMd5Digest, 0, sizeof(stMsgHead.arrayMD5));
                                       ...
                                       const ULONG ulLength, RDAMsgHead &stRDAMsgHead) const
{
    ...
    //解析消息头
    eSDK_MEMSET(&stRDAMsgHead, 0x00, sizeof(stRDAMsgHead));
