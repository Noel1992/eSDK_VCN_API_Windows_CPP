...
IVS_INT32 CControlBus::InitControlBusResource()
{
    ...
    //初始化StreamMgr
    (void)m_pStreamMgr->InitStreamMgr(HandleNotifyCallBack, this);
...
IVS_INT32 __stdcall CControlBus::HandleNotifyCallBack(CUMW_NOTIFY_INFO* pstNotifyInfo, void* pUserData)
{
    ...
    case CUMW_NOTIFY_TYPE_REMOTE_DECODER_REGISTER:
    {
        ...
        eSDK_MEMSET(pThis->m_objProcEventThread[lIndex].m_szDecoderID, 0, CUMW_DECODER_ID_LEN + 1);
	...
	case CUMW_NOTIFY_TYPE_REMOTE_DECODER_CAPABILITY:		//解码器能力
		{
			break;
		}
    default:
...
void CControlBus::ProcEventThread(int i)
{
    ...
    //清空事件同步队列
    while (NULL != pstEventInfo && VOS_OK == m_objProcEventThread[i].m_objEventSyncQueue.popFrontEv(pstEventInfo, 0, QUEUE_MODE_NOWAIT))
    {    
    ...
    IN const CUMW::BUSINESS_CTRL_CODE enCtrlCode)
{
    ...
    eSDK_MEMSET((char*) pCtrlPara, 0, ulCtrlParaLen);
...
* remark     : NA
**************************************************************************/
IVS_INT32 CControlBus::ReStartBusiness
	(   IN CBusiness& objBusiness)
{
	if (NULL == m_pStreamMgr)
	{
		IVS_RUN_LOG_ERR( "ReStart Business In Control Bus Failed. Stream Manager is Empty.");
		return IVS_TVWALL_MEMORY_NULL;
	}
	...
	IVS_DBG_LOG("[ AsyncSeq = %lu ][ BusinessID = %lu ] ReStart Business In Control Bus Begin.",
		objBusiness.m_ulAsyncSeq, ulBusinessId);
	...
	{
		IVS_RUN_LOG_ERR(
			"[ AsyncSeq = %lu ][ BusinessID = %lu ] ReStart Business Failed On Start Business In Control Bus.",
			objBusiness.m_ulAsyncSeq, ulBusinessId);
		return lResult;
	}
	...
	{
		IVS_RUN_LOG_ERR(
			"[ AsyncSeq = %lu ][ BusinessID = %lu ] Get Shared Business Object Failed On ReStart Business In Control Bus.",
			objBusiness.m_ulAsyncSeq, ulBusinessId);
		return lResult;
	}
	...
	IVS_DBG_LOG("[ AsyncSeq = %lu ][ BusinessID = %lu ] ReStart Business In Control Bus Success.",
		objBusiness.m_ulAsyncSeq, ulBusinessId);
	return IVS_SUCCEED;
}
    ...
    IN unsigned long   ulEventInfoLen)
{
	...
	case CUMW::ASYNC_TYPE_DECORDER_REINIT:
		{
			lResult = HandleEventReIniBusiness(stAsyncInfo);
			break;
		}
	...
	case CUMW::ASYNC_TYPE_DECORDER_RESTART:
	{
		lResult = HandleEventReStartBusiness(stAsyncInfo);
		break;
	}
...
* remark     : NA
**************************************************************************/
IVS_INT32 CControlBus::ReIniBusiness(IPI_INIT_BUSINESS_REQ* pParaInfo )
{
	IVS_DBG_LOG("ReIni Business Begin.");
	...
	IVS_DBG_LOG("ReIni Business, BusinessID = %lu.", ulBusinessId);

	//添加开始业务事件
	long lResult = AddBuesinessEvent(pParaInfo->ulAsyncSeq, CUMW::ASYNC_TYPE_DECORDER_REINIT, ulBusinessId, pBusiness);
	if (IVS_SUCCEED != lResult)
	{
		IVS_RUN_LOG_ERR( "[ AysncSeq = %lu ][ BusinessID = %lu ] Add Business Event Failed.",
			pParaInfo->ulAsyncSeq, ulBusinessId);
	...
	IVS_DBG_LOG("[ AysncSeq = %lu ][ BusinessID = %lu ] ReIni Decoder Success.", pParaInfo->ulAsyncSeq, ulBusinessId);
	return IVS_SUCCEED;
}//lint !e818
...
* remark     : NA
**************************************************************************/
IVS_INT32 CControlBus::HandleEventReIniBusiness(BUSINESS_ASYNC_INFO& stAsyncInfo)
{
	long lResult = IVS_SUCCEED;

	if (NULL == m_pStreamMgr)
	{
		IVS_RUN_LOG_ERR( "ReInit Business In Control Bus Failed. Stream Manager is Empty.");
	...
	IVS_DBG_LOG("[ BusinessID = %lu ] ReInit Business In Control Bus Begin.", ulBusinessId);

	//初始化业务
	lResult = m_pStreamMgr->ReInitBusiness(*pBusiness);
	if (IVS_SUCCEED != lResult)
	{
		IVS_RUN_LOG_ERR(
			"[ AsyncSeq = %lu ][ BusinessID = %lu ] ReInit Business Failed In Control Bus.",
			pBusiness->m_ulAsyncSeq, ulBusinessId);
...
/**************************************************************************
* name       : ReStartBusiness
* description: 远程解码器重新开始解码
* input      : pParaInfo   参数
* output     : NA
* return     : long 错误码
* remark     : NA
**************************************************************************/
IVS_INT32 CControlBus::ReStartBusiness(IPI_START_BUSINESS_REQ* pParaInfo)
{
	//输入参数检查
	//1、pParaInfo指针非空
	if (NULL == pParaInfo)
	{
		IVS_RUN_LOG_ERR( "ReStart Decoder Failed. Parameter Invalidate. Parameter is Empty.");
		return IVS_PARA_INVALID;
	}
	...
	if (0 == pParaInfo->szDecoderID[0])
	{
		IVS_RUN_LOG_ERR( "[ AysncSeq = %lu ] ReStart Business Failed. Decoder ID Invalidate.", pParaInfo->ulAsyncSeq);
		return IVS_PARA_INVALID;
	}
	...
	if ((RDA_PROTOCOL_NSS_OVER_UDP > pParaInfo->stMediaNetInfo.ucTransportProtocol)
		|| (RDA_PROTOCOL_RTP_OVER_TCP < pParaInfo->stMediaNetInfo.ucTransportProtocol))
	{
		IVS_RUN_LOG_ERR( "[ AysncSeq = %lu ] ReStart Business Failed. Parameter Invalidate. Protocol is error.", pParaInfo->ulAsyncSeq);
		return IVS_PARA_INVALID;
	}
	...
	if (NULL == pBusiness)
	{
		IVS_RUN_LOG_ERR( "[ AysncSeq = %lu ] Create Business Failed On ReStart Business.", pParaInfo->ulAsyncSeq);
		return IVS_ALLOC_MEMORY_ERROR;
	}
	...
	//添加开始业务事件
	long lResult = AddBuesinessEvent(pParaInfo->ulAsyncSeq, CUMW::ASYNC_TYPE_DECORDER_RESTART, 0, pBusiness);
	if (IVS_SUCCEED != lResult)
	{
		IVS_RUN_LOG_ERR( "[ AysncSeq = %lu ] Add Business Event Failed On ReStart Business.",
			pParaInfo->ulAsyncSeq);
	...
	IVS_DBG_LOG("[ AysncSeq = %lu ] ReStart Business Success.", pParaInfo->ulAsyncSeq);
...
/**************************************************************************
* name       : HandleEventReStartBusiness
* description: 处理启动解码器消息
* input      : objBusiness     业务对象引用
* output     : NA
* return     : long 错误码
* remark     : NA
**************************************************************************/
IVS_INT32 CControlBus::HandleEventReStartBusiness(BUSINESS_ASYNC_INFO& stAsyncInfo)
{
	CBusiness* pCurBusiness = (CBusiness*)stAsyncInfo.pParameter;
	...
	if (NULL == m_pStreamMgr)
	{
		...
		//释放内存
		VOS_DELETE(pCurBusiness);

		return IVS_TVWALL_MEMORY_NULL;
	}

	//开始业务
	long lResult = m_pStreamMgr->ReStartBusiness(*pCurBusiness);
	if (IVS_SUCCEED != lResult)
	{
		IVS_RUN_LOG_ERR( "[ BusinessID = %lu ] ReStart Business Failed On Handle ReStart Business Event.",
			pCurBusiness->GetBusinessId());
