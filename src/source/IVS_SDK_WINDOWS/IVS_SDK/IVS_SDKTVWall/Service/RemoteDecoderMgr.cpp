	...
	,m_objTriggerConnTimeOut(CUMW::TRIGGER_TYPE_DECODER_CONN_TIME_OUT)
{
    ...
    eSDK_MEMSET(m_szLocalIp, 0, sizeof(m_szLocalIp));
...
CRemoteDecoder* CRemoteDecoderArry::GetRemoteDecoder(void)
{
	...
	{
		(void)VOS_MutexUnlock(m_pMutexList);
		IVS_RUN_LOG_ERR("Get Remote Decoder Object Failed. pRemoteDecoder is NULL.");
		return NULL;
	}
...
CDecoderConnHandle* CRemoteDecoderArry::GetConnHandle(const char* pszDecoderID)
{
    ...
    //找到此解码器对应的连接
    CDecoderConnHandle* pDecoderConnHandle = pRemoteDecoder->m_pDecoderConnHandle;
    if (NULL == pDecoderConnHandle)
    {
...
LONG CRemoteDecoderReqProc::ProcMsg( CDecoderMgrEvent* pEventRecv)
{
	...
	do 
	{
		...
		case RDA_REINIT_BUSINESS_RSP:
		case RDA_RESTART_BUSINESS_RSP:
			{
				//处理同步响应
		...
		case RDA_CAPABILITY_REPORT_REQ:
			{
				//处理能力上报请求消息
				lRet = ProcCapabilityNotifyReq(*pEventRecv);
				break;
			}
		default:
			{
		...
		eSDK_MEMSET(pszBuf, 0, ulTotalRecvBytes + 1);
		eSDK_MEMCPY(pszBuf, ulTotalRecvBytes + 1, m_pRecvBuffer, ulTotalRecvBytes);

		eSDK_MEMSET(m_pRecvBuffer, 0, ulTotalRecvBytes + 1);
		eSDK_MEMCPY(m_pRecvBuffer, ulTotalRecvBytes + 1, &pszBuf[stRDAMsgHead.ulPackLength], ulTotalRecvBytes - ulHadDealBytes);
	
		eSDK_MEMSET(&stRDAMsgHead, 0x00, sizeof(RDAMsgHead));
...
long CRemoteDecoderReqProc::ProcCapabilityNotifyReq
	(   IN CDecoderMgrEvent& objEvent)
{
	IVS_DBG_LOG("Process Capability Notify Request Message Begin.");

	//检查是否已经初始化
	if (!CheckIsInited())
	{
		IVS_RUN_LOG_ERR( "Process Capability Notify Request Message Failed. Not Init yet.");
		return IVS_FAIL;
	}

	//解析请求消息
	CRDACapabilityNotifyReq objRDAReq;
	long lResult = m_pRDAProtStack->DecodeMsg(m_pRecvBuffer, MAX_RDAMSG_LEN, objRDAReq);
	if (VOS_SUCCESS != lResult)
	{
		IVS_RUN_LOG_ERR( "Decode Message Failed On Process Capability Notify Request Message.");
		return IVS_FAIL;
	}
	...
	//通知上层
	CUMW_NOTIFY_TYPE_DECODER_CAPABILITY_INFO* pstCapabilityInfo = VOS_NEW(pstCapabilityInfo);
	eSDK_MEMSET(pstCapabilityInfo, 0x00, sizeof(CUMW_NOTIFY_TYPE_DECODER_CAPABILITY_INFO));
	CUMW_NOTIFY_INFO* pstNotifyInfo = VOS_NEW(pstNotifyInfo);
	eSDK_MEMSET(pstNotifyInfo, 0x00, sizeof(CUMW_NOTIFY_INFO));

	if (!CToolsHelp::Strncpy(pstCapabilityInfo->szDecoderID, CUMW_DECODER_ID_LEN + 1, objRDAReq.m_szDecoderID, CUMW_DECODER_ID_LEN))
	{
		IVS_RUN_LOG_ERR("Proc Capability Notify Req Copy DecoderID error.");
		VOS_DELETE(pstCapabilityInfo);
		VOS_DELETE(pstNotifyInfo);
		return IVS_ALLOC_MEMORY_ERROR;
	}
	
	pstCapabilityInfo->ulSupportSwitch = 1;

	pstNotifyInfo->ulNotifyType = CUMW_NOTIFY_TYPE_REMOTE_DECODER_CAPABILITY;
	pstNotifyInfo->pNotifyInfo = (void*)pstCapabilityInfo;
	pstNotifyInfo->ulNotifyInfoLen = sizeof(CUMW_NOTIFY_INFO);
	...
	IVS_DBG_LOG("[ DecoderID = %.20s ] Process Capability Notify Request Message Success.", objRDAReq.m_szDecoderID);
	return IVS_SUCCEED;
}
...
LONG CRemoteDecoderReqProc::ProcRegisterMsg(CDecoderMgrEvent& objEvent)
{
    ...
    if (0 == memcmp(stDecoderInfo.szDecoderEncoding, "UTF-8", strlen("UTF-8")))
    {
        ...
        eSDK_MEMSET(stDecoderInfo.szDecoderName, 0, CUMW_DECODER_NAME_LEN + 1);
        ...
        eSDK_MEMSET(stDecoderInfo.szDecoderEncoding, 0, CUMW_DECODER_ENCODING_LEN + 1);
...
long CRemoteDecoderMgr::InitCheckHeartBeatTrigger()
{
    ...
    (void)m_pCheckHeartBeatTrigger->Init(m_pDecoderMgrEventQ);
...
(   IN CBusiness& objBusiness)
{
    ...
    eSDK_MEMSET((void*)&stInitInfo, 0, sizeof(CUMW_NOTIFY_INIT_DECODER_INFO));
...
long CRemoteDecoderMgr::ReInitBusiness
	(   IN CBusiness& objBusiness)
{
	const unsigned long ulBusinessId = objBusiness.GetBusinessId();

	IVS_RUN_LOG_INF(
		"[ BusinessID = %lu ] ReInit Business In Remote Decoder Manager Begin. DecoderID = %.20s. ChannelID = %lu.",
		ulBusinessId, objBusiness.m_szDecoderId, objBusiness.m_ulChannelId);

	//构造初始化业务请求消息
	CRDAReInitBusinessReq objRDAReq;
	objRDAReq.m_stMsgHead.usPackID = GetTransNo();
	objRDAReq.m_ulBusinessID = ulBusinessId;
	objRDAReq.m_usChannelNo = (RDA_UINT2)(objBusiness.m_ulChannelId);
	if(!CToolsHelp::Strncpy(objRDAReq.m_szDecoderID, sizeof(objRDAReq.m_szDecoderID), objBusiness.m_szDecoderId, sizeof(objRDAReq.m_szDecoderID) - 1))
	{
		IVS_RUN_LOG_ERR("ReInit Business Copy DecoderId error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	//协议类型值
	objRDAReq.m_ucTransportProtocol = objBusiness.GetDecodeProtocolType();
	...
	//发送请求消息，同步等待响应消息并解析
	CRDAReInitBusinessRsp objRDAResp;
	...
	if (VOS_SUCCESS != lResult)
	{
		...
		IVS_RUN_LOG_ERR( "[ BusinessID = %lu ] ReInit Business In Remote Decoder Manager Failed. "
			"Request or Response Message Problem.", ulBusinessId);
		return lResult;
	}

	IVS_RUN_LOG_INF("[ BusinessID = %lu ] ReInit Business Return: DecoderID = %s, ChannelId = %lu, ErrorCode = %lu, ErrorDescription = %s."
		"Request or Response Message Problem.", ulBusinessId, objRDAResp.m_szDecoderID, objBusiness.m_ulChannelId, objRDAResp.m_ulErrorCode, objRDAResp.m_szErrorDescription);
	...
	if(!CToolsHelp::Strncpy(stInitInfo.stMediaNetInfo.szMediaIP, sizeof(stInitInfo.stMediaNetInfo.szMediaIP), objRDAResp.m_szMediaIP, sizeof(stInitInfo.stMediaNetInfo.szMediaIP)-1))
	{
		IVS_RUN_LOG_ERR("ReInit Business Copy MediaIP error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	if(!CToolsHelp::Strncpy(stInitInfo.stErrorCode.szErrorDescription, sizeof(stInitInfo.stErrorCode.szErrorDescription), objRDAResp.m_szErrorDescription, sizeof(stInitInfo.stErrorCode.szErrorDescription) - 1))
	{
		IVS_RUN_LOG_ERR("ReInit Business Copy ErrorDescription error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	if ((RDA_PROTOCOL_RTP_OVER_TCP == objBusiness.GetDecodeProtocolType()) ||
		(RDA_PROTOCOL_NSS_OVER_TCP == objBusiness.GetDecodeProtocolType()))
	{
		...
		IVS_RUN_LOG_INF(
			"[ BusinessID = %lu ] ReInit Business In Remote Decoder Manager In. DecoderIp = %.15s. TcpInterleavedPort = %u.",
			ulBusinessId, objRDAResp.m_szMediaIP, objRDAResp.m_usTcpInterleavedPort);
	}
	else
	{
		...
		IVS_RUN_LOG_INF("[ BusinessID = %lu ] ReInit Business In Remote Decoder Manager In. DecoderIp = %.15s. "
			"Udp: VideoPort(Rtp = %u, Rtcp = %u). AudioPort(Rtp = %u, Rtcp = %u)",
			ulBusinessId, objRDAResp.m_szMediaIP,
			objRDAResp.m_usUdpVideoRtpPort,
			objRDAResp.m_usUdpVideoRtcpPort,
	...
	IVS_DBG_LOG("[ BusinessID = %lu ] ReInit Business In Remote Decoder Manager End. Result = 0x%04X.", ulBusinessId,
		lResult);
	return lResult;
}
...
(   IN const CBusiness& objBusiness)
{
    ...
    eSDK_MEMSET((void*)&stStartInfo, 0, sizeof(CUMW_NOTIFY_START_STOP_DECODER_INFO));
...
long CRemoteDecoderMgr::ReStartBusiness
	(   IN const CBusiness& objBusiness)
{
	const unsigned long ulBusinessId = objBusiness.GetBusinessId();

	IVS_RUN_LOG_INF(
		"[ BusinessID = %lu ] ReStart Business In Remote Decoder Manager Begin. DecoderID = %.20s. ChannelID = %lu.",
		ulBusinessId, objBusiness.m_szDecoderId, objBusiness.m_ulChannelId);

	//构造开始业务请求消息
	CRDAReStartBusinessReq objRDAReq;
	objRDAReq.m_stMsgHead.usPackID = GetTransNo();
	objRDAReq.m_ulBusinessID = ulBusinessId;
	objRDAReq.m_usChannelNo = (USHORT)objBusiness.m_ulChannelId;
	if(!CToolsHelp::Strncpy(objRDAReq.m_szDecoderID, sizeof(objRDAReq.m_szDecoderID), objBusiness.m_szDecoderId, sizeof(objRDAReq.m_szDecoderID) - 1))
	{
		IVS_RUN_LOG_ERR("ReStart Business Copy DecoderID error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	if (RDA_PROTOCOL_RTP_OVER_TCP == objBusiness.GetDecodeProtocolType())
	{
	...
	if(!CToolsHelp::Strncpy(objRDAReq.m_szMediaIP, sizeof(objRDAReq.m_szMediaIP), objBusiness.m_szServerIp, sizeof(objRDAReq.m_szMediaIP)-1))
	{
		IVS_RUN_LOG_ERR("ReStart Business Copy MediaIP error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	objRDAReq.m_ucVideoPayloadType = objBusiness.m_enVideoPt;
	objRDAReq.m_ucAudioPayloadType = objBusiness.m_enAudioPt;
	if(!CToolsHelp::Strncpy(objRDAReq.m_szSessionId, sizeof(objRDAReq.m_szSessionId), objBusiness.m_szSessionId, sizeof(objRDAReq.m_szSessionId) - 1))
	{
		IVS_RUN_LOG_ERR("ReStart Business Copy SessionId error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	if(!CToolsHelp::Strncpy(objRDAReq.m_szSessionUrl, sizeof(objRDAReq.m_szSessionUrl), objBusiness.m_szSessionUrl, sizeof(objRDAReq.m_szSessionUrl) - 1))
	{
		IVS_RUN_LOG_ERR("ReStart Business Copy SessionUrl error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	//回调函数数据
	CUMW_NOTIFY_INFO stNotifyInfo;
	...
	//发送请求消息，同步等待响应消息并解析
	CRDAReStartBusinessRsp objRDAMsgResp;
	long lResult = SendReqMsg(objRDAReq, objBusiness.m_szDecoderId, objRDAMsgResp);
	if (VOS_SUCCESS != lResult)
	{
		stStartInfo.stErrorCode.ulErrorCode = IVS_TVWALL_SEND_FAIL;
		...
		IVS_RUN_LOG_ERR( "[ BusinessID = %lu ] ReStart Business In Remote Decoder Manager Failed. "
			"Request or Response Message Problem.", ulBusinessId);
		return lResult;
	}

	IVS_DBG_LOG("[ BusinessID = %lu ] ReStart Business Return: DecoderID = %s, ChannelId = %lu, ErrorCode = %lu, ErrorDescription = %s.", ulBusinessId, objRDAMsgResp.m_szDecoderID, objBusiness.m_ulChannelId, objRDAMsgResp.m_ulErrorCode, objRDAMsgResp.m_szErrorDescription);

	stStartInfo.stErrorCode.ulErrorCode = objRDAMsgResp.m_ulErrorCode;
	if(!CToolsHelp::Strncpy(stStartInfo.stErrorCode.szErrorDescription, sizeof(stStartInfo.stErrorCode.szErrorDescription), objRDAMsgResp.m_szErrorDescription, sizeof(stStartInfo.stErrorCode.szErrorDescription) - 1))
	{
		IVS_RUN_LOG_ERR("ReStart Business Copy ErrorDescription error.");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	//回调通知
	lResult = m_objStatusNotifyCallBack.NotifyCallBack(stNotifyInfo);

	IVS_DBG_LOG("[ BusinessID = %lu ] ReStart Business In Remote Decoder Manager End. Result = 0x%04X.",
		ulBusinessId, lResult);
	return lResult;
}
...
{ 
    ...
    eSDK_MEMSET((void*)&stInfo, 0, sizeof(CUMW_NOTIFY_START_STOP_DECODER_INFO));
    ...
    IN BOOL                bStart)
{
    ...
    eSDK_MEMSET((void*)&stResInfo, 0, sizeof(CUMW_NOTIFY_COM_RES_INFO));
