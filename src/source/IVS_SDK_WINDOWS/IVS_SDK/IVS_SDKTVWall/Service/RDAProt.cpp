...
{
    eSDK_MEMSET(&m_stMsgHead, 0x00, sizeof(m_stMsgHead));
    eSDK_MEMSET(m_szRDAMsgTypeName, 0x00, LEN_RDA_MSGTYPE_NAME);
}
...
CRDARegisteReq::CRDARegisteReq( )
{
    ...
    eSDK_MEMSET(m_szDecoderID, 0, sizeof(m_szDecoderID));
    eSDK_MEMSET(m_szDecoderName, 0, sizeof(m_szDecoderName));
    eSDK_MEMSET(&m_stChannelsInfo, 0, sizeof(m_stChannelsInfo));
    eSDK_MEMSET(&m_stVideoPayloadTypes, 0, sizeof(m_stVideoPayloadTypes));
    eSDK_MEMSET(&m_stAudioPayloadTypes, 0, sizeof(m_stAudioPayloadTypes));
	eSDK_MEMSET(m_szDecoderEncoding, 0, sizeof(m_szDecoderEncoding));
}
...
CRDARegisteRsp::CRDARegisteRsp()
{
    ...
    m_ulErrorCode = 0;
    eSDK_MEMSET(m_szDecoderID, 0x00, sizeof(m_szDecoderID));
    eSDK_MEMSET(m_szErrorDescription, 0x00, sizeof(m_szErrorDescription));
}
...
CRDAInitBusinessRsp::CRDAInitBusinessRsp( )
{
    ...
    eSDK_MEMSET(m_szMediaIP, 0, sizeof(m_szMediaIP));
}
...
CRDAStartBusinessReq::CRDAStartBusinessReq()
{
    ...
    eSDK_MEMSET(m_szDecoderID, 0, sizeof(m_szDecoderID));
    eSDK_MEMSET(m_szMediaIP, 0, sizeof(m_szMediaIP));
    eSDK_MEMSET(m_szSessionId, 0, sizeof(m_szSessionId));
    eSDK_MEMSET(m_szSessionUrl, 0, sizeof(m_szSessionUrl));
}
...
CRDAReInitBusinessReq::CRDAReInitBusinessReq( )
{
	m_stMsgHead.usPackType = RDA_MSG_REQ;
	m_stMsgHead.ucProtType = RDA_PROT_TYPE;
	m_stMsgHead.ucVersion = RDA_PROT_VERSION;
	m_stMsgHead.usMsgType = RDA_REINIT_BUSINESS_REQ;

	if(!CToolsHelp::Strncpy(m_szRDAMsgTypeName, sizeof(m_szRDAMsgTypeName), RDA_MSG_NAME_REINIT_BUSINESS_REQ,
		LEN_RDA_MSGTYPE_NAME))
	{
		IVS_RUN_LOG_ERR("Copy RDAMsgTypeName failed");
	}
...
long CRDAReInitBusinessReq::RegisterAttrs( )
{
	if (NULL == m_pRDAMsgAttrMgr)
	{
		IVS_RUN_LOG_ERR("RDAInitBusinessReq RegisterAttrs RDAMsgAttrMgr is NULL");
...
CRDAReInitBusinessRsp::CRDAReInitBusinessRsp( )
{
	m_stMsgHead.usPackType = RDA_MSG_RSP;
	m_stMsgHead.ucProtType = RDA_PROT_TYPE;
	m_stMsgHead.ucVersion = RDA_PROT_VERSION;
	m_stMsgHead.usMsgType = RDA_REINIT_BUSINESS_RSP;

	if(!CToolsHelp::Strncpy(m_szRDAMsgTypeName, sizeof(m_szRDAMsgTypeName), RDA_MSG_NAME_REINIT_BUSINESS_RSP, LEN_RDA_MSGTYPE_NAME))
	{
		IVS_RUN_LOG_ERR("Copy RDAMsgTypeName failed");
	}
...
long CRDAReInitBusinessRsp::RegisterAttrs( )
{
	if (NULL == m_pRDAMsgAttrMgr)
	{
		IVS_RUN_LOG_ERR("RDAInitBusinessRsp RegisterAttrs RDAMsgAttrMgr is NULL");
...
CRDAReStartBusinessReq::CRDAReStartBusinessReq()
{
	m_stMsgHead.usPackType = RDA_MSG_REQ;
	m_stMsgHead.ucProtType = RDA_PROT_TYPE;
	m_stMsgHead.ucVersion = RDA_PROT_VERSION;
	m_stMsgHead.usMsgType = RDA_RESTART_BUSINESS_REQ;

	if(!CToolsHelp::Strncpy(m_szRDAMsgTypeName, sizeof(m_szRDAMsgTypeName), RDA_MSG_NAME_RESTART_BUSINESS_REQ, LEN_RDA_MSGTYPE_NAME))
	{
		IVS_RUN_LOG_ERR("Copy RDAMsgTypeName failed");
	}
...
long CRDAReStartBusinessReq::RegisterAttrs( )
{
	if (NULL == m_pRDAMsgAttrMgr)
	{
		IVS_RUN_LOG_ERR("RDAStartBusinessReq RegisterAttrs RDAMsgAttrMgr is NULL");
...
CRDAReStartBusinessRsp::CRDAReStartBusinessRsp( )
{
	m_stMsgHead.usPackType = RDA_MSG_RSP;
	m_stMsgHead.ucProtType = RDA_PROT_TYPE;
	m_stMsgHead.ucVersion = RDA_PROT_VERSION;
	m_stMsgHead.usMsgType = RDA_RESTART_BUSINESS_RSP;

	if(!CToolsHelp::Strncpy(m_szRDAMsgTypeName, sizeof(m_szRDAMsgTypeName), RDA_MSG_NAME_RESTART_BUSINESS_RSP,
		LEN_RDA_MSGTYPE_NAME))
	{
		IVS_RUN_LOG_ERR("Copy RDAMsgTypeName failed");
	}
...
long CRDAReStartBusinessRsp::RegisterAttrs( )
{
	if (NULL == m_pRDAMsgAttrMgr)
	{
		IVS_RUN_LOG_ERR("RDAStartBusinessRsp RegisterAttrs RDAMsgAttrMgr is NULL");
...
CRDACapabilityNotifyReq::CRDACapabilityNotifyReq( )
{
	m_stMsgHead.usPackType = RDA_MSG_REQ;
	m_stMsgHead.ucProtType = RDA_PROT_TYPE;
	m_stMsgHead.ucVersion = RDA_PROT_VERSION;
	m_stMsgHead.usMsgType = RDA_CAPABILITY_REPORT_REQ;

	eSDK_MEMSET(m_szDecoderID , 0x00, sizeof(RDA_STRING_ID));
	m_ulSupportSwitch = 0;
}

long CRDACapabilityNotifyReq::RegisterAttrs( )
{
	if (NULL == m_pRDAMsgAttrMgr)
	{
		IVS_RUN_LOG_ERR("CRDACapabilityNotifyReq RegisterAttrs RDAMsgAttrMgr is NULL");
		return VOS_FAIL;
	}
