...
IVS_INT32 CMotionDetecMgrXMLProcess::SetMotionDetecGetXML(const IVS_CHAR* pLoginId, const IVS_CHAR* pCameraCode, const IVS_CHAR* pDomainCode, const IVS_MOTION_DETECTION* pMotionDetection, CXml& xml)
{
	...
	(void)xml.AddElem(strMotionDetectionInfo.c_str());//lint !e64 匹配
	(void)xml.IntoElem();
	...
	const IVS_INDEX_RANGE* pIndexRange,CXml& xml)
{
	...
	(void)xml.AddChildElem(strFromIndex.c_str());//lint !e64 匹配
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pIndexRange->uiFromIndex)).c_str());//lint !e64 匹配
	(void)xml.AddElem(strToIndex.c_str());//lint !e64 匹配
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pIndexRange->uiToIndex)).c_str());//lint !e64 匹配
	...
	(void)xml.SetElemValue(strQueryType.c_str());//lint !e64 匹配
	xml.OutOfElem();
...
IVS_INT32 CMotionDetecMgrXMLProcess::ParseGetMotionRangeDataPlatformRspXML(CXml &RspXml, IVS_MOTION_RANGE_DATA_LIST* pMotionRangeData,IVS_UINT32 uiBufferSize)
{
	...
	if(RspXml.FindElemEx("Content/MotionList/MotionData"))
	{
		do{
			...
			eSDK_MEMSET(&stMotionRangeDataInfo, 0, sizeof(IVS_MOTION_RANGE_DATA_INFO));
