...
IVS_VOID CUserInfoMgrXMLProcess::GetAddModUserInfoXML(const IVS_CHAR* pDomainCode, const IVS_USER_INFO* pUserInfo, const CSDKSecurityClient& oSecurityClient, CXml &xml)
{
    ...
    eSDK_STRNCPY(cLoginName, sizeof(cLoginName), pUserInfo->cLoginName, IVS_NAME_LEN);
    (void)xml.SetElemValue(cLoginName);
    (void)xml.AddElem("LoginPWD");

	std::string strEncryptedData;
    IVS_CHAR cLoginPWD[IVS_PWD_LEN + 1] = {0};
    eSDK_STRNCPY(cLoginPWD, sizeof(cLoginPWD), pUserInfo->cPWD, IVS_PWD_LEN);
    ...
    eSDK_STRNCPY(cUserDomain, sizeof(cUserDomain), pUserInfo->cUserDomain, IVS_DOMAIN_LEN);
    ...
    eSDK_STRNCPY(cRegiterDate, sizeof(cRegiterDate), pUserInfo->cRegiterDate, IVS_TIME_LEN);

    (void)xml.SetElemValue(cRegiterDate);
    (void)xml.AddElem("UserDesc"); 
    IVS_CHAR cUserDesc[IVS_DESCRIBE_LEN + 1] = {0};
    eSDK_STRNCPY(cUserDesc, sizeof(cUserDesc), pUserInfo->cUserDesc, IVS_DESCRIBE_LEN);
    ...
    eSDK_STRNCPY(cUserName, sizeof(cUserName), pUserInfo->cUserName, IVS_NAME_LEN);
    ...
    pDataANSI = CToolsHelp::UTF8ToANSI(strEncryptedData.c_str());//lint !e838
    if (NULL!= pDataANSI)
    {
        (void)xml.SetElemValue(pDataANSI);
		free(pDataANSI);
    ...
    eSDK_STRNCPY(cEmail, sizeof(cEmail), pUserInfo->cEmail, IVS_EMAILE_LEN);
    strEncryptedData.clear();
    ...
    eSDK_STRNCPY(cPhone, sizeof(cPhone), pUserInfo->cPhone, IVS_PHONE_NUMBER_LEN);
    strEncryptedData.clear();
...
IVS_INT32 CUserInfoMgrXMLProcess::GetUserListParseXML(CXml &ReqXml, const CSDKSecurityClient& oSecurityClient, IVS_USER_INFO_LIST* pUserInfoList)
{
    ...
    do{
        ...
        GET_ELEM_VALUE_CHAR("LoginName", szElemValue, stUserInfo.cLoginName, IVS_NAME_LEN, ReqXml);
        ...
        if (NULL != szElemValue)
        {
            IVS_INT32 iRet = oSecurityClient.DecryptString(szElemValue, strEncryptedData);
			if (IVS_SUCCEED != iRet)
			{
				BP_RUN_LOG_ERR(IVS_DATA_DECRYPT_ERROR, "DecryptString failed", "return errocode: %d", iRet);
			...
			if (TRUE == bTranslated)
			{
				...
				if  (NULL  != pDataANSI)
				{
					eSDK_STRNCPY(stUserInfo.cUserDesc, sizeof(stUserInfo.cUserDesc), pDataANSI, IVS_DESCRIBE_LEN);
					free(pDataANSI);
					pDataANSI = NULL;
				}
			}//lint !e438
			...
			{
				eSDK_STRNCPY(stUserInfo.cUserDesc, sizeof(stUserInfo.cUserDesc), strEncryptedData.c_str(), strEncryptedData.length());
			}
        }//lint !e438
        ...
        if (NULL != szElemValue)
        {
			...
			if (TRUE == bTranslated)
			{
				...
				if  (NULL != pDataANSI)
				{
					eSDK_STRNCPY(stUserInfo.cUserName, sizeof(stUserInfo.cUserName), pDataANSI, IVS_NAME_LEN);
					free(pDataANSI);
					pDataANSI = NULL;
				}
			}//lint !e438
			...
			{
				eSDK_STRNCPY(stUserInfo.cUserName, sizeof(stUserInfo.cUserName), strEncryptedData.c_str(), strEncryptedData.length());
			}
        ...
        if (NULL != szElemValue)
        {
            (void)oSecurityClient.DecryptString(szElemValue, strEncryptedData);
            eSDK_STRNCPY(stUserInfo.cEmail, sizeof(stUserInfo.cEmail), strEncryptedData.c_str(), IVS_EMAILE_LEN);
        }
        ...
        {
            (void)oSecurityClient.DecryptString(szElemValue, strEncryptedData);
            eSDK_STRNCPY(stUserInfo.cPhone, sizeof(stUserInfo.cPhone), strEncryptedData.c_str(), IVS_PHONE_NUMBER_LEN);
        }
...
IVS_INT32 CUserInfoMgrXMLProcess::GetOnlineUserListParseXML(CXml &ReqXml, IVS_ONLINE_USER_LIST* pOnlUserList)
{
    ...
    do{
        ...
        eSDK_MEMSET(&stOnlUser, 0, sizeof(IVS_ONLINE_USER));
        ...
        GET_ELEM_VALUE_CHAR("LoginName", szElemValue, stOnlUser.cUserName, IVS_NAME_LEN, ReqXml);
...
{  
    ...
    if (NULL != szElemValue)
    {
		...
		if (TRUE == bTranslated)
		{
			...
			if  (NULL  != pDataANSI)
			{
				eSDK_STRNCPY(pUserInfo->cUserDesc, sizeof(pUserInfo->cUserDesc), pDataANSI, IVS_DESCRIBE_LEN);
				free(pDataANSI);
				pDataANSI = NULL;
			}
		}//lint !e438
		...
		{
			eSDK_STRNCPY(pUserInfo->cUserDesc, sizeof(pUserInfo->cUserDesc), strEncryptedData.c_str(), strEncryptedData.length());
		}
    }//lint !e438
    ...
    if (NULL != szElemValue)
    {
		...
		if (TRUE == bTranslated)
		{
			...
			if  (NULL  != pDataANSI)
			{
				eSDK_STRNCPY(pUserInfo->cUserName, sizeof(pUserInfo->cUserName), pDataANSI, IVS_NAME_LEN);
				free(pDataANSI);
				pDataANSI = NULL;
			}
		}//lint !e438
		...
		{
			eSDK_STRNCPY(pUserInfo->cUserName, sizeof(pUserInfo->cUserName), strEncryptedData.c_str(), strEncryptedData.length());
		}
    }//lint !e438
    ...
    {
        (void)oSecurityClient.DecryptString(szElemValue, strEncryptedData);
        eSDK_STRNCPY(pUserInfo->cEmail, sizeof(pUserInfo->cEmail), strEncryptedData.c_str(), IVS_EMAILE_LEN);
    }
    ...
    {
        (void)oSecurityClient.DecryptString(szElemValue, strEncryptedData);
        eSDK_STRNCPY(pUserInfo->cPhone, sizeof(pUserInfo->cPhone), strEncryptedData.c_str(), IVS_PHONE_NUMBER_LEN);
    }
...
IVS_INT32 CUserInfoMgrXMLProcess::UserDeviceListParseXML(CXml& xml, IVS_USER_DEVICE_INFO_LIST* pUserDevList)
{
    ...
    if (xml.FindElemEx("Content/DevList/DevInfo"))
    {
        do
        {
            ...
            eSDK_MEMSET(&stUserDev, 0, sizeof(IVS_USER_DEVICE_INFO));
...
{
    IVS_DEBUG_TRACE("");
	CHECK_POINTER(uiRoleID, IVS_PARA_INVALID);
...
IVS_INT32 CUserInfoMgrXMLProcess::GetRoleListParseXML(CXml &xml, const IVS_INDEX_RANGE* pIndexRange, IVS_ROLE_INFO_LIST* pRoleList)
{
    IVS_DEBUG_TRACE("");
    ...
    if(xml.FindElemEx("Content/PageInfo"))
    {
		...
		if (pRoleList->stIndexRange.uiToIndex > pIndexRange->uiToIndex)
		{
			pRoleList->stIndexRange.uiToIndex = pIndexRange->uiToIndex;
		}
        xml.OutOfElem();
    }
	...
	IVS_UINT32 uiReqNum = pIndexRange->uiToIndex - pIndexRange->uiFromIndex;
    IVS_UINT32 iSize = 0;
    do{
        ...
        eSDK_MEMSET(&stRoleInfo, 0, sizeof(IVS_ROLE_INFO));
        ...
        iSize ++;
    }while (xml.NextElem() && iSize <= uiReqNum);
...
IVS_INT32 CUserInfoMgrXMLProcess::GetUserGroupListParseXML(CXml &RsqXml, const IVS_INDEX_RANGE* pIndexRange, IVS_USER_GROUP_LIST* pUserGroupList)
{
    ...
    do 
    {
        ...
        eSDK_MEMSET(&stQueryInfo, 0, sizeof(IVS_USER_GROUP));
...
IVS_INT32 CUserInfoMgrXMLProcess::GetAutoCreatePWDParseXML(CXml &xml, IVS_CHAR* pPWD)
{
    ...
    const IVS_CHAR* szElemValue = NULL;
    GET_ELEM_VALUE_CHAR("LoginPWD", szElemValue, pPWD, IVS_PWD_LEN, xml);//lint !e838
...
IVS_INT32 CUserInfoMgrXMLProcess::GetOperationRightGroupParseXML(CXml &xml, IVS_OPERATION_RIGHT_GROUP_LIST* pRightGroupList)
{
    ...
    if(xml.FindElemEx("Content/OPGroupList/OPGroupInfo"))
    {
        do{
			...
			SecureZeroMemory(&stRightGroup, sizeof(IVS_OPERATION_RIGHT_GROUP));
            const IVS_CHAR* szElemValue = NULL;
            GET_ELEM_VALUE_CHAR("OPGroupName", szElemValue, stRightGroup.cRightGroupName, IVS_MAX_RIGHT_GROUP_NAME_LEN, xml);//lint !e838
            eSDK_MEMSET(chTemp, 0, IVS_MAX_OPERATION_RIGHT+1);
	...
	IVS_STREAM_LIST_BY_CAM* pStreamListByCam)
{
	...
	eSDK_MEMSET(pStreamListByCam, 0, uiBufSize);
	...
	const IVS_CHAR* szElemValue = NULL;
	GET_ELEM_VALUE_CHAR("CameraCode", szElemValue, pStreamListByCam->cCameraCode, IVS_DEV_CODE_LEN, xml);//lint !e838
	...
	IVS_STREAM_LIST_BY_USER* pStreamListByUser)
{
	...
	eSDK_MEMSET(pStreamListByUser, 0, uiBufSize);
	...
	if(xml.FindElemEx("Content/StreamList/StreamInfo"))
	{
		...
		do{
			...
			GET_ELEM_VALUE_CHAR("DomainCode", szElemValue, cDevDomainCode, IVS_DOMAIN_CODE_LEN, xml);
			std::string strCameraCode(cCameraCode);
			strCameraCode += "#";
			strCameraCode += cDevDomainCode;
			if (!CToolsHelp::Memcpy(stStreamCamInfo.cCameraCode, IVS_DEV_CODE_LEN, strCameraCode.c_str(), IVS_DEV_CODE_LEN))
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "GetStreamListByUserParseXML", "Strncpy failed");
				return IVS_ALLOC_MEMORY_ERROR;
			}
...
IVS_INT32 CUserInfoMgrXMLProcess::ShutdownStreamByUserGetXML(const IVS_CHAR* pUserDomainCode,
	IVS_UINT32 uiUserID, 
	const IVS_CHAR* pCameraCode,
	const IVS_CHAR* pDevDomainCode,
	IVS_INT32 iStreamType,
	IVS_INT32 iDuration,
	CXml &xml)
{
	IVS_DEBUG_TRACE("");
	CHECK_POINTER(pUserDomainCode, IVS_PARA_INVALID);
	CHECK_POINTER(pCameraCode, IVS_PARA_INVALID);
	CHECK_POINTER(pDevDomainCode, IVS_PARA_INVALID);

	CUserInfoMgrXMLProcess::CommDomainCodeXml(pDevDomainCode, xml);

	(void)xml.AddElem("UserID");
	std::string strUserID = CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiUserID));
	(void)xml.SetElemValue(strUserID.c_str());

	(void)xml.AddElem("UserDomainCode");
	IVS_CHAR cTmpUserDomCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
	if (!CToolsHelp::Strncpy(cTmpUserDomCode, IVS_DOMAIN_CODE_LEN + 1, pUserDomainCode, IVS_DOMAIN_CODE_LEN))
	{
		BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "ShutdownStreamByUserGetXML", "Strncpy failed");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	(void)xml.SetElemValue(cTmpUserDomCode);
