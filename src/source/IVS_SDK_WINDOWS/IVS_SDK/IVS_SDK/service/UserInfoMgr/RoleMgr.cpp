...
IVS_INT32 CRoleMgr::AddRole(const IVS_ROLE_INFO* pRoleInfo, IVS_UINT32* puiRoleID)
{
    ...
    IVS_INT32 iRet = m_UserMgr->SendCmd(sendNssMsgInfo,strRsp,iNeedRedirect);
    if(IVS_SUCCEED != iRet)
    {
...
IVS_INT32 CRoleMgr::DeleteRole(const IVS_CHAR* pDomainCode, IVS_UINT32 uiRoleID)
{
    ...
    eSDK_MEMSET(&pRoleInfo, 0, sizeof(IVS_ROLE_INFO));
...
IVS_INT32 CRoleMgr::GetRoleList(const IVS_CHAR* pDomainCode, const IVS_INDEX_RANGE* pIndexRange, IVS_ROLE_INFO_LIST* pRoleInfoList, IVS_UINT32 uiBufferSize)
{
	...
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "Get Role List failed", "Index range invalid");
		return IVS_PARA_INVALID;
	}
	//检查buffer的长度是否非法
	IVS_INT32 iRet = CheckRoleListBufSize(pIndexRange, uiBufferSize);
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Get Role List failed", "Buffer size invalid");
		return iRet;
	}
    ...
    iRet = CUserInfoMgrXMLProcess::GetRoleListParseXML(xmlRsp, pIndexRange, pRoleInfoList);
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "Get Role List failed", "parsee xml to buffer error");
	}
...
IVS_INT32 CRoleMgr::GetDefaultRoleAccountRule(const IVS_CHAR* pDomainCode, IVS_ROLE_ACCOUNT_RULE* pRoleAccountRule)
{
    ...
    IVS_INT32 iNeedRedirect = IVS_FAIL;
    IVS_INT32 iRet = m_UserMgr->SendCmd(sendNssMsgInfo,strResponse,iNeedRedirect);
    if(IVS_SUCCEED != iRet)
    {
...
IVS_INT32 CRoleMgr::GetOperationRightGroup(const IVS_CHAR* pDomainCode, IVS_OPERATION_RIGHT_GROUP_LIST* pRightGroupList)
{
    ...
    std::string sRsp;
    IVS_INT32 iNeedRedirect = IVS_FAIL;
    IVS_INT32 iRet = m_UserMgr->SendCmd(sendNssMsgInfo,sRsp,iNeedRedirect);
    if(IVS_SUCCEED != iRet)
    {
...
IVS_INT32 CRoleMgr::CheckRoleListBufSize(const IVS_INDEX_RANGE* pIndexRange, IVS_UINT32 uiBufferSize) const
{
	CHECK_POINTER(pIndexRange, IVS_PARA_INVALID);

	IVS_UINT32 uiReqNum = pIndexRange->uiToIndex - pIndexRange->uiFromIndex;
	IVS_UINT32 uiSize = sizeof(IVS_ROLE_INFO_LIST) + uiReqNum * sizeof(IVS_ROLE_INFO);
