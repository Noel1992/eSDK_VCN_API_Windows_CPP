...
IVS_INT32 CapturePictureXMLProcess::SetSnapshotPlanGetXML(const IVS_CHAR* pCameraCode,const IVS_SNAPSHOT_PLAN* pSnapShotPlan,CXml &xml)
{
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSnapShotPlan->bEnable)).c_str());//lint !e64 匹配
	(void)xml.AddElem("PlanType");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSnapShotPlan->uiPlanType)).c_str());//lint !e64 匹配
	...
	for (IVS_UINT32 i=0; i<pSnapShotPlan->uiPlanInfoNum; i++)
	{
		...
		(void)xml.IntoElem();//  -------------3
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSnapShotPlan->stPlanInfo[i].uiDayType)).c_str());//lint !e64 匹配
		...
		for (IVS_UINT32 j=0; j<pSnapShotPlan->stPlanInfo[i].uiTimeSpanInfoNum; j++)
		{
			...
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSnapShotPlan->stPlanInfo[i].stTimeSpanInfo[j].SnapInterval)).c_str()); //lint !e64 匹配
			xml.OutOfElem();//-------------------------1
		}  
		xml.OutOfElem();// ---------------------2
		xml.OutOfElem();//----------------3
...
IVS_INT32 CapturePictureXMLProcess::ParseGetSnapshotPlan(IVS_SNAPSHOT_PLAN* pSnapShotPlan,CXml& xml)
{
	...
	if (xml.FindElemEx("Content/SnapshotPlanInfo/PlanList"))
	{
		do 
		{
			...
			//解析PlanInfo
			(void)xml.IntoElem();
			eSDK_MEMSET(&pSnapShotPlan->stPlanInfo[iPlanInfoNum],0, sizeof(IVS_SNAPSHOT_PLAN_INFO));
...
IVS_INT32 CapturePictureXMLProcess::GetSnapshotListGetXML(const IVS_CHAR* pCameraCode, const IVS_QUERY_SNAPSHOT_PARAM* pQueryParam, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((int)pQueryParam->stIndexRange.uiFromIndex).c_str());//lint !e64 匹配
	(void)xml.AddElem("ToIndex");
	(void)xml.SetElemValue(CToolsHelp::Int2Str((int)pQueryParam->stIndexRange.uiToIndex).c_str());//lint !e64 匹配
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str((int)pQueryParam->uiSnapType).c_str());//lint !e64 匹配
...
IVS_INT32 CapturePictureXMLProcess::GetSnapshotListParseXML(CXml& xml, IVS_SNAPSHOT_INFO_LIST* pSnapshotList, IVS_UINT32 uiBufSize, const IVS_CHAR* pDomainCode)
{
	...
	IVS_INT32 iTemp;
	GET_ELEM_VALUE_NUM("FromIndex", pTemp, iTemp, xml);//lint !e838
	pSnapshotList->stIndexRange.uiFromIndex = static_cast<IVS_UINT32>(iTemp);

	GET_ELEM_VALUE_NUM("ToIndex", pTemp, iTemp, xml);//lint !e838
	pSnapshotList->stIndexRange.uiToIndex = static_cast<IVS_UINT32>(iTemp);

	GET_ELEM_VALUE_NUM("RealNum",pTemp, iTemp, xml);//lint !e838
	...
	if (xml.FindElemEx("Content/SnapshotList/SnapInfo"))
	{
		do 
		{
			...
			eSDK_MEMSET(&sSnapshoutInfo, 0, sizeof(IVS_SNAPSHOT_INFO));
			(void)xml.IntoElem();
			...
			if (!CToolsHelp::Memcpy(sSnapshoutInfo.cCameraCode, IVS_DEV_CODE_LEN, strCameraCode.c_str(), strCameraCode.length()))//lint !e64 匹配
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Get Snap shot List Get XML", "Memcpy cStart failed");
			}
			GET_ELEM_VALUE_CHAR("SnapTime",szElemValue,  sSnapshoutInfo.cSnapTime,IVS_TIME_LEN,xml);
			GET_ELEM_VALUE_NUM("SnapType", pTemp, iTemp, xml);//lint !e838
			sSnapshoutInfo.uiSnapType = static_cast<IVS_UINT32>(iTemp);
		
			GET_ELEM_VALUE_NUM("PictureSize", pTemp, iTemp, xml);//lint !e838
...
IVS_INT32 CapturePictureXMLProcess::DeleteSnapshotGetXML(const IVS_CHAR* pCameraCode, IVS_UINT32 uiPictureID, CXml &xml)
{
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiPictureID)).c_str());//lint !e64 匹配
	xml.OutOfElem();
