...
CTPPlayChannel::CTPPlayChannel(void)
				...
				, m_fSpeed(0.0)
{
	eSDK_MEMSET(&m_stuMediaParam, 0, sizeof(m_stuMediaParam));
											...
											IVS_MEDIA_ADDR& stMediaAddrSrc)
{
	...
	bool bSucc = CToolsHelp::Strncpy(stMediaAddrSrc.stIP.cIP, sizeof(stMediaAddrSrc.stIP.cIP), strIP.c_str(), strIP.length());//lint !e64 匹配
	if(!bSucc)
	{
		(void)RTSP_CLIENT_SendTeardownMsg(m_iRtspHandle, mediaRsp.szURL, 1);
		(void)RTSP_CLIENT_FreeHandle(m_iRtspHandle);
...
//开始平台录像回放
IVS_INT32 CTPPlayChannel::StartPlatformPlayBackByIP(const IVS_CHAR* pDomainCode,
											...
											IVS_FLOAT fSpeed)
{
	...
	ST_MEDIA_RSP mediaRsp;
	eSDK_MEMSET(&mediaRsp, 0, sizeof(ST_MEDIA_RSP));
	IVS_INT32 iRet = GetPlayBackURL(pDomainCode, pNVRCode, strCameraCode, stTimeSpanInter, stMediaAddrDst, mediaRsp);
	if (IVS_SUCCEED != iRet || 0 == strlen(mediaRsp.szURL))
	{
		BP_RUN_LOG_ERR(iRet,"TP Start Play Req", "get URL failed!");
		return iRet;
...
IVS_INT32 CTPPlayChannel::StopPlayByIP()
{
	IVS_DEBUG_TRACE("");
	//关闭rtsp
	(void)RTSP_CLIENT_SendTeardownMsg(m_iRtspHandle, m_strURL.c_str(), 1);//lint !e64 匹配
	(void)RTSP_CLIENT_FreeHandle(m_iRtspHandle); 
	m_iRtspHandle = 0;
	m_strURL = "";
	return IVS_SUCCEED;
...
IVS_INT32 CTPPlayChannel::GetRealPlayURL(const std::string& strCamera, 
const IVS_MEDIA_ADDR& stMediaAddrDst, 
				ST_MEDIA_RSP& stMediaRsp)
{
	...
	bool bSucc = CToolsHelp::Strncpy(stUrlMediaInfo.szSrcCode, sizeof(stUrlMediaInfo.szSrcCode), strCameraCode.c_str(), strCameraCode.length());//lint !e64 匹配
	CHECK_SUCCESS_RET(bSucc, IVS_ALLOC_MEMORY_ERROR);

	bSucc = CToolsHelp::Strncpy(stUrlMediaInfo.cDomainCode, sizeof(stUrlMediaInfo.cDomainCode), strDomainCode.c_str(), strDomainCode.length());//lint !e64 匹配
	CHECK_SUCCESS_RET(bSucc, IVS_ALLOC_MEMORY_ERROR);
	...
	CHECK_SUCCESS_RET(bSucc, IVS_ALLOC_MEMORY_ERROR);
	*/
	// "good stewards" project cannot NAT when iClientType = = 2
	...
	iRet = m_pTPMgr->GetURL(CToolsHelp::GetSCCHead().c_str(), stUrlMediaInfo, stMediaRsp);//lint !e64 匹配
	return iRet;
}

IVS_INT32 CTPPlayChannel::GetPlayBackURL(const IVS_CHAR* pDomainCode, 
					const IVS_CHAR* pNVRCode, 
					...
					ST_MEDIA_RSP& stMediaRsp)
{
	...
	{
		bSucc = CToolsHelp::Strncpy(stUrlMediaInfo.cDomainCode,IVS_DOMAIN_CODE_LEN+1,  pDomainCode, strlen(pDomainCode));
		CHECK_SUCCESS_RET(bSucc, IVS_ALLOC_MEMORY_ERROR);
	}
	...
	{
		(void)CToolsHelp::Strncpy(stUrlMediaInfo.szNVRCode, sizeof(stUrlMediaInfo.szNVRCode),pNVRCode, IVS_NVR_CODE_LEN);
	}
	else
	{
		eSDK_MEMSET(stUrlMediaInfo.szNVRCode,0,IVS_NVR_CODE_LEN);
		m_pTPMgr->GetRecordNVRInfo(stUrlMediaInfo.szSrcCode,stTimeSpanInter.iStartTime,stTimeSpanInter.iEndTime,stUrlMediaInfo.szNVRCode);
	}

	BP_RUN_LOG_INF("Get PlayBack URL", "CameraCode[%s]NVRCode[%s].", stUrlMediaInfo.szSrcCode, stUrlMediaInfo.szNVRCode);
	...
	*/
	...
	bSucc = CToolsHelp::Memcpy(stUrlMediaInfo.szTimeSpan,TIME_SPAN_LEN, strTime.c_str(), strTime.length());//lint !e64 匹配
	CHECK_SUCCESS_RET(bSucc, IVS_ALLOC_MEMORY_ERROR);
...
IVS_INT32 CTPPlayChannel::PlatformPlayBackPauseByIP(IVS_VOID)
{
	IVS_DEBUG_TRACE("");
	//暂停rtsp
	IVS_INT32 iRet = RTSP_CLIENT_SendPauseMsg(m_iRtspHandle, m_strURL.c_str(), RTSP_CLIENT_TIMEOUT);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CTPPlayChannel::PlatformPlayBackResumeByIP(IVS_VOID)
{
	...
	if (RTSP_SESSION_STATUS_READY == RTSP_CLIENT_GetStatus(m_iRtspHandle))
	{
		...
		IVS_INT32 iRet = RTSP_CLIENT_SendPlayMsg(m_iRtspHandle, m_strURL.c_str(), (double)fSendScale, &stRange, RTSP_CLIENT_TIMEOUT);//lint !e64 匹配
		if (IVS_SUCCEED != iRet)
		{
...
int CTPPlayChannel::SetPlayBackSpeedByIP(IVS_FLOAT fSpeed)
{
		...
		RTSP_SESSION_STATUS_READY   == state)
	{
		...
		IVS_INT32 iRet = RTSP_CLIENT_SendPlayMsg(m_iRtspHandle, m_strURL.c_str(), (double)fSendScale, &stMdieaRange, RTSP_CLIENT_TIMEOUT);//lint !e64 匹配
		if (IVS_SUCCEED != iRet)
		{  
			(void)RTSP_CLIENT_SendTeardownMsg(m_iRtspHandle, m_strURL.c_str(), 1);//lint !e64 匹配
			(void)RTSP_CLIENT_FreeHandle(m_iRtspHandle);
...
// 设置平台录像码流回放速度
int CTPPlayChannel::SetPlayBackSpeedByIPnTime(IVS_FLOAT fSpeed, IVS_UINT64 uiOffsetTime)
{
	IVS_DEBUG_TRACE("Speed: %f", fSpeed);
		...
		RTSP_SESSION_STATUS_READY   == state)
	{
			...
			(m_fSpeed > 0 && fSpeed < 0))
		{
			if (uiOffsetTime)
			{
				stMdieaRange.MediaBeginOffset = static_cast<unsigned int>(uiOffsetTime);
...
IVS_INT32 CTPPlayChannel::SetPlayBackTimeByIP(IVS_UINT32 uiTime)
{
	...
	BP_RUN_LOG_INF("Set PlayBack Time ByIP", "RTSP_CLIENT_SendPlayMsg begin, rtsphandle=0x%p, speed=%f, settime=%u.", m_iRtspHandle, m_fRTSPSpeed, uiTime);
	...
	iRet = RTSP_CLIENT_SendPlayMsg(m_iRtspHandle, m_strURL.c_str(), (double)m_fRTSPSpeed, &stRange, RTSP_CLIENT_TIMEOUT);//lint !e64 匹配
