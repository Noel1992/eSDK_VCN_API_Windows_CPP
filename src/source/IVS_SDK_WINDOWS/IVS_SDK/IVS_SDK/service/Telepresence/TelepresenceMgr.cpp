...
CTelepresenceMgr::~CTelepresenceMgr(void)
{
	(void)VOS_MutexLock(m_pChannelMapMutex);
	try
	{
												...
												IVS_ULONG& ulHandle)
{
	...
	eSDK_MEMSET(&stMediaPara, 0, sizeof(stMediaPara));
												...
												IVS_ULONG& ulHandle)
{
	...
	iRet = pPlayChannel->StartPlatformPlayBackByIP(NULL, stPlaybackParam.cNVRCode, strCameraCode, stMediaAddrDst, stMediaAddrSrc, stTimeSpanInter, stPlaybackParam.fSpeed);
	if(IVS_SUCCEED!=iRet)
	{
		RemovePlayChannel(ulHandle);
		ulHandle = 0;
...
void CTelepresenceMgr::StopAllPlayByIP()
{
	...
	try
	{
		for(; iter!=iter_end; iter++)
		{
			...
			(void)pPlayChannel->StopPlayByIP();
...
//获取实况句柄
IVS_ULONG CTelepresenceMgr::GetPlayHandlebyRtspHandle(IVS_ULONG iRtspHandle)
{
	...
	for (; iter != iterEnd; iter++)
	{
		...
		IVS_ULONG iRtspHandleTemp = pPlayChannel->GetRtspHandle();
...
//设置平台录像码流回放速度
IVS_INT32 CTelepresenceMgr::SetPlayBackSpeedByIPnTime(IVS_ULONG ulHandle, IVS_FLOAT fSpeed, IVS_UINT64 uiOffsetTime)
{
	IVS_DEBUG_TRACE("Handle: %lu, Speed: %f, OffsetTime: %lu", ulHandle, fSpeed, uiOffsetTime);
	CTPPlayChannel* pPlayChannel = GetPlayChannel(ulHandle);
	if(NULL==pPlayChannel)
	{
		BP_RUN_LOG_ERR(IVS_SDK_ERR_FINDVIDEOHANDLE_FAILED, "Set Platform PlayBack Speed", "Handle Invalid.");
	...
	return pPlayChannel->SetPlayBackSpeedByIPnTime(fSpeed, uiOffsetTime);
}
...
//获取摄像机在该时间段内所处的NVR信息
IVS_INT32 CTelepresenceMgr::GetRecordNVRInfo(const IVS_CHAR* pCameraCode,const IVS_INT32 iStarTime,const IVS_INT32 iEndTime, IVS_CHAR* pNVR)
{
	return m_pUserMgr->GetRecordList().GetRecordNVRInfo(pCameraCode,iStarTime,iEndTime,pNVR);//lint !e613
}
