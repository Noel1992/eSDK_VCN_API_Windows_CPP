...
IVS_INT32 CAlarmLinkagrXMLProcess::GetAndDelAlarmLinkageGetXML(const IVS_CHAR* pReqXml, CXml& xml, std::string& strDomainCode)
{
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "GetAndDel Alarm Linkage GetXML","Parse DevCode failed");
		return iRet;
	}
...
IVS_INT32 CAlarmLinkagrXMLProcess::GetAlarmInCode(const IVS_CHAR* pReqXML, IVS_CHAR*& pRspXml)
{
	...
	for(;;)
	{
        ...
        if (NULL != pDomainCode)
        {
            strDomainCode = pDomainCode;//lint !e63
        }
        ...
        {
            strAlarmInCode = pAlarmInCode;//lint !e63
        }
		if (!strAlarmInCode.empty())
		{
			strAlarmInCode.append("#");
			strAlarmInCode.append(strDomainCode);
			xmlRsp.ModifyElemValue(strAlarmInCode.c_str());//lint !e64 匹配
		}	
		xmlRsp.OutOfElem();
		if (!xmlRsp.NextElem())
		{
	...
	eSDK_MEMSET(pResultXml, 0x0, strlen(pRsp) + 1);
...
IVS_INT32 CAlarmLinkagrXMLProcess::SetAlarmInCodeValue(const IVS_CHAR* pReqXml, CXml& xml, std::string& strDomainCode)
{
	...
	do 
	{
		...
		do 
		{
			...
			std::string strParam;
			const IVS_CHAR *pParam = xmlModified.GetElemValue();
			if (NULL != pParam)
			{
				strParam = pParam;//lint !e63
			}
			...
			(void)xmlModified.SetElemValue(strParam.c_str());//lint !e64 匹配
	...
	pReq = xmlModified.GetXMLStream(xmlLen);
	CHECK_POINTER(pReq, IVS_FAIL);
	iRet = AddDomainCode4XML(strDomainCode.c_str(), "Content", "DomainCode", pReq, xml);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "AddDomainCode4XML fail", "NA");
		return iRet;
...
IVS_INT32 CAlarmLinkagrXMLProcess::GetAlarmLinkageListGetXml(const IVS_CHAR* pReqXml, CXml& xml, std::string& strDomainCode)
{
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Get Alarm LinkageList GetXml","Parse DevCode failed");
		return iRet;
	}
...
IVS_INT32 CAlarmLinkagrXMLProcess::AddDomainCode4XML(const char* pDomainCode, const IVS_CHAR* pDomainCodePath, const IVS_CHAR* pValueName, const IVS_CHAR* pReqXml, CXml& xml)
{
	...
	if (NULL == pXmlDoc)
	{
		...
		pXmlDoc = NULL;
		delete(pDomainCodeElem);
		return IVS_FAIL; //lint !e429 !e438 pXmlText在释放pDomainCodeElem时一起释放
	}
	...
	{		
		...
		if (strcmp(pDomainCodePath, "Content") == 0)
		{
			...
			{
				pContent = CToolsHelp::FindElementByPath(pXmlDoc->RootElement()->Parent(),"Content");//lint !e64 匹配
			}
			if (NULL == pContent)
			{
		...
		{
			TiXmlElement* pElem = CToolsHelp::FindElementByPath(pXmlDoc->RootElement()->Parent(),pDomainCodePath);//lint !e64 匹配
