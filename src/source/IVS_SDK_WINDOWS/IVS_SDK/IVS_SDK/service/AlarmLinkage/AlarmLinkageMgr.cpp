...
IVS_INT32 CAlarmLinkageMgr::AddAlarmLinkage(const IVS_CHAR* pReqXml,IVS_CHAR** pRspXml)
{
	...
	eSDK_MEMSET(pResult, 0x0, strpRsp.size() + 1);
	if (!CToolsHelp::Memcpy(pResult, strpRsp.size() + 1, (const void *)strpRsp.c_str(), strpRsp.size()))
	{
		IVS_DELETE(pResult, MUILI);
		BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Memcpy rsp fail", "NA");
...
IVS_INT32 CAlarmLinkageMgr::GetAlarmlinkageList(const IVS_CHAR* pReqXml, IVS_CHAR** pRspXml)
{
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Get Alarm Linkage List","Get Alarm LinkageList GetXml failed");
		return iRet;
	}
	IVS_UINT32 xmlLen = 0;
	const IVS_CHAR* pReq = xmlReq.GetXMLStream(xmlLen);
	...
	{
		BP_RUN_LOG_ERR(IVS_SMU_XML_INVALID, "xml parse failed", "NA");
...
IVS_INT32 CAlarmLinkageMgr::GetAlarmLinkageAction(const IVS_CHAR* pReqXml, IVS_CHAR** pRspXml)
{
	...
	const IVS_CHAR *pAlarmInCode = xmlModified.GetElemValue();
	if (NULL == pAlarmInCode)
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "AlarmInCode is NULL", "NA");
		return IVS_XML_INVALID;
	}
	std::string strAlarmInCode = pAlarmInCode;
	IVS_CHAR szAlarmInCode[IVS_DEV_CODE_LEN + 1] = {0};
	IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
	IVS_INT32 iRet = CXmlProcess::ParseDevCode(strAlarmInCode.c_str(), szAlarmInCode, szDomainCode);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Get Alarm Linkage Action fail","Parse DevCode failed");
		return iRet;
	}
	...
	if (xmlRsp.FindElemEx("Content/ActionList"))
	{
		...
		do 
		{
			...
			do 
			{
				...
				{
					strDevDomainCode = szDomainCode;//lint !e63
				}
