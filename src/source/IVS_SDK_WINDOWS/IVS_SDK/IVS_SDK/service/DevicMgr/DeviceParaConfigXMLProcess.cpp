...
IVS_INT32 CDeviceParaConfigXMLProcess::GetMainDevConfigParseXml(IVS_DEVICE_CFG* pMainDevCfg, CXml& xml)
{
    ...
    IVS_INT32 iTemp;
    GET_ELEM_VALUE_CHAR("DevCode", szElemValue, pMainDevCfg->stDeviceInfo.cCode, IVS_DEV_CODE_LEN, xml);//lint !e838
    ...
    GET_ELEM_VALUE_CHAR("NVRCode", szElemValue, pMainDevCfg->cNvrCode, IVS_NVR_CODE_LEN, xml);
    GET_ELEM_VALUE_CHAR("DCGCode", szElemValue, pMainDevCfg->cTasCode, IVS_TAS_CODE_LEN, xml);
...
IVS_INT32 CDeviceParaConfigXMLProcess::ParseCameraBriefInfoRsp(CXml& xml,IVS_CAMERA_CFG* pCameraCfg)
{
	...
	// 解析摄像机编码ParentCode
	GET_ELEM_VALUE_CHAR("CameraCode",szElemValue,pCameraCfg->cCameraCode,IVS_DEV_CODE_LEN,xml);//lint !e838
	// 解析父设备编码ParentCode
	...
	else if(0 == strcmp(tempChar,"BULLET"))
	{
		pCameraCfg->uiCameraType = CAMERA_BULLET;
	}
	else 
	{
		pCameraCfg->uiCameraType = CAMERA_FIX;
		BP_RUN_LOG_WAR("Parse CameraBriefInfoRsp fail", "Error CameraType:[%s]",tempChar);
	}
...
IVS_INT32 CDeviceParaConfigXMLProcess::ParseCameraPTZCfgRsp(CXml& xml,IVS_PTZ_CFG* pCameraPTZCfg, const IVS_CHAR* pDomainCode)
{
	...
	(void)CToolsHelp::Memcpy(pCameraPTZCfg->cSerialCode, IVS_DEV_SERIALNO_LEN, (const void *)strSerialCode.c_str(), IVS_DEV_SERIALNO_LEN);

	//其他值为默认
	eSDK_MEMSET(pCameraPTZCfg->cReserve,0,32);
...
IVS_INT32 CDeviceParaConfigXMLProcess::ParseAudioCfgRsp(CXml& xml,IVS_AUDIO_CFG* pAudioCfg)
{
	...
	// 解析出VoiceName
	GET_ELEM_VALUE_CHAR("VoiceName",szElemValue,pAudioCfg->cAudioName,IVS_AUDIO_NAME_LEN,xml);//lint !e838
	...
	// 其他值为默认
	eSDK_MEMSET(pAudioCfg->cReserve,0,32);
	eSDK_MEMSET(pAudioCfg->stAudioParam.cReserve,0,32);
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetMainParaConfigGetXml(const IVS_CHAR* pDomainCode, 
    const IVS_CHAR* pDevCode, const IVS_DEVICE_CFG* pDevCfg, CXml& xmlTemp)
{
    CHECK_POINTER(pDomainCode, IVS_OPERATE_MEMORY_ERROR);  
    ...
    (void)xmlTemp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)pDevCfg->iLoginType).c_str());//lint !e64 匹配
    // 主设备提供商类型
    ...
    (void)xmlTemp.SetElemValue(CToolsHelp::Int2Str(pDevCfg->bEnableAlarm).c_str());//lint !e64 匹配
    // 视频制式
    ...
    (void)xmlTemp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)pDevCfg->uiDirectConnectFirst).c_str());//lint !e64 匹配
    // 设备所属NVR编码
    ...
    (void)xmlTemp.SetElemValue(CToolsHelp::Int2Str(pDevCfg->bEnableSchedule).c_str()); //lint !e64 匹配
    xmlTemp.OutOfElem();
	...
	CXml& xmlTemp )
{
	...
	case CAMERA_BULLET:
		{
			(void)xmlTemp.SetElemValue("BULLET");
			break;
		}
	default:
		{
	...
	(void)xmlTemp.IntoElem();
	(void)xmlTemp.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraCfg->stBroadcast.uiPort)).c_str());//lint !e64 匹配

	(void)xmlTemp.AddElem("BroadcastTTL");
	(void)xmlTemp.IntoElem();
	(void)xmlTemp.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraCfg->stBroadcast.uiTTL)).c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::ParseCameraDispalyCfgRsp(CXml& xml,IVS_CAMERA_DISPLAY_CFG* pCameraBriefInfo)
{
	...
	IVS_INT32 iTemp;
	// 对比度
	GET_ELEM_VALUE_NUM("Contrast", szElemValue,iTemp, xml);//lint !e838
	pCameraBriefInfo->uiContrast = static_cast<IVS_UINT32>(iTemp);

	// 亮度
	GET_ELEM_VALUE_NUM("Bright", szElemValue,iTemp, xml);//lint !e838
	pCameraBriefInfo->uiBrightness = static_cast<IVS_UINT32>(iTemp);

	// 色度
	GET_ELEM_VALUE_NUM("Hue", szElemValue,iTemp, xml);//lint !e838
	pCameraBriefInfo->uiHue = static_cast<IVS_UINT32>(iTemp);

	// 饱和度
	GET_ELEM_VALUE_NUM("Saturation", szElemValue,iTemp, xml);//lint !e838
	...
	CXml& xml )
{
	...
	{	
		...
		{		
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraBasicImgCfg->uiContrast)).c_str());//lint !e64 匹配

			(void)xml.AddElem("Bright");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraBasicImgCfg->uiBrightness)).c_str());//lint !e64 匹配

			(void)xml.AddElem("Hue");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraBasicImgCfg->uiHue)).c_str());//lint !e64 匹配

			(void)xml.AddElem("Saturation");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraBasicImgCfg->uiSaturation)).c_str());//lint !e64 匹配
			xml.OutOfElem();
		}
		xml.OutOfElem();
	}	
	...
	CXml& reqXml )
{
	...
	(void)reqXml.AddElem("PTZInfo");
	{
		...
		(void)reqXml.IntoElem();
		(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraBasicImgCfg->uiDecoderType)).c_str());//lint !e64 匹配

		(void)reqXml.AddElem("DecoderAddress");
		(void)reqXml.IntoElem();
		(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraBasicImgCfg->uiDecoderAddress)).c_str());//lint !e64 匹配
	...
	CXml& reqXml )
{
	...
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiPitch)).c_str());//lint !e64 匹配

	// Volume
	(void)reqXml.AddElem("Volume");
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiVolume)).c_str());//lint !e64 匹配
			
	// InputType
	(void)reqXml.AddElem("InputType");
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiInputType)).c_str());//lint !e64 匹配
	...
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiSampleRate)).c_str());//lint !e64 匹配
			
	// Channel
	(void)reqXml.AddElem("Channel");
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiChannel)).c_str());//lint !e64 匹配
			
	// Gain
	(void)reqXml.AddElem("Gain");
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiGain)).c_str());//lint !e64 匹配

	// SampleBits
	(void)reqXml.AddElem("SampleBits");
	(void)reqXml.IntoElem();
	(void)reqXml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAudioCfg->stAudioParam.uiSampleBits)).c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::CameraStreamCfgParseXML(IVS_CAMERA_STREAM_CFG* pCameraStrCfg, CXml &xml)
{
	...
	IVS_INT32 iTemp = 0;
	GET_ELEM_VALUE_NUM("StreamNum", pTemp, iTemp, xml);//lint !e838
	...
	do 
	{
		...
		if (1 == pCameraStrCfg->stStreamInfo[i].uiEncodeType || 
			VIDEO_DEC_SVAC == pCameraStrCfg->stStreamInfo[i].uiEncodeType)
		{
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraStreamCfgGetXML(const IVS_CAMERA_STREAM_CFG* pCameraStrCfg, const IVS_CHAR* pDomainCode, const IVS_CHAR* pDevCode, CXml &xml)
{
	...
	(void)xml.AddElem("VideoInChanInfo");
	{
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->uiStreamInfoNum)).c_str());//lint !e64 匹配

		(void)xml.AddElem("StreamList");
		(void)xml.IntoElem();

		for(IVS_UINT32 i = 0;i < pCameraStrCfg->uiStreamInfoNum; i++){
			i == 0 ? (void)xml.AddChildElem("StreamInfo"):(void)xml.AddElem("StreamInfo");
			(void)xml.IntoElem();
			(void)xml.AddChildElem("StreamType");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiStreamType)).c_str());//lint !e64 匹配
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiPicQuality)).c_str());//lint !e64 匹配
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiBitRate)).c_str());//lint !e64 匹配
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiFrameRate)).c_str());//lint !e64 匹配
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiFrameInterval)).c_str());//lint !e64 匹配

			(void)xml.AddElem("TransProtocol");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiTransProtocol)).c_str());//lint !e64 匹配

			(void)xml.AddElem("PKTProtocol");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfg->stStreamInfo[i].uiPktProtocol)).c_str());//lint !e64 匹配
			xml.OutOfElem();
		}
		xml.OutOfElem();
		xml.OutOfElem();
	...
	CXml &xml)
{
	...
	(void)xml.AddElem("VideoInChanInfo");
	{
		...
		(void)xml.AddElem("StreamNum");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->uiStreamInfoNum)).c_str());//lint !e64 匹配

		(void)xml.AddElem("StreamList");
		(void)xml.IntoElem();

		for(IVS_UINT32 i = 0;i < pCameraStrCfgEx->uiStreamInfoNum; i++){
			i == 0 ? (void)xml.AddChildElem("StreamInfo"):(void)xml.AddElem("StreamInfo");
			(void)xml.IntoElem();
			(void)xml.AddChildElem("StreamType");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiStreamType)).c_str());//lint !e64 匹配
			...
			(void)xml.AddElem("PicQuality");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiPicQuality)).c_str());//lint !e64 匹配
			...
			(void)xml.AddElem("BitRate");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiBitRate)).c_str());//lint !e64 匹配
			...
			(void)xml.AddElem("FrameRate");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiFrameRate)).c_str());//lint !e64 匹配

			(void)xml.AddElem("EventRecordAdapt");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiEventRecordAdapt)).c_str());//lint !e64 匹配

			(void)xml.AddElem("EventBitRate");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiEventBitRate)).c_str());//lint !e64 匹配

			(void)xml.AddElem("EventFrameRate");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiEventFrameRate)).c_str());//lint !e64 匹配
			...
			(void)xml.AddElem("IframeInterval");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiFrameInterval)).c_str());//lint !e64 匹配

			(void)xml.AddElem("TransProtocol");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiTransProtocol)).c_str());//lint !e64 匹配

			(void)xml.AddElem("PKTProtocol");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraStrCfgEx->stStreamInfo[i].uiPktProtocol)).c_str());//lint !e64 匹配
			xml.OutOfElem();
		}
		xml.OutOfElem();
		xml.OutOfElem();
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetOSDParaParseXML(IVS_CAMERA_OSD_CFG* pOSDConfig, CXml& xml)
{
	...
	if (xml.FindElem("OSDTimeInfo"))
	{
		...
		pOSDConfig->stOSDTime.bEnableOSDTime = static_cast<IVS_BOOL>(CXmlProcess::TransOnOffToInt(szValue));//lint !e64 匹配
	...
	if (xml.FindElem("OSDNameInfo"))
	{
		...
		// 是否显示场名 ON/OFF
		eSDK_MEMSET(szValue, 0, MAXLEN);
		GET_ELEM_VALUE_CHAR("OSDNameEnable",pTemp,szValue,MAXLEN,xml);
		pOSDConfig->stOSDName.bEnableOSDName = static_cast<IVS_BOOL>(CXmlProcess::TransOnOffToInt(szValue));//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetAlarmInCfgGetXML(const IVS_ALARM_IN_CFG* pAlarmInCfg, const IVS_CHAR* pDomainCode, const IVS_CHAR* pDevCode, CXml &xml)
{
	...
	(void)xml.AddElem("AlarmInChanInfo");
	{
		...
		eSDK_MEMSET(cAlarmInName, 0x0, IVS_ALARM_IN_NAME_LEN + 1);
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmInCfg->uiAutoClearAlarmTime)).c_str());//lint !e64 匹配
		xml.OutOfElem();
	}
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetAlarmInCfgParseXML(IVS_ALARM_IN_CFG* pAlarmInCfg, CXml &xml)
{
	...
	const char* pTemp = NULL;
	{
		GET_ELEM_VALUE_CHAR("AlarmInName", pTemp, pAlarmInCfg->cAlarmInName, IVS_ALARM_IN_NAME_LEN, xml);//lint !e838
		GET_ELEM_VALUE_NUM("AlarmInChanIndex", pTemp, iTemp, xml);//lint !e838
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetAlarmOutCfgGetXML(const IVS_ALARM_OUT_CFG* pAlarmOutCfg, const IVS_CHAR* pDomainCode, const IVS_CHAR* pDevCode, CXml &xml)
{
	...
	(void)xml.AddElem("AlarmOutChanInfo");
	{
		...
		eSDK_MEMSET(cAlarmOutName, 0x0, IVS_ALARM_OUT_NAME_LEN + 1);
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmOutCfg->uiHoldTime)).c_str());//lint !e64 匹配
		xml.OutOfElem();
	}
	xml.OutOfElem();
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetAlarmOutCfgParseXML(IVS_ALARM_OUT_CFG* pAlarmOutCfg, CXml &xml)
{
	...
	const char* pTemp = NULL;
	IVS_INT32 iTemp = 0;
	GET_ELEM_VALUE_CHAR("AlarmOutName", pTemp, pAlarmOutCfg->cAlarmOutName, IVS_ALARM_OUT_NAME_LEN, xml);//lint !e838
	GET_ELEM_VALUE_NUM("AlarmOutChanIndex", pTemp, iTemp, xml);//lint !e838
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetVideoMaskConfigParseXml(IVS_VIDEO_MASK* pVideoMask, CXml& xml)
{
	...
	GET_ELEM_VALUE_CHAR("VideoMaskEnable", szElemValue, chTemp, maxLen, xml);//lint !e838
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraOsgCfgGetXML(const IVS_CHAR* pDomainCode, const IVS_CHAR* pDevCode, const IVS_CAMERA_OSD_CFG* pCameraOsgCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	{
		(void)xml.AddChildElem("OSDTimeInfo");
		{
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraOsgCfg->stOSDTime.uiTimeFormat)).c_str());//lint !e64 匹配
		...
		(void)xml.AddElem("OSDNameInfo");
		{
			...
			eSDK_MEMSET(cOsgNameText, 0x0, IVS_MAX_OSD_TEXT_LEN + 1);
			...
			(void)xml.IntoElem();
			IVS_CHAR cRight[STR_LEN] = {0};
			//modify by zwx211831, Date: 20150129, 应指明文字区域的左上角坐标，所以这里应该是top，而不是right
			if (!CToolsHelp::Snprintf(cRight, STR_LEN, "%f", pCameraOsgCfg->stOSDName.rectText.top))
			{
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraOsgCfg->stOSDName.bEnableSwitch).c_str());//lint !e64 匹配

			(void)xml.AddElem("SwitchInterval");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraOsgCfg->stOSDName.uiSwitchInterval)).c_str());//lint !e64 匹配

			(void)xml.AddElem("TextBlinkEnable");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraOsgCfg->stOSDName.bEnableTextBlink).c_str());//lint !e64 匹配

			(void)xml.AddElem("TextTranslucentEnable");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraOsgCfg->stOSDName.bEnableTextTranslucent).c_str());//lint !e64 匹配
	...
	CXml &xml)
{
	...
	(void)xml.AddChildElem("DomainCode");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(strDomainCode.c_str());//lint !e64 匹配

	(void)xml.AddElem("CameraCode");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(strDevCode.c_str());//lint !e64 匹配
	...
	(void)xml.IntoElem();
	{
		...
		for(IVS_UINT32 uiTimes = 0; uiTimes < uiOSDTimeNum; uiTimes++)
		{
			...
			(void)xml.AddElem("TimeFormatType");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pOSDTime.uiTimeFormat)).c_str());//lint !e64 匹配
			...
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Snprintf fail", "Source string is %s", pOSDTime.fTimeX);
				return IVS_ALLOC_MEMORY_ERROR;
			}
			(void)xml.SetElemValue(cTimeX);
			...
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Snprintf fail", "Source string is %s", pOSDTime.fTimeY);
				return IVS_ALLOC_MEMORY_ERROR;
			}
			(void)xml.SetElemValue(cTimeY);
...

	{
		...
		for(IVS_UINT32 uiTimes = 0; uiTimes < uiOSDNameNum; uiTimes++)
		{
			...
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Memcpy fail", "Source string is %s", pOSDName.cOSDNameText);
				return IVS_ALLOC_MEMORY_ERROR;
			}
			(void)xml.SetElemValue(cOsgNameText);
			...
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Snprintf fail", "Source string is %s", pOSDName.rectText.left);
				return IVS_ALLOC_MEMORY_ERROR;
			}
			(void)xml.SetElemValue(cLeft);
			...
			IVS_CHAR cRight[STR_LEN] = {0};
			//modify by zwx211831, Date: 20150129, 应指明文字区域的左上角坐标，所以这里应该是top，而不是right
			if (!CToolsHelp::Snprintf(cRight, STR_LEN, "%f", pOSDName.rectText.top))
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Snprintf fail", "Source string is %s", pOSDName.rectText.right);
				return IVS_ALLOC_MEMORY_ERROR;
			}
			(void)xml.SetElemValue(cRight);
			...
			(void)xml.AddElem("SwitchEnable");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(pOSDName.bEnableSwitch).c_str());//lint !e64 匹配

			(void)xml.AddElem("SwitchInterval");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pOSDName.uiSwitchInterval)).c_str());//lint !e64 匹配

			(void)xml.AddElem("TextBlinkEnable");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(pOSDName.bEnableTextBlink).c_str());//lint !e64 匹配

			(void)xml.AddElem("TextTranslucentEnable");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(pOSDName.bEnableTextTranslucent).c_str());//lint !e64 匹配
	...
	CXml& xml)
{
	...
	do 
	{
		if (IVS_CAMERA_OSD_NUM_MAX == uiOSDTimeNum)
		{
			BP_RUN_LOG_WAR("Set Camera MutlOSDCfg ParseXML", "uiOSDTimeNum is more than MaxNum, Ignore unnecessary");
			break;
		}

		eSDK_MEMSET(&stOSDTime, 0, sizeof(IVS_OSD_TIME));
		...
		// 是否显示时间，ON/OFF
		GET_ELEM_VALUE_CHAR("OSDTimeEnable",pTemp,szValue,MAXLEN,xml);
		stOSDTime.bEnableOSDTime = static_cast<IVS_BOOL>(CXmlProcess::TransOnOffToInt(szValue));//lint !e64 匹配
		// 时间格式：1、2009-09-09 09:09:09；2、2009年09月09日 09 :09 :09；3、UTC时间
		...
		pMultOSDConfig->stCameraOSDCfg[uiOSDTimeNum].stOSDTime = stOSDTime;//lint !e661
		uiOSDTimeNum++;
		xml.OutOfElem();
	...
	do 
	{
		if (IVS_CAMERA_OSD_NUM_MAX == uiOSDNameNum)
		{
			BP_RUN_LOG_WAR("Set Camera MutlOSDCfg ParseXML", "uiOSDNameNum is more than MaxNum, Ignore unnecessary");
			break;
		}

		eSDK_MEMSET(&stOSDName, 0, sizeof(IVS_OSD_NAME));
		...
		eSDK_MEMSET(szValue, 0, MAXLEN);
		GET_ELEM_VALUE_CHAR("OSDNameEnable",pTemp,szValue,MAXLEN,xml);
		stOSDName.bEnableOSDName = static_cast<IVS_BOOL>(CXmlProcess::TransOnOffToInt(szValue));//lint !e64 匹配
		// 场名内容，键盘可见字符和中文
		...
		pMultOSDConfig->stCameraOSDCfg[uiOSDNameNum].stOSDName = stOSDName;//lint !e661
		uiOSDNameNum++;
		xml.OutOfElem();
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetVideoHideAlarmGetXML(const IVS_CHAR* pDomainCode, const IVS_CHAR* pDevCode, const IVS_VIDEO_HIDE_ALARM* pVideoHideAlarm, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pVideoHideAlarm->uiSensitivity)).c_str());//lint !e64 匹配

	(void)xml.AddElem("AutoClearAlarmTime");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pVideoHideAlarm->uiAutoClearAlarmTime)).c_str());//lint !e64 匹配
	...
	for (IVS_UINT32 i = 0;i < pVideoHideAlarm->uiGuardTimeNum; i++)
	{
		...
		(void)xml.IntoElem();
		{
			...
			eSDK_MEMSET(cStartTime, 0x0, IVS_TIME_LEN + 1);
			...
			eSDK_MEMSET(cEndTime, 0x0, IVS_TIME_LEN + 1);
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetVideoHideAlarmParseXML(IVS_VIDEO_HIDE_ALARM* pVideoHideAlarm, CXml &xml)
{
	...
	const char* pTemp = NULL;
	IVS_INT32 iTemp = 0;
	GET_ELEM_VALUE_NUM("Sensitivity", pTemp, iTemp, xml);//lint !e838
...
IVS_INT32 CDeviceParaConfigXMLProcess::ParseMotionDetecXML(CXml& xml, IVS_MOTION_DETECTION* pMotionDetection)
{
	...
	eSDK_MEMSET(szTemp, 0x0, EABLE_LENGTH);
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetMotionDetecGetXML(const IVS_CHAR* pCameraCode, const IVS_CHAR* pDomainCode, const IVS_MOTION_DETECTION* pMotionDetection, CXml& xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pMotionDetection->uiDetectInterval)).c_str());//lint !e64 匹配

	(void)xml.AddElem("Sensitivity");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pMotionDetection->uiSensitivity)).c_str());//lint !e64 匹配

	(void)xml.AddElem("AutoClearAlarmTime");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pMotionDetection->uiAutoClearAlarmTime)).c_str());//lint !e64 匹配
	...
	(void)xml.SetElemValue(strEnableMotionDetec.c_str());//lint !e64 匹配
	...
	(void)xml.SetElemValue(strPlatformMotionDetection.c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetDevNetCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode, const IVS_DEVICE_NET_CFG* pDevNetCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	{
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pDevNetCfg->stPUIp.uiIPType)).c_str());//lint !e64 匹配

		(void)xml.AddElem("DevIP");
		(void)xml.IntoElem();
		IVS_CHAR cDevIp[IVS_IP_LEN + 1];
		eSDK_MEMSET(cDevIp, 0x0, IVS_IP_LEN + 1);
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pDevNetCfg->uiPUPort)).c_str());//lint !e64 匹配

		(void)xml.AddElem("DevGW");
		(void)xml.IntoElem();
		IVS_CHAR cDevGw[IVS_IP_LEN + 1];
		eSDK_MEMSET(cDevGw, 0x0, IVS_IP_LEN + 1);
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pDevNetCfg->uiMaxMtu)).c_str());//lint !e64 匹配

		(void)xml.AddElem("SubNetMask");
		(void)xml.IntoElem();
		IVS_CHAR cSubNetMask[IVS_IP_LEN + 1];
		eSDK_MEMSET(cSubNetMask, 0x0, IVS_IP_LEN + 1);
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetDevTimeCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode, const IVS_DEVICE_TIME_CFG* pDevTimeCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	{
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pDevTimeCfg->stNTP.stNtpServerIP.uiIPType)).c_str());//lint !e64 匹配

		(void)xml.AddElem("NTPServerIP");
		(void)xml.IntoElem();
		IVS_CHAR cNtpServerIp[IVS_IP_LEN + 1];
		eSDK_MEMSET(cNtpServerIp, 0x0, IVS_IP_LEN + 1);
		...
		eSDK_MEMSET(cNtpDns, 0x0, IVS_IP_LEN + 1);
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(pDevTimeCfg->iTimezone).c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetDevTimeCfgParseXML(IVS_DEVICE_TIME_CFG* pDevTimeCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	{
		...
		eSDK_MEMSET(cTimeType, 0x0, DEV_TIME_CFG_LEN);
		GET_ELEM_VALUE_CHAR("TimeType", pTemp, cTimeType, DEV_TIME_CFG_LEN, xml);//lint !e838
		...
		eSDK_MEMSET(cNtpType, 0x0, DEV_TIME_CFG_LEN);
		...
		eSDK_MEMSET(cDevDtsType, 0x0, DEV_TIME_CFG_LEN);
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetSerialChannelCfgGetXML(const IVS_CHAR* pSerialCode, const IVS_CHAR* pDomainCode, const IVS_SERIAL_CHANNEL_CFG* pSerialChannelCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSerialChannelCfg->uiBaudRate)).c_str());//lint !e64 匹配

	(void)xml.AddElem("DataBit");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSerialChannelCfg->uiDataBit)).c_str());//lint !e64 匹配

	(void)xml.AddElem("Parity");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSerialChannelCfg->uiParity)).c_str());//lint !e64 匹配

	(void)xml.AddElem("StopBit");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSerialChannelCfg->uiStopBit)).c_str());//lint !e64 匹配

	(void)xml.AddElem("FlowControl");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pSerialChannelCfg->uiFlowControl)).c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetSerialChannelCfgParseXML(IVS_SERIAL_CHANNEL_CFG* pSerialChannelCfg, CXml &xml)
{
	...
	const char* pTemp = NULL;
	(void)xml.IntoElem();
	GET_ELEM_VALUE_CHAR("SerialName", pTemp, pSerialChannelCfg->cSerialName, IVS_SERIAL_NAME_LEN, xml);//lint !e838
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraImgCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode, const IVS_CAMERA_IMAGING_CFG* pCameraImgCfg, CXml &xml)
{
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stBacklight.uiBacklightLevel)).c_str());//lint !e64 匹配
	xml.OutOfElem();
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stExposure.uiExposureTime)).c_str());//lint !e64 匹配
	// 曝光增益
	(void)xml.AddElem("ExposureGain");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stExposure.uiExposureGain)).c_str());//lint !e64 匹配
	// 光圈
	(void)xml.AddElem("ExposureIris");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stExposure.uiExposureIris)).c_str());//lint !e64 匹配
	xml.OutOfElem();
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stFocus.uiFocusSpeed)).c_str());//lint !e64 匹配
	// 最小变焦范围
	(void)xml.AddElem("NearLimit");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stFocus.uiNearLimit)).c_str());//lint !e64 匹配
	// 最大变焦范围
	(void)xml.AddElem("FarLimit");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stFocus.uiFarLimit)).c_str());//lint !e64 匹配
	xml.OutOfElem();
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stWideDynamic.uiWideDynamicLevel)).c_str());//lint !e64 匹配
	xml.OutOfElem();

	// 锐度
	(void)xml.AddElem("Sharpness");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->uiSharpness)).c_str());//lint !e64 匹配
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stWhiteBalance.uiCrGain)).c_str());//lint !e64 匹配
	// 蓝色增益
	(void)xml.AddElem("CbGain");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stWhiteBalance.uiCbGain)).c_str());//lint !e64 匹配
	xml.OutOfElem();
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraImgCfg->stStabilization.uiStabilizationLevel)).c_str());//lint !e64 匹配
	xml.OutOfElem();
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraExCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode, const IVS_CAMERA_EXTEND_CFG* pCameraExCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraExCfg->bEnableWatermark).c_str());//lint !e64 匹配

	(void)xml.AddElem("BandwidthAdaptive");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraExCfg->bEnableBandwidthAdaptive).c_str());//lint !e64 匹配

	(void)xml.AddElem("Cache");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraExCfg->bEnableCache).c_str());//lint !e64 匹配

	(void)xml.AddElem("MediaSecurityInfo");
	(void)xml.IntoElem();
	(void)xml.AddChildElem("MediaSecurityEnable");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(pCameraExCfg->stMediaSecurityCfg.bEnableMediaSecurity).c_str());//lint !e64 匹配

	(void)xml.AddElem("MediaSecurityAlgorithm");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraExCfg->stMediaSecurityCfg.uiMediaSecurityAlgorithm)).c_str());//lint !e64 匹配
	xml.OutOfElem();
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraSnapshotCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode, const IVS_CAMERA_SNAPSHOT_CFG* pCameraSnapshotCfg, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraSnapshotCfg->uiSnapCount)).c_str());//lint !e64 匹配

	(void)xml.AddElem("SnapInterval");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraSnapshotCfg->uiSnapInterval)).c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraRecordStreamCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode,IVS_UINT32 uiStreamType, CXml &xml)
{
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiStreamType)).c_str());//lint !e64 匹配
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetCameraRecordStreamCfgParseXML(IVS_UINT32* pStreamType, CXml &xml)
{
	...
	GET_ELEM_VALUE_NUM_FOR_UINT("StreamID", pTemp, ulpStreamType, xml);//lint !e838
...
//	if (RELIABLE_TRANSMISSION_FEC == pCameraFEC->uiReliableTransmission || RELIABLE_TRANSMISSION_SEC == pCameraFEC->uiReliableTransmission)
//	{
	 	(void)xml.AddElem("DataDropPercent");
	 	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraFEC->uiDataDropPercent)).c_str());//lint !e64 匹配
//	}
...
IVS_INT32 CDeviceParaConfigXMLProcess::GetCameraFECCfgParseXML(IVS_CAMERA_FEC_CFG* pCameraFEC, CXml &xml)
{
	...
	if (!xml.FindElemEx("Content/FECInfoOptions"))			//增加SEC，节点修改为FECInfoOptions
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "Get CameraFECCfg ParseXML fail", "xml.FindElem(FECInfoOptions) fail");
		return IVS_XML_INVALID;
	}
	...
	GET_ELEM_VALUE_CHAR("ReliableTransmission", pTemp, cReliableTransmission, CAMERA_EX_LEN, xml);//lint !e838
...
// 摄像机SEC参数拼装xml
IVS_INT32 CDeviceParaConfigXMLProcess::SetCameraSECCfgGetXML(const IVS_CHAR* pDevCode, const IVS_CHAR* pDomainCode, const IVS_CAMERA_SEC_CFG* pCameraSEC, CXml &xml)
{
	CHECK_POINTER(pDevCode, IVS_PARA_INVALID);
	CHECK_POINTER(pDomainCode, IVS_PARA_INVALID);
	CHECK_POINTER(pCameraSEC, IVS_PARA_INVALID);
	...
	(void)xml.AddChildElem("ReliableTransmission");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CDevEnum::GetReliableTransTypeCHAR(static_cast<IVS_INT32>(pCameraSEC->uiReliableTransmission)));


	(void)xml.AddElem("DataDropPercent");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pCameraSEC->uiDataDropPercent)).c_str());//lint !e64 匹配
...
// 摄像机SEC参数设置解析xml
IVS_INT32 CDeviceParaConfigXMLProcess::GetCameraSECCfgParseXML(IVS_CAMERA_SEC_CFG* pCameraSEC, CXml &xml)
{
	CHECK_POINTER(pCameraSEC, IVS_PARA_INVALID);
	IVS_DEBUG_TRACE("");
	...
	if (!xml.FindElemEx("Content/FECInfoOptions"))			//增加SEC，节点修改为FECInfoOptions
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "Get CameraSECCfg ParseXML fail", "xml.FindElem(FECInfoOptions) fail");
		return IVS_XML_INVALID;
	}
	...
	IVS_UINT32 i = 0;
	bool first_reliable_flag = true;
	bool first_droppercent_flag = true;

	do 
	{
		element_name = xml.GetElem();
		element_value = xml.GetElemValue();
		if (NULL == element_name || NULL == element_value)
		{
			return IVS_XML_INVALID;
		}

		if (0 == strncmp("ReliableTransmission" , element_name, strlen("ReliableTransmission"))) 
		{
			if (first_reliable_flag) 
			{
				pCameraSEC->uiReliableTransmission = static_cast<IVS_UINT32>(CDevEnum::GetReliableTransTypeINT(element_value));
				first_reliable_flag = false;
			}
			else
			{
				pCameraSEC->uiSelReliableTransmission[i] = static_cast<IVS_UINT32>(CDevEnum::GetReliableTransTypeINT(element_value));
			}
		}
		else if (0 == strncmp("DataDropPercent", element_name, strlen("DataDropPercent"))) 
		{
			if (first_droppercent_flag)
			{
				pCameraSEC->uiDataDropPercent = static_cast<IVS_UINT32>(atoi(element_value));
				first_droppercent_flag = false;
			}
			else
			{
				pCameraSEC->uiSelDataDropPercent[i] = static_cast<IVS_UINT32>(atoi(element_value));
				if (++i >= IVS_RELIABLE_TRANSMISSION_TYPE_NUM)
				{
					pCameraSEC->uiSelNum = IVS_RELIABLE_TRANSMISSION_TYPE_NUM;
					return IVS_SUCCEED;
				}
			}
		}else
		{
			BP_RUN_LOG_ERR(IVS_XML_INVALID, "Get CameraSECCfg ParseXML fail", "xml node[%s] fail", element_name);
			return IVS_XML_INVALID;
		}
	} while (xml.NextElem());

	pCameraSEC->uiSelNum = i;
