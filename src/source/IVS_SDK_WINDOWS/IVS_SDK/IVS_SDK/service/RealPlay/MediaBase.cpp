...
{
	/*01*/	{ RTSP_ERR_NOT_EXIST,		IVS_PLAYER_RET_RTSP_ERR_NOT_EXIST},		         //lint !e40 // 连接句柄不存在;
	/*02*/	{ RTSP_ERR_DISCONNECT,      IVS_PLAYER_RET_RTSP_ERR_DISCONNECT},	         //lint !e40 // 连接失败;
	/*03*/	{ RTSP_ERR_RESPONSE,        IVS_PLAYER_RET_RTSP_ERR_RESPONSE},		         //lint !e40 // 响应消息错误;
	/*04*/	{ RTSP_ERR_TIMEOUT,         IVS_PLAYER_RET_RTSP_ERR_TIMEOUT},		         //lint !e40 // 响应超时;
	/*05*/	{ RTSP_ERR_STAUTS_ABNORMAL, IVS_PLAYER_RET_RTSP_ERR_STAUTS_ABNORMAL},        //lint !e40 // 连接状态异常;
	/*06*/	{ RTSP_ERR_SEND_FAIL,       IVS_PLAYER_RET_RTSP_ERR_SEND_FAIL},		         //lint !e40 // 发送出错;
	/*07*/	{ RTSP_ERR_RECV_FAIL,       IVS_PLAYER_RET_RTSP_ERR_RECV_FAIL},		         //lint !e40 // 接收出错;
	/*08*/	{ RTSP_ERR_PARAMETER,       IVS_PLAYER_RET_RTSP_ERR_PARAMETER},		         //lint !e40 // 参数检查出错;
	/*09*/	{ RTSP_ERR_NORMAL_STOP,     IVS_PLAYER_RET_RTSP_ERR_NORMAL_STOP},	         //lint !e40 // 正常结束，ANNOUNCE正常结束请求;
	/*10*/	{ RTSP_ERR_ABNORMAL_STOP,   IVS_PLAYER_RET_RTSP_ERR_ABNORMAL_STOP},	         //lint !e40 // 异常结束，ANNOUNCE异常结束请求;
	/*11*/	{ RTSP_ERR_RET_SYS,         IVS_PLAYER_RET_RTSP_ERR_RET_SYS},		         //lint !e40 // 通用系统错误;
	/*12*/	{ RTSP_SERVER_INTERNAL,     IVS_PLAYER_RET_RTSP_ERR_SERVICE_500_RSP},		 //lint !e40 // 500错误;
	/*13*/	{ RTSP_SERVICE_UNAVAILABLE, IVS_PLAYER_RET_RTSP_ERR_SERVICE_UNAVAILABLE_RSP},//lint !e40 // 503错误;
	/*14*/  { RTSP_GONE,     IVS_PLAYER_RET_ERR_FILE_NOT_EXIST},		                 //lint !e40 // 410错误;
	/*15*/  { RTSP_SERVER_NOTIMPLEMENTED, IVS_DCG_DEV_UN_SUPPORT},		                 //lint !e40 // 501Not Implemented
	/*31*/  { RTSP_CLIENT_NOTENOUGH_BANDWIDTH,  IVS_PLAYER_RET_NOTENOUGH_BANDWIDTH},     //lint !e40 
	/*48*/  { RTSP_CLIENT_CPU_HIGH,     IVS_MU_NOTENOUGH_CPU_DILE},                      //lint !e40 
...
{//lint !e1927 !e1926
    ...
    eSDK_MEMSET(&m_stuMediaParam, 0, sizeof(IVS_MEDIA_PARA));
    ...
    eSDK_MEMSET(&m_stMediaRsp, 0, sizeof(ST_MEDIA_RSP));
    ...
    eSDK_MEMSET(m_szRemoteIp, 0, IVS_IP_LEN);
    m_uRemotePort = 0;
...
IVS_INT32 CMediaBase::GetPlayedTime(IVS_UINT32 uiChannel,IVS_UINT32 *puPlayTime) const
{
	...
	{
		*puPlayTime = (unsigned int)tick; //lint !e1924
		return IVS_SUCCEED;
	}
	return IVS_FAIL;
}
...
//从SMU/SCU获取媒体URL
IVS_INT32 CMediaBase::GetURL(const char * /*pszAuthHeader*/, CUserMgr *pUserMgr, ST_URL_MEDIA_INFO& stUrlMediaInfo, ST_MEDIA_RSP& stMediaRsp, bool& bAssociatedAudio, bool bPlaybackorDownload)
{
	IVS_DEBUG_TRACE("");
	...
	const CUserInfo* pUserInfo = pUserMgr->GetUserInfoEx();//lint !e838
	if (NULL == pUserInfo)
	{
	...
	IVS_INT32 iRet = pUserMgr->GetDomainRouteMgr().GetDomainRoutebyCode(stUrlMediaInfo.cDomainCode, stDomainRoute);//lint !e64 匹配
	if(IVS_SUCCEED != iRet)
	{
	...
	if(strlen(stUrlMediaInfo.szDstIP)==0)
	{
        ...
        if(!CToolsHelp::Strncpy(stUrlMediaInfo.szDstPort, PORT_LEN, strPort.c_str(),strPort.length()))//lint !e64 匹配
        {
    ...
    if (bPlaybackorDownload)
    {
        iRet = pUserMgr->SendMsg(cLocalDomainCode, NSS_GET_URL_REQ, pReq, strpRsp, stUrlMediaInfo.szSrcCode, -1, stUrlMediaInfo.szNVRCode);
    } 
    else
    {
        iRet = pUserMgr->SendMsg(cLocalDomainCode, NSS_GET_URL_REQ, pReq, strpRsp, stUrlMediaInfo.szSrcCode);
    }
...
IVS_INT32 CMediaBase::GetLocalIpPort()
{
	if (NULL != m_pUserMgr)
	{
		const CUserInfo *pUserInfo = NULL;
		pUserInfo = m_pUserMgr->GetUserInfoEx();//lint !e838 !e64 匹配
		if (NULL != pUserInfo)
		{
...
{
    m_iStartType = PLAY_TYPE_HWND;
    eSDK_MEMSET(m_szCameraID, 0, sizeof(CODE_LEN));
    eSDK_MEMSET(&m_stuMediaParam, 0, sizeof(IVS_MEDIA_PARA));
...
int CMediaBase::ReStartStream(const START_PLAY_PARAM& stParam)
{
...
#ifdef WIN32
	ResetEvent(m_hRtspSetupEvent);
#else
	VOS_ReSetEvent(m_hRtspSetupEvent);
#endif 
	m_bGetKeyIsSuccess = false;
	...
	//int iRet = RTSP_CLIENT_GetHandle(true, reinterpret_cast<int *>(&m_iRtspHandle)); //lint !e64
	...
	INET_ADDR stServiceAddr; //lint !e522 !e40 !e10
	eSDK_MEMSET(&stServiceAddr, 0x0, sizeof(INET_ADDR)); //lint !e40
	stServiceAddr.unAddrIp = htonl((unsigned int)inet_addr(m_stMediaRsp.szRTSPIP));        //lint !e732 !e63 !e1013 !e40 !e10 !e1924
	int iPort = CToolsHelp::StrToInt(m_stMediaRsp.szRTSPPORT);
	stServiceAddr.unAddrPort = (unsigned short)iPort;    //lint !e507 !e63 !e1013 !e40 !e10 !e1924

	MEDIA_ADDR stSourceMediaAddr; //lint !e522 !e40 !e10
	eSDK_MEMSET(&stSourceMediaAddr, 0x0, sizeof(MEDIA_ADDR)); //lint !e40
	...
	if (m_bReuseRtspConn)
	{
		iRet = RTSP_CLIENT_SetDataCallBack(m_iRtspHandle, stParam.cbRtspDataCallBack, this); //lint !e1013 !e40
		if (IVS_SUCCEED != iRet)
		{
			BP_RUN_LOG_ERR(IVS_FAIL,"ReStart RealPlay Stream", "Rtsp client set data callback fail, retcode=%d.", iRet);
		}
		iRet = RTSP_CLIENT_SendSetupMsg(m_iRtspHandle, &stServiceAddr, m_stMediaRsp.szURL, RTSP_CLIENT_TIMEOUT, true, NULL, NULL); //lint !e40 !e64 !e118 !e730 !e1786
	...
	else
	{
		...
		if (IVS_SUCCEED != iRet)
		{
			IVS_NETSOURCE_FreeChannel(m_ulNetSourceChannel); //lint !e64
			RTSP_CLIENT_FreeHandle(m_iRtspHandle);
            m_ulNetSourceChannel = 0;
			m_iRtspHandle = 0;
		...
		IVS_NETSOURCE_SetLocalRecvStreamIP(m_ulNetSourceChannel, const_cast<IVS_CHAR *>(m_strLocalIp.c_str()));
		...
		MEDIA_ADDR stLocalMediaAddr; //lint !e522 !e40 !e10
		eSDK_MEMSET(&stLocalMediaAddr, 0x0, sizeof(MEDIA_ADDR)); //lint !e40
		stLocalMediaAddr.unMediaIp = htonl((unsigned int)inet_addr(m_strLocalIp.c_str()));        //lint !e732 !e63 !e1013 !e40 !e10 !e1924
		stLocalMediaAddr.unMediaVideoPort = (unsigned short)uiLocalRecvStreamVideoRtpPort;        //lint !e63 !e1013 !e40 !e10 !e1924
		stLocalMediaAddr.unMediaAudioPort = (unsigned short)uiLocalRecvStreamAudioRtpPort;        //lint !e63 !e1013 !e40 !e10 !e1924

		if(PROTOCOL_RTP_OVER_TCP == m_stuMediaParam.ProtocolType)
		{
			iRet = RTSP_CLIENT_SendSetupMsg(m_iRtspHandle, &stServiceAddr, m_stMediaRsp.szURL, RTSP_CLIENT_TIMEOUT,  //lint !e40 !e64
				false, &stLocalMediaAddr, &stSourceMediaAddr);                                                       //lint !e40 !e118 !e730
		}
		else
		{
			iRet = RTSP_CLIENT_SendSetupMsg(m_iRtspHandle, &stServiceAddr, m_stMediaRsp.szURL, RTSP_CLIENT_TIMEOUT,  //lint !e40 !e64
		...
		stSourceMediaAddr.unMediaIp = ntohl(stSourceMediaAddr.unMediaIp);        //lint !e732 !e63 !e1013 !e40 !e10
		string strServiceIP = "";
		if (IVS_SUCCEED != AddrUint2Char(stSourceMediaAddr.unMediaIp, strServiceIP)) //lint !e1013 !e40 !e10
		{
			(void)IVS_NETSOURCE_CloseNetStream(m_ulNetSourceChannel);
		...
		iRet = IVS_NETSOURCE_OpenNetStream(m_ulNetSourceChannel, const_cast<IVS_CHAR *>(strServiceIP.c_str()), stSourceMediaAddr.unMediaVideoPort, stSourceMediaAddr.unMediaAudioPort);
		if (IVS_SUCCEED != iRet)
		{
			IVS_NETSOURCE_CloseNetStream(m_ulNetSourceChannel);
			RTSP_CLIENT_SendTeardownMsg(m_iRtspHandle, m_stMediaRsp.szURL, RTSP_CLIENT_TIMEOUT);
        ...
        (void)CToolsHelp::Strncpy(m_szRemoteIp, IVS_IP_LEN, strServiceIP.c_str(), strServiceIP.length());//lint !e64 匹配

        m_uLocalPort = stLocalMediaAddr.unMediaVideoPort;   //lint !e1013 !e40 !e10
        m_uRemotePort = stSourceMediaAddr.unMediaVideoPort; //lint !e1013 !e40 !e10
	}

	// 等待信号量，获取密钥不成功不发送Play消息
#ifdef WIN32
    DWORD ulRet = WaitForSingleObject(m_hRtspSetupEvent, 2000);
#else
    ULONG ulRet = VOS_WaitEvent(m_hRtspSetupEvent,2000);
#endif 
	BP_RUN_LOG_INF("ReStartStream", "WaitForSingleObject, ulRet:%08x", ulRet);
    if (!GetSecretFlag())
    {
        RTSP_CLIENT_SendTeardownMsg(m_iRtspHandle, m_stMediaRsp.szURL, RTSP_CLIENT_TIMEOUT);
	...
	iRet = RTSP_CLIENT_SendPlayMsg(m_iRtspHandle, m_stMediaRsp.szURL, (double)stParam.fSpeed, &stMdieaRange, RTSP_CLIENT_TIMEOUT); //lint !e40 !e1924
	if (IVS_SUCCEED != iRet)
	{
...
int CMediaBase::StartPlay(const START_PLAY_PARAM& stParam)
{
	...
	if (!m_hRtspSetupEvent)
	{
#ifdef WIN32
		m_hRtspSetupEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
#else
		VOS_CreateEvent(&m_hRtspSetupEvent);
#endif 
	}
 
	if (!m_hRtspSetupEvent)
	{
		BP_RUN_LOG_ERR(IVS_PLAYER_RET_ALLOC_MEM_ERROR,"Start RealPlay", "create sem fail");
   ...
   // int iRet = RTSP_CLIENT_GetHandle(true, reinterpret_cast<int *>(&m_iRtspHandle)); //lint !e64
    ...
    MEDIA_ADDR stSourceMediaAddr; //lint !e522 !e40 !e10
    eSDK_MEMSET(&stSourceMediaAddr, 0x0, sizeof(MEDIA_ADDR)); //lint !e40
	BP_RUN_LOG_INF("Start RealPlay", "IVS rtsp send setup message begin. Rtsp Handle = 0x%p.", m_iRtspHandle);
    ...
    else
    {
        ...
        m_uLocalPort = stLocalMediaAddr.unMediaAudioPort;   //lint !e1013 !e40 !e10
        m_uRemotePort = stSourceMediaAddr.unMediaAudioPort; //lint !e1013 !e40 !e10
    }
	BP_RUN_LOG_INF("Start RealPlay", "IVS rtsp send setup message end.");

    if (stParam.bBroadcast)
    {
        BP_DBG_LOG("StartPlay, Is Broad Cast, Not Need Wait.");
        iRet = ProcessRtspPlayMsg(m_stMediaRsp.szURL, stSourceMediaAddr, stParam);//lint !e119 !e40
        return iRet;
    }
    ...
    BP_RUN_LOG_INF("Start RealPlay", "WaitForSingleObject, ulRet:%08x", ulRet);
...
int CMediaBase::StartDownload(const START_PLAY_PARAM& stParam, const char *pCam, const IVS_DOWNLOAD_PARAM *pParam)
{
	...
	return ProcessRtspPlayMsgDownLoad(m_stMediaRsp.szURL, stSourceMediaAddr, stParam, pCam, pParam);//lint !e119 !e40
}
...
int CMediaBase::ProcessRtspPlayMsgDownLoad(const char* pszURL, const MEDIA_ADDR& /*stServerMediaAddr*/, const START_PLAY_PARAM& stParam, const char *pCam, const IVS_DOWNLOAD_PARAM *pParam)  //lint !e49 !e10 !e808
{//lint !e49
	...
	IVS_DEBUG_TRACE("");
	int iRet = IVS_PLAY_OpenStream(m_ulPlayerChannel, SERVICE_TYPE_DOWNLOAD, (IVS_UINT32)stParam.enSourceType); //lint !e1013 !e40 !e10 !e1924
	...
	//TODO fill
    RecordParam.uiDiskWarningValue = pParam->uiDiskWarningValue;  //lint !e1013 !e40 !e10
	RecordParam.uiNameRule         = pParam->uiNameRule;          //lint !e1013 !e40 !e10
	RecordParam.uiRecordFormat     = pParam->uiRecordFormat;      //lint !e1013 !e40 !e10
	RecordParam.uiRecordTime       = (unsigned int)-1;            //lint !e1924 禁用超时停止功能
	RecordParam.uiSplitterType     = pParam->uiSplitterType;     //lint !e1013 !e40 !e10
	RecordParam.uiSplitterValue    = pParam->uiSplitterValue;    //lint !e1013 !e40 !e10
	RecordParam.uiStopRecordValue  = pParam->uiStopRecordValue;  //lint !e1013 !e40 !e10
    
    RecordParam.bEncryptRecord     = pParam->bEncryptRecord;     //lint !e1013 !e40 !e10
	(void)CToolsHelp::Strncpy(RecordParam.cRecordPWD, sizeof(RecordParam.cRecordPWD) - 1,
		    pParam->cRecordPWD, strlen(pParam->cRecordPWD));     //lint !e1013 !e40 !e10
	(void)CToolsHelp::Strncpy(RecordParam.cSavePath, sizeof(RecordParam.cSavePath) - 1, 
		    pParam->cSavePath, strlen(pParam->cSavePath));       //lint !e1013 !e40 !e10


	const char *pCamName = NULL;
	IVS_CAMERA_BRIEF_INFO birefinfo;
	eSDK_MEMSET(&birefinfo, 0, sizeof(birefinfo));
	if (m_pUserMgr)
	{
		std::string strCamcode = pCam; //lint !e40
		m_pUserMgr->GetDeviceMgr().GetCameraBriefInfobyCode(strCamcode, birefinfo, true);
		pCamName = birefinfo.cName;
    }

	iRet = IVS_PLAY_StartRecord(m_ulPlayerChannel, pCam, pCamName, pParam->cFileName, &RecordParam, true); //lint !e1013 !e40 !e10
	if (IVS_SUCCEED != iRet)
	{
	...
	MEDIA_RANGE_S stMdieaRange; //lint !e522 !e40 !e10
	eSDK_MEMSET(&stMdieaRange, 0x0, sizeof(MEDIA_RANGE_S));//lint !e40
	stMdieaRange.enRangeType      = RANGE_TYPE_NPT;  //lint !e63 !e1013 !e40 !e10
	stMdieaRange.MediaBeginOffset = 0;               //lint !e63 !e1013 !e40 !e10

	BP_RUN_LOG_INF("Start RealPlay", "IVS rtsp play setup message start.");
	iRet = RTSP_CLIENT_SendPlayMsg(m_iRtspHandle, pszURL, (double)stParam.fSpeed, &stMdieaRange, RTSP_CLIENT_TIMEOUT); //lint !e1013 !e40 !e10 !e1924
...
//处理rtsp的play流程;
int CMediaBase::ProcessRtspPlayMsg(const char* pszURL, const MEDIA_ADDR& /*stServerMediaAddr*/, const START_PLAY_PARAM& stParam)  //lint !e49 !e10 !e808
{//lint !e49
	IVS_DEBUG_TRACE("");
    // 打开流、启动收流线程;
    int iRet = IVS_PLAY_OpenStream(m_ulPlayerChannel, (IVS_UINT32)stParam.enServiceType, (IVS_UINT32)stParam.enSourceType); //lint !e1013 !e40 !e10 !e1924
    if (IVS_SUCCEED != iRet)
    {
		IVS_PLAY_CloseStream(m_ulPlayerChannel);
        IVS_PLAY_FreeChannel(m_ulPlayerChannel);
	...
	// 64位IVS_Player库中没有IVS_PLAY_SetZigZagFlag函数实现
	if(DEVICE_PRO_HIK == GetAccessProtocolByCameraCode(m_szCameraID))
	{
		BP_RUN_LOG_INF("Start RealPlay", "AccessProtocol[%d] need Set ZigZagFlag True", DEVICE_PRO_HIK);
		IVS_PLAY_SetZigZagFlag(m_ulPlayerChannel,true);
	} 
	...
	if (stParam.fSpeed < 0 || 253.0 == stParam.fSpeed || 252.0 == stParam.fSpeed) //lint !e1013 !e40 !e10 /* 252.0 1倍速全量后退, 253.0 2倍速全量后退 */
	{
		stMdieaRange.MediaBeginOffset = static_cast<IVS_UINT32>(m_tmEnd - m_tmStart);  //lint !e63 !e1013 !e40 !e10
	}
	else
	{
		stMdieaRange.MediaBeginOffset = 0;  //lint !e63 !e1013 !e40 !e10
...
{
	BP_RUN_LOG_INF("Enter CMediaBase::StopPlay", "m_ulPlayerChannel = 0x%p, m_iRtspHandle = 0x%p, \
	m_ulNetSourceChannel = 0x%p, szURL = %s.", 
		m_ulPlayerChannel, m_iRtspHandle, m_ulNetSourceChannel, m_stMediaRsp.szURL);
	...
	if (m_hRtspSetupEvent)
	{
		...
		CloseHandle(m_hRtspSetupEvent);      
#else
		VOS_DestroyEvent(m_hRtspSetupEvent);
#endif
		m_hRtspSetupEvent = NULL;
	}

	BP_RUN_LOG_INF("Out CMediaBase::StopPlay", "iRet = %d.", iRet);
...
int CMediaBase::StartLocalRecord(const IVS_LOCAL_RECORD_PARAM* pRecordParam, const IVS_CHAR* pSaveFileName)
{
	...
	if(VIDEO_DEC_H264 != pVideoDecoderType && VIDEO_DEC_H265 != pVideoDecoderType)
	{
...
IVS_INT32 CMediaBase::SetNoNeedSecretKey(IVS_ULONG uiChannel, const MEDIA_KEY_INFO& stMediaKeyInfo)
{
    IVS_DEBUG_TRACE("");
    ...
    //将空密钥设置到MP中
    IVS_INT32 iRet = IVS_PLAY_SetSecretKey(uiChannel, stMediaKeyInfo.iVideoKeyType, NULL);
    if (IVS_SUCCEED != iRet)
    {
...
IVS_INT32 CMediaBase::SetSecretKey(IVS_ULONG uiChannel, const MEDIA_KEY_INFO& stMediaKeyInfo)
{
    IVS_DEBUG_TRACE("");

	//定义密钥存储密钥数组
	IVS_CHAR cSercetKey[ENCRY_KEY_LEN + 1] = {0};
    IVS_INT32 iRet = GetStreamencryKey(cSercetKey, stMediaKeyInfo);
    if (IVS_SUCCEED != iRet)
    {
...
IVS_INT32 CMediaBase::GetStreamencryKey(IVS_CHAR* cSecretKey,const MEDIA_KEY_INFO& stMediaKeyInfo) const
{
    ...
    IVS_INT32 iRet = CToolsHelp::SplitCameraDomainCode(m_szCameraID, strCameraCode, strDomainCode);
    if (IVS_SUCCEED != iRet)
    {
        return iRet;
    }
    ...
    int iInitSecurityClient = oSecurityClient.Init(NET_ELE_SMU_NSS, oLinkIDInfo.GetStrLinkId(), strCameraCode.c_str(),
                                                   strDomainCode.c_str(), strDestModule.c_str(), "", m_szNvrCode);
	...
	BP_RUN_LOG_INF("Get Streamencry Key", "CameraCode[%s], DomainCode[%s], NVRCode[%s]", 
		           strCameraCode.c_str(), strDomainCode.c_str(), m_szNvrCode);

    iRet = CMediaXMLProcess::ApplyStreamencryPwdGetXML(strCameraCode, strDomainCode, m_szNvrCode, strDestModule, iStreanmType,
    ...
    //创建要发送的CMD，拼装了NSS消息头
    CCmd *pCmd = CNSSOperator::instance().BuildCmd(oLinkIDInfo.GetStrLinkId(), NET_ELE_SMU_NSS, NSS_GET_APPLY_STREAMENCRY_PWD_REQ, pReq, strSecurityTransID);//lint !e64 匹配
	...
	eSDK_MEMSET(szEncryPWD, 0, ENCRY_KEY_LEN+1);
	...
	(void)CMediaXMLProcess::ParseStreamencryKey(xmlRsp, szEncryPWD);
	BP_DBG_LOG("GetStreamencryKey EncryPWD:%s,len:%d", szEncryPWD, strlen(szEncryPWD));

    IVS_DELETE(pRsp,MUILI);
	std::string strSecretKey = "";
	iRet = oSecurityClient.DecryptString(szEncryPWD, strSecretKey);//lint !e838
	if (IVS_SUCCEED != iRet)
	{
		return iRet;
	}    
			...
			HW_IPC_ENCRYPT_SIMPLE_HEADER == stMediaKeyInfo.iVideoKeyType))
	{
		...
		eSDK_MEMCPY(pszInitKey, XOR_INIT_KEY_LEN + 1, &strSecretKey[0],XOR_INIT_KEY_LEN);
		//加密用的初始化向量，随机数的后14个字节
		char pszSalt[XOR_INIT_SALT_LEN + 1] = {0};       
		eSDK_MEMCPY(pszSalt, XOR_INIT_SALT_LEN + 1, &strSecretKey[XOR_INIT_KEY_LEN],XOR_INIT_SALT_LEN);
		...
		eSDK_MEMCPY(cSecretKey, ENCRY_KEY_LEN + 1, pszKeyBuffer,XOR_INIT_KEY_LEN);
		BP_DBG_LOG("Init SecretKey Len ;%d  , SecretKey Len :%d", uiStrLen,strSecretKey.length());
        return iRet;
	}

    //AES256密钥拷贝
    eSDK_STRNCPY(cSecretKey, ENCRY_KEY_LEN + 1, strSecretKey.c_str(), strSecretKey.length());
...
int CMediaBase::ParseURL(const char* pUrl, const char* pKey, std::string& strValue)
{
    ...
    strSrc.append(pUrl);
    std::string::size_type pos1 = strSrc.find(pKey); //lint !e1924
    if (std::string::npos == pos1)  //lint !e1924
    {
        return IVS_FAIL;
    }
    strSrc = strSrc.substr((unsigned int)pos1,strSrc.size()-(unsigned int)pos1); //lint !e1924
    std::string::size_type pos2 = strSrc.find("&"); //lint !e1924
    if (std::string::npos == pos2) //lint !e1924
    {
        return IVS_FAIL;
    }
    strValue = strSrc.substr(strlen(pKey), (unsigned int)pos2 - strlen(pKey)); //lint !e1924
    return IVS_SUCCEED;
}//lint !e1762
...
int CMediaBase::SetDrawCallBack(DRAW_PROC callback, void *pUserData)
{
	...
	return IVS_PLAY_SetDrawCallBack(m_ulPlayerChannel, (DRAW_CALLBACK)callback, pUserData);//lint !e64 匹配
}
...
void CMediaBase::UTCStr2Span(std::string& strTime, const char* stStartTime, const char* stEndTime)
{
	if(stStartTime == NULL || stEndTime == NULL)
	{
		BP_RUN_LOG_ERR(IVS_DATA_INVALID, "UTCStr2Span", "para is null");
		return ;
	}
	strTime = stStartTime;
	strTime.insert(8, "T");
	strTime.insert(15, "Z");
	strTime.append("-");
	std::string strEndTime = stEndTime;
	strEndTime.insert(8, "T");
	strEndTime.insert(15, "Z");
...
IVS_INT32 CMediaBase::NotifyStreamBufferState(IVS_INT32 iBufferState)
{
    ...
    if (1 == iBufferState)
    {
		const RTSP_SESSION_STATUS iState = RTSP_CLIENT_GetStatus(m_iRtspHandle); //lint !e10 !e808
        BP_DBG_LOG("Client VCR control, Buffer over max, rtsp client status[%d].", iState); //lint !e40
        if (RTSP_SESSION_STATUS_READY == iState       //lint !e40
			|| RTSP_SESSION_STATUS_PLAYING == iState) //lint !e40
        {
    ...
    {
        const RTSP_SESSION_STATUS iState = RTSP_CLIENT_GetStatus(m_iRtspHandle);          //lint !e10 !e808
        BP_DBG_LOG("Client VCR control, Buffer low min. rtsp client status[%d].",iState); //lint !e40

		if (RTSP_SESSION_STATUS_PLAYING != iState) //lint !e40
		{
			...
			stRange.MediaBeginOffset = OFFSET_CUR;  //lint !e63 !e1013 !e40 !e10
...
{//lint !e18 !e745
    ...
    // 产生信号量
#ifdef WIN32
    bool bIsSuccess = SetEvent(m_hRtspSetupEvent); //lint !e1786
    if (bIsSuccess)
    {
        BP_DBG_LOG("SetEvent Is Success.");
    }
#else
    ULONG vosret = VOS_SetEvent(m_hRtspSetupEvent);
    if(VOS_OK == vosret)    
    {
        BP_DBG_LOG("SetEvent Is Success.");
    }
...
IVS_INT32 CMediaBase::GetAccessProtocolByCameraCode(const IVS_CHAR* pCameraCode)
{
	if (NULL != m_pUserMgr)
	{
		return m_pUserMgr->GetDeviceMgr().GetAccessProtocolByCameraCode(pCameraCode);
	}
