...
/***************************************************************************
 ...
 a=crypto:1 HW_IVS_IPC_ENCRYPT_SIMPLE StartTime:YYYYMMddHHmmss StopTime:YYYYMMddHHmms
***************************************************************************/
IVS_INT32 parseSdp(const char* pSdp, MEDIA_KEY_INFO* pMediaKeyInfo,  /*lint -esym(714,parseSdp)*/
    ...
    while (bRet && (IVS_SUCCEED == nRet))
    {
        ...
        if ('=' == *pTmp)
        {
            ...
            case 'o':
                ...
                break;
            case 'u':
                eSDK_STRNCPY(pMediaInfo->SessionUrl, sizeof(pMediaInfo->SessionUrl), pTmp, strlen(pTmp));
                break;
            case 'm':
...
int parseStreamID(const char* pSdpMsg, char* pSessionId)
{
    ...
    eSDK_STRNCPY(pSessionId, strValue.length() + 1, strValue.c_str(), strValue.length());
...
/***************************************************************************
 ...
 返 回 值 : IVS_SUCCEED-成功           其他-失败
***************************************************************************/
int parseApplications(const char* pTmp, IVS_ULONG* pPort)
{
    ...
    *pPort = (unsigned long)atoi(strValue.c_str()); //lint !e1924
    return IVS_SUCCEED;
}
...
int parseDecoderType(const char* pTmp, IVS_ULONG* pDecoderType)
{
    ...
    *pDecoderType = (IVS_ULONG)atoi(strValue.c_str()); //lint !e1924
    return IVS_SUCCEED;
}
...
int parseManuType(char* pTmp, IVS_ULONG* ulManuType)
{
    ...
    *ulManuType = (IVS_ULONG)atoi(strTmp.c_str()); //lint !e1924
    return IVS_SUCCEED;
} //lint !e818
...
int parseRtpMap(const char* pTmp,MEDIA_KEY_INFO* pMediaKeyInfo)
{
	...
	* decode Sample Rate
	*/
	unsigned int nSampleRateVal = 8000;
	const char *pSampleRateVal = strchr(pTmp, '/');
	if (NULL != pSampleRateVal)
	{
		pSampleRateVal++;
		nSampleRateVal = atoi(pSampleRateVal);//lint !e732
	}
	...
	do 
	{
		...
		// h265(hevc)
		pEndPos = strTmp.find(SDP_VIDEO_H265_TYPE);
		if (std::string::npos != pEndPos)
		{
			pMediaKeyInfo->enVideoDecoderType = VIDEO_DEC_H265;
			pMediaKeyInfo->uPTVideo = PtVal;
			break;
		}

		// svac(svac)
		pEndPos = strTmp.find(SDP_VIDEO_SVAC_TYPE);
		if (std::string::npos != pEndPos)
		{
			pMediaKeyInfo->enVideoDecoderType = VIDEO_DEC_SVAC;
			pMediaKeyInfo->uPTVideo = PtVal;
			break;
		}
		...
		if (std::string::npos != pEndPos)
		{
			...
			pMediaKeyInfo->uAudioSampleRate = nSampleRateVal;
			break;
		}
		//PCMA
...
int parseStorageSecretKey(const char* pTmp, STORAGESECRET_INFO* pStorageSecretInfo)
{
    ...
    pStorageSecretInfo->keyTpye = (IVS_ULONG)atoi(strValue.c_str()); //lint !e613 !e1924
    ...
    eSDK_STRNCPY(pStorageSecretInfo->keyValue, sizeof(pStorageSecretInfo->keyValue), strValue.c_str(), strValue.length());
...
int parseCerIndex(const char* pTmp, STORAGESECRET_INFO* pStorageSecretInfo)
{
    ...
    pStorageSecretInfo->cerIndex = (IVS_ULONG)atoi(strValue.c_str()); //lint !e613 !e1924
...
/***************************************************************************
 ...
 返 回 值 : IVS_SUCCEED-成功        其他-失败
***************************************************************************/
int parseStarTimeAndEndTime(const char* pTmp,MEDIA_KEY_INFO* pMediaKeyInfo)/*lint -esym(714,parseStarTimeAndEndTime)*/
{
...
int parseSessionID(const char* pTmp,MEDIA_KEY_INFO* pMediaKeyInfo)
{
	...
	eSDK_MEMSET(&pMediaKeyInfo->szSessionID,0,SESSION_ID_LEN);//lint !e545
