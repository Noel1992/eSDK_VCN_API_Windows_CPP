...
IVS_INT32 CRecordXMLProcess::ParseCamerTraceInfo(CXml& xml,std::list<std::string>& nvrList)
{
	...
	xml.IntoElem();
	std::string strNvr;
	const char*AttriValue = NULL;   //¡Ÿ ±¥Ê¥¢µ•∏ˆΩ⁄µ„÷µ
...
IVS_INT32 CRecordXMLProcess::GetRecordPlanParseRSP(IVS_RECORD_PLAN* pRecordPlan, CXml& xml)
{
	...
	if (xml.FindElemEx("Content/Plan/PlanList"))
	{
		do 
		{
			...
			//Ω‚ŒˆPlanInfo
			(void)xml.IntoElem();
			eSDK_MEMSET(&pRecordPlan->stPlanInfo[iPlanInfoNum],0, sizeof(IVS_PLAN_INFO));
	...
	const IVS_CHAR* pDomainCode,CXml &xml)
{
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiRecordMode)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("AllocMethod");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(0).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("Time");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiTime)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("FrameExtractRecordTime");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiFrameExtractRecordTime)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("AlarmTime");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiAlarmTime)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("KeyframeTime");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiKeyframeTime)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("BeforeAlarm");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiBeforeAlarm)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("AfterAlarm");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiAfterAlarm)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("PlanStreamID");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiPlanStreamType)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("AlarmStreamID");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiAlarmStreamType)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("AlarmRecordTTL");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiAlarmRecordTTL)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("ManualRecordTTL");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiManualRecordTTL)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("PreRecord");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiPreRecord)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("PreRecordTime");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->uiPreRecordTime)).c_str()); //lint !e64 ∆•≈‰

	(void)xml.AddElem("AssociatedAudio");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPolicy->bAssociatedAudio)).c_str()); //lint !e64 ∆•≈‰
	                                            ...
	                                            IVS_UINT32& uiRecordNum,IVS_UINT32 uiFromIndex,IVS_UINT32 uiToIndex)
{
	...
	eSDK_MEMSET(cCameraCode,0,IVS_DEV_CODE_LEN+1);
	if (xml.FindElemEx("Content/CameraCode"))
	{
		...
		if (NULL != pRecordData)
		{
			(void)CToolsHelp::Memcpy(&(pRecordData->cCameraCode), IVS_DEV_CODE_LEN, &cCameraCode, IVS_DEV_CODE_LEN); //lint !e545
	...
	if (xml.FindElemEx("Content/RecordDataList/RecordDataInfo"))
	{
		do
		{
			...
			eSDK_MEMSET(&sRecordInfo, 0, sizeof(IVS_RECORD_INFO));
			RECORD_DATA_PLACE stRecordDataPlace;
			eSDK_MEMSET(&stRecordDataPlace,0,sizeof(RECORD_DATA_PLACE));
			...
			eSDK_MEMSET(cRecordStatus,0,ulRecordTypeLength+1);
			GET_ELEM_VALUE_CHAR("RecordType",pTemp,cRecordStatus,ulRecordTypeLength,xml);
			IVS_UINT32 ulRecordType = CXmlProcess::TransBinStrToDec(cRecordStatus);//lint !e64 ∆•≈‰
    ...
    const IVS_RECORD_PLAN* pRecordPlan, CXml &xml)
{
    ...
    for (IVS_UINT32 i=0; i<uiDeviceNum; i++)
    {
        ...
        (void)xml.IntoElem();
        (void)xml.SetElemValue(chDevCode);
        (void)xml.AddElem(strExDomainCode.c_str());//lint !e64 ∆•≈‰
        (void)xml.SetElemValue(chDomaCode);
        xml.OutOfElem();
        pDeviceList++;
    }
    ...
    (void)xml.AddChildElem("RecordMethod");
    (void)xml.IntoElem();//------------5
    (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPlan->uiRecordMethod)).c_str()); //lint !e64 ∆•≈‰
    (void)xml.AddElem("IsEnable");
    (void)xml.SetElemValue(CToolsHelp::Int2Str(pRecordPlan->bEnableRecordPlan).c_str()); //lint !e64 ∆•≈‰
    (void)xml.AddElem("PlanType");
    (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPlan->uiRecordPlanType)).c_str()); //lint !e64 ∆•≈‰
    if(pRecordPlan->uiRecordPlanType !=2)
    {
        ...
        for (IVS_UINT32 i=0; i<pRecordPlan->uiPlanInfoNum; i++)
        {
            ...
            (void)xml.IntoElem();//  -------------3
            (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPlan->stPlanInfo[i].uiDayType)).c_str());//lint !e64 ∆•≈‰
            (void)xml.AddElem("TimeInfoNum"); 
            (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordPlan->stPlanInfo[i].uiTimeSpanInfoNum)).c_str()); //lint !e64 ∆•≈‰
            (void)xml.AddElem("TimeList"); 
            ...
            for (IVS_UINT32 j=0; j<pRecordPlan->stPlanInfo[i].uiTimeSpanInfoNum; j++)
            {
                ...
                (void)xml.AddChildElem(strStartTime.c_str());//lint !e64 ∆•≈‰
                (void)xml.IntoElem();//-------------------------1
                (void)xml.SetElemValue(cStartTime);

                (void)xml.AddElem(strEndTime.c_str());//lint !e64 ∆•≈‰
                (void)xml.SetElemValue(cEndTime); 
    ...
    IVS_UINT32 uiRecordMethod, CXml &xml)
{
    ...
    (void)xml.AddChildElem("RecordMethod");
    (void)xml.IntoElem();
    (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiRecordMethod)).c_str());//lint !e64 ∆•≈‰
    xml.OutOfElem();
...
IVS_CHAR* CRecordXMLProcess::GetCameraDomainCodeByCameraCode(IVS_UINT32 uiDeviceNum,const IVS_DEVICE_CODE* pDeviceList,const IVS_CHAR CameraCode[64])
{
	...
	for (IVS_UINT32 i=0; i<uiDeviceNum; i++)
	{
		...
		{
			return const_cast<IVS_CHAR *>(pDeviceList[i].cDevCode);
		}
	}
	return const_cast<IVS_CHAR *>(CameraCode);
}
//ªÒ»°¬ºœÒ≤ﬂ¬‘µƒœÏ”¶XML
	...
	const IVS_INDEX_RANGE* pIndexRange,CXml& xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pIndexRange->uiFromIndex)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("ToIndex");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pIndexRange->uiToIndex)).c_str());//lint !e64 ∆•≈‰
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(iRecordMethod).c_str());//lint !e64 ∆•≈‰
...
IVS_INT32 CRecordXMLProcess::GetRecordStatusReqXML(const IVS_CHAR* pCameraCode,IVS_UINT32 iRecordMethod,CXml& xml)
{
	...
	(void)xml.SetElemValue(chDevCode);
	(void)xml.AddElem("RecordMethod");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(iRecordMethod)).c_str());//lint !e64 ∆•≈‰
	xml.OutOfElem();

	return IVS_SUCCEED;
}
//≤È—Ø¬ºœÒ◊¥Ã¨µƒœÏ”¶Ω‚ŒˆXML
IVS_INT32 CRecordXMLProcess::ParseRecordStatus(CXml& xml,IVS_UINT32* ulStatus)
{
	...
	eSDK_MEMSET(cRecordStatus,0,iRecordStatusLength+1);

	GET_ELEM_VALUE_CHAR("RecordState",pTemp,cRecordStatus,iRecordStatusLength,xml);//lint !e838
	
	IVS_UINT32 ulRecordStatus = CXmlProcess::TransBinStrToDec(cRecordStatus);//lint !e64 ∆•≈‰
	*ulStatus = ulRecordStatus;

	return IVS_SUCCEED;
}
//ø™ º‘∂≥Ã¬ºœÒªÿ∑≈µƒ«Î«ÛXML
IVS_INT32 CRecordXMLProcess::StartRemoteRecordReqXML(const IVS_UINT32 uiUserId,const IVS_CHAR* pUserDomainCode,const IVS_CHAR* pCameraCode,IVS_UINT32 iRecordType,IVS_UINT32 iRecordTime,CXml& xml)
{
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiUserId)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("RecordMethod");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(iRecordType)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("RecordType");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(1).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("RecordTime");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(iRecordTime)).c_str());//lint !e64 ∆•≈‰
	xml.OutOfElem();

	return IVS_SUCCEED;
}
//Õ£÷π‘∂≥Ã¬ºœÒªÿ∑≈µƒ«Î«ÛXML
	...
	const IVS_CHAR* pCameraCode,IVS_UINT32 uiBookmarkID,const IVS_CHAR* pNewBookmarkName, CXml &xml)
{
	...
	(void)xml.AddElem(strRecordMethod.c_str());//lint !e64 ∆•≈‰
	(void)xml.SetElemValue(CToolsHelp::Int2Str(iRecordMethod).c_str()); //lint !e64 ∆•≈‰
	(void)xml.AddElem("RecordBookmark");
	(void)xml.AddChildElem("BookmarkID");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiBookmarkID)).c_str()); //lint !e64 ∆•≈‰
	(void)xml.AddElem("BookmarkName");
...
IVS_INT32 CRecordXMLProcess::ParseGetBookmarkList(CXml& xml,IVS_RECORD_BOOKMARK_INFO_LIST* pRecordBookmarkInfoList,const std::string& /*strDomainCode*/,IVS_UINT32 uiBufferSize)
{
	...
	const char* pTemp = NULL;
	GET_ELEM_VALUE_NUM("FromIndex", pTemp, pRecordBookmarkInfoList->stIndexRange.uiFromIndex, xml);//lint !e732 !e64 ∆•≈‰
	GET_ELEM_VALUE_NUM("ToIndex", pTemp, pRecordBookmarkInfoList->stIndexRange.uiToIndex, xml);//lint !e732 !e64 ∆•≈‰
	GET_ELEM_VALUE_NUM("RealNum",pTemp, pRecordBookmarkInfoList->uiTotal, xml);//lint !e734 !e732 !e64 ∆•≈‰
	...
	if (xml.FindElemEx("Content/RecordBookmarkList/RecordBookmark"))
	{
		do
		{
			...
			eSDK_MEMSET(&stRecordBookmarkInfo, 0, sizeof(IVS_RECORD_BOOKMARK_INFO));

			(void)xml.IntoElem();
			GET_ELEM_VALUE_NUM_FOR_UINT("BookmarkID",szElemValue,  stRecordBookmarkInfo.uiBookmarkID,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("NVRCode",szElemValue,  stRecordBookmarkInfo.cNvrCode,IVS_NVR_CODE_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("MBUDomain",szElemValue,  stRecordBookmarkInfo.cMBUDomain,IVS_DOMAIN_CODE_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("CameraCode",szElemValue,  stRecordBookmarkInfo.cCameraCode,IVS_DEV_CODE_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("BookmarkName",szElemValue,  stRecordBookmarkInfo.cBookmarkName,IVS_BOOKMARK_NAME_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("BookmarkTime",szElemValue,  stRecordBookmarkInfo.cBookmarkTime,IVS_TIME_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("BookmarkCreatorName",szElemValue,  stRecordBookmarkInfo.cBookmarkCreatorName,IVS_NAME_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_CHAR("BookmarkCreatorDomain",szElemValue,  stRecordBookmarkInfo.cUserDomain,IVS_DOMAIN_CODE_LEN,xml);//lint !e64 ∆•≈‰
			GET_ELEM_VALUE_NUM_FOR_UINT("BookmarkCreatorID",szElemValue,stRecordBookmarkInfo.uiBookmarkCreatorID,xml);//lint !e64 ∆•≈‰
			xml.OutOfElem();
...
IVS_INT32 CRecordXMLProcess::ParseManualRecordStateXML(CXml& xml,IVS_MANUAL_RECORD_STATUS_NOTIFY* pManualRecordStatusNotify)
{
	...
	GET_ELEM_VALUE_CHAR("CameraCode",szElemValue,cCameraCode,IVS_DEV_CODE_LEN,xml);//lint !e838
	...
	bool bRet = CToolsHelp::Memcpy(pManualRecordStatusNotify->cCameraCode,IVS_DEV_CODE_LEN, strBigCode.c_str(), strlen(strBigCode.c_str()));//lint !e64 ∆•≈‰
	if(false == bRet)
	{
...
IVS_INT32 CRecordXMLProcess::LockRecordReqXML(IVS_INT32 iRecordMethod,const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,const IVS_CHAR* pCameraCode,const IVS_RECORD_LOCK_INFO* pRecordLockInfo, CXml &xml)
{
	...
	(void)xml.AddDeclaration("1.0","UTF-8","");
	(void)xml.AddElem("Content");
	(void)xml.AddChildElem(strDomainCode.c_str());//lint !e64 ∆•≈‰
	(void)xml.IntoElem();
	if(RECORD_METHOD_MBU == iRecordMethod)
	{
		(void)xml.SetElemValue(pDomainCode);
		(void)xml.AddElem(strNVRCode.c_str());//lint !e64 ∆•≈‰
		(void)xml.SetElemValue(pNVRCode);
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordLockInfo->uiOperatorID)).c_str()); //lint !e64 ∆•≈‰
...
IVS_INT32 CRecordXMLProcess::ModLockRecordReqXML(IVS_INT32 iRecordMethod,const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,const IVS_CHAR* pCameraCode,const IVS_RECORD_LOCK_INFO* pRecordLockInfo, CXml &xml)
{
	...
	(void)xml.IntoElem(); 
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordLockInfo->uiLockID)).c_str()); //lint !e64 ∆•≈‰
	(void)xml.AddElem("LockDesc");
	(void)xml.SetElemValue(cLockDesc); 
	(void)xml.AddElem("OperatorID");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pRecordLockInfo->uiOperatorID)).c_str()); //lint !e64 ∆•≈‰
...
IVS_INT32 CRecordXMLProcess::UnLockRecordReqXML(IVS_INT32 iRecordMethod,const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,const IVS_CHAR* pCameraCode,const IVS_RECORD_LOCK_INFO* pRecordLockInfo, CXml &xml)
{
	...
	if(RECORD_METHOD_MBU == iRecordMethod)
	{
		...
		(void)xml.AddElem(strNVRCode.c_str());//lint !e64 ∆•≈‰
		(void)xml.SetElemValue(pNVRCode);
		(void)xml.AddElem(strCameraDomain.c_str());//lint !e64 ∆•≈‰
		(void)xml.SetElemValue(chDomaCode);
	}
	else
	{
...
IVS_VOID CRecordXMLProcess::GetCameraPlanInfoGetXML(IVS_UINT32 uiPlanType, const IVS_CHAR* pDomainCode, CXml &xml)
{
    ...
    (void)xml.AddElem("PlanType");
    (void)xml.IntoElem();
    (void)xml.SetElemValue((CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiPlanType))).c_str());//lint !e64 ∆•≈‰
    xml.OutOfElem();
}
...
IVS_INT32 CRecordXMLProcess::GetCameraPlanInfoParseXML(CXml &xml, PLAN_INFO_VEC &cameraPlanInfoList)
{
    ...
    if (xml.FindElemEx("Content/CameraList/CameraInfo"))
    {
        do
        {
            ...
            eSDK_MEMSET(&stcCameraPlan, 0, sizeof(IVS_CAMERA_PLAN_SET_INFO));
...
IVS_VOID CRecordXMLProcess::GetCameraPlanInfoGetXml(const std::map<std::string, PLAN_INFO_VEC> &planMap, CXml &xml)
{
    ...
    for (planMapIter = planMap.begin(); planMapIter != planMap.end(); planMapIter++)
    {
        ...
        {
            pPlanInfo = &vec_planList->at(i);//lint !e64 ∆•≈‰
            if(iSize > 0)
            {
                (void)xml.AddElem("CameraInfo");
            }
