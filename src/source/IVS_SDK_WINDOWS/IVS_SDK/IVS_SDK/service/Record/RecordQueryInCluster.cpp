...
struct tm _tm;                                         \
	localtime_s(&_tm, &(t));                                  \
	eSDK_SPRINTF(buf, 14, "%04d%02d%02d%02d%02d%02d",         \
...
IVS_INT32 CRecordQueryInCluster::GetRecordInCluster(CSendNssMsgInfo& sendNssMsgInfo, const IVS_QUERY_UNIFIED_FORMAT* pUnifiedQuery, std::string strClusterCode, std::string& strSendXml)
{
	IVS_DEBUG_TRACE("");
	...
	eSDK_MEMSET(&timeSpan,0,sizeof(IVS_TIME_SPAN));
	IVS_INT32 iRet = GetTimeSpan(pUnifiedQuery,timeSpan);
	if (IVS_SUCCEED != iRet)
	{
	...
	iRet = GetCamerTraceInfo(stSendNssInfo, strClusterCode);
	if (IVS_SUCCEED != iRet)
	{
		IVS_DELETE(pReqData, MUILI);
...
IVS_INT32 CRecordQueryInCluster::GetCamerTraceInfo(CSendNssMsgInfo& sendNssMsgInfo, std::string strClusterCode)
{
	IVS_DEBUG_TRACE("");
	if (NULL == m_pUserMgr)
	{
	...
	IVS_INT32 iRet = m_pUserMgr->SendRedirectServe(sendNssMsgInfo,strpRsp);//lint !e1013 !e1055 !e746 !e526 !e628
	if (IVS_SUCCEED != iRet)
	{
		//如果获取摄像机轨迹失败则需要向m_nvrList插入当前集群下所有的NVRCode后直接如return IVS_SUCCESS;
		//根据集群编码获取NVRList;
		iRet = m_pUserMgr->GetDeviceMgr().GetNvrListByClusterCode(strClusterCode, m_nvrList);
		if (0 == m_nvrList.size())
		{
			BP_RUN_LOG_ERR(iRet,"Get Camer Trace Info","Get Camer Trace Info Error");
			return iRet;
...
IVS_INT32 CRecordQueryInCluster::GetRecordSegment(CSendNssMsgInfo& sendNssMsgInfo,const IVS_QUERY_UNIFIED_FORMAT* pUnifiedQuery)
{
	...
	CXml xml;
	std::string strDomainCode = sendNssMsgInfo.GetDomainCode();
	IVS_INT32 iRet = CXmlProcess::GetUnifiedFormatQueryXML(pUnifiedQuery,xml,strDomainCode.c_str());//lint !e64 匹配
	if (IVS_SUCCEED != iRet)//lint !e774 上面放开后这个去掉
	{
		BP_RUN_LOG_ERR(iRet, "Send Cmd by ReDirect","Build Cmd Error");
		return iRet;
	...
	for (;beginIter != endIter; beginIter++ )
	{
		...
		//解析数据
		CXml xmlRsp;
		(void)xmlRsp.Parse(strNSSRsp.c_str());//lint !e64 匹配
		(void)ParseRecordSegmentInfo(xmlRsp);
	}
	return IVS_SUCCEED;
}
...
int CRecordQueryInCluster::SortAndSetRecordInfo(const std::string& strRecordInfo)
{
	...
	unsigned int uiBeginIndex = strRecordInfoBack.find("<RecordDataInfo>");
	unsigned int uiEndIndex   = strRecordInfoBack.find("</RecordDataInfo>");
...
IVS_INT32 CRecordQueryInCluster::SetTimeToRecordInfo(std::string& strRecordInfo,time_t& start,time_t& end)
{
	...
	uiBeginIndex      += strlen("<StartTime>");
	char cStartTime[256];
	eSDK_MEMSET(cStartTime,0,sizeof(cStartTime));
	...
	char cEndTime[256];
	eSDK_MEMSET(cEndTime,0,sizeof(cEndTime));
...
int CRecordQueryInCluster::AssembleTimeSpanXml(std::string& strXml)
{
	...
	char StartTime[256];
	eSDK_MEMSET(StartTime,0,sizeof(StartTime));
	...
	char EndTime[256];
	eSDK_MEMSET(EndTime,0,sizeof(EndTime));
