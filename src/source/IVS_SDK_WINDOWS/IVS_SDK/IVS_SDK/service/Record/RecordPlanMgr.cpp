	...
	const IVS_RECORD_PLAN* pRecordPlan,IVS_RESULT_LIST* pResultList,IVS_UINT32 uiResultBufferSize)const
{
		...
		(PLAN_ALLDAY == pRecordPlan->uiRecordPlanType && 0 != pRecordPlan->uiPlanInfoNum))
	{
		...
		for (IVS_UINT32 i=0; i<pResultList->uiNum; i++)
		{
			...
			SecureZeroMemory(pResultList->stInfo[i].cCode, sizeof(pResultList->stInfo[i].cCode));
			eSDK_STRNCPY(pResultList->stInfo[i].cCode, sizeof(pResultList->stInfo[i].cCode), pDeviceList[i].cDevCode, IVS_DEV_CODE_LEN);
		}
	...
	CmdMap cmdMap;
	CCmd *pCmd = NULL;
	(void)m_UserMgr->RecollectCameraDomainCodes2Map(uiDeviceNum,pDeviceList,deviceCodeMap);
    IVS_INT32 iRet;
...
IVS_INT32 CRecordPlanMgr::DeleteRecordPlan(IVS_UINT32 uiDeviceNum,const IVS_DEVICE_CODE* pDeviceList,
	IVS_UINT32 uiRecordMethod,IVS_RESULT_LIST* pResultList,IVS_UINT32 uiResultBufferSize)const
 {
	...
	for (; deviceCodeMapIter != deviceCodeMapIterEnd; deviceCodeMapIter++)
	{
		...
		(void)CRecordXMLProcess::DelRecordPlanGetXML(strSendDomainCode,devList, uiRecordMethod, delRecordPlanXmlReq);
		unsigned int xmlLen = 0;
	...
	std::string strpRsp;
	IVS_UINT32 uiTimeOut =  CSDKConfig::instance().GetTimeOutValue();
	IVS_INT32 iRet = CDispatcher::instance().dispatcherSyncMsgOutBatch(cmdMap, uiTimeOut);
	 ...
	 IVS_RECORD_PLAN* pRecordPlan)
 {
	 ...
	 eSDK_MEMSET(chDevCode, 0, sizeof(chDevCode));

	 IVS_CHAR chDomaCode[IVS_DOMAIN_CODE_LEN+1];
	 eSDK_MEMSET(chDomaCode, 0, sizeof(chDomaCode));
	 ...
	 {
		 BP_RUN_LOG_ERR(iParseRSPRet, "Get Platform Record Plan", "Get RecordPlan Parse RSP failed.");
		 return iParseRSPRet;
	 }
...
IVS_INT32 CRecordPlanMgr::GetCameraPlanInfo(IVS_UINT32 uiPlanType, IVS_CHAR** pRspXml)
{
    ...
    IVS_INT32 iRet = m_UserMgr->GetDeviceMgr().GetDomainListByUserID(domainCodeList);
    if(IVS_SUCCEED != iRet)
    {
    ...
    for(IVS_UINT32 i = 0; i < iSize; i++)
    {
        ...
        CXml xmlReq;
        CRecordXMLProcess::GetCameraPlanInfoGetXML(uiPlanType, strDomainCode.c_str(), xmlReq);//lint !e64 匹配
        IVS_UINT32 xmlLen = 0;
