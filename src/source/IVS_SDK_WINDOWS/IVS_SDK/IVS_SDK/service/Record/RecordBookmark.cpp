...
IVS_INT32 CRecordBookmark::AddBookmark(IVS_INT32 iRecordMethod,const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,const IVS_CHAR* pCameraCode,const IVS_CHAR* pBookmarkName,const IVS_CHAR* pBookmarkTime,IVS_UINT32* uiBookmarkID)const
{
	...
	(void)CRecordXMLProcess::AddBookmarkGetXML(iRecordMethod,m_UserMgr->GetUserID(),strUserDomainCode.c_str(),userName.c_str(),pDomainCode,pNVRCode,pCameraCode,pBookmarkName, pBookmarkTime, xmlReq);//lint !e64 匹配
	...
	IVS_INT32 iRet = m_UserMgr->SendCmd(sendNssMsgInfo,strpRsp,iNeedRedirect);
...
IVS_INT32 CRecordBookmark::ModifyBookmark(IVS_INT32 iRecordMethod,const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,const IVS_CHAR* pCameraCode,IVS_UINT32 uiBookmarkID,const IVS_CHAR* pNewBookmarkName)const
{
	...
	std::string strUserDomainCode;
	m_UserMgr->GetDomainCode(strUserDomainCode);
	(void)CRecordXMLProcess::ModifyBookmarkGetXML(iRecordMethod,m_UserMgr->GetUserID(),userName.c_str(),strUserDomainCode.c_str(),pDomainCode,pNVRCode,pCameraCode,uiBookmarkID, pNewBookmarkName, xmlReq);//lint !e64 匹配
...
IVS_INT32 CRecordBookmark::DeleteBookmark(IVS_INT32 iRecordMethod,const IVS_CHAR* pDomainCode,const IVS_CHAR* pNVRCode,const IVS_CHAR* pCameraCode,IVS_UINT32 uiBookmarkID)const
{
	...
	(void)CRecordXMLProcess::DeleteBookmarkGetXML(iRecordMethod,pDomainCode,pNVRCode,pCameraCode,uiBookmarkID, xmlReq);
	unsigned int xmlLen = 0;
	...
	IVS_RECORD_BOOKMARK_INFO_LIST* pBookmarkList,IVS_UINT32 uiBufSize)const
{
	...
	CXml xmlReq;
	//拼装XML查询请求
	IVS_INT32 iRet = CXmlProcess::GetUnifiedFormatQueryXML(pUnifiedQuery, xmlReq, chDomaCode);
	if (IVS_SUCCEED != iRet)
	{
	...
	iRet = CXmlProcess::AddQueryFieldForUnifiedFormatQueryXML(sRecordMethod.c_str(), CToolsHelp::Int2Str(iRecordMethod).c_str(), xmlReq);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
	...
	IVS_RECORD_BOOKMARK_INFO_LIST* pBookmarkList,IVS_UINT32 uiMaxBufSize)const
{
	...
	IVS_INT32 i = 0;
	IVS_UINT32 j = 0;
	for (i=0;i<pUnifiedQuery->iFieldNum;i++)//lint !e838
	{
		if (QUERY_CAMERA_CODE == pUnifiedQuery->stQueryField[i].eFieID)
		{
			(IVS_VOID)CXmlProcess::ParseDevCode(pUnifiedQuery->stQueryField[i].cValue, chDevCode, chDomaCode);
	...
	***************************************/
    ...
    eSDK_MEMSET(pMBUInfoListBuf, 0, uiBufSize);
	...
	***************************************/
	...
	eSDK_MEMSET(pTotalMBUInfoListBuf, 0, uiBufSize);
	...
	if(strlen(strNVRCode.c_str()) > 0)
	{
		...
		IVS_DELETE(pMBUInfoListBuf, MUILI);
		IVS_DELETE(pTotalMBUInfoListBuf, MUILI);
		return GetBookmarkListByBackupServerForOneDomain(pUnifiedQuery->stIndex.uiFromIndex,pUnifiedQuery->stIndex.uiToIndex,strNVRCode.c_str(),cMBUDomainCode,pUnifiedQuery,pBookmarkList,uiMaxBufSize);//lint !e64 匹配
	}
	...
	***************************************/
	...
	for (j = 0; j < uiDiffMBUChangeInfoNum; j++)
	{
		if(j < pTotalMBUInfoList->uiTotal)
		{
			...
			(void)GetBookmarkListByBackupServerForOneDomain(1,1,cMBUCode,cMBUDomainCode,pUnifiedQuery,pInfoList,uiMaxBufSize);
	...
	for (IVS_UINT32 k = 0; k < uiDiffMBUChangeInfoNum; k++)
	{
		if ((pTotalMBUInfoList->stMBUChangeInfo[k].uiFromIndex != 0) && (pTotalMBUInfoList->stMBUChangeInfo[k].uiToIndex != 0))
		{
			...
			std::string sRecordMethod = "RecordMethod";
			iRet = CXmlProcess::AddQueryFieldForUnifiedFormatQueryXML(sRecordMethod.c_str(),CToolsHelp::Int2Str(RECORD_METHOD_MBU).c_str(), xmlReq);//lint !e64 匹配
			if (IVS_SUCCEED != iRet)
			{
				IVS_DELETE(pMBUInfoListBuf, MUILI);
				IVS_DELETE(pTotalMBUInfoListBuf, MUILI);
			...
			if (xmlReq.FindElemEx("Content/PageInfo"))
			{
				...
				{
					xmlReq.ModifyElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pUnifiedQuery->stIndex.uiFromIndex)).c_str());//lint !e64 匹配
				}
				if (xmlReq.FindElem("ToIndex"))
				{
					xmlReq.ModifyElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pUnifiedQuery->stIndex.uiToIndex)).c_str());//lint !e64 匹配
				}
				xmlReq.OutOfElem();
			}
			...
			{
				//解析查询请求XML
				if (IVS_SUCCEED != CRecordXMLProcess::ParseGetBookmarkListForBackup(xmlRsp,pBookmarkList,cMBUDomainCode,uiMaxBufSize,	//lint !e64 匹配
	...
	const IVS_QUERY_UNIFIED_FORMAT* pUnifiedQuery,IVS_RECORD_BOOKMARK_INFO_LIST* pBookmarkList,IVS_UINT32 uiMaxBufSize)const
{
	...
	eSDK_MEMSET(chMBUDomainCode,0x0,IVS_DOMAIN_CODE_LEN+1);
	...
	if (xmlReq.FindElemEx("Content/PageInfo"))
	{
		...
		if (xmlReq.FindElem("FromIndex"))
		{
			xmlReq.ModifyElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiFromIndex)).c_str());//lint !e64 匹配
		}
		if (xmlReq.FindElem("ToIndex"))
		{
			xmlReq.ModifyElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiToIndex)).c_str());//lint !e64 匹配
		}
		xmlReq.OutOfElem();
	}
	...
	//解析查询请求XML
	if (IVS_SUCCEED != CRecordXMLProcess::ParseGetBookmarkList(xmlRsp,pBookmarkList,cMBUDomainCode,uiMaxBufSize))//lint !e64 匹配
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID,"Get Bookmark List By BackupServer","Parse response xml failed");
		return IVS_XML_INVALID;
	}
