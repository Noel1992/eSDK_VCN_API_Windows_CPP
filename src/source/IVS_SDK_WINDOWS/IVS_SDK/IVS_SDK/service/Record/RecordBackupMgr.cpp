...
IVS_INT32 CRecordBackupMgr::GetCameraBackupCfg(const IVS_CHAR* pCameraCode, IVS_CHAR** pBackConfigXml)
{
	...
	(void)IVS_NEW((IVS_CHAR *&)*pBackConfigXml, strpRsp.size() + 1); //lint !e1924
	eSDK_MEMSET(*pBackConfigXml, 0, strpRsp.size() + 1);
	if (!CToolsHelp::Memcpy(*pBackConfigXml, strpRsp.size() + 1, (const void *)strpRsp.c_str(), strpRsp.size()))
	{
...
IVS_INT32 CRecordBackupMgr::GetRecordBackupPlan(const IVS_CHAR* pQueryInfo, IVS_CHAR** pBackupPlan)
{
	...
	eSDK_MEMSET(*pBackupPlan, 0, strpRsp.size() + 1);
	if (!CToolsHelp::Memcpy(*pBackupPlan, strpRsp.size() + 1, (const void *)strpRsp.c_str(), strpRsp.size()))
	{
...
IVS_INT32 CRecordBackupMgr::SetNodeFileNameValue(CXml& xml) const
{
	...
	if (xml.FindElemEx("Content"))
	{
		...
		// 如果是手动录像备份，需要开始和结束时间
		IVS_UINT32 uiBackupMethod = 0;
		GET_ELEM_VALUE_NUM_FOR_UINT("BackupMethod", pTemp, uiBackupMethod, xml); //lint !e838
		if (0 == uiBackupMethod)
		{
		...
		// 检查是否有备份时长节点
		if (!xml.FindElem("BackupTime"))
		{
			BP_RUN_LOG_ERR(IVS_XML_INVALID, "FindElem BackupTime failed", "NA");
			return IVS_XML_INVALID;
		}
		...
		if(1 == iSourceType)//录像来源(实时备份只有平台):0-平台 1-前端
		{
			...
			eSDK_MEMSET(&stRecordInfo, 0, sizeof(IVS_RECORD_INFO));
