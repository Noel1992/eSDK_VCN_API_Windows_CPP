...
IVS_INT32 CRecordService::GetRecordPolicy(const IVS_CHAR* pCameraCode, IVS_RECORD_POLICY_TIME* pRecordPolicy)const
{
	...
	string chDevCode = "";
	string chDomaCode = "";
	(void)CToolsHelp::SplitCameraDomainCode(pCameraCode,chDevCode,chDomaCode);//lint !e64 匹配
	//数据结构拼装XML并转换为const IVS_CHAR*
	CXml xmlReq;
	IVS_INT32 iRet = CRecordXMLProcess::GetRecordPolicyGetXML(xmlReq,chDevCode.c_str(),chDomaCode.c_str());//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CRecordService::SetRecordPolicy(const IVS_CHAR* pCameraCode,const IVS_RECORD_POLICY_TIME* pRecordPolicy)const
{
	...
	CXml xmlReq;
	IVS_INT32 iRet = CRecordXMLProcess::SetRecordPolicyGetXML(pRecordPolicy,chDevCode.c_str(),chDomaCode.c_str(),xmlReq);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CRecordService::GetExDomainRecordPolicy(const IVS_CHAR* pReqXml,IVS_CHAR** pRspXml)const
{
	...
	if (xmlRsp.FindElemEx("Content/CameraCode"))
	{
			...
			GET_ELEM_VALUE_CHAR("ExDomainCode",szElemValue,cDomainCode,IVS_DEV_CODE_LEN,xmlRsp);//lint !e838
			xmlRsp.RemoveElem();
	...
	if (!CToolsHelp::Memcpy(*pRspXml, strpRsp.size() + 1, strpRsp.c_str(), strpRsp.size()))//lint !e64 匹配
	{
...
IVS_INT32 CRecordService::GetPrerecordCameraList(IVS_CHAR** pRspXml)
{
	...
	for(IVS_UINT32 i=0;i<iSize;i++)
	{
		...
		(void)xmlReq.SetElemValue(strDomainCode.c_str());//lint !e64 匹配
	...
	if (!CToolsHelp::Memcpy(*pRspXml, strResult.size() + 1, strResult.c_str(), strResult.size()))//lint !e64 匹配
	{
...
IVS_INT32 CRecordService::AddExDomainRecordPlan( IVS_UINT32 uiDeviceNum,const IVS_DEVICE_CODE* pDeviceList, const IVS_RECORD_PLAN* pRecordPlan,IVS_RESULT_LIST* pResultList,IVS_UINT32 uiResultBufferSize ) const
{
    ...
    //数据结构拼装XML并转换为const IVS_CHAR *
    CXml xmlReq;
    (void)CRecordXMLProcess::AddOrModExDomainRecordPlanGetXML(uiDeviceNum,pDeviceList, pRecordPlan, xmlReq);
	...
	if (IVS_SUCCEED != iParseRSPRet)
	{
		BP_RUN_LOG_ERR(iParseRSPRet, "Add ExDomain Record Plan", "RecordPlan ParseRSP failed.");
		return iParseRSPRet;
	}
    ...
    const IVS_RECORD_PLAN* pRecordPlan,IVS_RESULT_LIST* pResultList,IVS_UINT32 uiResultBufferSize)const
{
    ...
    //数据结构拼装XML并转换为const IVS_CHAR *
    CXml xmlReq;
    (void)CRecordXMLProcess::AddOrModExDomainRecordPlanGetXML(uiDeviceNum,pDeviceList, pRecordPlan, xmlReq); //CRecordXMLProcess::AddOrModRecordPlanGetXML(uiDeviceNum,pDeviceList, pRecordPlan, xmlReq);
	...
	if (IVS_SUCCEED != iParseRSPRet)
	{
		BP_RUN_LOG_ERR(iParseRSPRet, "Mod ExDomain Record Plan", "RecordPlan Parse RSP failed.");
		return iParseRSPRet;
	}
...
IVS_INT32 CRecordService::DelExDomainRecordPlan( IVS_UINT32 uiDeviceNum,const IVS_DEVICE_CODE* pDeviceList, IVS_UINT32 uiRecordMethod,IVS_RESULT_LIST* pResultList,IVS_UINT32 uiResultBufferSize ) const
{
    ...
    //数据结构拼装XML并转换为const IVS_CHAR *
    CXml xmlReq;
    (void)CRecordXMLProcess::DelExDomainRecordPlanXML(uiDeviceNum,pDeviceList, uiRecordMethod, xmlReq); //CRecordXMLProcess::DelRecordPlanGetXML(uiDeviceNum,pDeviceList, uiRecordMethod, xmlReq);
	...
	if (IVS_SUCCEED != iParseRSPRet)
	{
		BP_RUN_LOG_ERR(iParseRSPRet, "Delete Platform Record Plan", "RecordPlan Parse RSP failed.");
		return iParseRSPRet;
	}
...
IVS_INT32 CRecordService::GetExDomainRecordPlan( const char* pCameraCode,IVS_UINT32 uiRecordMethod, IVS_RECORD_PLAN* pRecordPlan )
{
    ...
    //数据结构拼装XML并转换为const IVS_CHAR *
    CXml xmlReq;
    (void)CRecordXMLProcess::GetRecordPlanGetXML(pCameraCode, uiRecordMethod, xmlReq);
	...
	if (IVS_SUCCEED != iParseRSPRet)
	{
		BP_RUN_LOG_ERR(iParseRSPRet, "Get ExDomain Record Plan", "Get RecordPlan Parse RSP failed.");
		return iParseRSPRet;
	}
