...
{//lint !e1927 !e1926 !e1928
	...
	m_CodeHandle.clear();
	eSDK_MEMSET(&m_mediaPara,0,sizeof(m_mediaPara));
	eSDK_MEMSET(m_fileName,0,FILE_NAME_LEN+1);
	m_iFileBroadcastCycle = -1;
	m_iBroadcastType = -1;
	InitializeCriticalSectionAndSpinCount(&m_csCodeHandleLock, 4000 );
}
...
ULONG CAudioObj::GetProtocolType(char *pProtocol, ULONG ulProtocolType)
{
    ...
    case PROTOCOL_RTP_OVER_UDP:
        eSDK_STRNCPY(pProtocol, strlen("UDP") + 1, "UDP", strlen("UDP"));
        return PROTOCOL_RTP_OVER_UDP;
    case PROTOCOL_RTP_OVER_TCP:
        eSDK_STRNCPY(pProtocol, strlen("TCP")  + 1, "TCP", strlen("TCP"));
                                  ...
                                  const char* pCameraCode)
{
    ...
    iRet = IVS_PLAY_StartVoiceTalkback(const_cast<IVS_CHAR*>(m_strLocalIp.c_str()),
                              ...
                              BROADCAST_RTSP_INFO* pRtspInfo)
{
    ...
    if (iter != m_CodeHandle.end())
    {
        ...
        eSDK_MEMCPY(pstInfo, sizeof(*pstInfo), pRtspInfo, sizeof(*pRtspInfo));
    }
    else
    {
...
int CAudioObj::AddBroadcastDevice(const IVS_CHAR* pCameraCode)
{
	...
	int strVoiceCodeLen = (int)strlen(pCameraCode); //lint !e1924
	...
	// 如果是外域设备，不需要去获取设备的音频编码格式;
	std::string strTmpCode = pCameraCode;
	BP_DBG_LOG("AddBroadcastDevice Code[%s]", strTmpCode.c_str());

	strTmpCode = strTmpCode.substr(0, strTmpCode.rfind("#"));
	int iPos = (int)strTmpCode.find("#"); //lint !e1924
	if (0 > iPos)
	{
		iRet = GetSupportAudioType(pCameraCode, unAudioType);
		if (iRet != IVS_SUCCEED)
		{
    ...
    eSDK_MEMSET(pRtspInfo, 0, sizeof(BROADCAST_RTSP_INFO));
...
int CAudioObj::DeleteBroadcastDevice(const IVS_CHAR* pCameraCode)
{
	...
	if(m_bIsBroadcastOpenedFlag)
	{
		...
		if ( IVS_SUCCEED != iRet )
		{
            //从列表中删除.检查IVS_PLAY_StopVoiceBroadcast接口，
            //只有在校验阶段才返回失败，这样的情况下也应该删除列表
            DeleteCodeHandle(pCameraCode);
            return IVS_SDK_VOICE_ERR_STOP_BROADCAST_FAIL;
		}

        // modify by wanglei 00165153:2013.4.26 不发送停止，再次进行广播MU就会返回失败
        (void)StopPlay(pRtspInfo); // 发送RTSP停止广播
	}
                                    ...
                                    int iCycle)
{
    ...
    pRtspInfo->RtspHandle = (ULONG)m_iRtspHandle; //lint !e1924
...
int CAudioObj::StartRealBroadcast()
{
	...
	int codesSize = (int)m_CodeHandle.size(); //lint !e1924
	if(codesSize <= 0)
	{
		LeaveCriticalSection(&m_csCodeHandleLock);
    ...
    m_ulHandle = (unsigned long)this; //lint !e1924
	//下面开始遍历设备组并一一开启广播
	CODE_HANDLE_ITER iter = m_CodeHandle.begin();
	for( ;iter != m_CodeHandle.end();iter++)
	{
		...
		if(IVS_SUCCEED != iRet)//如果某路开启失败，不处理，继续开启其他设备
		{
            ...
            eSDK_MEMSET(&exceptionInfo, 0, sizeof(exceptionInfo));
            exceptionInfo.iSessionID = m_pUserMgr->GetSessionID();
            exceptionInfo.ulHandle = m_ulHandle;
           
            eSDK_STRNCPY(exceptionInfo.szDeviceCode, sizeof(exceptionInfo.szDeviceCode), pCameraCode.c_str(), IVS_DEV_CODE_LEN - 1);
...
int CAudioObj::StopSingleVoiceBroadcast(const BROADCAST_RTSP_INFO* pRtspInfo)
{
	...
	int iRet =  IVS_PLAY_StopVoiceBroadcast(const_cast<IVS_CHAR*>(pRtspInfo->ServiceIP),
                                            pRtspInfo->ServicePort);
...
int CAudioObj::StopPlay(const BROADCAST_RTSP_INFO* pRtspInfo) const
{
    ...
    int iRet = RTSP_CLIENT_SendTeardownMsg((int)pRtspInfo->RtspHandle,	//lint !e732
                                           pRtspInfo->szUrl,
                                           RTSP_CLIENT_TIMEOUT);
    if (RET_OK != iRet)
    {
    ...
    iRet = RTSP_CLIENT_FreeHandle((int)pRtspInfo->RtspHandle);//lint !e732
    if (IVS_SUCCEED != iRet)
    {
...
int CAudioObj::CheckFileExt(const char*pFileName)
{
	...
	int fileNameLen = (int)strlen(pFileName); //lint !e1924
	...
	eSDK_MEMCPY(fileNameExt, fileExtLen+1, &pFileName[tempFileNameLen],fileExtLen);
...
int CAudioObj::StartFileBroadcast(const IVS_CHAR* pFileName,IVS_INT32 iCycle)
{
	...
	//备份文件名
	eSDK_MEMCPY(m_fileName, FILE_NAME_LEN+1, pFileName,(unsigned long)fileNameLen);
	//备份文件广播方式
...
int CAudioObj::GetSupportAudioType(const char* pszCameraCode, unsigned int& unAudioType)
{
    ...
    eSDK_MEMSET(&stCameraCfg, 0, sizeof(stCameraCfg));
    ...
    eSDK_MEMSET(&stAudioCfg, 0, sizeof(stAudioCfg));
...
{
	return ulRtspHandle == (IVS_ULONG)m_iRtspHandle ? true : false; //lint !e1924
}

bool CAudioObj::IsExistBroadCastRtspHandle(IVS_ULONG ulRtspHandle)
{
     BP_RUN_LOG_ERR(11,"IsExistBroadCastRtspHandle",  "Param[0x%p]", ulRtspHandle);
    // 打印map值
    ...
    {
        const BROADCAST_RTSP_INFO *pRtspInfoTmp = iterBegin->second;
        BP_RUN_LOG_ERR(11,"IsExistBroadCastRtspHandle", "Map[%d]. RtspHandle[0x%p] NetSourceHandle[%lu]", iIndex, pRtspInfoTmp->RtspHandle, pRtspInfoTmp->NetSourceHandle);
        iIndex++;
    }

	if(iIndex == 0 && iterBegin == iterEnd)
	{
		BP_RUN_LOG_ERR(11,"IsExistBroadCastRtspHandle", "m_CodeHandle is null");
		return false;
	}
    ...
    for (; iter != m_CodeHandle.end(); ++iter)
    {
        const BROADCAST_RTSP_INFO* pstBroadCastRtspInfo = iter->second;
        if (NULL == pstBroadCastRtspInfo)
        {
            continue;
        }

        if (ulRtspHandle == (IVS_ULONG)pstBroadCastRtspInfo->RtspHandle) //lint !e1924
        {
...
int CAudioObj::SetAudioSampleRate(IVS_ULONG uAudioSampleRate)const
{
	return IVS_PLAY_SetVoiceSampleRate(uAudioSampleRate);	
}
