...
*********************************************************************/
#ifndef _SDK_TVWALL_DECODER_CHANNEL_H_
#define _SDK_TVWALL_DECODER_CHANNEL_H_
...
typedef std::map<IVS_ULONG, std::string> RTSP_HANDLE_MAP;	//key为RTSP handle，value为URL
typedef RTSP_HANDLE_MAP::iterator RTSP_HANDLE_MAP_ITER;
...
public:
	...
	// 启动电视墙实时浏览
	IVS_INT32 SwitchRealPlay(const IVS_CHAR* pCameraCode, 
		const IVS_REALPLAY_PARAM& stRealplayParam, 
		const IVS_TVWALL_PARAM& stTVWallParam);
...
public:
	...
	//设置大屏实况切换标识
	void SetSwitchPlay(const IVS_BOOL bIsSwitchPlay) {m_bIsSwitchPlay = bIsSwitchPlay;}
	//获取大屏实况切换标识
	const IVS_BOOL GetSwitchPlay(void) const {return m_bIsSwitchPlay;}
...
protected:
	...
	// 重新初始化通道
	virtual IVS_INT32 ReInitChannel(IVS_UINT32 uiProtocolType, MEDIA_ADDR& stDecoderAddr) = 0;
	// 启动通道开始解码
	virtual IVS_INT32 StartChannel(const std::string strURL, const MEDIA_ADDR stSourceMediaAddr) = 0;
	// 重新启动通道开始解码
	virtual IVS_INT32 ReStartChannel(const std::string strURL, const MEDIA_ADDR stSourceMediaAddr) = 0;
	// 结束通道解码
...
private:
    ...
    **************************************************************************/
	...
	//开始播放
	IVS_INT32 SwitchPlay(const TVWALL_PLAY_PARAM& stPlayParam,
		const IVS_TVWALL_PARAM& stTVWallParam,
		const IVS_CHAR* pFileName = NULL);
	...
	IVS_INT32 NotifySwitchPlay(const TVWALL_PLAY_PARAM& stPlayParam, const IVS_TVWALL_PARAM& stTVWallParam, 
		const ST_MEDIA_RSP& mediaRsp, const MEDIA_ADDR& stDecoderAddr);
...
public:
    ...
    IVS_ULONG m_ulHandle;							//当前播放的大屏切换前的RTSP Handle
    WND_STATUS m_wndStatus;					//窗格状态
    bool m_bReqStop;						//请求停止

	RTSP_HANDLE_MAP m_RtspHandleMap;		//存放多次切屏时对应的handle
	VOS_Mutex* m_pRtspHandleMapMutex;		//m_RtspHandleMap锁
...
protected:
	...
	IVS_BOOL m_bIsSwitchPlay;						//是否为大屏无缝实况切换
	CUserMgr* m_pUserMgr;
public:
	IVS_ULONG GetHandle(void)const {return m_ulHandle;}
	void FreeRTSPHandle(RTSP_HANDLE_MAP_ITER HandleIter) const;
	void FreeRTSPHandle(IVS_ULONG ulHandle);
	void FreeAllRTSPHandle(void);
};

#endif	//_SDK_TVWALL_DECODER_CHANNEL_H_
