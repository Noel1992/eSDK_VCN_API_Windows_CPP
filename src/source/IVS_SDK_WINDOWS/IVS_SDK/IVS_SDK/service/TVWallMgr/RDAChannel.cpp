...
IVS_INT32 CRDAChannel::ReInitChannel(IVS_UINT32 uiProtocolType, MEDIA_ADDR& stDecoderAddr)
{
	// 初始化解码器通道
	IPI_INIT_BUSINESS_REQ pParaInfo = {0};
	pParaInfo.ulAsyncSeq = m_ulAsyncSeq;
	...
	pParaInfo.ucTransportProtocol = (1==uiProtocolType) ? RDA_PROTOCOL_RTP_OVER_UDP:RDA_PROTOCOL_RTP_OVER_TCP;
	m_ucTransportProtocol = pParaInfo.ucTransportProtocol;
	IVS_INT32 iRet = IVS_TVWall_ReInitBusiness(&pParaInfo);
	if(IVS_SUCCEED!=iRet)
	{
		BP_RUN_LOG_ERR(iRet, "ReInit Channel", "Init Business failed!");
		return iRet;
	}

	BP_DBG_LOG("CRDAChannel::ReInitChannel: Send ReInit Channel Info Suc. usChannelNo = %d.", pParaInfo.usChannelNo);
	...
	if(IVS_SUCCEED!=iRet)
	{
		BP_RUN_LOG_ERR(iRet, "ReInit Channel", "Get Decoder ReInit Info Failed!");
		HandlePlayException();
		return iRet;
	}
	...
	if (IVS_SUCCEED != decoderInitInfo.stErrorCode.ulErrorCode)
	{
		BP_RUN_LOG_ERR(IVS_TVWALL_DECODER_INIT_CHANNEL_FAIL, "ReInit Channel", "ReInit Business failed!");
		(void)StopChannel();
		HandlePlayException();
		return IVS_TVWALL_DECODER_INIT_CHANNEL_FAIL;
	}
	...
	BP_DBG_LOG("CRDAChannel::ReInitChannel, get DecoderInitInfo success: \
			   DecoderID=%s,TCPPort=%d,UDPVideoPort=%d,UDPAudioPort=%d", 
			   decoderInitInfo.szDecoderID,
			   decoderInitInfo.stMediaNetInfo.usTcpInterleavedPort, 
			   decoderInitInfo.stMediaNetInfo.usUdpVideoRtpPort,
...
IVS_INT32 CRDAChannel::ReStartChannel(const std::string strURL, const MEDIA_ADDR stSourceMediaAddr)
{
	// 命令远程解码器开始解码
	IPI_START_BUSINESS_REQ stStartParaInfo = {0};
	stStartParaInfo.ulAsyncSeq = m_ulAsyncSeq;
	...
	if (IVS_SUCCEED != CMediaBase::AddrUint2Char(ntohl(stSourceMediaAddr.unMediaIp), strServiceIP))
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID,"ReStart Channel","Parse IP Failed!");  
		return IVS_PARA_INVALID;
	}
	...
	if (IVS_SUCCEED!=iRet)
	{
		BP_RUN_LOG_ERR(iRet, "ReStart Channel", "Parse TOKEN type failed from url, AsyncSeq=%lu", m_ulAsyncSeq);
		return iRet;
	}
	bSucc = CToolsHelp::Strncpy(stStartParaInfo.szSessionId,sizeof(stStartParaInfo.szSessionId), strSessionID.c_str(),strSessionID.length());
	CHECK_SUCCESS_RET(bSucc, IVS_ALLOC_MEMORY_ERROR);
	...
	iRet = IVS_TVWall_ReStartBusiness(&stStartParaInfo);
	return iRet;
}
