        ...
        if (!pRelink->GetIsConnected())
        {
            ...
            if (NET_ELE_OMU_NSS == pRelink->GetNetElemType())
            {
                ...
                else
                {
                    ...
                    // 重连失败，需要删除BP底层重连列表
                    (void)CNetFactory::Instance().DeleteClientLinkByAddr(pRelink->GetIP().c_str(), pRelink->GetPort(), pRelink->GetNetElemType());//lint !e64 匹配
            ...
            else
            {
                ...
                if (IVS_SUCCEED != iRet)
                {
                    ...
                    eSDK_STRNCPY(pszDomainCode, sizeof(pszDomainCode), stDomainRoute.cDomainCode, IVS_DOMAIN_CODE_LEN);
                    ...
                    || IVS_SMU_USER_PWD_NEED_MOD == iRet)
                {
                    ...
                    IVS_DELETE(pRelink);
                    EventCallBack fnCallBack = m_pUserMgr->GetEventCallBack();//lint !e64 匹配
                    if (NULL == fnCallBack)
                    {
                        continue;
                    }
                    ...
                    {
                        // 抛一个重连成功事件，并停止重连
                        fnCallBack(IVS_EVENT_AUTO_CONNECT_SUCCEED, NULL, 1, m_pUserMgr->GetEventUserData());//lint !e64 匹配
                    ...
                    else if (IVS_PASSWORD_ERROR == iRet)
                    {
                        ...
                        fnCallBack(IVS_EVENT_LOGIN_FAILED, NULL, 1, m_pUserMgr->GetEventUserData());//lint !e64 匹配
...
//判断断线重连的域是否为本域
bool CReLink::IsLocalDomain(const CRelinkInfo* pRelink) const
{
