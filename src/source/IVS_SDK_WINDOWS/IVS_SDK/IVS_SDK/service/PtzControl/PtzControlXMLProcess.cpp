...
IVS_INT32 PtzControlProcess::GetPtzPositionParseXML(const std::string& strRsp, IVS_PTZ_ABSPOSITION* pPTZPosInfo)
{
	...
	eSDK_MEMSET(pPTZPosInfo, 0, sizeof(IVS_PTZ_ABSPOSITION));

	CXml xml;
	CHECK_SUCCESS_RET(xml.Parse(strRsp.c_str()), IVS_SMU_XML_INVALID);//lint !e64 ∆•≈‰
...
IVS_INT32 PtzControlProcess::PtzControlParseXML(const std::string& strRsp, IVS_PTZ_CONTROL_INFO* pPtzControlInfo)
{
	...
	eSDK_MEMSET(pPtzControlInfo, 0, sizeof(IVS_PTZ_CONTROL_INFO));
	CXml xml;
	CHECK_SUCCESS_RET(xml.Parse(strRsp.c_str()), IVS_SMU_XML_INVALID);//lint !e64 ∆•≈‰
	...
	if (xml.FindElemEx("Content/LockUserInfo"))
	{
		xml.IntoElem();
		const char* AttriValue = NULL;
		GET_ELEM_VALUE_NUM_FOR_UINT("UserID", AttriValue, pPtzControlInfo->uiUserID, xml);
		...
		GET_ELEM_VALUE_NUM_FOR_UINT("ReleaseTimeRemain", AttriValue, pPtzControlInfo->uiReleaseTimeRemain, xml);
	}
...
IVS_INT32 PtzControlProcess::AddPTZPresetParseXML(const std::string& strRsp, IVS_UINT32* uiPresetIndex)
{
	...
	CXml xml;
	if (!xml.Parse(strRsp.c_str()))//lint !e64 ∆•≈‰
	{
...
IVS_INT32 PtzControlProcess::DelPTZPresetGetXML(const IVS_CHAR* pCameraCode, IVS_UINT32 uiPresetIndex, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)uiPresetIndex).c_str());//lint !e64 ∆•≈‰
	xml.OutOfElem();
	xml.OutOfElem();
...
IVS_INT32 PtzControlProcess::ModPTZPresetGetXML(const IVS_CHAR* pCameraCode, const IVS_PTZ_PRESET* pPTZPreset, CXml &xml)
{
	...
	(void)xml.AddElem("Index");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)(pPTZPreset->uiPresetIndex)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("Name");
	(void)xml.IntoElem();
...
IVS_INT32 PtzControlProcess::SetGuardPosGetXML(const IVS_CHAR* pCameraCode, const IVS_GUARD_POS_INFO* pGuardPosInfo, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)(pGuardPosInfo->bEnableGuardPosition)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("WaitTime");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)(pGuardPosInfo->uiWaitTime)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("PresetIndex");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)(pGuardPosInfo->uiPresetIndex)).c_str());//lint !e64 ∆•≈‰
	xml.OutOfElem();
	xml.OutOfElem();
...
IVS_INT32 PtzControlProcess::AddCruiseTrackGetXML(const IVS_CHAR* pCameraCode, const IVS_CRUISE_TRACK* pCuriseTrackInfo, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)(pCuriseTrackInfo->uiCruiseType)).c_str());//lint !e64 ∆•≈‰
	...
	for (unsigned int i=0; i < pCuriseTrackInfo->uiTrackPointNum; i++)
	{
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruiseTrackPoint.uiPresetIndex).c_str());//lint !e64 ∆•≈‰
		(void)xml.AddElem("DwellTime");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruiseTrackPoint.uiDwellTime).c_str());//lint !e64 ∆•≈‰
		(void)xml.AddElem("Speed");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruiseTrackPoint.uiSpeed).c_str());//lint !e64 ∆•≈‰
		xml.OutOfElem();
	}
...
IVS_INT32 PtzControlProcess::DelCruiseTrackGetXML(const IVS_CHAR* pCameraCode, IVS_UINT32 uiTrackIndex, IVS_UINT32 uiCruiseType, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)uiTrackIndex).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("CruiseType");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)uiCruiseType).c_str());//lint !e64 ∆•≈‰
	xml.OutOfElem();
	xml.OutOfElem();
...
IVS_INT32 PtzControlProcess::ModCruiseTrackGetXML(const IVS_CHAR* pCameraCode, const IVS_CRUISE_TRACK* pCuriseTrackInfo, CXml &xml)
{
	...
	(void)xml.AddElem("CruiseNO");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)(pCuriseTrackInfo->uiCruiseNO)).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("Name");
	(void)xml.IntoElem();
	IVS_CHAR szTemp[IVS_CURISE_TRACK_NAME_LEN + 1] = {0};
	(void)CToolsHelp::Memcpy(szTemp, IVS_CURISE_TRACK_NAME_LEN + 1, pCuriseTrackInfo->cTrackName, IVS_CURISE_TRACK_NAME_LEN);
...
IVS_INT32 PtzControlProcess::SetCruisePlanGetXML(const IVS_CHAR* pCameraCode, const IVS_CRUISE_PLAN* pCruisePlan, CXml &xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((int)pCruisePlan->uiPlanType).c_str());//lint !e64 ∆•≈‰
	(void)xml.AddElem("IsEnable");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str((int)pCruisePlan->bEnable).c_str());//lint !e64 ∆•≈‰
	...
	for (unsigned int i=0; i < pCruisePlan->uiPlanInfoNum; i++)
	{
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruisePlanInfo.uiDayType).c_str());//lint !e64 ∆•≈‰
		(void)xml.AddElem("TimeInfoNum");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruisePlanInfo.uiTimeSpanInfoNum).c_str());//lint !e64 ∆•≈‰
		...
		for (unsigned int j = 0; j < stCruisePlanInfo.uiTimeSpanInfoNum; j++)
		{
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruiseTimeInfo.uiCruiseType).c_str());//lint !e64 ∆•≈‰
			(void)xml.AddElem("TrackIndex");
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stCruiseTimeInfo.uiCruiseNO).c_str());//lint !e64 ∆•≈‰
			xml.OutOfElem();
			xml.OutOfElem();
		}
		xml.OutOfElem();
...
IVS_INT32 PtzControlProcess::GetCruisePlanParseXML(const std::string& strRsp, IVS_CRUISE_PLAN* pCruisePlan)
{
	...
	CXml xml;
	CHECK_SUCCESS_RET(xml.Parse(strRsp.c_str()), IVS_FAIL);//lint !e64 ∆•≈‰
