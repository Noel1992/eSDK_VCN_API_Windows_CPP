	...
	const IVS_CHAR* pConfirmTime,const std::string& strUserDomainCode,const std::string& strDomainCode,CXml &xml)
{
	...
	eSDK_SPRINTF(szAlarmEventID, sizeof(szAlarmEventID), "%lld",ullAlarmEventID);
		...
		IVS_UINT32 uiBufferSize, CXml &Rspxml)
{
	...
	GET_ELEM_VALUE_NUM("RealNum", pTemp, iTotal, Rspxml);//lint !e838 !e64 匹配
	GET_ELEM_VALUE_NUM("ToIndex", pTemp,  iToTemp, Rspxml);//lint !e838 !e64 匹配
	GET_ELEM_VALUE_NUM("FromIndex", pTemp, iFromTemp, Rspxml);//lint !e838 !e64 匹配
	...
	do 
	{
		...
		SecureZeroMemory(&stAlarmInfo, sizeof(stAlarmInfo));
		SecureZeroMemory(cDevCode, IVS_DEV_CODE_LEN + 1);
		
		GET_ELEM_VALUE_CHAR("CheckoutOperator", pTemp, stAlarmInfo.cCheckoutOperator ,IVS_NAME_LEN, Rspxml);//lint !e64 匹配

		GET_ELEM_VALUE_NUM_FOR_UINT64("AlarmEventID", pTemp, stAlarmInfo.ullAlarmEventID,Rspxml); //lint !e64 匹配

		GET_ELEM_VALUE_CHAR("AlarmType", pTemp, stAlarmInfo.cAlarmType , IVS_ALARM_TYPE_LEN, Rspxml); //lint !e64 匹配 

		//对设备编码需要添加上域编码返回个CU
		GET_ELEM_VALUE_CHAR("CameraCode", pTemp, cDevCode , IVS_DEV_CODE_LEN, Rspxml);//lint !e64 匹配
		strDevCode = cDevCode;
		strDevCode.append(strAppendDomainCode);
		eSDK_MEMCPY(stAlarmInfo.cCameraCode, IVS_DEV_CODE_LEN, strDevCode.c_str(),strDevCode.size());

		GET_ELEM_VALUE_CHAR("OccurStartTime", pTemp, stAlarmInfo.cOccurStartTime ,IVS_TIME_LEN, Rspxml); //lint !e64 匹配
 
		GET_ELEM_VALUE_CHAR("OccurStopTime", pTemp, stAlarmInfo.cOccurStopTime , IVS_TIME_LEN, Rspxml);  //lint !e64 匹配

		GET_ELEM_VALUE_NUM_FOR_UINT("WatermarkAlarmStatus", pTemp, stAlarmInfo.uiWatermarkAlarmStatus,Rspxml); //lint !e64 匹配

		GET_ELEM_VALUE_CHAR("ConfirmOperator", pTemp, stAlarmInfo.cConfirmOperator, IVS_NAME_LEN, Rspxml); //lint !e64 匹配
	
		GET_ELEM_VALUE_CHAR("ConfirmTime", pTemp,  stAlarmInfo.cConfirmTime, IVS_TIME_LEN, Rspxml); //lint !e64 匹配

		GET_ELEM_VALUE_CHAR("ConfirmInfo", pTemp,stAlarmInfo.cConfirmInfo, IVS_WATERMARK_ALARM_DESCRIPTION_LEN, Rspxml);//lint !e64 匹配
...
IVS_INT32 CAlarmMgrXMLProcess::AlarmLevelGetXML(const IVS_ALARM_LEVEL *pAlarmLevel, IVS_INT32 iReqType, CXml &xml)
{
	...
	eSDK_MEMSET(pointer, 0, POINTER_MAX_NUM);
	(void)xml.AddDeclaration("1.0","UTF-8","");
	(void)xml.AddElem("Content");
	...
	(void)xml.AddChildElem("AlarmLevel");
	{
		...
		if (1 == iReqType)
		{
			...
			(void)xml.IntoElem();
			(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmLevel->uiAlarmLevelId)).c_str());//lint !e64 匹配
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmLevel->uiValue)).c_str());//lint !e64 匹配
		...
		(void)xml.SetElemValue(pointer);
		SecureZeroMemory(pointer, POINTER_MAX_NUM);

		(void)xml.AddElem("AlarmLevelDesc");
		(void)xml.IntoElem();
		IVS_CHAR cAlarmLevelDesc[IVS_ALARM_DESCRIPTION_LEN + 1];
		eSDK_MEMSET(cAlarmLevelDesc, 0x0, IVS_ALARM_DESCRIPTION_LEN + 1);
...
IVS_INT32 CAlarmMgrXMLProcess::DeleteAlarmLevelGetXML(const IVS_UINT32 uiAlarmLevelId, CXml &xml)
{
	...
	(void)xml.AddChildElem("AlarmLevelID");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(uiAlarmLevelId).c_str());//lint !e713 !e64 匹配
	xml.OutOfElem();
...
IVS_INT32 CAlarmMgrXMLProcess::GetAlarmLevelGetXML(IVS_UINT32 uiAlarmLevelID, CXml& xml)
{
	...
	(void)xml.AddChildElem("AlarmLevelID");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiAlarmLevelID)).c_str());//lint !e64 匹配
	xml.OutOfElem();
...
IVS_INT32 CAlarmMgrXMLProcess::HelpInfoGetXML(const IVS_HELP_INFO *helpInfo, CXml &xml)
{
	...
	eSDK_MEMSET(pointer, 0, IVS_MAX_HELP_LEN + 1);
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(helpInfo->uiSourceId)).c_str());//lint !e64 匹配

	(void)xml.AddElem("SourceType");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(helpInfo->uiSourceType)).c_str());//lint !e64 匹配
...
IVS_INT32 CAlarmMgrXMLProcess::DeleteHelpInfoGetXML(const IVS_UINT32 uiSourceId,IVS_UINT32 uiSourceType, CXml &xml)
{
	...
	(void)xml.AddChildElem("SourceID");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiSourceId)).c_str());//lint !e64 匹配

	(void)xml.AddElem("SourceType");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiSourceType)).c_str());//lint !e64 匹配
	xml.OutOfElem();
...
{	
	...
	(void)xml.AddChildElem("SourceID");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiAlarmHelpSourceId)).c_str());//lint !e64 匹配

	(void)xml.AddElem("SourceType");
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiAlarmHelpSourceType)).c_str());//lint !e64 匹配
...
IVS_INT32 CAlarmMgrXMLProcess::SetAlarmTypeLevelGetXML(const IVS_ALARM_TYPE_LIST* pAlarmTypeList, CXml& xml)
{
	...
	for (unsigned int i=0;i<pAlarmTypeList->iTotal;i++)
	{
		...
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stAlarmType.uiID).c_str());//lint !e64 匹配
		(void)xml.AddElem("AlarmLevelID");
		(void)xml.SetElemValue(CToolsHelp::Int2Str((int)stAlarmType.stAlarmLevel.uiAlarmLevelId).c_str());//lint !e64 匹配
		xml.OutOfElem();
	}
	xml.OutOfElem();
	xml.OutOfElem();
...
IVS_INT32 CAlarmMgrXMLProcess::GetAlarmTypeListParseXML(IVS_ALARM_TYPE_LIST* pAlarmTypeList, CXml &xml, IVS_UINT32 uiBufferSize)
{
	...
	const char* pTemp = NULL;
	IVS_INT32 iTemp;
	GET_ELEM_VALUE_NUM("FromIndex", pTemp, iTemp, xml);//lint !e732 !e838
	pAlarmTypeList->stIndexRange.uiFromIndex = static_cast<IVS_UINT32>(iTemp);

	GET_ELEM_VALUE_NUM("ToIndex", pTemp, iTemp, xml);//lint !e732 !e838
	pAlarmTypeList->stIndexRange.uiToIndex = static_cast<IVS_UINT32>(iTemp);

	GET_ELEM_VALUE_NUM("RealNum", pTemp, iTemp , xml);//lint !e734 !e732 !e838
...
IVS_INT32 CAlarmMgrXMLProcess::GetAlarmNotifyGetXML(const IVS_ALARM_NOTIFY* palarmNotify,CXml& xml)
{
	...
	(void)xml.IntoElem();
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<int>(palarmNotify->ullAlarmEventID)).c_str());//lint !e64 匹配
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(palarmNotify->uiAlarmInType)).c_str());//lint !e64 匹配
	...
	eSDK_MEMSET(pointer, 0, POINTER_MAX_NUM);
	(void)xml.AddElem("AlarmLevelValue");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(palarmNotify->uiAlarmLevelValue)).c_str());//lint !e64 匹配
	(void)xml.AddElem("AlarmLevelName");
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(palarmNotify->uiAlarmStatus)).c_str());//lint !e64 匹配
	(void)xml.AddElem("OccurNumber");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(palarmNotify->uiOccurNumber)).c_str());//lint !e64 匹配

	(void)xml.AddElem("IsCommission");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(palarmNotify->bIsCommission).c_str());//lint !e64 匹配
	(void)xml.AddElem("AlarmDesc");
	IVS_CHAR cAlarmDesc[IVS_ALARM_DESCRIPTION_LEN + 1];
	eSDK_MEMSET(cAlarmDesc, 0, IVS_ALARM_DESCRIPTION_LEN + 1);
	...
	(void)xml.SetElemValue(CToolsHelp::Int2Str(palarmNotify->bExistsRecord).c_str());//lint !e64 匹配
	xml.OutOfElem();
...
// 查询告警类型列表拼接xml
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetAlarmAreaPlanXML(const IVS_CHAR* pDomainCode, IVS_UINT32 uiAlarmAreaID, CXml& xml)
 {
	IVS_DEBUG_TRACE("");
	 ...
	 (void)xml.IntoElem();
	 (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(uiAlarmAreaID)).c_str());//lint !e64 匹配
	 xml.OutOfElem();
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetAlarmListParseXML(IVS_ALARM_EVENT_LIST* pAlarmEventList, IVS_UINT32 uiBufferSize, CXml &xml, const std::string& strDomainCode)
 {
	 ...
	 GET_ELEM_VALUE_NUM("RealNum", pTemp, iTotal , xml);//lint !e838 !e64 匹配
	 GET_ELEM_VALUE_NUM("FromIndex", pTemp, iFromIndex, xml);//lint !e838 !e64 匹配
	 GET_ELEM_VALUE_NUM("ToIndex",pTemp, iToIndex , xml);//lint !e838 !e64 匹配
	 ...
	 while(i < uiResNum)
	 {
		 ...
		 const char* szElemValue = NULL;
		 IVS_INT32 iTemp = 0;
		 GET_ELEM_VALUE_NUM("AlarmEventID", szElemValue,  iTemp, xml);//lint !e838 !e64 匹配
		 pAlarmEvent.stAlarmNotify.ullAlarmEventID = static_cast<IVS_UINT64>(iTemp);//lint !e571

		 GET_ELEM_VALUE_CHAR("AlarmInCode", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmInCode, IVS_ALARM_CODE_LEN, xml);//lint !e64 匹配
		 if (!CToolsHelp::Memcpy(pAlarmEvent.stAlarmNotify.cDevDomainCode, IVS_DOMAIN_CODE_LEN, strDomainCode.c_str(), strDomainCode.size()))//lint !e64 匹配
		 {
		 ...
		 GET_ELEM_VALUE_NUM("AlarmInType", szElemValue, iTemp, xml);//lint !e64 匹配
		 pAlarmEvent.stAlarmNotify.uiAlarmInType = static_cast<IVS_UINT32>(iTemp);
		 GET_ELEM_VALUE_CHAR("AlarmInName", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmInName, IVS_ALARM_NAME_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_NUM("AlarmLevelValue", szElemValue, iTemp, xml);//lint !e64 匹配
		 pAlarmEvent.stAlarmNotify.uiAlarmLevelValue = static_cast<IVS_UINT32>(iTemp);
		 GET_ELEM_VALUE_CHAR("AlarmLevelName", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmLevelName, IVS_NAME_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("AlarmLevelColor", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmLevelColor, IVS_ALARM_LEVEL_COLOR_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("AlarmType", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmType, IVS_ALARM_CODE_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("AlarmCategory", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmCategory, IVS_ALARM_CATEGORY_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("OccurTime", szElemValue, pAlarmEvent.stAlarmNotify.cOccurTime, IVS_TIME_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_NUM_FOR_UINT("OccurNumber", szElemValue, pAlarmEvent.stAlarmNotify.uiOccurNumber, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_NUM("AlarmStatus", szElemValue, iTemp, xml);//lint !e64 匹配
		 pAlarmEvent.stAlarmNotify.uiAlarmStatus = static_cast<IVS_UINT32>(iTemp);
		 GET_ELEM_VALUE_CHAR("Operator", szElemValue, pAlarmEvent.stOperateInfo.cOperatorName, IVS_NAME_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("OperateInfo", szElemValue, pAlarmEvent.stOperateInfo.cOperateInfo, IVS_ALARM_DESCRIPTION_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("OperateTime", szElemValue, pAlarmEvent.stOperateInfo.cOperateTime, IVS_TIME_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("AlarmDesc", szElemValue, pAlarmEvent.stAlarmNotify.cAlarmDesc, IVS_ALARM_DESCRIPTION_LEN, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_NUM("ExistsRecord", szElemValue, pAlarmEvent.stAlarmNotify.bExistsRecord, xml);//lint !e64 匹配
		 GET_ELEM_VALUE_CHAR("PreviewUrl", szElemValue, pAlarmEvent.stAlarmNotify.cPreviewUrl, IVS_URL_LEN, xml);//lint !e64 匹配
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetAlarmStatusGetXML(const IVS_ALARM_EVENT* pAlarmEvent, CXml& xml)
 {
	...
	(void)xml.IntoElem();
	{
		(void)xml.AddChildElem("AlarmEventId");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmEvent->stAlarmNotify.ullAlarmEventID)).c_str());//lint !e64 匹配

		(void)xml.AddElem("AlarmInCode");
		(void)xml.IntoElem();
		IVS_CHAR cAlarmInCode[IVS_ALARM_CODE_LEN + 1];
		eSDK_MEMSET(cAlarmInCode, 0x0, IVS_ALARM_CODE_LEN + 1);
		...
		eSDK_MEMSET(cAlarmType, 0x0, IVS_ALARM_CODE_LEN + 1);
		...
		(void)xml.AddElem("AlarmStatus");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmEvent->stAlarmNotify.uiAlarmStatus)).c_str());//lint !e64 匹配

		(void)xml.AddElem("OccurNumber");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmEvent->stAlarmNotify.uiOccurNumber)).c_str());//lint !e64 匹配
	}
	xml.OutOfElem();
	...
	{	
		...
		{
			strOperatorId = CToolsHelp::Int2Str(static_cast<IVS_INT32>(pAlarmEvent->stOperateInfo.uiOperatorID)).c_str();//lint !e64 匹配
		}


		(void)xml.AddChildElem("OperatorId");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(strOperatorId.c_str());//lint !e64 匹配

		(void)xml.AddElem("OperatorName");
		(void)xml.IntoElem();
		IVS_CHAR cOperatorName[IVS_NAME_LEN + 1];
		eSDK_MEMSET(cOperatorName, 0x0, IVS_NAME_LEN + 1);
		...
		eSDK_MEMSET(cOperateTime, 0x0, IVS_TIME_LEN + 1);
		...
		eSDK_MEMSET(cReserve, 0x0, ALARM_RESERVER_LEN + 1);
		...
		eSDK_MEMSET(cOperateInfo, 0x0, IVS_ALARM_DESCRIPTION_LEN + 1);
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetAlarmInfoGetXML(IVS_UINT64 ullAlarmEventID, CXml &xml)
 {
	 ...
	 (void)xml.AddChildElem("AlarmEventID");
	 (void)xml.IntoElem();
	 (void)xml.SetElemValue(CToolsHelp::Int2Str(static_cast<IVS_INT32>(ullAlarmEventID)).c_str());//lint !e64 匹配
	 xml.OutOfElem();
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetAlarmInfoParseXML(IVS_ALARM_EVENT* pAlarmEvent, CXml &xml, const IVS_CHAR* pDomainCode)
 {
	 ...
	 {
		 IVS_INT32 iTemp = 0;
		 GET_ELEM_VALUE_NUM("AlarmEventID", pTemp, iTemp, xml);//lint !e838
		 pAlarmEvent->stAlarmNotify.ullAlarmEventID = static_cast<IVS_UINT64>(iTemp); //lint !e571
 ...
 // 遍历是存在这个节点数据
	 ...
	 TiXmlElement* pConnListElem = CToolsHelp::FindElementByPath(pXmlDoc,"Content/AbilityList/Ability");//lint !e64 匹配
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetDeviceUserListParseXML(IVS_DEVICE_USER_INFO_LIST* pDevUserList, IVS_UINT32 uiBufferSize, CXml &xml)
 {
	 ...
	 IVS_INT32 iFromIndex = 0;
	 IVS_INT32 iToIndex = 0;
	 GET_ELEM_VALUE_NUM("RealNum", pTemp, iTotal , xml);//lint !e838
	 GET_ELEM_VALUE_NUM("FromIndex", pTemp, iFromIndex, xml);//lint !e838
	 GET_ELEM_VALUE_NUM("ToIndex",pTemp, iToIndex , xml);//lint !e838
	 ...
	 do 
	 {
		 ...
		 const char* szElemValue = NULL;
		 IVS_INT32 iTemp = 0;
		 GET_ELEM_VALUE_NUM("UserID", szElemValue,  iTemp, xml);//lint !e838
	 ...
	 xml.OutOfElem();
	 BP_RUN_LOG_INF("Parse DevUserInfo xml","RealGetNum: %d, FromIndex: %d, ToIndex : %d ",i,iFromIndex,iToIndex);
	 return IVS_SUCCEED;
 }
 ...
 IVS_INT32 CAlarmMgrXMLProcess::SetAlarmInCodeValue(CXml& xml)
 {
    ...
    do 
    {
        ...
        do 
        {
			...
			if (IVS_SUCCEED != iRet)
			{
				BP_RUN_LOG_ERR(iRet, "Set AlarmIn Code Value", "Parse DevCode failed");
				return iRet;
			}
 ...
 IVS_INT32 CAlarmMgrXMLProcess::ModifyAlarmInCodeOfGetAlarmList(CXml& xml, std::string& strDstDomainCode)
 {
	 ...
	 do 
	 {
		 ...
		 {
			 xml.OutOfElem();
			 eSDK_MEMSET(szElemField, 0x0, IVS_DEV_CODE_LEN + 1);
			 continue;
		 }
		 ...
		 {
			 char* pNext = NULL;
			 char* token = strtok_s(cAlarmInCode, ",", &pNext); //lint !e64 匹配
             while (NULL!= token)
             {
                 IVS_CHAR szAlarmInCode[IVS_ALARM_CODE_LEN + 1] = {0};
                 IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
                 iRet += CXmlProcess::ParseDevCode(token, szAlarmInCode, szDomainCode);
                 strDstDomainCode = szDomainCode;   //lint !e64 匹配 // 消息发送目标域
                 strAlarmInCode.append(szAlarmInCode).append(",");
                 token = strtok_s(NULL, ",", &pNext);//lint !e64 匹配
             }
			 strAlarmInCode = strAlarmInCode.substr(0, strAlarmInCode.size() - 1);
			 xml.ModifyElemValue(strAlarmInCode.c_str());//lint !e64 匹配
			 break;
		 }
		 else  if(!strElemValue.empty())
		 {
			 IVS_CHAR szAlarmInCode[IVS_ALARM_CODE_LEN + 1] = {0};
			 IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
			 iRet = CXmlProcess::ParseDevCode(strElemValue.c_str(), szAlarmInCode, szDomainCode);//lint !e64 匹配
             strDstDomainCode = szDomainCode;   //lint !e64 匹配 // 消息发送目标域
	 ...
	 if (xml.FindElemEx("Content/PageInfo/QueryCond"))
	 {
		...
		do 
		{
			...
			if (!strValue.empty())
			{
				IVS_CHAR szAlarmInCode[IVS_ALARM_CODE_LEN + 1] = {0};
				IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
				(void)CXmlProcess::ParseDevCode(strValue.c_str(), szAlarmInCode, szDomainCode);//lint !e64 匹配
				xml.ModifyElemValue(szAlarmInCode);
			}
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetRsqXML(const IVS_CHAR* pRsp, IVS_CHAR*& pRspXml)
 {
	 ...
	 eSDK_MEMSET(pResult, 0x0, strlen(pRsp) + 1);
	 ...
	 if (xml.FindElemEx("Content/AlarmAreaInfo/AlarmInList"))
	 {
		 ...
		 if (xml.FindElem("AlarmInInfo"))
		 {
			 do 
			 {
				 ...
				 iRet = CXmlProcess::ParseDevCode(strAlarmInCode.c_str(), szAlarmInCode, szDomainCode);//lint !e64 匹配
				 xml.ModifyElemValue(szAlarmInCode);
				 (void)xml.AddElem("DevDomainCode");
				 (void)xml.SetElemValue(szDomainCode);
...
// 获得消息目标域
 ...
 IVS_INT32 CAlarmMgrXMLProcess::SetDevCode(CXml& xml, const IVS_CHAR* pReqXml, const std::string& strPath, IVS_CHAR* pDevDomainCode, IVS_CHAR* pDevCode)
 {
	 ...
	 BP_RUN_LOG_INF("strPath: %s",strPath.c_str());//lint !e64 匹配
	 IVS_DEBUG_TRACE("");
	 ...
	 std::string strDevCode;
     const IVS_CHAR* pDevCodeTemp = xml.GetElemValue();//lint !e64 匹配
     if (NULL != pDevCodeTemp)
     {
         strDevCode = pDevCodeTemp;
     }
	 IVS_INT32 iRet = CXmlProcess::ParseDevCode(strDevCode.c_str(), pDevCode, pDevDomainCode);//lint !e64 匹配
	 if (!strDevCode.empty())
	 {
 ...
 IVS_INT32 CAlarmMgrXMLProcess::ParseLocalDevCode(CXml& xml, const IVS_CHAR* pReqXml, const IVS_CHAR* pGrandPath, const IVS_CHAR* pParentPath)
 {
	 ...
	 do 
	 {
		 ...
		 iRet = CXmlProcess::ParseDevCode(strLocalDevCode.c_str(), szDevLocalCode, szDevDomainCode);//lint !e64 匹配
 ...
 IVS_INT32 CAlarmMgrXMLProcess::ModifyLocalDevCode(CXml& xmlRsp, const std::string& strDomainCode, const IVS_CHAR* pGrandPath, const IVS_CHAR* pParentPath)
 {
	 ...
	 if (xmlRsp.FindElemEx(pGrandPath))
	 {
		 ...
		 do 
		 {
			 ...
			 std::string strLocalDevCode;
             const IVS_CHAR* pLocalDevCode = xmlRsp.GetElemValue();//lint !e64 匹配
             if (NULL != pLocalDevCode)
             {
                 strLocalDevCode = pLocalDevCode;
             }
 ...
 IVS_INT32 CAlarmMgrXMLProcess::GetAlarmSnapshotListGetXml(const std::string& strInXml, std::string& strOutXml, IVS_INT32& iFlag)
 {
	 ...
	 (void)(pSnapShotXmlDoc.Parse(strOutXml.c_str(), NULL, TIXML_ENCODING_LEGACY));//lint !e64 匹配
	 if (0 != pSnapShotXmlDoc.Error())
	 {
		 BP_RUN_LOG_ERR(IVS_PARA_INVALID, "parse xml failed","NA");
		 return IVS_PARA_INVALID;
	 }
	 TiXmlElement* pSnapshotListElem = CToolsHelp::FindElementByPath(&pSnapShotXmlDoc,"Content/SnapshotList");//lint !e64 匹配
	 ...
	 (void)(pXmlDoc.Parse(strInXml.c_str(), NULL, TIXML_ENCODING_LEGACY));//lint !e64 匹配
	 if (0 != pXmlDoc.Error())
	 {
		 BP_RUN_LOG_ERR(IVS_PARA_INVALID, "parse xml failed","NA");
		 return IVS_PARA_INVALID;
	 ...
	 // 如果没有信息返回，smu返回空字符串
	 TiXmlElement* pListElem = CToolsHelp::FindElementByPath(&pXmlDoc,"Content/SnapshotList");//lint !e64 匹配
	 if (NULL != pListElem)
	 {
