...
CDeviceUserListReqItem::CDeviceUserListReqItem(void)
{
	iFromIndex = 0;
	strDeviceUserGroupIDs = "";
	time_t tCreateTime = 0;
	(void)BP_OS::time(&tCreateTime);
...
IVS_INT32 CAlarmMgr::SubscribeQuery(const IVS_CHAR* pReqXml, IVS_CHAR** pRspXml) const
{
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Subscribe Query", "Get Dst DomainCode failed");
		return iRet;
	}
	...
	else if (xmlRsq.FindElem("SubscribeInfo"))
	{
		for (;;)
		{
            ...
            {
                pDevDoaminCode = strDstDomainCode.c_str();//lint !e64 匹配
            }
			...
			{				
				...
				xmlRsq.ModifyElemValue(strBigCode.c_str());//lint !e64 匹配
			}			
			xmlRsq.OutOfElem();
			if (!xmlRsq.NextElem())
			{
	...
	eSDK_MEMSET(pResult, 0x0, strlen(pRsq) + 1);
...
IVS_INT32 CAlarmMgr::ConfirmAlarm(IVS_UINT64 ullAlarmEventID, const IVS_CHAR* pAlarmInCode, const IVS_ALARM_OPERATE_INFO* pAlarmOperateInfo)
{
	...
	eSDK_MEMSET(pData, 0, iBufferSize);

	IVS_UINT64 uiID = 0;
	BP_htonll(ullAlarmEventID, uiID);//lint !e838
	...
	if (!CToolsHelp::Memcpy(pData + sizeof(IVS_UINT64), iBufferSize - sizeof(IVS_UINT64), pOperationInfo, strlen(pOperationInfo) + 1))
	{
		...
		free(pOperationInfo);
		pOperationInfo = NULL;
		return IVS_ALLOC_MEMORY_ERROR;//lint !e438
	}
...
IVS_INT32 CAlarmMgr::ConfirmWaterAlarm(const IVS_WATERMARK_ALARM_CONFIRM* pConfirmInfo)
{
	...
	IVS_UINT64 uiID = pConfirmInfo->ullAlarmEventID;
	std::string strCameraDomainCode = pConfirmInfo->cCameraCode;
	std::string strCameraCode;
	std::string strDomainCode;
	std::string strUserDomainCode;
	IVS_INT32 iRet = CToolsHelp::SplitCameraDomainCode(strCameraDomainCode.c_str(), strCameraCode, strDomainCode);
	if (IVS_SUCCEED != iRet)
	{
	...
	CXml xmlReq;
	IVS_UINT32 xmlLen = 0;
	(void)CAlarmMgrXMLProcess::GetConfirmWaterAlarmXML(uiID,pConfirmInfo->cConfirmInfo,pConfirmInfo->cConfirmTime,strUserDomainCode,strDomainCode, xmlReq);
	const IVS_CHAR* pReq = xmlReq.GetXMLStream(xmlLen);
	CHECK_POINTER(pReq, IVS_OPERATE_MEMORY_ERROR);
...
IVS_INT32 CAlarmMgr::CancelWaterAlarm(const IVS_WATERMARK_ALARM_CANCEL* pstConfirmInfo)
{
	...
	IVS_UINT64 uiID = pstConfirmInfo->ullAlarmEventID;
	...
	CXml xmlReq;
	IVS_UINT32 xmlLen = 0;
	(void)CAlarmMgrXMLProcess::GetCancelWaterAlarmXML(uiID,pstConfirmInfo->cCancelInfo,pstConfirmInfo->cCancelTime,strUserDoaminCode,strDomainCode, xmlReq);
	const IVS_CHAR* pReq = xmlReq.GetXMLStream(xmlLen);
	CHECK_POINTER(pReq, IVS_OPERATE_MEMORY_ERROR);
	//创建要发送的CMD，拼装了NSS消息头
...
IVS_INT32 CAlarmMgr::AlarmCommission(const IVS_CHAR* pAlarmInCode, const IVS_ALARM_COMMISSION* pAlarmCommission)
{
	...
	eSDK_MEMSET(pData, 0x0, uiBufferSize);
	
	IVS_UINT64 ulAlarmEventID = 0;
	BP_htonll(pAlarmCommission->ullAlarmEventID, ulAlarmEventID);//lint !e838
	if (!CToolsHelp::Memcpy(pData, uiBufferSize, &ulAlarmEventID, sizeof(ulAlarmEventID)))
	{
	...
	for (IVS_UINT32 ui = 0; ui < pAlarmCommission->uiUserNum; ui++ )
	{	
...
IVS_INT32 CAlarmMgr::UserAlarmAuthorization(const IVS_CHAR *pPWD) const
{
    ...
    ************************************************************************/
	...
	CXml xmlReq;
	IVS_UINT32 xmlLen = 0;
	IVS_INT32 iRet = CAlarmMgrXMLProcess::UserAlarmAuthorGetXML(strPWDCipher.c_str(), xmlReq);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "User Alarm Authorization", "User Alarm Author GetXML failed");
		return iRet;
	}
    const IVS_CHAR* pReq = xmlReq.GetXMLStream(xmlLen);
    CHECK_POINTER(pReq, IVS_OPERATE_MEMORY_ERROR);
    ...
    //创建要发送的CMD，拼装了NSS消息头
    CCmd *pCmd = CNSSOperator::instance().BuildCmd(strLinkID, NET_ELE_SMU_NSS, NSS_USER_AUTHENTICATION_REQ, pReq, strSecurityTransID);//lint !e64 匹配
    CHECK_POINTER(pCmd, IVS_OPERATE_MEMORY_ERROR);
...
IVS_INT32 CAlarmMgr::AlarmReport(const IVS_ALARM_REPORT* pAlarmReport)
{
	...
	IVS_UINT32 strAlarmDescLen = strlen(pAlarmDesc);
	...
	IVS_UINT32 strExtParamLen = strlen(pExtParam);
	...
	eSDK_MEMSET(&stAlarmReport, 0x0, sizeof(IVS_ALARM_REPORT));
	...
	IVS_UINT32 uiLength = htonl(strlen(pAlarmDesc));
	...
	uiSTSize += strlen(pAlarmDesc);

	usiTag = htons(static_cast<IVS_USHORT>(EXTPARAM_TAG));
	uiLength = htonl(strlen(pExtParam));	
	(void)CToolsHelp::Memcpy(pData + uiSTSize, uiBufferSize - uiSTSize, &usiTag, sizeof(IVS_USHORT));
    ...
    if (IVS_SUCCEED != iGetLinkRet)
    {
		...
		IVS_DELETE(pData);
        BP_RUN_LOG_ERR(iGetLinkRet, "Get LocalDomainLinkID failed", "NA");
        return iGetLinkRet;//lint !e438
    }
	CCmd* pCmd = CNSSOperator::instance().BuildCmd(strSMULinkID, NET_ELE_SMU_NSS, NSS_ALARM_REPORT_REQ, pData, static_cast<IVS_INT32>(uiBufferSize));//lint !e64 匹配
	if (NULL == pCmd)
	{
		free(pAlarmDesc);
		pAlarmDesc = NULL;
		...
		return IVS_OPERATE_MEMORY_ERROR;//lint !e438
	}
	...
	if (NULL == pCmdRsp)
	{
		...
		pExtParam = NULL;
		IVS_DELETE(pData);
		BP_RUN_LOG_ERR(IVS_NET_RECV_TIMEOUT, "SendSyncCmd failed", "NA");
		return IVS_NET_RECV_TIMEOUT;//lint !e438
	}
...
IVS_INT32 CAlarmMgr::StartAlarmOut(const IVS_CHAR* pAlarmOutCode)
{
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Start Alarm Out", "Parse DevCode failed");
		return iRet;
	}
...
IVS_INT32 CAlarmMgr::StopAlarmOut(const IVS_CHAR* pAlarmOutCode)
{
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Stop AlarmOut", "Parse DevCode failed");
		return iRet;
	}
...
IVS_INT32 CAlarmMgr::GetAlarmInfo(IVS_UINT64 ullAlarmEventID, const IVS_CHAR* pAlarmInCode, IVS_ALARM_EVENT* pAlarmEvent)
{
	...
	CXml xmlReq;
	(void)CAlarmMgrXMLProcess::GetAlarmInfoGetXML(ullAlarmEventID, xmlReq);
    IVS_CHAR szAlarmInCode[IVS_ALARM_CODE_LEN + 1] = {0};
    IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
    IVS_INT32 iRet = CXmlProcess::ParseDevCode(pAlarmInCode, szAlarmInCode, szDomainCode);
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet,  "Get AlarmInfo", "Parse DevCode failed");
		return iRet;
	}
...
IVS_INT32 CAlarmMgr::QueryWatermarkAlarmList(const IVS_QUERY_UNIFIED_FORMAT *pQueryFormat, IVS_VOID* pBuffer,IVS_UINT32 uiBufferSize)
{
	...
	IVS_UINT32 uiCheckBufferSize = sizeof(IVS_WATERMARK_ALARM_INFO_LIST) + sizeof(IVS_WATERMARK_ALARM_INFO_LIST)*(uiReqNum-1);       
	...
	IVS_INT32 iRet = CXmlProcess::GetUnifiedFormatQueryXML(pNoConst, xmlReq,chDomaCode,strCamerCode.c_str());  //lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
{	
	...
	std::string strpRsp;
	IVS_INT32 iNeedRedirect = IVS_FAIL;
	if(cDevUserListReq.strDeviceUserGroupIDs.empty())
	{
		(void)ClearDevUserListReq();
		(void)BP_OS::time(&cDevUserListReq.tCreateTime);
		cDevUserListReq.iFromIndex = (int)pIndexRange->uiFromIndex;

		// 根据设备编码获取设备组ID
		IVS_CHAR szDevCode[IVS_DEV_CODE_LEN + 1] = {0};
		IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
		iRet = CXmlProcess::ParseDevCode(pDevCode, szDevCode, szDomainCode);
		if (IVS_SUCCEED != iRet)
		{
			BP_RUN_LOG_ERR(iRet, "Get Device User List", "Parse DevCode failed");
			return iRet;
		}
		// 拼装响应消息
		CXml xmlReqGroupID;
		...
		// 将响应消息中的GroupID取出，作为获取用户列表的条件，多个GroupID是中间用","隔开。
		//CXml xmlGrpIDRsp;
		xmlReqGroupID.RemoveElem();
		//std::string strGroupIDs;
		if (!xmlReqGroupID.Parse(strpRsp.c_str()))//lint !e64 匹配
		{
			BP_RUN_LOG_ERR(IVS_SMU_XML_INVALID, "xmlReqGroupID.Parse(strpRsp)", "NA");
			return IVS_SMU_XML_INVALID;
		}
		iRet = CAlarmMgrXMLProcess::GetGroupID(xmlReqGroupID, cDevUserListReq.strDeviceUserGroupIDs);
		if (IVS_SUCCEED != iRet)
		{
			BP_RUN_LOG_ERR(iRet, "Get Device User List", "Get GroupID failed");
			return iRet;
		}
	}
	...
	CXml xmlReq;
	iRet = CAlarmMgrXMLProcess::GetDeviceUserListGetXML(pDomainCode, pDevCode, uiIsUserOnline, pIndexRange,cDevUserListReq.strDeviceUserGroupIDs,  xmlReq);
	if (IVS_SUCCEED != iRet)
	{
	...
	if((pDevUserList->uiTotal) <= (((pIndexRange->uiToIndex) - ((IVS_UINT32)cDevUserListReq.iFromIndex)) + 1))
	{
		cDevUserListReq.strDeviceUserGroupIDs = "";
		cDevUserListReq.iFromIndex = 0;
	}
...
IVS_INT32 CAlarmMgr::SetDeviceGuard(const IVS_CHAR* pReqXml)
{
	...
	IVS_CHAR szDevCode[IVS_DEV_CODE_LEN + 1] = {0};
	IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
	IVS_INT32 iRet = CAlarmMgrXMLProcess::SetDevCode(xml, pReqXml, "Content/DevInfo/DevCode", szDomainCode, szDevCode);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CAlarmMgr::ResetAlarm(const IVS_CHAR* pReqXml)
{
	...
	IVS_INT32 iRet = CAlarmMgrXMLProcess::SetDevCode(xmlReq, pReqXml, "Content/AlarmOutChanInfo/AlarmOutCode", szDstDomainCode, szDevCode);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CAlarmMgr::ExEventSubscribe(const IVS_CHAR* pReqXml)
{
	...
	std::string strDstDomainCode;
	m_pUserMgr->GetDomainCode(strDstDomainCode);
	iRet = CXmlProcess::GetCommSendMgr(xml, m_pUserMgr, NSS_EVENT_SUBSCRIBE_REQ, BUILDRET, strDstDomainCode.c_str());//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CAlarmMgr::ExCatalogSubscribe(const IVS_CHAR* pReqXml)
{
	...
	std::string strDomainCode;
	m_pUserMgr->GetDomainCode(strDomainCode);
	iRet = CXmlProcess::GetCommSendMgr(xmlReq, m_pUserMgr, NSS_CATALOG_SUBSCRIBE_REQ, BUILDRET, strDomainCode.c_str());//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32  CAlarmMgr::GetExEventSubscribe(const IVS_CHAR* pReqXml, IVS_CHAR** pRsqXml)
{
	...
	std::string strDomainCode;
	m_pUserMgr->GetDomainCode(strDomainCode);
	IVS_INT32 iRet = CXmlProcess::GetCommSendMgr(xml, m_pUserMgr, NSS_GET_EVENT_SUBSCRIBE_LIST_REQ, BUILSTRING, strDomainCode.c_str());//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CAlarmMgr::GetExCatalogSubscribeList(const IVS_CHAR* pReqXml, IVS_CHAR** pRsqXml)
{
	...
	IVS_INT32 iRet = CXmlProcess::GetCommSendMgr(xmlReq, m_pUserMgr, NSS_GET_CATALOG_SUBSCRIBE_LIST_REQ, BUILSTRING, strLocalDomainCode.c_str());//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CAlarmMgr::GetAlarmSnapshotList(const IVS_CHAR* pReqXml, IVS_CHAR** pRsqXml)
{
	...
	IVS_CHAR szAlarmInCode[IVS_ALARM_CODE_LEN + 1] = {0};
	GET_ELEM_VALUE_NUM_FOR_UINT("AlarmEventID", szElemValue, ulAlarmEventID, xml);//lint !e838
	...
	do 
	{
		...
		IVS_CHAR szDomainCode[IVS_DOMAIN_CODE_LEN + 1] = {0};
		iRet = CXmlProcess::ParseDevCode(strDevCode.c_str(), szDevCode, szDomainCode);//lint !e64 匹配
	...
	while(iter != ivecDomainCode.end())
	{
		...
		if (IVS_SUCCEED != iRet)
		{
			BP_RUN_LOG_WAR("Ge tAlarm Snapshot List", "Get Alarm Snap List GetXml failed, ErrorCode is %d", iRet);
			iter++;
			continue;
		}
		IVS_UINT32 uiXmlLen = 0;
		...
		CCmd* pCmd = m_pUserMgr->BuildCmd(sendNssMsgInfo);//lint !e64 匹配
		if (NULL == pCmd)
		{
			BP_RUN_LOG_INF("DomainCode: %s, NULL == pCmd", (*iter).c_str());//lint !e64 匹配
			iter++;
			continue;
		}
	...
	// 发送消息
	iRet = CNSSOperator::instance().SendSyncCmd(cmdMap, 35);//lint !e64 匹配
	if (IVS_SUCCEED != iRet)
	{
	...
	eSDK_MEMSET(pResult, 0x0, strNSSRsq.size() + 1);
	if (!CToolsHelp::Memcpy(pResult, strNSSRsq.size() + 1, (const void *)strNSSRsq.c_str(), strNSSRsq.size() + 1))
	{
		IVS_DELETE(pResult, MUILI);
...
IVS_INT32 CAlarmMgr::ClearDevUserListReq()
{
	time_t tNowTime = 0;
	(void)BP_OS::time(&tNowTime);

	//time_t对象只能大值减小值进行判断
	if((tNowTime - cDevUserListReq.tCreateTime) >=  60 )
	{
		cDevUserListReq.strDeviceUserGroupIDs = "";
		cDevUserListReq.iFromIndex = 0;
	}
