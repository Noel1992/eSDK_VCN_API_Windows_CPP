...
CUserInfo::CUserInfo(void)
    ...
    m_strLocalIp("")
{
	eSDK_MEMSET(&m_stSRULoginRsp, 0, sizeof(m_stSRULoginRsp));
...
IVS_INT32 CUserInfo::Init(IVS_INT32 iSdkSessionID, const IVS_LOGIN_INFO* pLoginReqInfo, const ST_SRU_LOGIN_RSP &stSRULoginRsp )
{
	...
	SecureZeroMemory(cPWDtemp, IVS_PWD_LEN + 1);
	m_strPWD = cPWDCipher;
...
{
    (void)CToolsHelp::Memcpy(m_stSRULoginRsp.szLoginID, LOGIN_ID_LEN, (const void *)strLoginID.c_str(), strLoginID.length());
}
...
std::string CUserInfo::GetPWD() const
{
    ...
    char cPWDtemp[128] = {0};
    //解密
    (void)Aes256Operator::instance().Aes256CbcDecode(m_strPWD.c_str(),cPWDtemp, (IVS_UINT64)128);//lint !e64 匹配
