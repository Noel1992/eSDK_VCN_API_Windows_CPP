...
CLoginMgr::~CLoginMgr(void)
{
    ...
    if (m_bLoginOmuThreadRun  &&  (NULL != m_pLoginOMUThread)   &&  (NULL != m_pLoginOMUThread->pthead))
    {
        ...
        //线程未结束，强制杀死线程
        if (WAIT_OBJECT_0 != uiRet)//lint !e835
        {
...
long __SDK_CALL CLoginMgr::LoginOmuThreadEntry(void* pArg)
{
    ...
    while (pLoginMgr->m_bLoginOmuThreadRun)
    {
        ...
        if (IVS_SUCCEED == iRet || IVS_USER_NOT_EXIST == iRet  ||  IVS_NO_ENOUGH_RIGHT == iRet)
        {
			if(IVS_SUCCEED == iRet)
			{
				BP_RUN_LOG_INF("LoginOmuThreadEntry", "Login Omu Sucess");
			}
			else
			{
				BP_DBG_LOG("Login Omu failed, Not need login OMU, return: %d", iRet);
			}
        ...
        BP_DBG_LOG("Login Omu failed, wait next, return: %d", iRet);
...
IVS_INT32 CLoginMgr::Login(const IVS_LOGIN_INFO* pLoginReqInfo)
{
	...
	eSDK_MEMSET(&stLoginRspInfo, 0, sizeof(stLoginRspInfo));
    ...
    if (CLIENT_PC_CHILD ==  pLoginReqInfo->uiClientType 
		...
		|| CLIENT_VCM == pLoginReqInfo->uiClientType)
    {
        return iRet;
    }
...
IVS_INT32 CLoginMgr::LoginByTicket(const IVS_LOGIN_INFO_EX* pLoginReqInfoEx)
{
	...
	eSDK_MEMSET(&stLoginRspInfoEx, 0, sizeof(stLoginRspInfoEx));
	...
	if (CLIENT_PC_CHILD ==  pLoginReqInfoEx->uiClientType 
		...
		|| CLIENT_VCM == pLoginReqInfoEx->uiClientType)
	{
		return iRet;
	}
...
{
    return m_userInfo.GetLoginId();//lint !e64 匹配
}

// 获取用户登录返回的UserID
IVS_UINT32 CLoginMgr::GetUserID() const
{
    return m_userInfo.GetUserID();//lint !e64 匹配
}
...
IVS_INT32 CLoginMgr::LoginSmu(const IVS_LOGIN_INFO* pLoginReqInfo, ST_SRU_LOGIN_RSP &stSRULoginRsp)
{
    ...
    //初次建立传入空
    string strLinkID;
	int resultValue = CNSSOperator::instance().CreateSyncClientLink(NET_ELE_SMU_NSS, pLoginReqInfo->stIP.cIP, static_cast<int>(pLoginReqInfo->uiPort), strLinkID);
    ...
    eSDK_STRNCPY(cLoginName, sizeof(cLoginName), pLoginReqInfo->cUserName, IVS_NAME_LEN);

	char cLoignPWD[IVS_PWD_LEN + 1] = {0};
    eSDK_STRNCPY(cLoignPWD, sizeof(cLoignPWD), pLoginReqInfo->pPWD, IVS_PWD_LEN);
	...
	SecureZeroMemory(cLoignPWD, IVS_PWD_LEN + 1);
    ...
    CCmd *pCmd = CNSSOperator::instance().BuildCmd(strLinkID, NET_ELE_SMU_NSS, NSS_LOGIN_REQ, pszData, strSecurityTransID);//lint !e64 匹配
    if (NULL == pCmd)
    {
        CLinkRouteMgr::instance().DeleteLinkList(iSessionID);
...
IVS_INT32 CLoginMgr::LoginSmuByTicket(const IVS_LOGIN_INFO_EX* pLoginReqInfoEx, ST_SRU_LOGIN_RSP &stSRULoginRspEx)
{
	...
	//初次建立传入空;
	string strLinkID;
	int resultValue = CNSSOperator::instance().CreateSyncClientLink(NET_ELE_SMU_NSS, pLoginReqInfoEx->stIP.cIP, static_cast<int>(pLoginReqInfoEx->uiPort), strLinkID);
	if (IVS_SUCCEED != resultValue)
	{
		BP_RUN_LOG_ERR(IVS_NET_CONNECT_ERROR, "Create client smuLink failed", "failed");
		return IVS_NET_CONNECT_ERROR;
	...
	CHECK_POINTER(pszData, IVS_FAIL);
	std::string strSecurityTransID  = oSecurityClient.GetSecurityDataTransID();
	CCmd *pCmd = CNSSOperator::instance().BuildCmd(strLinkID, NET_ELE_SMU_NSS, NSS_SSO_LOGIN_REQ, pszData, strSecurityTransID);//lint !e64 匹配
	if (NULL == pCmd)
	{
		CLinkRouteMgr::instance().DeleteLinkList(iSessionID);
...
IVS_INT32 CLoginMgr::LoginScu(const std::string &strDomainCode,const std::string &strNVRCode, std::string &strLinkID)
{
    ...
    int resultValue = deviceMgr.GetNVRInfoByDomainCodeAndNVRCode(strDomainCode, strNVRCode, stDeviceBasicInfo);
    if (IVS_SUCCEED != resultValue)
    {
    ...
    eSDK_STRNCPY(cPeerIPTemp, sizeof(cPeerIPTemp), stDeviceBasicInfo.stIP.cIP, IVS_IP_LEN);
    ...
    do
    {
        ...
        eSDK_STRNCPY(cLoginID, sizeof(cLoginID), pLoginID, LOGIN_ID_LEN);
        ...
        resultValue = SendGeneralAuthCmd(NET_ELE_SCU_NSS, strLinkID, xml, strTransID);//lint !e64 匹配
        if (IVS_SUCCEED != resultValue)
        {
...
IVS_INT32 CLoginMgr::GeneralLoginSmu(const std::string &strDomainCode, std::string &strLinkID)
{
    ...
    int resultValue = domainRouteMgr.GetDomainRoutebyCode(strDomainCode, stDomainRouteInfo);
    if (IVS_SUCCEED != resultValue)
    {
    ...
    char cPeerIPTemp[IVS_IP_LEN + 1] = {0};
    eSDK_STRNCPY(cPeerIPTemp, sizeof(cPeerIPTemp), stDomainRouteInfo.stIP.cIP, IVS_IP_LEN);
    ...
    // 拼装xml
    CXml xml;
    int iRet = CLoginXMLProcess::GetGeneralLoginSmuXML(strCipherLoignID.c_str(), strLocalDomainCode, stDomainRouteInfo, xml);//lint !e64 匹配
    if (IVS_SUCCEED != iRet)
    {
      //  (void)CLinkRouteMgr::instance().RemoveLinkInfo(iSessionID, strLinkID);
    ...
    std::string strTransID = oSecurityClient.GetSecurityDataTransID();
    //std::string strTransID = CMKTransID::Instance().GenerateTransID();
    resultValue = SendGeneralAuthCmd(NET_ELE_SMU_NSS, strLinkID, xml, strTransID);//lint !e64 匹配
    if (IVS_SUCCEED != resultValue)
    {
        // 通用业务请求失败，删除linkIDRoute中strLinkID，断开strLinkID对应连接
       // (void)CLinkRouteMgr::instance().RemoveLinkInfo(iSessionID, strLinkID);
...
IVS_INT32 CLoginMgr::LogoutSmu(const string& sSmuLinkId)
{
	...
	CXml xml;
	int iRet = CLoginXMLProcess::GetLogoutXML(xml);
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CLoginMgr::SendGeneralAuthCmd(int iNetElementType, const string& strLinkId, CXml& xml, const std::string& strTransID) const
{
    ...
    unsigned int iLen = 0;
    const char* pszData = xml.GetXMLStream(iLen);//lint !e64 匹配
    CHECK_POINTER(pszData, IVS_OPERATE_MEMORY_ERROR);
    ...
    // 构造发送CMD对象
	CCmd *pCmd = CNSSOperator::instance().BuildCmd(strLinkId, iNetElementType, iReqID, pszData, strTransID);//lint !e64 匹配
    if (NULL == pCmd)
    {
	...
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Send General AuthCmd", "Parse Cmd to NSS failed, return: %d", iRet);
	}
    return iRet;
}
...
IVS_INT32 CLoginMgr::LoginOmu()
{
    ...
    eSDK_MEMSET(&stSRULoginRsp, 0, sizeof(stSRULoginRsp));
    ...
    // 用于建立TCP连接
    string strLinkID;
	int iLinkRet = CNSSOperator::instance().CreateSyncClientLink(NET_ELE_OMU_NSS, m_userInfo.GetServerIP().c_str(), stSRULoginRsp.iOmuPort, strLinkID);//lint !e64 匹配
    CLinkInfo oLinkIDInfoTemp;
    ...
    ************************************************************************/
    ...
    do
    {
        ...
        std::string strTransID = oSecurityClient.GetSecurityDataTransID();
        iRet = SendGeneralAuthCmd(NET_ELE_OMU_NSS, strLinkID, xml, strTransID);//lint !e64 匹配
        if (IVS_SUCCEED != iRet)
        {
...
void CLoginMgr::GetInitData()
{
    ...
    if (IVS_SUCCEED == domainRouteMgr.GetLocalDomainCode(strLocalDomainCode))
    {
		...
		IVS_INT32 iRet = deviceMgr.GetAllNVRInfo(strLocalDomainCode.c_str(), 0); //lint !e64 匹配//0是NVRType，表示查询所有。
        if (IVS_SUCCEED != iRet)
        {
...
IVS_INT32 CLoginMgr::ReLoginSMU(const std::string& strIP, int iPort) 
{
    ...
    eSDK_STRNCPY(cDomainCode, sizeof(cDomainCode), stDomainRoute.cDomainCode, IVS_DOMAIN_CODE_LEN);
    ...
    else
    {
        ...
        iRet = GeneralLoginSmu(cDomainCode, strLinkID); //lint !e64 匹配
    }
    ...
    if (IVS_SUCCEED == iRet || IVS_SMU_USER_FIRST_LOGIN == iRet)
    {
        ...
        bool bRet = CLinkRouteMgr::instance().GetSmuLinkIDInfoByDomainCode(m_pUserMgr->GetSessionID(), cDomainCode, oLinkIdInfo);//lint !e64 匹配// 此处调用查找LinkRouteMgr的函数；

        if (bRet)
        {
            oLinkIdInfo.SetStrLinkId(strLinkID);
            oLinkIdInfo.SetLinkStatus(LINKID_STATUS_OPENED);
            iRet = CLinkRouteMgr::instance().UpdateLinkInfo(m_pUserMgr->GetSessionID(), oLinkIdInfo);
...
IVS_INT32 CLoginMgr::ReLogin(std::string& strLinkID)
{
    ...
    // 建立连接
    // string strLinkID;
    int resultValue = CNSSOperator::instance().CreateSyncClientLink(NET_ELE_SMU_NSS, m_userInfo.GetServerIP().c_str(), m_userInfo.GetServerPort(), strLinkID);
    if (IVS_SUCCEED != resultValue)
    {
        BP_RUN_LOG_ERR(IVS_NET_CONNECT_ERROR, "Create client smuLink failed", "failed");
        return IVS_NET_CONNECT_ERROR;
    ...
    if (IVS_SUCCEED == resultValue)
    {
        ...
        if (CLIENT_PC_CHILD == m_userInfo.GetClientType() 
			...
			|| CLIENT_VCM == m_userInfo.GetClientType())
        {
            return resultValue;
        }
        //与OMU建立tcp长连接,传送loginId鉴权
...
{
    CSDKSecurityClient oSecurityClient;
    IVS_INT32 iRet = oSecurityClient.Init(NET_ELE_SMU_NSS, strLinkID);
    if (IVS_SUCCEED != iRet)
    {
    ...
    //拼装请求xml
    CXml xml;
    char* pLoginID = m_userInfo.GetLoginId();//lint !e64 匹配
    if (NULL == pLoginID)
    {
    ...
    CHECK_POINTER(pszData, IVS_FAIL);
    std::string strSecurityTransID  = oSecurityClient.GetSecurityDataTransID();
    CCmd *pCmd = CNSSOperator::instance().BuildCmd(strLinkID, NET_ELE_SMU_NSS, NSS_RECONNECT_SERVER_REQ, pszData, strSecurityTransID);//lint !e64 匹配
    if (NULL == pCmd)
    {
...
IVS_INT32 CLoginMgr::SendLoginCmd(const std::string &strLinkID, std::string &strLoginID)
{
    ...
    std::string strLoignCipherName;
    std::string strLoignCipherPWD;
    int iEncryptRet = oSecurityClient.EncryptString(m_userInfo.GetLoginName().c_str(), strLoignCipherName);//lint !e64 匹配
    iEncryptRet += oSecurityClient.EncryptString(m_userInfo.GetPWD().c_str(), strLoignCipherPWD);//lint !e64 匹配
    if (IVS_SUCCEED != iEncryptRet)
    {
...
IVS_INT32 CLoginMgr::ReLoginOmu(const std::string& strIP, int iPort)
{
    ...
    // 用于建立TCP连接
    string strLinkID;
    int iCreatLinkRet = CNSSOperator::instance().CreateSyncClientLink(NET_ELE_OMU_NSS, strIP.c_str(), iPort, strLinkID);//lint !e64 匹配
    ...
    // 执行重连OMU操作
    IVS_INT32 iReloginRet = SendReLoginOMUCmd(strLinkID);
...
{
    CSDKSecurityClient oSecurityClient;
    IVS_INT32 iRet = oSecurityClient.Init(NET_ELE_OMU_NSS, strLinkID);
    if (IVS_SUCCEED != iRet)
    {
    ...
    CHECK_POINTER(pszData, IVS_FAIL);
    std::string strSecurityTransID  = oSecurityClient.GetSecurityDataTransID();
    CCmd *pCmd = CNSSOperator::instance().BuildCmd(strLinkID, NET_ELE_OMU_NSS, NSS_LOGIN_REQ, pszData, strSecurityTransID);//lint !e64 匹配
    if (NULL == pCmd)
    {
        BP_RUN_LOG_ERR(IVS_FAIL, "ReLogin Smu Send ReConnectCmd", "Build SMUCmd failed");
        return IVS_FAIL;
