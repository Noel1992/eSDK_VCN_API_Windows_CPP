...
IVS_INT32 CDomainRouteMgr::GetDomainRoute()
{
    ...
    if (IVS_SUCCEED == iRet)
    {
        ...
        for (; iterBegin != iterEnd; iterBegin++)
        {
			...
			if (DEV_STATUS_ONLINE == pstDomainRout->uiStatus)
			{
				oLinkIDInfoTemp.Init(NET_ELE_SMU_NSS, LINKID_STATUS_ENABLED, "", szDomainCode, pstDomainRout->stIP.cIP, "");

				(void)CLinkRouteMgr::instance().UpdateLinkInfo(m_pUserMgr->GetSessionID(), oLinkIDInfoTemp, false);

				BP_DBG_LOG("get domain list, domain:[%s-%s:%u] is online.", szDomainCode, pstDomainRout->stIP.cIP, pstDomainRout->uiPort);
			}
			else
			{
				oLinkIDInfoTemp.Init(NET_ELE_SMU_NSS, LINKID_STATUS_CLOSED, "", szDomainCode,  pstDomainRout->stIP.cIP, "");
				(void)CLinkRouteMgr::instance().UpdateLinkInfo(m_pUserMgr->GetSessionID(), oLinkIDInfoTemp, false);

				// 当前域不可用，添加到重连列表中(键值为 IP + Port)
				m_pUserMgr->AddLinkIdToBusinessResumeList("", NET_ELE_SMU_NSS, pstDomainRout->stIP.cIP, pstDomainRout->uiPort, TYPE_ADD_RELINK);

				BP_RUN_LOG_INF("get domain list,", "domain:[%s-%s:%u] is offline, have been push into resume list.", szDomainCode,
					pstDomainRout->stIP.cIP, pstDomainRout->uiPort);
			}
        }
...
IVS_INT32 CDomainRouteMgr::GetDomainRoute(IVS_DOMAIN_ROUTE_LIST* pDomainRouteList, IVS_UINT32 uiBufferSize )
{
    ...
    {
        eSDK_MEMSET(szDomainCode, 0, sizeof(szDomainCode));
        eSDK_STRNCPY(szDomainCode, sizeof(szDomainCode), pDomainRouteList->stDomainRoute[i].cDomainCode, IVS_DOMAIN_CODE_LEN);
...
//域路由(转发域扩展)
IVS_INT32 CDomainRouteMgr::GetDomainRouteEx(IVS_DOMAIN_ROUTE_LIST_EX* pDomainRouteList, IVS_UINT32 uiBufferSize )
{
	BP_RUN_LOG_INF("Get DomainRoute", "Enter");
	CHECK_POINTER(pDomainRouteList, IVS_OPERATE_MEMORY_ERROR);
	CHECK_POINTER(m_pUserMgr, IVS_OPERATE_MEMORY_ERROR);
	...
	iRet = GeDomainRouteParseXMLEx(pDomainRouteList, xmlRsp);
	m_proxyDomainCodeMap.clear(); // 清除代理域map
...
IVS_INT32 CDomainRouteMgr::GetDomainRouteToListParseXML(DOMAIN_CODE_LIST& stCodeInfoList, CXml &xmlRsp)
{
    ...
    do
    {
        ...
        eSDK_MEMSET(pDomain_route, 0, uiSTLen);
    ...
    BP_RUN_LOG_INF("Parse XML to List", "Leave, Domain Route Total: %d", stCodeInfoList.size());
    return IVS_SUCCEED;
} //lint !e1762

// 解析域路由到List(转发域扩展)
IVS_INT32 CDomainRouteMgr::GetDomainRouteToListParseXMLEx(DOMAIN_CODE_LIST_EX& stCodeInfoList, CXml &xmlRsp)
{
	BP_RUN_LOG_INF("Parse XML to List", "Enter");
	...
	do
	{
		...
		xmlRsp.IntoElem();
		IVS_DOMAIN_ROUTE_EX *pDomain_route = IVS_NEW(pDomain_route);
		if (NULL == pDomain_route)
		{
			return IVS_FAIL;
		}
		...
		GET_ELEM_VALUE_NUM("IsLocalDomain", pSzElement, pDomain_route->bIsLocalDomain, xmlRsp);
		//是否支持转发
		GET_ELEM_VALUE_NUM("IsProgressionTransmit", pSzElement, pDomain_route->bProgressionTransmit, xmlRsp);
		pDomain_route->uiPort = DEFAULT_SMU_PORT;
...
IVS_INT32 CDomainRouteMgr::GeDomainRouteParseXML( IVS_DOMAIN_ROUTE_LIST* pDomainRouteList, CXml &xmlRsp)
{
    ...
    if (IVS_SUCCEED == iRet)
    {
        ...
        for (; iterBegin != iterEnd; iterBegin++)
        {
            ...
            eSDK_MEMSET(&stDomainRouteTmp, 0, sizeof(stDomainRouteTmp));
            ...
            eSDK_MEMSET(szDomainCode, 0, IVS_DOMAIN_CODE_LEN + 1);
            eSDK_STRNCPY(szDomainCode, sizeof(szDomainCode), (*iterBegin)->cDomainCode, IVS_DOMAIN_CODE_LEN);
    ...
    // TODO for 通用请求
    BP_RUN_LOG_INF("Get Parse Domain Route XML Info", "Leave, Domain Route Total: %d", pDomainRouteList->uiTotal);
    return iRet;
}


// 解析获取域路由响应(转发域扩展)
IVS_INT32 CDomainRouteMgr::GeDomainRouteParseXMLEx( IVS_DOMAIN_ROUTE_LIST_EX* pDomainRouteList, CXml &xmlRsp)
{
	BP_RUN_LOG_INF("Get Parse Domain Route XML Info", "Enter");
	CHECK_POINTER(pDomainRouteList, IVS_OPERATE_MEMORY_ERROR);

	DOMAIN_CODE_LIST_EX stCodeInfoList;
	IVS_INT32 iRet = IVS_FAIL;

	iRet = GetDomainRouteToListParseXMLEx(stCodeInfoList, xmlRsp);
	if (IVS_SUCCEED == iRet)
	{
		pDomainRouteList->uiTotal = stCodeInfoList.size();
		DOMAIN_CODE_LIST_EX_ITER iterBegin = stCodeInfoList.begin();
		DOMAIN_CODE_LIST_EX_ITER iterEnd = stCodeInfoList.end();

		IVS_UINT32 uiSTLen = sizeof(IVS_DOMAIN_ROUTE_EX);
		IVS_UINT32 uiIndex = 0;
		for (; iterBegin != iterEnd; iterBegin++)
		{
			if (NULL == *iterBegin)
			...
			IVS_DOMAIN_ROUTE_EX &stDomainRouteTmp = pDomainRouteList->stDomainRoute[uiIndex];
			eSDK_MEMSET(&stDomainRouteTmp, 0, sizeof(stDomainRouteTmp));
...
IVS_INT32 CDomainRouteMgr::GetShareCatalogList(const IVS_CHAR* pQueryInfo, IVS_CHAR** pRspXml)
{
	...
	if (!xml.FindElem("ShareDevInfo"))
	{
		...
		eSDK_MEMSET(cTmp, 0, strlen(pRsp) + 1);
		eSDK_MEMCPY(cTmp, strlen(pRsp) + 1, pRsp, strlen(pRsp));
		*pRspXml = cTmp;
		return IVS_SUCCEED;
	}
	do 
	...
	eSDK_MEMSET(cTmp, 0, strlen(pRsq) + 1);
	eSDK_MEMCPY(cTmp, strlen(pRsq) + 1, pRsq, strlen(pRsq));
	*pRspXml = cTmp;
...
IVS_INT32 CDomainRouteMgr::GetDomainRouteStruct( IVS_DOMAIN_ROUTE_LIST* pDomainRouteList, IVS_UINT32 uiReqNum )
{
    ...
    for (IVS_UINT32 uiIndex = 0; uiIndex < uiReqNum && domainInter != domainInterEnd; uiIndex++, domainInter++)
    {
        ...
        {
            eSDK_MEMCPY(&stRoute, sizeof(stRoute), domainInter->second, uiStLen);
        }
    }
...
bool CDomainRouteMgr::FindProxyDomainCode(const std::string& strDestDomainCode, std::string& strProxyDomainCode)
{
    ...
    {
        eSDK_MEMSET(szDomainCode, 0, IVS_DOMAIN_CODE_LEN);
