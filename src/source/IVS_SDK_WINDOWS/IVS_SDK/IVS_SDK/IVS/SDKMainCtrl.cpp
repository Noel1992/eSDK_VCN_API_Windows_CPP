...
int CSDKMainctrl::OnAlarmNotify(CCmd* pCmd)const
{
	...
	eSDK_MEMSET(pstAlarmNotify->cAlarmInCode, 0, IVS_ALARM_CODE_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmNotify->cAlarmDesc, 0, IVS_ALARM_DESCRIPTION_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmNotify->cAlarmInName, 0, IVS_ALARM_NAME_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmNotify->cAlarmLevelName, 0, IVS_NAME_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmNotify->cAlarmTypeName, 0, IVS_ALARM_NAME_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmNotify->cExtParam, 0, IVS_ALARM_EX_PARAM_LEN);
...
int CSDKMainctrl::OnAlarmStatusNotify(CCmd* pCmd)const
{
	...
	std::string strCode = cAlarmInCodeTemp;
	strCode.append("#").append(strDomainCode);
	eSDK_MEMSET(pstAlarmStatusNotify->cAlarmInCode, 0, IVS_ALARM_CODE_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmStatusNotify->stOperateInfo.cOperatorName, 0, IVS_NAME_LEN);
	...
	if (TRUE == bTranslated)
	{
		...
		{
			// 清理掉先
			eSDK_MEMSET(pstAlarmStatusNotify->stOperateInfo.cOperateInfo, 0, IVS_ALARM_DESCRIPTION_LEN);
...
int CSDKMainctrl::OnUserOffLineNotify(CCmd* pCmd)const
{
    ...
    eSDK_MEMSET(&pstUserOffline, 0, sizeof(IVS_USER_OFFLINE_INFO));

    IVS_CHAR cLoginID[LOGIN_ID_LEN + 1];
    eSDK_MEMSET(cLoginID, 0, LOGIN_ID_LEN+1);
...
int CSDKMainctrl::OnDevAlarmNotify(CCmd* pCmd)const
{
    ...
    CHECK_POINTER(pData, IVS_OPERATE_MEMORY_ERROR);
	eSDK_MEMSET(pData, 0x0, sizeof(IVS_DEVICE_ALARM_NOTIFY));
...
//     }
	...
	eSDK_MEMSET(&m_header, 0, sizeof(TNssMsgHeader));
	...
	if (IVS_SUCCEED == iRet)
	{
		...
		if (TRUE == bTranslated)
		{
			...
			{
				// free source
				eSDK_MEMSET(pDevAlarmNotify->cAlarmDest, 0, IVS_ALARM_DESCRIPTION_LEN);
		...
		if (TRUE == bTranslated)
		{
			...
			{
				// free source
				eSDK_MEMSET(pDevAlarmNotify->cAlarmInName, 0, IVS_ALARM_NAME_LEN);
		...
		{
			//Node Type
			char cNodeType[IVS_NODE_TYPE_LEN + 1] = {0};
			(void)CToolsHelp::Memcpy(cNodeType, IVS_NODE_TYPE_LEN, pDevAlarmNotify->cNodeType, IVS_NODE_TYPE_LEN);
			char* pNodeType = CToolsHelp::UTF8ToANSI(cNodeType);
			if (NULL != pNodeType)
			{
				// free source
				eSDK_MEMSET(pDevAlarmNotify->cNodeType, 0, IVS_NODE_TYPE_LEN);
				(void)CToolsHelp::Memcpy(pDevAlarmNotify->cNodeType, IVS_NODE_TYPE_LEN, pNodeType, strlen(pNodeType));
				free(pNodeType);
				pNodeType = NULL;
			}
		}
		...
		{
			//Location Info
			char cLocationInfo[IVS_DEVICE_ALARM_LOCATION_INFO_LEN + 1] = {0};
			(void)CToolsHelp::Memcpy(cLocationInfo, IVS_DEVICE_ALARM_LOCATION_INFO_LEN, pDevAlarmNotify->cLocationInfo, IVS_DEVICE_ALARM_LOCATION_INFO_LEN);
			char* pLocationInfo = CToolsHelp::UTF8ToANSI(cLocationInfo);
			if (NULL != pLocationInfo)
			{
				// free source
				eSDK_MEMSET(pDevAlarmNotify->cLocationInfo, 0, IVS_NODE_TYPE_LEN);
				(void)CToolsHelp::Memcpy(pDevAlarmNotify->cLocationInfo, IVS_NODE_TYPE_LEN, pLocationInfo, strlen(pLocationInfo));
				free(pLocationInfo);
				pLocationInfo = NULL;
			}
		}
		...
		{
			//Net Name
			char cNetName[IVS_NAME_LEN + 1] = {0};
			(void)CToolsHelp::Memcpy(cNetName, IVS_NAME_LEN, pDevAlarmNotify->cNetName, IVS_NAME_LEN);
			char* pNetName = CToolsHelp::UTF8ToANSI(cNetName);
			if (NULL != pNetName)
			{
				// free source
				eSDK_MEMSET(pDevAlarmNotify->cNetName, 0, IVS_NAME_LEN);
				(void)CToolsHelp::Memcpy(pDevAlarmNotify->cNetName, IVS_NAME_LEN, pNetName, strlen(pNetName));
				free(pNetName);
				pNetName = NULL;
			}
		}
		...
		if (TRUE == bTranslated)
		{
			//Extended Param.
			char cExtParam[IVS_ALARM_EX_PARAM_LEN + 1] = {0};
			(void)CToolsHelp::Memcpy(cExtParam, IVS_ALARM_EX_PARAM_LEN, pDevAlarmNotify->cExtParam, IVS_ALARM_EX_PARAM_LEN);
			char* pExtParam = CToolsHelp::UTF8ToANSI(cExtParam);
			if (NULL != pExtParam)
			{
				// free source
				eSDK_MEMSET(pDevAlarmNotify->cExtParam, 0, IVS_ALARM_EX_PARAM_LEN);
				(void)CToolsHelp::Memcpy(pDevAlarmNotify->cExtParam, IVS_ALARM_EX_PARAM_LEN, pExtParam, strlen(pExtParam));
				free(pExtParam);
				pExtParam = NULL;
			}
		}
...
int CSDKMainctrl::OnManualRecordStateNotify(CCmd* pCmd)const
{
	...
	eSDK_MEMSET(&stManualRecordStatusNotify, 0, sizeof(IVS_MANUAL_RECORD_STATUS_NOTIFY));
	IVS_INT32 iRet = CSDKMainService::ProcessManualRecordStateNotify(pCmd,stManualRecordStatusNotify);
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(IVS_FAIL, "ProcessManualRecordStateNotify failed","NA");
		return iRet;
	}
...
int CSDKMainctrl::HandleGetUserChannelListRsp(CCmd* pCmd)const
{
    ...
    eSDK_MEMSET(&channelListRetriveStatus, 0, sizeof(channelListRetriveStatus));
