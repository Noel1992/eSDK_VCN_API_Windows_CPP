...
int CNSSOperator::CreateSyncClientLink(int iNetElementType, const char* pszPeerIP, int iPeerPort, std::string& strNetElementID)const
{
	...
	eSDK_MEMSET(&stCreateClientInfoSycn, 0, sizeof(CREATE_CLIENT_INFO));
...
char* CNSSOperator::ParseCmd2XML(CCmd* pCmd, int& iRspCode) const
{
	...
	BP_RUN_LOG_INF("Login End", "return :%d", iRspCode);
	...
	// 将utf8->ansi
	char* xml2ANSI = const_cast<char *>(xml);
	IVS_BOOL bNeedFree = FALSE;	// 是否需要调用free释放转码开辟的空间
	IVS_BOOL bTranslated = CSDKConfig::instance().GetTranslateConfig();
	if (TRUE == bTranslated)
	{
		xml2ANSI = CToolsHelp::UTF8ToANSI(xml);
		if (NULL == xml2ANSI)
		{
...
char* CNSSOperator::ParseCmd2Data(CCmd* pCmd, int& length)
{
	...
	if (NULL == xmlBuf)
	{
		BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "New data buffer failed", "NA");
		return NULL;
	}
...
CCmd* CNSSOperator::BuildCmd(const std::string& strLinkID, int iNetElementType, unsigned short iReqID, const char* pData, int length) const
{
	...
	eSDK_STRNCPY(m_header.szTransactionNo, sizeof(m_header.szTransactionNo), strTransID.c_str(), MaxTransactionNoLength - 1);
...
// 直接传入拼装完成的内容部分，组成CMD，传递文件使用
CCmd* CNSSOperator::BuildCmd(const std::string& strLinkID, int iNetElementType, unsigned short iReqID, const char* pData, int length, const std::string& strTransID) const
{
	CCmd* pCmd = NULL;
	HW_NEW(pCmd, CCmd); //lint !e774
	CHECK_POINTER(pCmd, NULL);
	...
	m_header.uiSeqID = 0; 
	m_header.uiTraceID = 0;
	(void)CToolsHelp::Strncpy(m_header.szTransactionNo, TRANSACTIONNO_LEN, strTransID.c_str(), MaxTransactionNoLength - 1);
	m_header.uiRspCode = 0;  
	m_header.uiPacketLength = sizeof(TNssMsgHeader) + static_cast<IVS_UINT32>(length);
...
int CNSSOperator::ConfCmd(CCmd* pCmd,unsigned short iReqID,const char* pData, const std::string& strTransID)const
{
	...
	if (NssRet_Succeed != iRet)
	{
		...
		//add by zwx211831, Date:20150930, memory leak
		delete pMsgBlock;
		//end
		HW_DELETE(pCmd);
