...
IVS_INT32 CXmlProcess::PrintfXML2File(const IVS_CHAR* FileName,const IVS_CHAR* XMLContent)
{
	...
	eSDK_FPRINTF(streamW,"%s",XMLContent);
...
IVS_INT32 CXmlProcess::ParseDevCodeList(std::list<std::string>& lDevCodeList,std::string& strCamerCode,IVS_CHAR* pDomainCode )
{
	...
	eSDK_MEMSET(pDomainCode, 0, IVS_DOMAIN_CODE_LEN+1);
...
IVS_INT32 CXmlProcess::ParseDevCode(const IVS_CHAR *pDevBigCode, IVS_CHAR *pDevCode, IVS_CHAR* pDomainCode )
{
    ...
    eSDK_MEMSET(pDevCode, 0, IVS_SDK_DEV_CODE_LEN+1);
    eSDK_MEMSET(pDomainCode, 0, IVS_DOMAIN_CODE_LEN+1);
...
long CXmlProcess::TransStrToLong(const std::string& str)
{
	...
	{
		eSDK_SSCANF( str.c_str(), STR_FORMAT_L, &lRet);
		return lRet;
	}
	else
		return lRet;
 ...
 int CXmlProcess::TransStrToInt(const std::string& str)
{
	...
	if (CToolsHelp::IsNum(str))
	{
		eSDK_SSCANF( str.c_str(), STR_FORMAT_D, &iRet);
		return iRet;
	}
	else
		return iRet;
 ...
 void CXmlProcess::SetElemValueStr(const IVS_CHAR* pXmlValue,IVS_UINT32 ulLength,CXml &xml)
 {
	 ...
	 eSDK_MEMSET(pXmlNewValue,0,ulLength+1);
 ...
 // 实现外域发送消息公共方法
 IVS_INT32 CXmlProcess::GetCommSendMgr(CXml &xml, CUserMgr* pUserMgr, NSS_MSG_TYPE_E msgType, IVS_INT32 iType, const IVS_CHAR* pDomainCode, const IVS_CHAR* pCameraCode)
 {
	 CHECK_POINTER(pDomainCode, IVS_OPERATE_MEMORY_ERROR);
	 ...
	 BP_RUN_LOG_INF("Get CommSendMgr", "send XML");
	 ...
	 {
		 iRet = pUserMgr->SendMsg(pDomainCode, msgType, pReq, strpRsp, pCameraCode);
		 if (IVS_SUCCEED != iRet)
		 {
			 BP_RUN_LOG_ERR(iRet, "send common message", "msg-type:0x%04x", msgType);
			 return iRet;
		 }
	 }
	 //std::string strpRsp;
	 //IVS_INT32 iRet = pUserMgr->SendMsg(pDomainCode, msgType, pReq, strpRsp, )
	 ...
	 if (BUILSTRING == iType)
	 {
		 ...
		 BP_RUN_LOG_INF("Get CommSendMgr", "revice XML");
	 }
 ...
 // 实现加密消息发送公共方法
 IVS_INT32 CXmlProcess::GetEncrypSendMgr(CXml &xml, CUserMgr* pUserMgr, NSS_MSG_TYPE_E msgType, IVS_INT32 iType, const IVS_CHAR* pDomainCode, 
	 std::string strLinkID, std::string strSecurityTransID, IVS_UINT32 uiTimeOut)
 {
	 IVS_UINT32 uLen = 0;
	 const IVS_CHAR* pReq = xml.GetXMLStream(uLen);
	 CHECK_POINTER(pReq, IVS_OPERATE_MEMORY_ERROR); 

	 BP_RUN_LOG_INF("Get Encrpy Seng Mgr", "send XML");
	 ...
	 //发送消息
	 CCmd *pCmdRsp = CDispatcher::instance().dispatcherSyncMsgOut(pCmd, uiTimeOut);
	 CHECK_POINTER(pCmdRsp, IVS_NET_RECV_TIMEOUT);
	 ...
	 if (BUILSTRING == iType)
	 {
		 ...
		 BP_RUN_LOG_INF("Get Encrpy Seng Mgr", "revice XML");
	 }
	 IVS_DELETE(pRsp, MUILI);
	 return iRet;
 }
 ...
 IVS_INT32 CXmlProcess::GetNodeValueParseXML(CXml& xml, const IVS_CHAR* pNodePath, std::string& strNodeValue)
 {
	 if (NULL == pNodePath)
	 {
		 BP_RUN_LOG_ERR(IVS_XML_INVALID, "get node value from request xml", "search path is null");
		 return IVS_XML_INVALID;
	 }

	 if (!xml.FindElemEx(pNodePath))
	 {
		 BP_RUN_LOG_ERR(IVS_XML_INVALID, "get node value from request xml", "fail to find node:%s in xml", pNodePath);
		 return IVS_XML_INVALID;
	 }

	 const IVS_CHAR* pNodeCode = xml.GetElemValue();
	 if (NULL == pNodeCode)
	 {
		 BP_RUN_LOG_ERR(IVS_XML_INVALID, "get node value from request xml", "fail to find node in xml");
		 return IVS_XML_INVALID;
	 }
	 strNodeValue = pNodeCode;
	 return IVS_SUCCEED;
 }

 IVS_INT32 CXmlProcess::ModifyNodeValueParseXML(CXml& xml, const IVS_CHAR* pNodePath, std::string& strNodeValue)
 {
	 if (NULL == pNodePath)
	 {
		 BP_RUN_LOG_ERR(IVS_XML_INVALID, "modify node value for request xml", "search path is null");
		 return IVS_XML_INVALID;
	 }

	 if (!xml.FindElemEx(pNodePath))
	 {
		 BP_RUN_LOG_ERR(IVS_XML_INVALID, "modify node value for request xml", "fail to find node:%s in xml", pNodePath);
		 return IVS_XML_INVALID;
	 }
	 // (void)xml.SetElemValue(strNodeValue.c_str());
	 (void)xml.ModifyElemValue(strNodeValue.c_str());
	 return IVS_SUCCEED;
 }