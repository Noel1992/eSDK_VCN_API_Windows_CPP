...
public:
	...
	// 获取NVR通道号管理类对象
	CNVRChannelMgr& GetNVRChannelMgr(){ return m_NVRChannelMgr; };//lint !e1536

	//设置逐级转发
	IVS_INT32  SetProgressionTransmit(IVS_BOOL  bSupportProgressionTransmit, const IVS_CHAR* pProgressionDomainCode);

	IVS_BOOL IsSupportProgressionTransmit() const;

	void ProgressionTransmitDomainCode(std::string& strProgressionDomainCode) const;

	IVS_INT32 CreateDomianCameraCode(IVS_CHAR* pDevCode);
    ...
    // 根据播放通道号获取播放句柄
    IVS_ULONG GetPlayHandlebyPlayChannel(IVS_ULONG ulPlayChannel);

    // 根据NetSource的通道号获取播放句柄
    IVS_ULONG GetPlayHandlebyNet(IVS_ULONG ulNetChannel);
	...
	// 获取发现前端设备的目标域编码
	void GetDevDomainCode(std::string &strDomainCode)const { strDomainCode = m_strDevDomainCode; };

	// 设置发现前端设备的目标域编码
	void SetDevDomainCode(const std::string& strDomainCode){ m_strDevDomainCode = strDomainCode; };
    ...
    *******************************************************************/
    ...
    IVS_INT32 SendCmdEX(CSendNssMsgInfo& sendNssMsgInfo, std::string& strNSSRsp, IVS_INT32& iNeedRedirect, IVS_INT32 iTimeOut = -1);
	
	IVS_INT32 SendSynCmdContainsExchangeKey(CSendNssMsgInfo& sendNssMsgInfo,const std::string& strLinkID, std::string& strNSSRsp, std::string strSecurityTransID, uint32_t uiTimeout = 35, IVS_BOOL bTimeout = 0);

    IVS_INT32 SendMsg(const IVS_CHAR* pDomainCode, 
        unsigned short usReqID, 
        const IVS_CHAR* pReqMsg,
        std::string& strRspMsg, 
        const IVS_CHAR*  pCameraCode,
        IVS_INT32 iTimeOut = -1, const IVS_CHAR* pszNVRCode = NULL);
    ...
    IVS_INT32 ProcrssEnableLinkNode(CLinkInfo& oLinkIdInfo);
     ...
     return     : IVS_INT32 IVS_SUCCEED成功 IVS_FAIL失败
    *******************************************************************/
	IVS_INT32 SendRedirectServe(CSendNssMsgInfo& sendNssMsgInfo,std::string& strNSSRsp, const IVS_CHAR* pszNVRCode = NULL);
	...
	*******************************************************************/
	...
	IVS_INT32 SendSynCmdEX(CSendNssMsgInfo& sendNssMsgInfo,const std::string& strLinkID, std::string& strNSSRsp, IVS_INT32 iTimeout = -1);  // 默认35秒
    ...
    *******************************************************************/
	...
	/******************************************************************
     ...
     return     : IVS_INT32 IVS_SUCCEED成功 IVS_FAIL失败
    *******************************************************************/
	IVS_INT32 SetPlaySecretKeybyRtspHandle(IVS_ULONG iRtspHandle,const char* pSdp);
...
private:
    ...
    *******************************************************************/
    ...
    /******************************************************************
     ...
     return     : IVS_INT32 IVS_SUCCEED成功 IVS_FAIL失败
    *******************************************************************/
    IVS_INT32 SendCmdbyReDirect(const IVS_DOMAIN_ROUTE& stDomainRoute, std::string strDevNvrCode, CSendNssMsgInfo& sendNssMsgInfo,std::string &strNSSRsp);
...
private:
	...
	CNVRChannelMgr	m_NVRChannelMgr;	//NVR通道号管理类
private:
	...
	std::string         m_strDevDomainCode;     // 发现前端设备的目标域编码
	...
	IVS_BOOL                m_bSupportProgressionTransmit; //是否支持逐级转发
	std::string				m_strProgressionDomainCode;    //转发域编码
};
#endif
