...
{
	eSDK_MEMSET(&m_IAGraphic, 0, sizeof(IA_INNER_GRAPHICS));
	eSDK_MEMSET(&m_RecordPoint, 0, sizeof(POINT));
	eSDK_MEMSET(m_OriginIAPoints, 0, sizeof(POINT) * POINTS_NUM_MAX);
	eSDK_MEMSET(&m_bf, 0, sizeof(BLENDFUNCTION));
	eSDK_MEMSET(&m_bmi, 0, sizeof(BITMAPINFO));
	eSDK_MEMSET(&m_lf, 0, sizeof(LOGFONT));
...
LONG CIADrawer::InitIADrawer(HWND hMsgWnd)
{
	...
	RECT Wndrect;
    eSDK_MEMSET(&Wndrect, 0, sizeof(Wndrect));
...
LONG CIADrawer::InsertIntoRelationShipList(ULONG ulGraphicGroupA, ULONG ulGraphicGroupB, RELATION_SHIP_TYPE ulRelationShipType) 
{
	...
	eSDK_MEMSET(pRelationShip, 0, sizeof(IA_RELATIONS));
...
{
	IA_ARROW iaArrow;
	eSDK_MEMSET(&iaArrow, 0, sizeof(iaArrow));
...
// 	IVS_TRACE();
	...
	if (m_bActive)
	{
		if (m_bWindowPoint)
		{
			(void)WindowPointToGraphicPoint(&m_IAGraphic.iaGraphic.IAPoints[0]);
			(void)WindowPointToGraphicPoint(&m_IAGraphic.iaGraphic.IAPoints[1]);
...
LONG CIADrawer::ModifyIARectanglePoint(MOUSE_EVENT_TYPE MouseEvent, ULONG ulPointIndex, POINT /*point*/) 
{
	...
	POINT tmp_Points[5];
	eSDK_MEMSET(tmp_Points, 0, sizeof(tmp_Points));
...
LONG CIADrawer::UpdateIAMove(POINT point, ULONG ulGraphicGroup, BOOL bFinish)
{
	...
	eSDK_MEMSET(tmp_Points, 0 , sizeof(POINT) * POINTS_NUM_MAX);
