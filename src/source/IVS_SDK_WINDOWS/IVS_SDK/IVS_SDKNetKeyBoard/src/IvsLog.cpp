...
CIvsLog::CIvsLog()
{
    ...
    eSDK_MEMSET(m_szLogFilePathName,0,MAX_LOG_FILE_PATH_NAME_LEN);
}
...
long CIvsLog::Start()
{
    ...
    if(::strlen(m_szLogFilePathName) == 0)
    {
        ...
        //添加后缀
        eSDK_SPRINTF(pszFind, sizeof(m_szLogFilePathName) - (pszFind- m_szLogFilePathName), ".log");
    }
    //打开文件
...
bool CIvsLog::SetLogFilePathName(const char* szPathName)
{
    ...
    if((NULL!=szPathName) && (::strlen(szPathName)<MAX_LOG_FILE_PATH_NAME_LEN))
    {
        ...
        {
            //去掉斜杠
            eSDK_SPRINTF(m_szLogFilePathName, sizeof(m_szLogFilePathName), "%s", szPathName+1);
        }
        else
        {
            eSDK_SPRINTF(m_szLogFilePathName, sizeof(m_szLogFilePathName), "%s", szPathName);
        }
                ...
                long lLevel, const char* format, va_list argp)
{
    ...
    if(!m_bAllowWrite)
    {
       ...
       eSDK_STRNCPY(szDirectory, sizeof(szDirectory), m_szLogFilePathName, MAX_LOG_FILE_PATH_NAME_LEN-1);
...
//额外信息准备完成
    ...
    //首先向最终日志信息中加入额外信息
    eSDK_SPRINTF(szLogTmp, sizeof(szLogTmp), "%04d-%02d-%02d %02d:%02d:%02d.%03d[%8s|%20s:%05d|PID:0x%04X|TID:0x%04X|Err:0x%04X]    ",
        st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds,
        pszLevel, pszFileName, lLine, ulProcessID, ulThreadID, ulErr);//lint !e705

    //将用户信息接在额外信息后写进日志中
    unsigned long ulLen = ::strlen(szLogTmp);
    eSDK_VSPRINTF(szLogTmp+ulLen, (MAX_LEN_OF_SINGLE_LOG-ulLen)-1, format, argp);
...
bool CIvsLog::LogFileLenDeal()
{
    ...
    eSDK_MEMCPY(szTemp, sizeof(szTemp), m_szLogFilePathName, strlen(m_szLogFilePathName) - 4);
    eSDK_SPRINTF(szNewFileName, sizeof(szNewFileName), "%s_%4d%02d%02d%02d%02d%02d%s",
...
void CIvsLog::DeleteBackLogFiles() const
{
    ...
    eSDK_MEMCPY(szAbsolutePath, sizeof(szAbsolutePath), m_szLogFilePathName, strlen(m_szLogFilePathName) - 4);
    ...
    *(pszFind + 1) = '\0';    //替换为结束符
    ...
    eSDK_MEMCPY(szTemp, sizeof(szTemp), m_szLogFilePathName, strlen(m_szLogFilePathName) - 4);
    eSDK_SPRINTF(szLogFileNameFilter, sizeof(szLogFileNameFilter), "%s*", szTemp);
    ...
    for (; IVSLOG::LOG_FILE_COUNT_MAX <= ulFileCount && Iter != LogFileList.end(); ++Iter)
    {
        ...
        eSDK_SPRINTF(szDelLogFileName, sizeof(szDelLogFileName), "%s\\%s", 
