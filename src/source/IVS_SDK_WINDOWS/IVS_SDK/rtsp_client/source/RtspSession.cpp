...
{
	{
		IVS_RUN_LOG_CRI("RtspSession Destroy 0x%08p 0x%08p", this, m_pRecvBuffer);
...
int CRtspSession::handle_timeout(const ACE_Time_Value &tv, const void *arg)
{
	...
	{
		KillRtpTimer();
		CRtspSessionManager::instance()->m_fExceptionCallBack((IVS_ULONG)this, 
...
int CRtspSession::handleRecvedData(const char* pData, unsigned int unDataSize)
{
    ...
    {
		//add by zwx211831,Date:20151216，和设置回调函数使用同一把锁，放在被设置为空
		{
			ACE_Guard <ACE_Thread_Mutex> locker (m_CallBackMutex);
...
int CRtspSession::parseRtpRtcpPacket(const char* pData, unsigned int unDataSize)
{
    ...
    if((NULL != m_fDataCallBack) && !m_bDisconnect)
    {
