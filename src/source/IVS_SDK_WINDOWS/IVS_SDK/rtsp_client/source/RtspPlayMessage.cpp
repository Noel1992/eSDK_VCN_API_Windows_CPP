...
CRtspPlayMessage::CRtspPlayMessage()
{
    ...
    m_dScale       = 0;
    eSDK_MEMSET (&m_stRange, 0x0, sizeof(MEDIA_RANGE_S));
}
...
void CRtspPlayMessage::encodeRangeField(std::string &strMessage)
{
    ...
    if (RANGE_TYPE_UTC == m_stRange.enRangeType)
    {
        ...
        eSDK_SPRINTF(strTime, sizeof(strTime), "%04d%02d%02dT%02d%02d%02dZ", tmv.tm_year
        ...
        if (OFFSET_END != m_stRange.MediaEndOffset)
        {
            ...
            (void) ACE_OS::localtime_r(&rangeTime, &tmv);
            eSDK_SPRINTF(strTime, sizeof(strTime), "%04d%02d%02dT%02d%02d%02dZ",
    ...
    else
    {
        ...
        eSDK_SPRINTF(strTime, sizeof(strTime), "%u", m_stRange.MediaBeginOffset);
        strMessage += strTime;
    }
    strMessage += SIGN_H_LINE;
    ...
    if (OFFSET_END != m_stRange.MediaEndOffset)
    {
        eSDK_SPRINTF(strTime, sizeof(strTime), "%u", m_stRange.MediaEndOffset);
        strMessage += strTime;
    }
    strMessage += RTSP_END_TAG;
