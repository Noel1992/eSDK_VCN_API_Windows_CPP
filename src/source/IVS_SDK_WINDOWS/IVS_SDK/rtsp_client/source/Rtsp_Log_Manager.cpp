...
CRtsp_Log::CRtsp_Log()
{
    ...
    // 日志文件名称
    eSDK_MEMSET(m_strFileAllPath, 0, sizeof(m_strFileAllPath));
    eSDK_MEMSET(m_strFileName, 0, sizeof(m_strFileName));
    eSDK_MEMSET(m_strPrefix, 0, sizeof(m_strPrefix));

    // 日志缓存区
    eSDK_MEMSET(m_strBuff, 0, sizeof(m_strBuff));
...
{
    eSDK_SPRINTF(m_strFileName, sizeof(m_strFileName), 
                           "%s",
                           strFileName);
    
    m_enLogType = enLogType;

	eSDK_SPRINTF(m_strPrefix, sizeof(m_strPrefix), 
		"%s",
		strPrefix);

    // 日志文件全路径
    eSDK_SPRINTF(m_strFileAllPath, 
    ...
    eSDK_SPRINTF(strTempFileAllPath, 
                        ...
                        const char* format, va_list args)
{
    ...
    if(LOG_TYPE_DBG == m_enLogType)
    {
        ...
        if(0 != errno)
        {
            eSDK_SPRINTF(strErrno, sizeof(strErrno), "[%d:%s]", errno, strerror(errno));
        }
        
        ulLogLen = eSDK_SPRINTF(strSingleLog, 
                                sizeof(strSingleLog), 
                                "%s [%s] [0x%04x] | (%s:%d%s) %s ",
                                strTime, 
    ...
    {
        ulLogLen = eSDK_SPRINTF(strSingleLog, sizeof(strSingleLog), 
                                "%s [%s] [0x%04x] | %s ",
                                strTime, 
                                g_strRtspLogLevel[logLevel],
                                (uint32_t)ACE_OS::thr_self(),
	...
	if (NSS_SINGLE_LOG_LEN_MAX < ulLogLen)
	{
		return;
	}
    ulLogLen += eSDK_VSPRINTF(strSingleLog + ulLogLen,
...
int CRtsp_Log::openLogFile()
{
    ...
    {
        strNewPos++;
        eSDK_STRNCPY(strTempDir, sizeof(strTempDir), m_strFileAllPath, (uint32_t)(strNewPos - m_strFileAllPath));
...
{
    m_bRunFlag = false;
    eSDK_MEMSET(m_strLogRootPath, 0 , sizeof(m_strLogRootPath));
    eSDK_MEMSET(m_strConfigPath, 0 , sizeof(m_strConfigPath));
    eSDK_MEMSET(m_strServerName, 0 , sizeof(m_strServerName));
                               ...
                               unsigned int unLevel)
{
    ...
    eSDK_SPRINTF(m_strServerName, sizeof(m_strServerName), "%s", strServerName);
    if ('\0' == m_strLogRootPath[0])
    {
        eSDK_STRNCPY(m_strLogRootPath, sizeof(m_strLogRootPath), strLogRootPath, strlen(strLogRootPath)+1);
    }
...
int CRtsp_Log_Manager::open(void *)
{
	...
	ACE_thread_t threadID[1] = {0};
	ACE_hthread_t hThread[1];
	const char* pszThreadName[1] = {""};
    // 启动媒体处理线程
                ...
                const char * file, long line, const char* format, va_list args)
{
    ...
    (void)ACE_OS::localtime_r( &currTime, &tmv );
    char strTime[64];
    eSDK_SPRINTF(strTime, sizeof(strTime), 
    ...
    {
        case LOG_TYPE_SEC:
            eSDK_SPRINTF(strAddInfo, sizeof(strAddInfo), "%s %s |",usr, ret);
            m_securityLog.printLog(strTime, strAddInfo,logLevel,file,line, format,args);
            break;
        
        case LOG_TYPE_OPR:
            eSDK_SPRINTF(strAddInfo, sizeof(strAddInfo), "%s %s %s |",usr, ip, ret);
        ...
        case LOG_TYPE_USR:
            eSDK_SPRINTF(strAddInfo, sizeof(strAddInfo), "%s |[%s] [%s]",usr, flowID, sessionID);
