...
#ifdef WIN32
static const std::string DEFAULT_CONFIG_DIR_NAME = "\\config";
...
CSDKConfig::CSDKConfig(void)
	...
	, m_bIsTranslated(TRUE)
{
	m_NetSourceLogPath = "..\\log";
}
...
IVS_INT32 CSDKConfig::LoadConfigFile()
{
	...
	CToolsHelp::GetDLLPath("IVS_SDK.dll", strConfigDir);
	strConfigDir += DEFAULT_CONFIG_DIR_NAME;
    std::string strConfigFilePath = strConfigDir;
    strConfigFilePath += DEFAULT_CONFIG_FILE_NAME;
	BP_RUN_LOG_INF("Get Config File Path", "$filepath=%s", strConfigFilePath.c_str());
...
IVS_INT32 CSDKConfig::InitMember(CBB::CXml& oCXml)
{
	...
	// 初始化NetSource日志路径配置项值
	iRet += SetNetSourceLogPath(oCXml);

	// 加载TranslateConfig配置项值
	LoadTranslateConfig(oCXml);
...
IVS_INT32 CSDKConfig::SetNetSourceLogPath(CBB::CXml& oCXml)
{
	int iRet = IVS_FAIL;
	// 初始化NetSourceLogPath配置项值
	const char* pTmpVal = NULL;
	if(oCXml.FindElemEx("Config/NetSourceConfig/LogBasePath"))
	{
		pTmpVal = oCXml.GetElemValue();
		if (NULL != pTmpVal)
		{
			iRet = IVS_SUCCEED;
			m_NetSourceLogPath = pTmpVal;
		}
	}
	return iRet;
...
IVS_VOID CSDKConfig::LoadTranslateConfig(CBB::CXml& oCXml)
{
	// 加载TranslateConfig配置项值
	if(oCXml.FindElemEx("Config/SDKConfig/IsTranslated"))
	{
		const char*  pElemValue = oCXml.GetElemValue();
		if (NULL != pElemValue)
		{
			std::string strElemValue = pElemValue;
			if(!strElemValue.compare("FALSE") || !strElemValue.compare("false"))
			{
				SetTranslateConfig(FALSE);
				return;
			}
		}
		else
		{
			BP_RUN_LOG_INF("Can not load Translate Config", "use default configuration");
			return;
		}
	}
	else
