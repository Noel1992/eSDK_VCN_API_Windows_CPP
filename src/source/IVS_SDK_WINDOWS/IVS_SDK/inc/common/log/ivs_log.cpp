...
#  define LOCK_INIT(p)    InitializeCriticalSectionAndSpinCount((p), 4000 )
...
typedef struct LogFileName
{
    ...
    {
        this->uiTime = uiTime;
        eSDK_MEMSET(szFileName, 0, MAX_FNAME_LEN);
        eSDK_MEMCPY(szFileName, MAX_FNAME_LEN, pFileName,strlen(pFileName));
    };
...
static int SftpInsertTransList(char *szName)
{
    ...
    {
        eSDK_MEMSET(newNode, 0, sizeof(FNAME_NODE));
        newNode->bIsZip = strstr(szName, ".zip")? 1 : 0;
        eSDK_STRCPY(newNode->Name, sizeof(newNode->Name), szName);
        LOCK(&g_lockSftp);
...
static void BackupFinishedFile(LOG_FILE *pFile, time_t _tt)
{
    ...
    eSDK_STRNCPY(pFile->FinishedFiles[uInsAt], sizeof(pFile->FinishedFiles[uInsAt]), NewFileName, MAX_FNAME_LEN-1);
    pFile->uFinished++;
}
...
static size_t LogFileWrite(LOG_FILE *pFile, const char *data, size_t len)
{
    ...
    if ((pFile->uPos + len) > FILE_BUFFER_SIZE)
    {
        ...
        else
        {
            ...
            pBuf = pChunk->buf;
            eSDK_MEMCPY(pBuf, len, data, len);
            pChunk->size = pFile->uPos = len;
        }
    }
    else
    {
        eSDK_MEMCPY(pBuf + pFile->uPos, len, data, len);
        pFile->uPos  += len;
        pChunk->size += len;
    }
...
static size_t LogFileWrite(LOG_FILE *pFile, char *data, size_t len)
{
    ...
    eSDK_MEMCPY(pFile->Buffer + pFile->uPos, len, data, len);
    pFile->uPos += len;
    UNLOCK(&pFile->lockWrite);
    return len;
}
...
static void InitRingQueue( std::list<LOG_FILENAME> &aFileNameList, LOG_FILE *&pFile )
{
    ...
    while (aFileNameList.end() != iter)
    {
        ...
        eSDK_STRNCPY(pFile->FinishedFiles[uiInsertIndex], sizeof(pFile->FinishedFiles[uiInsertIndex]), iter->szFileName, MAX_FNAME_LEN);
        ++pFile->uFinished;
        ++iter;
    }
}
...
static void ScanDirectory(char *szDir, LOG_FILE *&pFile)
{
    ...
    eSDK_MEMSET(&FileData, 0, sizeof(FileData));
...
{ 
	...
	char DirName[256] = {0};      
	eSDK_STRCPY(DirName, sizeof(DirName), pszDir);      
	int len = strlen(DirName);      
	if(DirName[len-1]!='/')
	{
		eSDK_STRCAT(DirName, sizeof(DirName), "/");
	}
	len = strlen(DirName);      
	for(int i=1;i<len;i++)      
	{      
...
static int LogFileInit(LOG_FILE *pFile, int nType)
{
    ...
    eSDK_MEMCPY(subDir, sizeof(subDir), g_LogRootDir, pathLen);
    subDir[pathLen++] = DIR_SEP;

    /* 检查目录 */
    eSDK_STRNCPY(subDir + pathLen, strlen(g_strLogType[nType])+1, g_strLogType[nType], strlen(g_strLogType[nType]));
    pathLen = strlen(subDir);
    subDir[pathLen++] = DIR_SEP;
#ifdef WIN32
	int iRet = IVS_access(subDir, 0);
	if (0 > iRet)
	{
		iRet = create_directory_recursion(subDir);
    ...
    eSDK_MEMSET(pFile, 0, sizeof(LOG_FILE));
                ...
                int nModuleID, void (*callback)(int code))
{
...
#ifdef LOG_HAS_CONFIG_FILE
    eSDK_MEMSET(g_ConfFileName, 0, sizeof(g_ConfFileName));
    eSDK_STRNCPY(g_ConfFileName, sizeof(g_ConfFileName), szConf, sizeof(g_ConfFileName) - 1);
    if (ReadConfigure(g_ConfFileName))
    {
        printf("Log Error: Read Configure file %s Fail Use Default Configure\n", g_ConfFileName);
    }
#endif
    eSDK_MEMSET(g_FileName, 0, sizeof(g_FileName));
    eSDK_STRNCPY(g_FileName,  sizeof(g_FileName), szFileName, sizeof(g_FileName) - 1);

    eSDK_MEMSET(g_ModuleType, 0, sizeof(g_ModuleType));
    eSDK_STRNCPY(g_ModuleType,  sizeof(g_ModuleType), szModuleType, sizeof(g_ModuleType) - 1);
    g_ModuleCode = nModuleID;

    /* 检查并创建日志根目录 */
    int pathLen = IVS_snprintf(g_LogRootDir, sizeof(g_LogRootDir) - 1,
             "%s%c%s", szDir, DIR_SEP, szModuleType);
	if (pathLen > sizeof(g_LogRootDir) - 2)
	{
	...
	int iRet = IVS_access(g_LogRootDir, 0);
	if (0 > iRet)
	{
		//TCHAR cPath[MAX_PATH] = {0};
		//GetModuleFileName(::GetModuleHandle("IVS_SDK.dll"), cPath, MAX_PATH-sizeof(TCHAR));
		//std::string strPath((char *)cPath);
		//strPath = strPath.substr(0, strPath.rfind("\\")+1);
		//strPath += g_LogRootDir;

		//iRet = SHCreateDirectoryEx(NULL , g_LogRootDir, NULL);
		iRet = create_directory_recursion(g_LogRootDir);
		if (ERROR_SUCCESS != iRet)
		{
			printf("Log Error: Create Log Root Directory Fail\n");
			return -1;
...
*CreateLocalPath
*/
void CreateLocalPath(const char * pszFilename)
{
	if (NULL == pszFilename) 
	{
		return; 
	...
	unsigned int uiLength = strlen(pszFilename); 
	if (MAX_FNAME_LEN <= uiLength) 
	{
		return; 
	}

	char szFilename[MAX_FNAME_LEN] = { 0 }; 
	for (unsigned int uiIndex = 0; uiIndex < uiLength; ++uiIndex) 
	{
		if ('/' == pszFilename[uiIndex] || '\\' == pszFilename[uiIndex]) 
		{
			szFilename[uiIndex] = DIR_SEP; 
			(void)IVS_mkdir(szFilename); 
		}
		else 
		{
			szFilename[uiIndex] = pszFilename[uiIndex]; 
		}
	}
...
/*
*create_directory_recursion
*/
bool create_directory_recursion(const char * pszFilename)
{
	CreateLocalPath(pszFilename); 
	if (0 != IVS_mkdir(pszFilename)) 
	{
		return false; 
	}

	return(0 == IVS_access(pszFilename, 0));
}
                ...
                char *szSvrAddr, unsigned short usSvrPort)
{
    ...
    eSDK_STRNCPY(g_SftpPath, sizeof(g_SftpPath), szSvrDir, sizeof(g_SftpPath) - 1);
    eSDK_STRNCPY(g_SftpUser, sizeof(g_SftpUser), szUser, sizeof(g_SftpUser) - 1);
    eSDK_STRNCPY(g_SftpPass, sizeof(g_SftpPass), szPassWord, sizeof(g_SftpPass) - 1);
    eSDK_STRNCPY(g_SftpSvrAddr, sizeof(g_SftpSvrAddr), szSvrAddr, sizeof(g_SftpSvrAddr) - 1);
...
static unsigned int FillTime(char *buf, unsigned int len)
{
    ...
    return IVS_snprintf(buf, len, "%04d-%02d-%02d %02d:%02d:%02l.%03d%c%02u:%02u", 
