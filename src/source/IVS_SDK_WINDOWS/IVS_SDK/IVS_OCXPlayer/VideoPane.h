...
*****************************************************************/
#ifndef _VIDEO_PANE_H_
#define _VIDEO_PANE_H_
...
public:
	...
	ULONG GetWndID() const {return m_ulWndID;}
	void SetWndID(ULONG val) {m_ulWndID = val;}
private:
    // 是否是活动窗格
...
private:
	...
	public:
		...
		REALPLAY_CMD (const REALPLAY_CMD& rhs)
		{
			transid = rhs.transid;
			bStop   = rhs.bStop;
			eSDK_MEMCPY(CamID, sizeof(CamID), rhs.CamID, sizeof(CamID));
			eSDK_MEMCPY(&Param, sizeof(Param), &rhs.Param, sizeof(Param));
		}
	};
...
public:
	// 是否启动画图回调，TURE-启动，FALSE-关闭
	IVS_INT32 IsEnableDrawCallBackEx(IVS_BOOL bEnable);
};

#endif	//_VIDEO_PANE_H_
