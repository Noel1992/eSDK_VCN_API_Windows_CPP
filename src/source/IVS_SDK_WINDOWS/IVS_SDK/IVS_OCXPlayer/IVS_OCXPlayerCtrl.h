...
*****************************************************************/
#ifndef _IVS_OCX_PLAYER_CTRL_H_
#define _IVS_OCX_PLAYER_CTRL_H_
...
typedef struct
{
	ULONG ulWindowID;	// 窗口ID
	HDC hDc;			// 显示DC
	RECT rect;			// DC大小
	ULONG64 ulFrameID;	// 当前时间戳
}IA_CallBackData;
...
public:
	...
	afx_msg LRESULT OnRemoteDownLoadEvent(WPARAM wParam,LPARAM lParam);
	...
	//响应关闭指定用户视频业务事件
	afx_msg LRESULT OnShutdownStreamNotify(WPARAM wParam, LPARAM lParam);

	afx_msg LRESULT OnIADrawCallBackEx(WPARAM wParam, LPARAM lParam);
...
public:
	enum {
		dispidIVS_OCX_DisableDrawCallBackEx = 570L,
		dispidIVS_OCX_EnableDrawCallBackEx = 569L,
		dispidIVS_OCX_GetDomainDeviceList = 568L,
    ...
    enum
    {
		eventidIVS_OCX_DrawCallBackEx_Event = 25L,
...
#pragma region EventDef
	...
	void IVS_OCX_DrawCallBackEx_Event(ULONG ulWindowID, LONG hDC, LONG left, LONG top, LONG right, LONG bottom, ULONGLONG ulFrame)
	{
		FireEvent(eventidIVS_OCX_DrawCallBackEx_Event, EVENT_PARAM(VTS_UI4 VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_UI8), ulWindowID, hDC, left, top, right, bottom, ulFrame);
	}
...
protected:
	...
	BSTR IVS_OCX_GetDomainDeviceList(LPCTSTR pDomainCode, ULONG ulDeviceType, ULONG ulFromIndex, ULONG ulToIndex);
	LONG IVS_OCX_EnableDrawCallBackEx(ULONG ulWindowID);
	LONG IVS_OCX_DisableDrawCallBackEx(ULONG ulWindowID);
}; //lint !e1712  MFC自动生成的的函数

#endif	//_IVS_OCX_PLAYER_CTRL_H_
