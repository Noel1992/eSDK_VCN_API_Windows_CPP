...
int CSyncPlayBackMgr::GetPlayedTime(IVS_PLAYBACK_TIME &IvsTm)const
{
	...
	eSDK_SPRINTF(IvsTm.cPlayBackStartTime, sizeof(IvsTm.cPlayBackStartTime), 
		"%04d%02d%02d%02d%02d%02d", _tm.tm_year + 1900, _tm.tm_mon + 1, _tm.tm_mday,
		_tm.tm_hour, _tm.tm_min, _tm.tm_sec);

	gmtime_s(&_tm, &m_tmEnd);
	eSDK_SPRINTF(IvsTm.cPlayBackEndTime, sizeof(IvsTm.cPlayBackEndTime), 
			"%04d%02d%02d%02d%02d%02d", _tm.tm_year + 1900, _tm.tm_mon + 1, _tm.tm_mday,
			_tm.tm_hour, _tm.tm_min, _tm.tm_sec);

	gmtime_s(&_tm, &m_tmPlayTime);

	eSDK_SPRINTF(IvsTm.cPlayBackTime, sizeof(IvsTm.cPlayBackTime), 
		"%04d%02d%02d%02d%02d%02d", _tm.tm_year + 1900, _tm.tm_mon + 1, _tm.tm_mday,
		_tm.tm_hour, _tm.tm_min, _tm.tm_sec);
...
void CSyncPlayBackMgr::StartTask(unsigned long long refTick)
{
	...
	TASK_ITER iter = m_TaskWaiting.begin();
	TASK_ITER end  = m_TaskWaiting.end();
	char pNVRCode[IVS_NVR_CODE_LEN + 1] = {0};
	eSDK_MEMCPY(pNVRCode, IVS_NVR_CODE_LEN + 1, m_stNVRCode, IVS_NVR_CODE_LEN + 1);
	for (; iter != end; )
	{
		CSyncTask *pTask = *iter;
		if (IVS_SUCCEED == pTask->StartPlayExpired(now, !!m_TaskRunning.size(), pNVRCode))
		{
	          ...
	          unsigned int uProto, float fSpeed)
{
	...
	eSDK_STRNCPY(m_CamID, sizeof(m_CamID), pCamID, sizeof(m_CamID) - 1);
    m_bPlaying    = false; 
...
#define FORMAT_TIME(t, buf)                                   \
	do {                                                      \
       struct tm _tm;                                         \
       gmtime_s(&_tm, &(t));                                  \
       eSDK_SNPRINTF(buf, 14, "%04d%02d%02d%02d%02d%02d",         \
 ...
 * 启动到时的任务
 */
int CSyncTask::StartPlayExpired(time_t now, bool bRunning, const char* pNVRCode)
{
	/*
	 * 没有正在播放的录像，
	...
	if (NULL != pCtrl)//lint !e774
	{
		...
		eSDK_MEMCPY(Param.cNVRCode, IVS_NVR_CODE_LEN, pNVRCode, IVS_NVR_CODE_LEN);
		...
		int scale = static_cast<int>(pVideoPaneMgr->GetDispalyScale());
