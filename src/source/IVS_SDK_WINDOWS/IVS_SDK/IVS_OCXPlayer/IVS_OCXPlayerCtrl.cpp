...
BEGIN_MESSAGE_MAP(CIVS_OCXPlayerCtrl, COleControl)
	...
	ON_MESSAGE(WM_EVENT_REMOTE_DOWNLOAD_CALLBACK, &CIVS_OCXPlayerCtrl::OnRemoteDownLoadEvent)
	...
	ON_MESSAGE(WM_IA_DRAW_CALLBACK_EX, &CIVS_OCXPlayerCtrl::OnIADrawCallBackEx)
END_MESSAGE_MAP()
...
BEGIN_DISPATCH_MAP(CIVS_OCXPlayerCtrl, COleControl)
	...
	DISP_FUNCTION_ID(CIVS_OCXPlayerCtrl, "IVS_OCX_GetDomainDeviceList", dispidIVS_OCX_GetDomainDeviceList, IVS_OCX_GetDomainDeviceList, VT_BSTR, VTS_BSTR VTS_UI4 VTS_UI4 VTS_UI4)
	DISP_FUNCTION_ID(CIVS_OCXPlayerCtrl, "IVS_OCX_EnableDrawCallBackEx", dispidIVS_OCX_EnableDrawCallBackEx, IVS_OCX_EnableDrawCallBackEx, VT_I4, VTS_UI4)
	DISP_FUNCTION_ID(CIVS_OCXPlayerCtrl, "IVS_OCX_DisableDrawCallBackEx", dispidIVS_OCX_DisableDrawCallBackEx, IVS_OCX_DisableDrawCallBackEx, VT_I4, VTS_UI4)
	END_DISPATCH_MAP()
...
BEGIN_EVENT_MAP(CIVS_OCXPlayerCtrl, COleControl)
	...
	EVENT_CUSTOM_ID("IVS_OCX_DrawCallBackEx_Event", eventidIVS_OCX_DrawCallBackEx_Event, IVS_OCX_DrawCallBackEx_Event, VTS_UI4 VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_UI8)
END_EVENT_MAP()
//lint +e786 这部分代码由MS自动产生
...
LONG CIVS_OCXPlayerCtrl::WM_KEYUPEvent(const MSG* pMsg)
{
	...
	case 'R':
		if (pMsg->message == WM_KEYDOWN)
		{
			...
			{
				::PostMessage(pActiveVedioPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_RECORD, BN_CLICKED), (LPARAM)pActiveVedioPane->GetVideoRealtimeBar()->GetRecordBtnHwnd());//lint !e835 !e845
			}
		}
		break;
// 	case 0x41://VK_A
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_Init(void)
{
	...
	GetModuleFileName(AfxGetStaticModuleState()->m_hCurrentInstanceHandle ,pBuf, MAX_PATH-sizeof(TCHAR));
	char* cTemp = (char*)pBuf;
	std::string strOCXPath = cTemp;
	strOCXPath = strOCXPath.substr(0,strOCXPath.rfind('\\')+1);

	//获取统计日志的配置文件路径
	std::string strInitPath = strOCXPath;
	strInitPath.append(__CLIENT_LOG_CFG__);
	//获取日志输出路径
	//std::string strLogOutPath = strOCXPath;
    ...
    if (1 == g_nNumberOfOcx)
    {
		...
		{
			pLogPath = strLogOutPath.c_str();
			(void)IVS_SDK_SetLogPath(strLogOutPath.c_str());
		}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_GetVersion(void)const
{
    ...
    WORD nlVersion = MAKEWORD(IVS_OCX_FORTH_VERSION,IVS_OCX_THIRD_VERSION);	//lint !e835 !e845
    WORD nhVersion = MAKEWORD(IVS_OCX_SECOND_VERSION,IVS_OCX_MAIN_VERSION);
    LONG dTmp = MAKELONG(nlVersion, nhVersion);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_AddUser(LPCTSTR pReqXml)
{
	...
	// 清理内存，保证内存中不会有密码的存在
	eSDK_MEMSET((char*)pReqXml,0,strlen(pReqXml));
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetHelpInfoText(LPCTSTR pAlarmInCode, LPCTSTR pAlarmType)
{
 ...
 //       return strResult.AllocSysString();
 //   }
 //   eSDK_MEMSET(pHelpInfoText, 0, IVS_MAX_HELP_LEN + 1);
 //   // 获取告警帮助信息正文
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteUser(LPCTSTR pDomainCode, ULONG ulUserID)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::DeleteUser(m_iSDKSessionID, pDomainCode, ulUserID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModifyUser(LPCTSTR pReqXml)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::ModifyUser(m_iSDKSessionID, pReqXml);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_LockUser(LPCTSTR pDomainCode, ULONG ulUserID, ULONG ulLock)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::LockUser(m_iSDKSessionID, pDomainCode, ulUserID, ulLock);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ChangePWD(LPCTSTR pOldPWD, LPCTSTR pNewPWD)
{
	...
	IVS_INT32 iRet = COCXUserInfoMgr::ChangePWD(m_iSDKSessionID, pOldPWD, pNewPWD);

	// 防止内存中密码出现
	eSDK_MEMSET((char*)pOldPWD,0,strlen(pOldPWD));
	eSDK_MEMSET((char*)pNewPWD,0,strlen(pNewPWD));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ResetPWD(LPCTSTR pDomainCode, ULONG ulUserID, LPCTSTR pNewPWD)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::ResetPWD(m_iSDKSessionID, pDomainCode, ulUserID, pNewPWD);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_CheckPWDByRole(LPCTSTR pDomainCode, ULONG ulRoleID, LPCTSTR pLoginName, LPCTSTR pPWD)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::CheckPWDByRole(m_iSDKSessionID, pDomainCode, ulRoleID, pLoginName, pPWD);

	// 防止内存中密码出现
	eSDK_MEMSET((char*)pPWD,0,strlen(pPWD));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetUserDeviceList(LPCTSTR pReqXml)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::SetUserDeviceList(m_iSDKSessionID, pReqXml);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SignOutUser(LPCTSTR pDomainCode, ULONG ulUserID)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::SignOutUser(m_iSDKSessionID, pDomainCode, ulUserID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SignOutUserByRole(LPCTSTR pDomainCode, ULONG ulRoleID)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::SignOutUserByRole(m_iSDKSessionID, pDomainCode, ulRoleID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SignOutOnlineUser(LPCTSTR pDomainCode, ULONG ulUserOnlineID)
{
    ...
    IVS_INT32 iRet = COCXUserInfoMgr::SignOutOnlineUser(m_iSDKSessionID, pDomainCode, ulUserOnlineID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteRole(LPCTSTR pDomainCode, ULONG ulRoleID)
{
    ...
    IVS_INT32 iRet = COCXRoleMgr::DeleteRole(m_iSDKSessionID, pDomainCode, ulRoleID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModifyRole(LPCTSTR pRoleInfo)
{
    ...
    IVS_INT32 iRet = COCXRoleMgr::ModifyRole(m_iSDKSessionID, pRoleInfo);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteUserGroup(LPCTSTR pDomainCode, ULONG ulUserGroupID)
{
    ...
    IVS_INT32 iRet = COCXUserGroupMgr::DeleteUserGroup(m_iSDKSessionID, pDomainCode, ulUserGroupID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModifyUserGroup(LPCTSTR pUserGroup)
{
    ...
    IVS_INFO_TRACE("OCX Address: %x,UserGroup: %s", this,pUserGroup);
    IVS_INT32 iRet = COCXUserGroupMgr::ModifyUserGroup(m_iSDKSessionID, pUserGroup);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_MoveUserGroup(LPCTSTR pUserGroup)
{
    ...
    IVS_INFO_TRACE("OCX Address: %x,UserGroup: %s", this,pUserGroup);
    IVS_INT32 iRet = COCXUserGroupMgr::MoveUserGroup(m_iSDKSessionID, pUserGroup);
...
ULONG CIVS_OCXPlayerCtrl::IVS_OCX_StopAllRealPlay(void)
{
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,static_cast<IVS_INT32>(ulRet),"");
    return ulRet;
}
//**********************************************
...
// TODO: GetMagnifyOrMinifyEventMsgXML(xmlToCU,ulWinID);
	BP_DBG_LOG("Post change size event massage, IsMagnify = %d", ulIsMagnify);
...
ULONG CIVS_OCXPlayerCtrl::StopRealPlayByPane(CVideoPane *pVideoPane)
{
	...
	if (MAGNIFY_OPEN == pVideoPane->GetRealtimeMagnifyState())
	{
		...
		::SendMessage(pVideoPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_MAGNIFY, BN_CLICKED), (LPARAM)pVideoPane->GetVideoRealtimeBar()->GetMagnifyBtnHwnd());//lint !e835 !e845
	}
...
//		::SendMessage(pVideoPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_TALKBACK, BN_CLICKED), (LPARAM)pVideoPane->GetVideoRealtimeBar()->GetTalkbackBtnHwnd());
	...
	{
		// 发送一个点击事件给放大按钮，让他关闭自己，这里使用同步的sendmessage,不要使用postmessage
		::SendMessage(pVideoPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_PTZ, BN_CLICKED), (LPARAM)pVideoPane->GetVideoRealtimeBar()->GetPTZBtnHwnd());//lint !e835 !e845
	}
...
// 	eSDK_MEMSET(&stIArectPercent,0,sizeof(IVS_RECT));
	//bool bOpen = false;
    ...
    if (SERVICE_TYPE_REALVIDEO == enServiceType)
    {
		...
		(void)pVideoPane->StopRealStreamSync();
    }
...
afx_msg LRESULT CIVS_OCXPlayerCtrl::OnCommEventCallback(WPARAM wParam, LPARAM lParam)
{
	...
	SetThreadLocale(LOCALE_SYSTEM_DEFAULT);	//lint !e835
	...
	case IVS_EVENT_MODE_CRUISE_RECORD_OVER:
		...
		break;
	case IVS_EVENT_SHUTDOWN_STREAM:
		OnShutdownStreamNotify(wParam, lParam);
		break;
...
inline void HandleRemotePlaybackEvent(void *pEventBuf, size_t iBufSize, void *pUserData)//lint !e830
{
    ...
    CIVS_OCXPlayerCtrl * pOcxCtrl = iter->second;
    ...
    try
    {
        if ((NULL != pEventBuf) && (0 != iBufSize))
        {
            try
            {
				...
				eSDK_MEMSET(pEvent, 0, sizeof(iBufSize));
            }
            catch(...)
            {
...
//#
    ...
    case IVS_EVENT_FILE_BROADCAST_END:
	case IVS_EVENT_SHUTDOWN_STREAM:						//关闭指定用户视频业务事件
        //如果是文件广播结束事件，则停止广播
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_Login(LPCTSTR pUserName, LPCTSTR pPWD, LPCTSTR pServerIP,LPCTSTR pPort, ULONG ulClientType)
{
    ...
    eSDK_MEMSET(&sLoginInfo,0,sizeof(IVS_LOGIN_INFO));
	...
	if (IVS_SUCCEED == iRet||IVS_SMU_USER_FIRST_LOGIN == iRet || IVS_SMU_USER_PWD_NEED_MOD == iRet)
	{
        ...
        // 子进程不需要上传下载功能
        if (CLIENT_PC_CHILD != ulClientType && CLIENT_TV_WALL != ulClientType && CLIENT_VCM != ulClientType)
        {
#ifdef ZIP_UNZIP
            //启动上传、下载文件的功能
	...
	// 清理内存，保证内存中不会有密码的存在
	SecureZeroMemory(&sLoginInfo, sizeof(IVS_LOGIN_INFO));
	SecureZeroMemory((char*)pPWD, strlen(pPWD));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartRealPlay(LPCTSTR pMediaPara, LPCTSTR pCameraCode, LONG lWndID)
{
    ...
    eSDK_MEMSET(&stRealplayParam, 0, sizeof(stRealplayParam));
	...
	CVideoPane* pVideoPane1 = NULL;
	if(GetVideoPaneMgr())
	{
		pVideoPane1 = GetVideoPaneMgr()->GetWndPointerByID((ULONG)lWndID);//lint !e613
	}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopRealPlay(LONG lWndID)
{
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	LONG iRet =  (LONG)StopRealPlayByPane(pVideoPane);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_Logout(void)
{
	...
	IVS_INFO_TRACE("OCX Address: %x", this);
	//delete by zwx211831, Date:20150829, 去掉该功能
#ifdef ZIP_UNZIP
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetRecordPolicyByTime(LPCTSTR pCameraCode)
{
	...
	eSDK_MEMSET(&stRecordPoly,0,sizeof(IVS_RECORD_POLICY_TIME));

	IVS_INT32 iRet = IVS_SDK_GetRecordPolicyByTime(m_iSDKSessionID,pCameraCode,&stRecordPoly);
	if (iRet!=IVS_SUCCEED)
	{
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return strResult.AllocSysString();//lint !e438
}//lint !e529
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetRecordPolicyByTime(LPCTSTR pReqXml)
{
	...
	eSDK_MEMSET(&sRecordPolicy,0,sizeof(IVS_RECORD_POLICY_TIME));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetRecordSpace(LPCTSTR pRecordSpace)
{
 ...
 //   eSDK_MEMSET(pCameraCode,0,sizeof(pCameraCode));
 //   IVS_INT32 pRecordMethod = NULL;
 //   eSDK_MEMSET(&sRecordSpace,0,sizeof(IVS_RECORD_POLICY_TIME));
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_QueryRecordList(LPCTSTR pUnifiedQuery)
{
	...
	IVS_INT32 iFieldNum = COCXXmlProcess::GetFieldNum(xmlReq);
...
BOOL CIVS_OCXPlayerCtrl::IVS_OCX_GetGPUDecodeAbility(void)
{
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,bRet,"");
    return bRet;
}
...
// 	}
	...
	LONG iRet =  m_pVideoPaneMgr->SetToolbarEnableStatus(static_cast<IVS_ULONG>(lWndID),
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_Aes256CbcDecode(LPCTSTR pszCipher)
{
 ...
 //   char* pszPlain_pwd = IVS_NEW(pszPlain_pwd, 128);
	...
	//	return strResult.AllocSysString();
	//}
	//eSDK_MEMSET(pszPlain_pwd,0,128);
 //   //解密
 //   IVS_INT32 iRet = IVS_SDK_Aes256CbcDecode(pszCipher,pszPlain_pwd, (IVS_UINT64)128);
	//// 防止密码在内存中显示
	//eSDK_MEMSET((char*)pszCipher, 0, strlen(pszCipher));
 //   if (IVS_SUCCEED != iRet)
 //   {
 //       IVS_DELETE(pszPlain_pwd, MULTI);
	//	SecureZeroMemory(pszPlain_pwd, 128);
 //       return NULL;
 //   }
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_AddFavoriteDevice(LPCTSTR pReqXml)
{
    ...
    CXml xmlRsp;
    IVS_INT32 iRet = CFavorite::AddFavoriteDevice(pReqXml, strDeviceGUID);
    if (IVS_SUCCEED != iRet)
    {
        COCXXmlProcess::GetErrString(strResult,iRet);
        return strResult.AllocSysString();
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelFavoriteDevice(LPCTSTR pReqXml)
{
	...
	IVS_INT32 iRet = CFavorite::DelFavoriteDevice(pReqXml);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
    return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_MoveFavoriteDevice(LPCTSTR pReqXml)
{
    ...
    IVS_INT32 iRet = CFavorite::MoveFavoriteDevice(pReqXml);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_AddFavoriteFolder(LPCTSTR pReqXml)
{
    ...
    CXml xmlRsp;
    IVS_INT32 iRet = CFavorite::AddFavoriteFolder(pReqXml, strFolderGUID);
    if (IVS_SUCCEED != iRet)
    {
        COCXXmlProcess::GetErrString(strResult,iRet);
        return strResult.AllocSysString();
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelFavoriteFolder(LPCTSTR pReqXml)
{
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);	
    IVS_INT32 iRet = CFavorite::DelFavoriteFolder(pReqXml);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModFavoriteFolder(LPCTSTR pReqXml)
{
    ...
    IVS_INT32 iRet = CFavorite::ModFavoriteFolder(pReqXml);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);	
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_MoveFavoriteFolder(LPCTSTR pReqXml)
{
    ...
    IVS_INT32 iRet = CFavorite::MoveFavoriteFolder(pReqXml);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_PtzControl(LPCTSTR pCameraCode,LONG lOperateCode, LPCTSTR pControlPara1, LPCTSTR pControlPara2)
{
	...
	IVS_INT32 iRet = IVS_SDK_PtzControl(m_iSDKSessionID,pCameraCode,lOperateCode,pControlPara1,pControlPara2, &uiLockStatus);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetUserID(void)
{
    ...
    IVS_INT32 iRet = IVS_SDK_GetUserID(m_iSDKSessionID, &uiUserID);
    if (IVS_SUCCEED != iRet)
    {
        COCXXmlProcess::GetErrString(strResult,iRet);
        return strResult.AllocSysString();
...
// 	SoundMgr.SetSoundingPanel(pVideoPane);
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,lRet,"");
    return lRet;
}
...
LONG CIVS_OCXPlayerCtrl::GetVolume(ULONG ulWndID, ULONG ulType, unsigned int* puiVolumeValue)
{
    ...
    IVS_INT32 iRet = IVS_SDK_GetVolume(m_iSDKSessionID,
                             ulHandle,
                             puiVolumeValue);
...
IVS_INT32 __stdcall CIVS_OCXPlayerCtrl::HandleTVWallNotifyCallBack(CUMW_NOTIFY_INFO* pstNotifyInfo, void* pUserData)
{
    ...
    eSDK_MEMSET(pstCallback, 0, sizeof(ST_TV_WALL_CALLBACK_INFO));
    ...
    eSDK_MEMSET(pstNotify, 0, sizeof(CUMW_NOTIFY_INFO));
    eSDK_MEMCPY(pstNotify, sizeof(CUMW_NOTIFY_INFO), pstNotifyInfo, sizeof(CUMW_NOTIFY_INFO));
    ...
    case CUMW_NOTIFY_TYPE_ASYNC_RET:
        {
            ...
            eSDK_MEMSET(pstContent, 0, sizeof(CUMW_NOTIFY_ASYNC_RET_EX));

            eSDK_MEMCPY(pstContent, sizeof(CUMW_NOTIFY_ASYNC_RET_EX), pstNotifyInfo->pNotifyInfo, sizeof(CUMW_NOTIFY_ASYNC_RET_EX));
    ...
    case CUMW_NOTIFY_TYPE_REMOTE_DECODER_REGISTER:
        {
            ...
            eSDK_MEMSET(pstContent, 0, sizeof(CUMW_NOTIFY_REMOTE_DECODER_STATUS));

            eSDK_MEMCPY(pstContent, sizeof(CUMW_NOTIFY_REMOTE_DECODER_STATUS), pstNotifyInfo->pNotifyInfo, sizeof(CUMW_NOTIFY_REMOTE_DECODER_STATUS));
            pstNotify->pNotifyInfo = (void*)pstContent;
        }
        break;
    ...
    case CUMW_NOTIFY_TYPE_REMOTE_DECODER_DISCONNECT:
        {
            ...
            eSDK_MEMSET(pstContent, 0, sizeof(CUMW_NOTIFY_REMOTE_DECODER_OFF_LINE_INFO));

            eSDK_MEMCPY(pstContent, sizeof(CUMW_NOTIFY_REMOTE_DECODER_OFF_LINE_INFO), pstNotifyInfo->pNotifyInfo, sizeof(CUMW_NOTIFY_REMOTE_DECODER_OFF_LINE_INFO));
            pstNotify->pNotifyInfo = (void*)pstContent;
        }
        break;
    ...
    case CUMW_NOTIFY_TYPE_DECODER_ERROR:	//断流
        {
            ...
            eSDK_MEMSET(pstContent, 0, sizeof(CUMW_NOTIFY_TYPE_DECODER_ERROR_INFO));

            eSDK_MEMCPY(pstContent, sizeof(CUMW_NOTIFY_TYPE_DECODER_ERROR_INFO), pstNotifyInfo->pNotifyInfo, sizeof(CUMW_NOTIFY_TYPE_DECODER_ERROR_INFO));
            pstNotify->pNotifyInfo = (void*)pstContent;
        }
        break;
...
{
	CHECK_POINTER(CIVS_OCXPlayerCtrl::m_pOCX, IVS_OPERATE_MEMORY_ERROR);
    ...
    eSDK_MEMSET(szBuf, 0, sizeof(szBuf));
    ...
    const char* pXml = xml.GetXMLStream(nXMLStreamLength);
     ...
     eSDK_MEMSET(g_szNetKeyBoardXML, 0, sizeof(g_szNetKeyBoardXML));
     eSDK_MEMCPY(g_szNetKeyBoardXML, sizeof(g_szNetKeyBoardXML), pXml, nXMLStreamLength);
	 if (nXMLStreamLength >= 1024)
	 {
		 g_szNetKeyBoardXML[1023] = '\0';
	 }
	 else
	 {
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetIAToolbar(ULONG ulToolbarFlag)
{
	...
	(void)m_pVideoPaneMgr->SetIAToolbar(ulToolbarFlag);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartPlatformPlayBack(LPCTSTR pCameraCode, LPCTSTR pPlaybackParam, LONG lWndID)
{
    ...
    eSDK_MEMSET(&stPlaybackParam, 0, sizeof(IVS_PLAYBACK_PARAM));
    ...
    IVS_INT32 iRet = IVS_SDK_StartPlatformPlayBack(m_iSDKSessionID, pCameraCode, &stPlaybackParam, pVideoPane->GetVideoWnd(), &ulHandle);
    if (IVS_SUCCEED != iRet)
    {
...
ULONG CIVS_OCXPlayerCtrl::StopPlayBackByPane(CVideoPane * pVideoPane,IVS_INT32 iRecordMethod)
{
	...
	eSDK_MEMSET(&graphicsTemp,0,sizeof(IA_GRAPHICS));
	bool bOpen = false;
	CVideoPane* pMagnifySrcPane = NULL;
	if (BOTTOMBAR_MAGNIFY_OPEN == pVideoPane->GetDstMagnifyState())
	{
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_PlayBackPause(LONG lWndID)
{
    ...
    IVS_INT32 iRet = IVS_SDK_PlayBackPause(m_iSDKSessionID, (ULONG)pVideoPane->GetPlayHandle());
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_PlaybackFrameStepForward(LONG lWndID)
{
	...
	IVS_INT32 iRet = IVS_SDK_PlayBackFrameStepForward(m_iSDKSessionID, (IVS_ULONG)pVideoPane->GetPlayHandle());
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetPlayBackSpeed(LONG lWndID, FLOAT fSpeed)
{
	...
	IVS_INT32 iRet = IVS_SDK_SetPlayBackSpeed(m_iSDKSessionID, (IVS_ULONG)pVideoPane->GetPlayHandle(), fSpeed);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetPlayBackTime(LONG lWndID)
{
	...
	eSDK_MEMSET(&stPlaybackTime, 0, sizeof(IVS_PLAYBACK_TIME));
	int iRet = IVS_FAIL;
...
//平台录像下载
afx_msg LRESULT CIVS_OCXPlayerCtrl::OnRemoteDownLoadEvent(WPARAM wParam,LPARAM lParam)
{
    // 向界面抛事件;
    LPCTSTR pszXML = (LPCTSTR)lParam;
	ULONG EventID        = (ULONG)wParam;

    IVS_OCX_Windows_Event(static_cast<IVS_LONG>(EventID), pszXML);
	IVS_OCX_Event(static_cast<IVS_LONG>(EventID), pszXML);

	char *pEvent = const_cast<char *>(pszXML);
	IVS_UINT32 uiStrLen = strlen(pEvent);
	IVS_DELETE(pEvent, uiStrLen);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_EnableDraw(ULONG ulWindowID)
{
	...
	LONG iRet = pVideoPane->EnableDraw(m_iSDKSessionID, pVideoPane->GetPlayHandle());
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DisableDraw(ULONG ulWindowID)
{
	...
	IVS_INT32 iRet = pVideoPane->DisableDraw(m_iSDKSessionID, pVideoPane->GetPlayHandle());
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetGraphics(ULONG ulWindowID, LPCTSTR pGraphicsXml)
{
	...
	eSDK_MEMSET(pGraphicsArr, 0, sizeof(IA_GRAPHICS) * GRAPHICS_NUM_MAX);
	ULONG ulNum = 0;
	SIZE sz = {0};
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetRelationShip(ULONG ulWindowID, ULONG ulGraphicGroupA, ULONG ulGraphicGroupB, ULONG ulRelationShipType)
{
	...
	IVS_INT32 iRet = pVideoPane->SetRelationShip(ulGraphicGroupA, ulGraphicGroupB, ulType);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetGraphicSelected(ULONG ulWindowID)
{
	...
	eSDK_MEMSET(pGraphic, 0, sizeof(IA_GRAPHICS) * GRAPHICS_NUM_MAX);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DrawGraphics(ULONG ulWindowID, LPCTSTR pGraphicsXml)
{
	...
	eSDK_MEMSET(&GraphicAttribute, 0, sizeof(IA_GRAPHICS));
	...
	// 调用函数
	LONG lRet = pVideoPane->SetCurrentGraphicAttribute(&GraphicAttribute, ulGroupDrawMaxNum);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,lRet,"");
	return lRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopDrawGraphics(ULONG ulWindowID, ULONG ulGraphicGroup)
{
	...
	IVS_INT32 iRet =  pVideoPane->StopDrawGraphics(ulGraphicGroup);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteGraphicsSelected(ULONG ulWindowID)
{
	...
	IVS_INT32 iRet = pVideoPane->DeleteGraphicsSelected();
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteGraphicsByGroup(ULONG ulWindowID, ULONG ulGraphicGroup)
{
	...
	IVS_INT32 iRet = pVideoPane->DeleteGraphicsByGroup(ulGraphicGroup);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteGraphics(ULONG ulWindowID)
{
	...
	IVS_INT32 iRet = pVideoPane->DeleteGraphics();
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SelectGraphicByGroup(ULONG ulWindowID, ULONG ulGraphicGroup)
{
	...
	IVS_INT32 iRet = pVideoPane->SelectGraphicByGroup(ulGraphicGroup);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SelectCancel(ULONG ulWindowID)
{
	...
	IVS_INT32 iRet = pVideoPane->SelectCancel();
...
ULONG CIVS_OCXPlayerCtrl::IVS_OCX_StartLocalRecord(LPCTSTR pRecordPara, LONG lWndID)
{
	...
	eSDK_MEMSET(&RecordParam, 0, sizeof(RecordParam));
	if (NULL == pRecordPara)
	{
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopPlatformPlayBackTVWall(ULONG ulTVWallID, ULONG ulWndID)
{
	...
	//ulWndID为真实的电视墙窗格，先根据真实的窗格找到虚拟的窗格
	TVWall_VPANEID vPaneID;
	eSDK_MEMSET(&vPaneID,0,sizeof(vPaneID));
	if(!m_pVideoPaneMgr->GetVirtualIDByRealID(ulWndID,vPaneID))
	{
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetTempScenario(LPCTSTR pScenarioXml)
{
	...
	IVS_INT32 iRet= CFavorite::AddScenarioFavorite(pScenarioXml, strTempScenarioInfoID);
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iRet,"");
	return iRet;
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModScenarioFavorite(LPCTSTR pScenarioXml)
{
	...
	CHECK_POINTER(pScenarioXml,IVS_PARA_INVALID);
	IVS_INFO_TRACE("OCX Address: %x,ScenarioXml: %s", this,pScenarioXml);
	IVS_INT32 iRet = CFavorite::ModScenarioFavorite(pScenarioXml);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelScenarioFavorite(LPCTSTR pScenarioID)
{
	...
	IVS_INT32 iRet = CFavorite::DelScenarioFavorite(pScenarioID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetVolume(LONG lWndID, ULONG ulVolumeValue)
{
	...
	LONG lRet = SetVolume((ULONG)lWndID, SOUNDCTRL_WND_TYPE_SOUND, ulVolumeValue);
...
ULONG CIVS_OCXPlayerCtrl::IVS_OCX_Set3DPTZ(ULONG ulWndID, ULONG ulIsStart3D)
{
    ...
    CHECK_POINTER(m_pVideoPaneMgr,IVS_OPERATE_MEMORY_ERROR);
    ULONG ulRet = m_pVideoPaneMgr->Set3DPTZState(ulWndID,ulIsStart3D);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_AddBookmark(LPCTSTR pNVRCode,LPCTSTR pCameraCode,LPCTSTR pBookmarkName,LPCTSTR pBookmarkTime)
{
	...
	IVS_INT32 iRet = IVS_SDK_AddBookmark(m_iSDKSessionID,pCameraCode,pNVRCode,pBookmarkName,pBookmarkTime,&uiBookmarkID);
	if (IVS_SUCCEED != iRet)
	{
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetBookmarkList(LPCTSTR pQueryXml)
{
	...
	if (IVS_SUCCEED != iRet)
	{
		IVS_DELETE(pQueryFiledBuff, MUILI);
		IVS_DELETE(pRecordListBuf, MUILI);
		(void)IVS_SDK_ReleaseBuffer(reinterpret_cast<IVS_CHAR *>(pBookmarkInfoList));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModifyBookmark(LPCTSTR pNVRCode,LPCTSTR pCameraCode,ULONG ulBookmarkID,LPCTSTR pNewBookmarkName)
{
	...
	IVS_INT32 iRet = IVS_SDK_ModifyBookmark(m_iSDKSessionID,pNVRCode,pCameraCode,ulBookmarkID,pNewBookmarkName);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteBookmark(LPCTSTR pNVRCode,LPCTSTR pCameraCode,ULONG ulBookmarkID)
{
	...
	IVS_INT32 iRet = IVS_SDK_DeleteBookmark(m_iSDKSessionID,pNVRCode,pCameraCode,ulBookmarkID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelPTZPreset(LPCTSTR pCameraCode, ULONG ulPresetIndex)
{
	...
	CHECK_POINTER(pCameraCode,IVS_PARA_INVALID);
	IVS_INFO_TRACE("OCX Address: %x,ulPresetIndex:%lu", this, ulPresetIndex);
	IVS_INT32 iRet = IVS_SDK_DelPTZPreset(m_iSDKSessionID, pCameraCode, ulPresetIndex);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModPTZPreset(LPCTSTR pCameraCode, ULONG ulPresetIndex, LPCTSTR pPresetName)
{
	...
	IVS_INFO_TRACE("OCX Address: %x,CameraCode: %s,ulPresetIndex:%lu,PresetName: %s", this, pCameraCode, ulPresetIndex, pPresetName);
	
	// 构造输入参数
	IVS_PTZ_PRESET stPTZPreset;
	eSDK_MEMSET(&stPTZPreset, 0, sizeof(IVS_PTZ_PRESET));
	...
	IVS_INT32 iRet = IVS_SDK_ModPTZPreset(m_iSDKSessionID, pCameraCode, &stPTZPreset);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetPTZPresetList(LPCTSTR pCameraCode)
{
	...
	eSDK_MEMSET(pPTZPresetList, 0, uiBufferSize);
	// 输出参数
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetGuardPos(LPCTSTR pGuardPosInfo)
{
	...
	eSDK_MEMSET(&stGuardPosInfo, 0, sizeof(IVS_GUARD_POS_INFO));
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_AddCruiseTrack(LPCTSTR pCuriseTrackInfo)
{
	...
	eSDK_MEMSET(&stCuriseTrackInfo, 0, sizeof(IVS_CRUISE_TRACK));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelCruiseTrack(LPCTSTR pCameraCode, ULONG ulTrackIndex, ULONG ulCruiseType)
{
	...
	CHECK_POINTER(pCameraCode,NULL);
	IVS_INFO_TRACE("OCX Address: %x,ulTrackIndex: %lu", this, ulTrackIndex);
	IVS_INT32 iRet = IVS_SDK_DelCruiseTrack(m_iSDKSessionID, pCameraCode, ulTrackIndex, ulCruiseType);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetCruiseTrackList(LPCTSTR pCameraCode)
{
	...
	eSDK_MEMSET(pCruiseTrackList, 0, uiBufferSize);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetCruiseTrack(LPCTSTR pCameraCode, ULONG ulTrackIndex)
{
	...
	CHECK_POINTER(pCameraCode,NULL);
	IVS_INFO_TRACE("OCX Address: %x,CameraCode: %s, ulTrackIndex:%lu", this, pCameraCode, ulTrackIndex);
	CString strResult;

	IVS_CRUISE_TRACK stCruiseTrackInfo;
	eSDK_MEMSET(&stCruiseTrackInfo, 0, sizeof(IVS_CRUISE_TRACK));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetCruisePlan(LPCTSTR pCruisePlan)
{
	...
	eSDK_MEMSET(pstCruisePlan, 0, sizeof(IVS_CRUISE_PLAN));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelCameraSwitch(LPCTSTR pCameraSwitchID)
{
	...
	IVS_INT32 iRet = CFavorite::DelCameraSwitch(pCameraSwitchID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_LocalSnapshot(LONG lWndID)
{
	...
	eSDK_MEMSET(&LocalCaptureParam, 0, sizeof(LocalCaptureParam));
	// 获取本地抓拍参数配置
	...
	IVS_INT32 iRet = pVideoPane->LocalSnapshot(m_iSDKSessionID, LocalCaptureParam);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_PlatformSnapshot(LPCTSTR pCameraCode)
{
	...
	IVS_INFO_TRACE("OCX Address: %x,CameraCode: %s",this,pCameraCode);
	IVS_INT32 iRet = IVS_SDK_PlatformSnapshot(m_iSDKSessionID,pCameraCode);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteSnapshot(LPCTSTR pCameraCode, ULONG ulPictureID)
{
	...
	IVS_INT32 iRet = OCXCapturePicture::DeleteSnapshot(m_iSDKSessionID, pCameraCode, ulPictureID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_UploadStatement(LPCTSTR pLanguageType,LPCTSTR pStatementFile)
{
	...
	IVS_INFO_TRACE("OCX Address: %x", this);
	//IVS_INT32  iRet = IVS_SDK_UploadStatement(m_iSDKSessionID,pLanguageType,pStatementFile);
	IVS_INT32  iRet = IVS_FAIL;
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetWarnState(ULONG ulWndID, ULONG ulWarnState, ULONG ulWarnSeconds)
{
	...
	ULONG ulRet = m_pVideoPaneMgr->SetWarnState(ulWndID,ulWarnState,ulWarnSeconds);

	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,static_cast<IVS_INT32>(ulRet),"");
	return static_cast<IVS_LONG>(ulRet);
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartPlatformRecord(LPCTSTR pCameraCode,ULONG ulRecordTime)
{
	...
	// TODO: Add your dispatch handler code here
	IVS_INT32 iRet = IVS_SDK_StartPlatformRecord(m_iSDKSessionID,pCameraCode,static_cast<IVS_INT32>(ulRecordTime));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopPlatformRecord (LPCTSTR pCameraCode)
{
	...
	IVS_INFO_TRACE("OCX Address: %x,CameraCode: %s", this,pCameraCode);
	IVS_INT32 iRet = IVS_SDK_StopPlatformRecord(m_iSDKSessionID,pCameraCode);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartPURecord(LPCTSTR pCameraCode,ULONG ulRecordTime)
{
	...
	IVS_INFO_TRACE("OCX Address: %x,CameraCode: %s,RecordTime:%lu", this,pCameraCode,ulRecordTime);
	IVS_INT32 iRet = IVS_SDK_StartPURecord(m_iSDKSessionID,pCameraCode,static_cast<IVS_INT32>(ulRecordTime));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopPURecord (LPCTSTR pCameraCode)
{
	...
	IVS_INFO_TRACE("OCX Address: %x,CameraCode: %s", this,pCameraCode);
	IVS_INT32 iRet = IVS_SDK_StopPURecord(m_iSDKSessionID,pCameraCode);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetCameraBackupConfig(LPCTSTR pBackupXml)
{
	...
	IVS_INT32 iRet = OCXRecordMgr::SetCameraBackupConfig(m_iSDKSessionID, pBackupXml);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetWndType(ULONG ulWindID,ULONG ulTyle)
{
	...
	//入参窗格ID为0表示没有指定窗格，默认使用当前活动窗格
	CVideoPane *pWnd = ((CVideoPaneMgr*)m_pVideoPaneMgr)->GetWndPointerByID(ulWindID);
	if (NULL == pWnd)
	{
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_VerifyDevice(LPCTSTR pDomainCode, LPCTSTR pVerifyInfo)
{
	...
	// 防止内存中密码出现
	eSDK_MEMSET((char*)pVerifyInfo,0,strlen(pVerifyInfo));
...
ULONG CIVS_OCXPlayerCtrl::StartTalkback(ULONG ulWinID)
{
	...
	******************************/
	...
	eSDK_MEMSET(&stTalkbackParam, 0, sizeof(stTalkbackParam));
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_StartVoiceTalkback(LPCTSTR pTalkParam, LPCTSTR pCameraCode)
{
	...
	if(GetVideoPaneMgr())
	{
		pVideoPane = GetVideoPaneMgr()->GetWndPointerByCameraCode(pCameraCode);//lint !e613
	}
	if (NULL != pVideoPane)
	{
		m_pVoiceTalkbackVideoPane = pVideoPane;
		...
		if (NULL != pRealtimeBar)
		{
			...
			::SendMessage(pRealtimeBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_TALKBACK, BN_CLICKED), (LPARAM)pRealtimeBar->GetTalkbackBtnHwnd());//lint !e835 !e845
			iRet = IVS_SUCCEED;
		}
	}
	else
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopVoiceTalkback(ULONG ulTalkbackHandle)
{
	...
	if (NULL != m_pVoiceTalkbackVideoPane)
	{
        ...
        {
            (void)IVS_SDK_StopVoiceTalkback(m_iSDKSessionID, ulTalkbackHandle);
        ...
        else
        {
            ...
            if (NULL != pRealtimeBar)
            {
                ...
                ::SendMessage(m_pVoiceTalkbackVideoPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_TALKBACK, BN_CLICKED), (LPARAM)pRealtimeBar->GetTalkbackBtnHwnd());//lint !e835 !e845
            }
        }
		iRet = IVS_SUCCEED;
	}
...
ULONG CIVS_OCXPlayerCtrl::IVS_OCX_SetCameraAlias(LPCTSTR pCameraCode, LPCTSTR pAlias)
{
	...
	while(NULL != pNode)
	{
		...
		if (!strcmp(pCameraCode, pCode))
		{
			TiXmlNode *pTiXmlNode = pNode->FirstChild();
			ASSERT(NULL != pTiXmlNode);
			TiXmlText *pTiXmlText = pTiXmlNode->ToText();
			ASSERT(NULL != pTiXmlText);
			pTiXmlText->SetCDATA(true);
			pTiXmlText->SetValue(pAlias);
			(void)pXmlDoc->SaveFile(strPath.c_str());
			IVS_DELETE(pXmlDoc);
			return 0 ;
		}
	...
	try{
		...
		TiXmlNode *pXmlNode = pConnElem->LinkEndChild(new TiXmlText(pAlias));
		ASSERT(NULL != pXmlNode);
		TiXmlText *pXmlText = pXmlNode->ToText();
		ASSERT(NULL != pXmlNode);
		pXmlText->SetCDATA(true);
	}
	catch(...)
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_AddRecordBackupPlan(LPCTSTR pBackupPlan)
{
	...
	IVS_INT32 iRet = OCXRecordMgr::AddRecordBackupPlan(m_iSDKSessionID, pBackupPlan);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModifyRecordBackupPlan(LPCTSTR pBackupPlan)
{
	...
	CHECK_POINTER(pBackupPlan,IVS_PARA_INVALID);
	IVS_INFO_TRACE("OCX Address: %x,pBackupPlan: %s", this, pBackupPlan);
	IVS_INT32 iRet = OCXRecordMgr::ModifyRecordBackupPlan(m_iSDKSessionID, pBackupPlan);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteRecordBackupPlan(LPCTSTR pBackupPlan)
{
	...
	CHECK_POINTER(pBackupPlan,IVS_PARA_INVALID);
	IVS_INFO_TRACE("OCX Address: %x,pBackupPlan: %s", this, pBackupPlan);
	IVS_INT32 iRet = OCXRecordMgr::DeleteRecordBackupPlan(m_iSDKSessionID, pBackupPlan);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelSnapshotPlan(LPCTSTR pCameraCode)
{
	...
	CHECK_POINTER(pCameraCode,IVS_PARA_INVALID);
	IVS_INFO_TRACE("OCX Address: %x,pCameraCode: %s", this, pCameraCode);
	IVS_INT32 iRet = OCXCapturePicture::DelSnapshotPlan(m_iSDKSessionID, pCameraCode);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartRealBroadcast(void)const
{
    ...
    // TODO: Add your dispatch handler code here
    IVS_INT32 iRet = IVS_SDK_StartRealBroadcast(m_iSDKSessionID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopRealBroadcast(void)const
{
    ...
    // TODO: Add your dispatch handler code here
    IVS_INT32 iRet = IVS_SDK_StopRealBroadcast(m_iSDKSessionID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartFileBroadcast(LPCTSTR pFileName, ULONG ulCycle)const
{
    ...
    IVS_INT32 iRet = IVS_SDK_StartFileBroadcast(m_iSDKSessionID, pFileName, ulCycle);//lint !e713
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopFileBroadcast(void)const
{
    ...
    // TODO: Add your dispatch handler code here
    IVS_INT32 iRet = IVS_SDK_StopFileBroadcast(m_iSDKSessionID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetCameraSwitchWnd(ULONG ulWndID,ULONG ulStyle)
{
	...
	//入参窗格ID为0表示没有指定窗格，默认使用当前活动窗格
	CVideoPane *pWnd = ((CVideoPaneMgr*)m_pVideoPaneMgr)->GetWndPointerByID(ulWndID);
	if (NULL == pWnd)
	{
	...
	{
		iRet = (LONG)((CVideoPaneMgr*)m_pVideoPaneMgr)->SetTurnCruiseStyle(ulWndID,ulStyle);
	}
	else
	{
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ShareCatalog(LPCTSTR pShareInfo)
{
	...
	//// TODO: Add your dispatch handler code here
	//IVS_INT32 iRet =  IVS_SDK_ShareCatalog(m_iSDKSessionID, pShareInfo);
	LONG iRet = IVS_FAIL;
...
LONG CIVS_OCXPlayerCtrl::SetNoMalWnd(CVideoPane * pWnd)
{
	...
	{
		(void)static_cast<LONG>(SetAlarmState(pWnd->GetVideoWnd(), FALSE));	
	}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_PlaybackFrameStepBackward(LONG lWndID)
{
	...
	IVS_INT32 iRet = IVS_SDK_PlayBackFrameStepBackward(m_iSDKSessionID, (IVS_ULONG)pVideoPane->GetPlayHandle());
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartSyncPlay(LPCTSTR pSyncPlayInfo)
{
	...
	for (; iter != end; iter++)
	{
		...
		for (; _iter != _end; _iter++)
		{
			...
			rts.Start = ti.beg;
			rts.End   = ti.end;
			eSDK_MEMSET(SyncMgr.m_stNVRCode, 0, IVS_NVR_CODE_LEN+1);
			eSDK_MEMCPY(SyncMgr.m_stNVRCode, IVS_NVR_CODE_LEN+1, PlayInfo.NVRCode, IVS_NVR_CODE_LEN);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_StartPlatformDownload(LPCTSTR pCameraCode, LPCTSTR pStartTime, LPCTSTR pEndTime, LPCTSTR pDownloadParamXml)
{
	...
	eSDK_MEMSET(&DLParam, 0, sizeof(DLParam));
	...
	// 清理内存，保证内存中不会有密码的存在
	eSDK_MEMSET((char*)pDownloadParamXml,0,strlen(pDownloadParamXml));

	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,iResultCode,"");
	return strResult.AllocSysString();
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopPlatformDownload(ULONG ulDownloadHandle)const
{
	...
	// TODO: Add your dispatch handler code here
	IVS_INT32 iRet = IVS_SDK_StopPlatformDownload(m_iSDKSessionID, ulDownloadHandle);
    ...
    ULONG ulDownloadHandle)
{
    ...
    // TODO: Add your dispatch handler code here
    //IVS_INT32 iRet = IVS_SDK_StopDisasterRecoveryDownload(m_iSDKSessionID, ulDownloadHandle);
	IVS_INT32 iRet = IVS_FAIL;
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopPUDownload(ULONG ulDownloadHandle)const
{
	...
	// TODO: Add your dispatch handler code here
	IVS_INT32 iRet = IVS_SDK_StopPUDownload(m_iSDKSessionID, ulDownloadHandle);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DownloadPause(ULONG ulDownloadHandle)const
{
	...
	// TODO: Add your dispatch handler code here
	IVS_INT32 iRet = IVS_SDK_DownloadPause(m_iSDKSessionID, ulDownloadHandle);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DownloadResume(ULONG ulDownloadHandle)const
{
	...
	// TODO: Add your dispatch handler code here
	IVS_INT32 iRet = IVS_SDK_DownloadResume(m_iSDKSessionID, ulDownloadHandle);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetDownloadInfo(ULONG ulDownloadHandle)const
{
	...
	eSDK_MEMSET(&DownloadInfo, 0, sizeof(DownloadInfo));
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,ResultCode,"");
	return strResult.AllocSysString();
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_AddRecordBackupTask(LPCTSTR pBackupTask)
{
	...
	IVS_INT32 iRet = OCXRecordMgr::AddRecordBackupTask(m_iSDKSessionID, pBackupTask);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_AddCameraSwitchPlan(LPCTSTR pSwitchPlanInfo)
{
	...
	(void)CFavorite::GetCameraSwitchID(pSwitchPlanInfo, pCameraSwitchID);
	...
	IVS_INT32 iRet = CFavorite::AddCameraSwitchPlan(pSwitchPlanInfo);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelCameraSwitchPlan(LPCTSTR pCameraSwitchID)
{
	...
	CHECK_POINTER(pCameraSwitchID,IVS_PARA_INVALID);
	IVS_INFO_TRACE("OCX Address: %x,pCameraSwitchID: %s", this, pCameraSwitchID);
	IVS_INT32 iRet = CFavorite::DelCameraSwitchPlan(pCameraSwitchID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_LockRecord(LPCTSTR pLockXml)
{
	...
	//eSDK_MEMSET(&pRecordLockInfo,0,sizeof(IVS_RECORD_LOCK_INFO));
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetMotionRangeDataPlatform(LPCTSTR pCameraCode,LPCTSTR pStartTime,LPCTSTR pEndTime,LONG lFromIndex,LONG lToIndex)
{
	...
	//	return strResult.AllocSysString();
	//}
	//eSDK_MEMSET(pMotionRangeDataBuf, 0, uiBufSize);
	//IVS_MOTION_RANGE_DATA_LIST* pMotionRangeData = reinterpret_cast<IVS_MOTION_RANGE_DATA_LIST*>(pMotionRangeDataBuf); //lint !e826
	...
	//// 校验已申请内存大小
	//if(uiTotal + 1 < pMotionRangeData->uiTotal)
	//{
	//	pMotionRangeData->uiTotal = uiTotal + 1;
	//}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_OMUModAlarmLinkageRule(LPCTSTR pReqXml)
{
	...
	IVS_INT32 iRet = COCXClientNetworkMgr::ModAlarmLinkageRule(m_iSDKSessionID, pReqXml);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_OMUDelAlarmLinkageRule(LPCTSTR pReqXml)
{
	...
	IVS_INT32 iRet = COCXClientNetworkMgr::DelAlarmLinkageRule(m_iSDKSessionID, pReqXml);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_WatermarkAlarmConfirm(LPCTSTR pConfirmInfo)
{
	...
	//eSDK_MEMSET(&stConfirmInfo,0,sizeof(IVS_WATERMARK_ALARM_CONFIRM));
	...
	//iRet = IVS_SDK_WatermarkAlarmConfirm(m_iSDKSessionID,&stConfirmInfo);
	IVS_INT32 iRet = IVS_FAIL;
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_WatermarkAlarmCancel(LPCTSTR pCancelInfo)
{
	...
	//eSDK_MEMSET(&stCancelInfo,0,sizeof(IVS_WATERMARK_ALARM_CANCEL));
	//IVS_INT32 iRet = COCXAlarmMgrXMLProcess::GetWatermarkAlarmCancelInfo(pCancelInfo,&stCancelInfo);
	//if (IVS_SUCCEED != iRet)
	//{
	//	return iRet;
	//}
	//iRet = IVS_SDK_WatermarkAlarmCancel(m_iSDKSessionID,&stCancelInfo);
	IVS_INT32 iRet = IVS_FAIL;
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_QueryWatermarkAlarmList(LPCTSTR pQueryXml)
{
	...
	//	return strResult.AllocSysString();
	//}
	//eSDK_MEMSET(pWatermarkAlarmListBuf, 0, uiBufSize);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartLocalPlayBack(LONG lWndID, LPCTSTR pFileName, LPCTSTR pPlaybackParam)
{
	...
	eSDK_MEMSET(&Param, 0, sizeof(Param));
	...
	// 清理内存，保证内存中不会有密码的存在
	eSDK_MEMSET((char*)pPlaybackParam,0,strlen(pPlaybackParam));
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopBackupDownload(ULONG ulDownloadHandle)const
{
	...
	//IVS_INT32 iRet = IVS_SDK_StopBackupDownload(m_iSDKSessionID, ulDownloadHandle);
	LONG iRet = IVS_FAIL;
...
BSTR CIVS_OCXPlayerCtrl:: IVS_OCX_AddRealPlayBookmark(LPCTSTR pCameraCode,LPCTSTR pBookmarkName,LPCTSTR pBookmarkTime)
{
	...
	IVS_INT32 iRet = IVS_SDK_AddRealPlayBookmark(m_iSDKSessionID,pCameraCode,pBookmarkName,pBookmarkTime,&uiBookmarkID);
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet,"IVS OCX Add Bookmark List Failed", "Reason:IVS SDK Add Bookmark Failed.");
		COCXXmlProcess::GetErrString(strResult,iRet);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModifyRealPlayBookmark(LPCTSTR pCameraCode,ULONG ulBookmarkID,LPCTSTR pNewBookmarkName)
{
	...
	IVS_INT32 iRet = IVS_SDK_ModifyRealPlayBookmark(m_iSDKSessionID,pCameraCode,ulBookmarkID,pNewBookmarkName);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DeleteRealPlayBookmark(LPCTSTR pCameraCode,ULONG ulBookmarkID)
{
	...
	IVS_INT32 iRet = IVS_SDK_DeleteRealPlayBookmark(m_iSDKSessionID,pCameraCode,ulBookmarkID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartSyncPlayTVWall(ULONG ulTVWallID, LPCTSTR pSyncPlayInfo)
{
	...
	//任务准备好后，开启任务
	IVS_INT32 iRet = TVWallSyncMgr.StartTVwallSyncPlayBack();
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StopSyncPlayTVWall(ULONG ulTVWallID)
{
	...
	IVS_INFO_TRACE("OCX Address: %x,TVWallID: %lu.", this,ulTVWallID);
	CTVWallSyncPlayBackMgr &TVWallSyncMgr = CTVWallSyncPlayBackMgr::Instance();
	IVS_INT32 iRet = TVWallSyncMgr.StopTVwallSyncPlayBack();
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModNetkeyboardCamera(LPCTSTR pNetboardCamera)
{
    ...
    {
        BP_RUN_LOG_INF("NetKeyBoard Info isn't exist,add to xml file", "NA");
        (void)xml.AddElem("NetboardCameraInfo");
        (void)xml.IntoElem();
        (void)xml.AddChildElem("CameraCode");
        (void)xml.IntoElem();
        (void)xml.SetElemValue(strCameraCode.c_str());
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_AddCameraSwitchPlanTVWall(LPCTSTR pSwitchPlanInfo)
{
	...
	IVS_INT32 iRet = CFavorite::AddCameraSwitchPlanTVWall(pSwitchPlanInfo);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DelCameraSwitchPlanTVWall(ULONG ulTVWallID,LPCTSTR pCameraSwitchID)
{
	...
	IVS_INT32 iRet = CFavorite::DelCameraSwitchPlanTVWall(ulTVWallID,pCameraSwitchID);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetRecordThumbnail(LPCTSTR pThumbnailQuery)const
{
	...
	CXml        ResultXml;
	SNAP_VEC    SnapVec;
	eSDK_MEMSET(&SnapVec,0,sizeof(SNAP_VEC));
	...
	eSDK_MEMSET((void*)&param, 0, sizeof(IVS_PLAYBACK_PARAM));
	param.fSpeed = 2;
	param.uiProtocolType  = PROTOCOL_RTP_OVER_TCP;
	eSDK_MEMCPY(&param.stTimeSpan, sizeof(param.stTimeSpan), &SnapInfo.RecTimeSpan, sizeof(SnapInfo.RecTimeSpan));
	eSDK_MEMCPY(param.cNVRCode, sizeof(param.cNVRCode), SnapInfo.NvrCode, sizeof(SnapInfo.NvrCode));
	...
	for (unsigned int i = 0; i < SnapInfo.SnapPoint.size(); i++)
	{
		...
		eSDK_SPRINTF(FileName, sizeof(FileName), 
		...
		eSDK_MEMSET(&Snap, 0, sizeof(Snap));

	
		eSDK_STRNCPY(Snap.FileName, sizeof(Snap.FileName), FileName, sizeof(Snap.FileName) - 1);
...
ULONG CIVS_OCXPlayerCtrl::IVS_OCX_GetAbilityInfo(void)const
{
	...
	IVS_UINT32 uiAbility = COCXAlarmMgr::GetAbilityInfo(m_iSDKSessionID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetRealTimeTempScenario(LPCTSTR pRealTimeScenarioXml, LPCTSTR pScenarioID)
{
	...
	IVS_INT32 iRet = CFavorite::AddScenarioFavorite(pRealTimeScenarioXml, strTempScenarioInfoID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetRecordTempScenario(LPCTSTR pRecordScenarioXml, LPCTSTR pScenarioID)
{
	...
	std::string strTempScenarioInfoID = pScenarioID;
	IVS_INT32 iRet = CFavorite::AddScenarioFavorite(pRecordScenarioXml, strTempScenarioInfoID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetEmapTempScenario(LPCTSTR pEmapScenarionXml, LPCTSTR pScenarioID)
{
	...
	std::string strTempScenarioInfoID = pScenarioID;
	IVS_INT32 iRet = CFavorite::AddScenarioFavorite(pEmapScenarionXml, strTempScenarioInfoID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_ModCameraSwitch(LPCTSTR pSwitchInfo)
{
    ...
    IVS_INFO_TRACE("OCX Address: %x", this);
    IVS_INT32 iRet = CFavorite::ModCameraSwitch(pSwitchInfo);
...
void CIVS_OCXPlayerCtrl::StopPlayResource(CVideoPane* pVideoPane)const
{
	...
	{
		// 发送一个点击事件给放大按钮，让他关闭自己，这里使用同步的sendmessage,不要使用postmessage
		::SendMessage(pVideoPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_TALKBACK, BN_CLICKED), (LPARAM)pVideoPane->GetVideoRealtimeBar()->GetTalkbackBtnHwnd());//lint !e835 !e845
// 		pVideoPane->GetVideoRealtimeBar()->StopTalkbackSound();
// 		pVideoPane->GetVideoRealtimeBar()->DeleteTalkbackCtrl();
	}
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetDeviceEncodeCapabilities(LPCTSTR pDevCode)const
{
	...
	//eSDK_MEMSET(pStreamInfoListBuf, 0, uiBufSize);
...
{  
	...
	HRESULT retval = pThis->ExternalQueryInterface(&riid,   
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_RefreshCameraList()const
{
    ...
    IVS_INT32 iRet = COCXDeviceMgr::RefreshCameraList(m_iSDKSessionID);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetCUVersion(LPCTSTR pReqXml)
{
	...
	IVS_INT32 iRet = COCXUpgradeMgrMgr::GetCUVersion(m_iSDKSessionID, reqXml, strResult);
	if (IVS_SUCCEED != iRet)
	{
...
ULONG CIVS_OCXPlayerCtrl::StopRealPlayByPaneEx(CVideoPane * pVideoPane)
{
    ...
    if (SERVICE_TYPE_REALVIDEO == enServiceType)
    {
		...
		(void)pVideoPane->StopRealStreamAsync();
    }
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_StartRealPlayTVWall_Async(ULONG ulTVWallID, LPCTSTR pCameraCode, LPCTSTR pRealplayParam, ULONG ulWndID, ULONG ulTransID)
{
    ...
    LONG lRet = DealStartRealPlayTVWall(ulTVWallID, pCameraCode, pRealplayParam, ulWndID, FALSE, ulTransID);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_LoginByTicket(LPCTSTR pTicketID, LPCTSTR pServerIP, LPCTSTR pPort, ULONG ulClientType)
{
	...
	eSDK_MEMSET(&sLoginInfoEx,0,sizeof(IVS_LOGIN_INFO_EX));
	...
	if (IVS_SUCCEED == iRet||IVS_SMU_USER_FIRST_LOGIN == iRet || IVS_SMU_USER_PWD_NEED_MOD == iRet)
	{
		...
		eSDK_MEMSET(&stUserInfo, 0, sizeof(IVS_USER_INFO));
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetStreamListByCam(LPCTSTR pDomainCode, LPCTSTR pCameraCode)
{
	...
	eSDK_MEMSET(pStreamListByCamBuf, 0, uiBufSize);
	IVS_STREAM_LIST_BY_CAM *pStreamListByCam = (IVS_STREAM_LIST_BY_CAM *)pStreamListByCamBuf;	//lint !e826
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetStreamListByUser(LPCTSTR pDomainCode, ULONG ulUserID)
{
	...
	eSDK_MEMSET(pStreamListByUserBuf, 0, uiBufSize);
	IVS_STREAM_LIST_BY_USER *pStreamListByUser = (IVS_STREAM_LIST_BY_USER *)pStreamListByUserBuf;	//lint !e826
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetMicVolume(ULONG ulVolumeValue)
{
	...
	SYSTEMTIME rspTime; GetLocalTime(&rspTime);
	INT_LOG_NO_SESSION(reqTime,rspTime,IVS_SUCCEED,"");
	return static_cast<IVS_LONG>(m_pVideoPaneMgr->SetMicVolume(ulVolumeValue));
}
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_LocalSnapshotToDir(LONG lWndID, ULONG ulPictureFormat, LPCTSTR pFileName)
{
	...
	IVS_INT32 iRet = pVideoPane->LocalSnapshotToDir(m_iSDKSessionID, ulPictureFormat, pFileName);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_SetVideoEffect(LONG lWndID, LONG lBrightValue, LONG lContrastValue, LONG lSaturationValue, LONG lHueValue)
{
	...
	IVS_INT32 iRet = pVideoPane->SetVideoEffect(m_iSDKSessionID, lBrightValue, lContrastValue, lSaturationValue, lHueValue);
...
BSTR CIVS_OCXPlayerCtrl::IVS_OCX_GetDomainDeviceList(LPCTSTR pDomainCode, ULONG ulDeviceType, ULONG ulFromIndex, ULONG ulToIndex)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	SYSTEMTIME reqTime; GetLocalTime(&reqTime);	
	...
	if(NULL == pDomainCode)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "IVS OCX Get Domain DeviceList", "DomainCode is NULL.");
		return NULL;
	}
	IVS_INFO_TRACE("OCX Address: %x, DomainCode: %s, DeviceType: %lu,ulFromIndex: %lu,ulToIndex: %lu", 
		this, pDomainCode, ulDeviceType, ulFromIndex, ulToIndex);

	CString strResult;
	CXml xmlRsp;
	IVS_INT32 iRet = COCXDeviceMgr::GetDomainDeviceList(m_iSDKSessionID, pDomainCode, ulDeviceType, ulFromIndex,ulToIndex, xmlRsp);
	if (IVS_SUCCEED == iRet)
	{
		IVS_UINT32 uiLen = 0;
		strResult = xmlRsp.GetXMLStream(uiLen);
	...
	{
		BP_RUN_LOG_ERR(iRet,"IVS OCX Get Domain DeviceList","Get Domain DeviceList Failed.");
		COCXXmlProcess::GetErrString(strResult, iRet);
	}
...
afx_msg LRESULT CIVS_OCXPlayerCtrl::OnShutdownStreamNotify(WPARAM wParam, LPARAM lParam)
{
	IVS_INFO_TRACE("OCX Address: %x", this);

	if (IVS_EVENT_SHUTDOWN_STREAM != wParam)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "IVS OCX Notify Shutdown Stream", "Reason: wParam is error.");
		return IVS_PARA_INVALID;
	}
	if (NULL == lParam)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "IVS OCX Notify Shutdown Stream", "Reason: lParam in NULL.");
		return IVS_PARA_INVALID;
	}

	IVS_SHUTDOWN_NOTIFY_INFO *pShutdownNotifyInfo = (IVS_SHUTDOWN_NOTIFY_INFO*)(lParam);

	CXml xml;
	(void)COCXUserInfoMgrXMLProcess::ShutdownStreamGetXml(pShutdownNotifyInfo, xml);
	unsigned int uiLent = 0;
	const char* pszResult = xml.GetXMLStream(uiLent);
	IVS_OCX_Event((long)wParam, pszResult);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_EnableDrawCallBackEx(ULONG ulWindowID)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	IVS_INFO_TRACE("OCX Address: %x, WindowID: %d", this, ulWindowID);

	CVideoPane *pVideoPane = GetVideoPanel((ULONG)ulWindowID);
	if (NULL == pVideoPane)
	{
		BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "IVS OCX enable draw callback failed.", "Reason: pVideoPane is NULL.");
		return IVS_OPERATE_MEMORY_ERROR;
	}

	IVS_INT32 iRet = pVideoPane->IsEnableDrawCallBackEx(TRUE);
...
LONG CIVS_OCXPlayerCtrl::IVS_OCX_DisableDrawCallBackEx(ULONG ulWindowID)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	if (NULL == pVideoPane)
	{
		BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "IVS OCX disable draw callback failed.", "Reason: pVideoPane is NULL.");
		return IVS_OPERATE_MEMORY_ERROR;
	}
	IVS_INT32 iRet = pVideoPane->IsEnableDrawCallBackEx(FALSE);
...
afx_msg LRESULT CIVS_OCXPlayerCtrl::OnIADrawCallBackEx(WPARAM wParam, LPARAM lParam)
{
	//IVS_INFO_TRACE("OCX Address: %x", this);
	if (IVS_EVENT_IA_PUSH_DRAW_CALLBACK != wParam)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "IVS OCX Notify draw callback", "Reason: event type is error.");
		return IVS_PARA_INVALID;
	}
	if (NULL == lParam)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "IVS OCX Notify draw callback", "Reason: lParam in NULL.");
		return IVS_PARA_INVALID;
	}

	IA_CallBackData *pIACallBackData = reinterpret_cast<IA_CallBackData *>(lParam);

	IVS_OCX_DrawCallBackEx_Event(pIACallBackData->ulWindowID, (LONG)(pIACallBackData->hDc), pIACallBackData->rect.left, pIACallBackData->rect.top, 
		pIACallBackData->rect.right, pIACallBackData->rect.bottom, pIACallBackData->ulFrameID);
	return IVS_SUCCEED;
}
