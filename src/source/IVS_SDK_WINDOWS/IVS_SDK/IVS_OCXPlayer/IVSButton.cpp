...
void CIVSButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if(m_bIsBordLine) { 
		...
		//add by zwx211831, Resource leak
		CBitmap* pOld = memDC.SelectObject(&btp);
		if(NULL == pOld)
		{
			::DeleteObject(hBitMap);
			BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "Draw Line Header Or Trail failed", "SelectObject return NULL");
			return;
		}
	...
	if (isRequireProtractCharacter(strText))
	{
		...
		COLORREF crBak = pDC->SetTextColor(RGB(0,0,0));	//lint !e835 !e845
...
COLORREF CIVSButton::SetTextColor( CDC &MemDC )
{
	...
	if (isHover())
	{
		...
		crBak = MemDC.SetTextColor(RGB(0,0,0)); //lint !e835 !e845
	}	
