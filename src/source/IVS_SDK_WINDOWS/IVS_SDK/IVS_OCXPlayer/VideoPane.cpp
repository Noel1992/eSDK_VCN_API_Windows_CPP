...
{
    eSDK_MEMSET(m_szCameraID,0x0, IVS_DEV_CODE_LEN + 1);
    eSDK_MEMSET(m_szToolbarStatus,0x0,sizeof(m_szToolbarStatus));
	eSDK_MEMSET(&m_CurrentGraphic,0x0,sizeof(IA_GRAPHICS));
	eSDK_MEMSET(m_CamReal, 0, sizeof(m_CamReal));
	eSDK_MEMSET(&m_LocalCaptureParam, 0, sizeof(m_LocalCaptureParam));
	eSDK_MEMSET(m_MagnifyNameCount,0,sizeof(m_MagnifyNameCount));
	eSDK_MEMSET(m_szGraphics,0,MAGNIFY_MAX_NUM* sizeof(IA_GRAPHICS));
	m_IAColor.Alpha = 255;
	m_IAColor.Blue = 246;
	m_IAColor.Red = 41;
	m_IAColor.Green = 154 ;
	...
	eSDK_MEMSET(&m_IArectPercent,0,sizeof(IVS_RECT));
	eSDK_MEMSET(&m_MagnifyLastPoint,0,sizeof(POINT));
	eSDK_MEMSET(&m_szOpendMagnifySize,0,sizeof(SIZE));
}
...
}   //lint !e1740 !e1579 m_pVideoPaneMgr m_pOcxCtrl 只是指针的引用
...
BOOL CVideoPane::OnEraseBkgnd(CDC* pDC)
{
    ...
    IVS_INT32 nWidth  = _Rect.Width();
    IVS_INT32 nHeight = _Rect.Height();
	...
	if (m_bIsTvWall && !m_bIsResume)
	{
		...
		{
			pDC->SetTextColor(RGB(0, 0, 0));	//lint !e835 !e845
...
{
	IVS_INFO_TRACE("On left button double clink");
    if ((CURSOR_STATE_PTZ == m_ulCursorState)
        && (FALSE == IsInCenter(point)) || (MAGNIFY_OPEN == m_ulIsRealtimeMagnifying))
    {
		BP_RUN_LOG_INF("On left button double clink", "CursorState = %d, IsInCenter = %d. IsRealtimeMagnifying = %d", m_ulCursorState, IsInCenter(point), m_ulIsRealtimeMagnifying);
        return;
    }
	...
	{
		BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "GetVideoPaneMgr failed", "NA");
	}
}//lint !e715
...
void CVideoPane::GetDisplayScaleRect( CRect &mainRect )
{
	...
	{
		BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "Get Display Scale Rect failed", "m_pVideoPaneMgr is NULL");
		return;
	}
...
void CVideoPane::PTZ3DMouseDown( const CPoint &point )
{
    ...
    eSDK_MEMSET(&ptz_3dcolor,0x0,sizeof(IA_COLOR));
    ...
    eSDK_MEMSET(&ptz_3dgraphics,0x0,sizeof(IA_GRAPHICS));
...
IVS_INT32 CVideoPane::GetIARectByGroup(ULONG uiGraphicGroup, CRect& rect)
{
	...
	IVS_INT32 iRet = Draw_GetGraphicsByGroup((DRAW_HANDLE)m_ulDrawHandle, uiGraphicGroup, &graphicsTemp, &ulNum);
	if (IVS_SUCCEED != iRet)
	{
...
BOOL CVideoPane::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
    ...
    if(BOTTOMBAR_MAGNIFY_OPEN == m_ulIsDstMagnify)
    {
		if (m_pZoomBar!=NULL)
		{
			...
			{
				// 直接发送一个按钮消息给放大窗口
				::PostMessage(m_pZoomBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_SHRINK, BN_CLICKED), (LPARAM)m_pZoomBar->GetZoomInHwnd());//lint !e835 !e845
			} else {        // 往远滚，放大
				::PostMessage(m_pZoomBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_MAGNIFY, BN_CLICKED), (LPARAM)m_pZoomBar->GetZoomOutHwnd());//lint !e835 !e845
			}
		}
		return TRUE;
    }
	...
	IVS_LONG ulRet = PostEventXMLImpl.GetPTZConfig(strXML,&LResult,&lSpeed);
...
void CVideoPane::Set3DPTZState( ULONG ulIsStart3D )
{
    ...
    if (ulIsStart3D == PTZ_3D_OPEN)
    {
        ...
        if (MAGNIFY_OPEN == m_ulIsRealtimeMagnifying)
        {
			...
			::SendMessage(m_pRealTimeToolBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_MAGNIFY, BN_CLICKED), (LPARAM)m_pRealTimeToolBar->GetMagnifyBtnHwnd());//lint !e835 !e845
        }
...
void CVideoPane::SetActiveBorder()
{
    ...
    if (pVideoPaneMgr->m_VideoPaneBKGColor ==  RGB(0,0,0))	//lint !e835
    {
...
void CVideoPane::ResetPane()
{
    ...
    eSDK_MEMSET(m_szCameraID, 0, IVS_DEV_CODE_LEN + 1 );
	//隐藏透明Bar
	HideTranslucenWnd();
}
...
CIVS_OCXPlayerCtrl *CVideoPane::GetOCXPlayerCtrl()
{
    ...
    {
		CWnd *pWnd = GetParent();
		CHECK_POINTER(pWnd, NULL);
        m_pOcxCtrl = dynamic_cast<COleControl *>(pWnd->GetParent());
    }
	CHECK_POINTER(m_pOcxCtrl, NULL);
    CIVS_OCXPlayerCtrl *pOcxCtrl = dynamic_cast<CIVS_OCXPlayerCtrl *>(m_pOcxCtrl);
    CHECK_POINTER(pOcxCtrl, NULL);
...
void CVideoPane::CreateRealTimeBar()
{
	...
	if (GetVideoTitleBar() == nullptr)
	{
		...
		{
			// 创建标题栏
			Gdiplus::Image* pImage = SkinMgr.GetImageTitleBarBKG();
			if (NULL == pImage)
			{
				return;
			}
	...
	if (GetVideoRealtimeBar() == nullptr)
	{
		...
		{
			// 创建实况工具栏
			Gdiplus::Image * pToolImage = SkinMgr.GetImageBarBKG();
			if (NULL == pToolImage)
			{
				return;
			}
			...
			if (NULL == m_pVideoPaneMgr)
			{
				BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "Create RealTime Bar failed", "m_pVideoPaneMgr is NULL");
				continue;
			}
...
void CVideoPane::ShowInstantReplayBar(ULONG val)
{
	...
	eSDK_MEMSET(pRecordList,0,uiBufSize);
	...
	eSDK_MEMSET(&stFirstStartTime,0,sizeof(ST_TIME));
	...
	if (IVS_SUCCEED == iRet)
	{
		...
		for (int iIndex=0;iIndex<iTotalNum && iIndex < (int)stIndex.uiToIndex;iIndex++)
		{
			...
			eSDK_MEMSET(&stGreen,0,sizeof(ST_TIME));
	...
	else
	{
		...
		CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
		if (NULL == pOCXPlayerCtrl)
		{
			BP_RUN_LOG_ERR(IVS_OPERATE_MEMORY_ERROR, "Show Instant Replay Bar failed", "Get OCXPlayer Ctrl failed");
			return;
		}
		IVS_SDK_DelPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), GetPlayHandle(), GetVideoWnd());
	}
}
...
BOOL CVideoPane::TransStrToTime(const CString &str,ST_TIME* pTime)const
{
	...
	currenttime += (localttime - systemttime);//lint !e647
	CTime newCurrentTime(currenttime);
...
void CVideoPane::PTZ3DMouseUp(const CRect &mainRect)
{
	...
	IVS_UINT32 uiLockStatus = 0;
	IVS_INT32 iRet = IVS_SDK_PtzControl(p_OcxCtrl->GetSDKSessionID(),szCameraCode, PTZ_FAST_LOCATE, csPara1.GetBuffer(0), csPara2.GetBuffer(0), &uiLockStatus);
...
static IVS_VOID __SDK_CALL DrawCallBackEx(IVS_VOID *hDc, IVS_VOID *rc, ULONG64 ulFrameID, IVS_VOID *pUser)
{
	if(NULL == hDc || NULL == rc || NULL == pUser)
	{
		return;
	}
	CVideoPane *pVideoPane = reinterpret_cast<CVideoPane *>(pUser);

	IA_CallBackData *pCallBackData = IVS_NEW(pCallBackData);
	if (NULL == pCallBackData)
	{
		return;
	}
	pCallBackData->ulWindowID = pVideoPane->GetWndID();
	pCallBackData->hDc = reinterpret_cast<HDC>(hDc);
	pCallBackData->rect.bottom = reinterpret_cast<RECT *>(rc)->bottom;
	pCallBackData->rect.left = reinterpret_cast<RECT *>(rc)->left;
	pCallBackData->rect.right = reinterpret_cast<RECT *>(rc)->right;
	pCallBackData->rect.top = reinterpret_cast<RECT *>(rc)->top;
	pCallBackData->ulFrameID = ulFrameID;

	//PostMessage(CIVS_OCXPlayerCtrl::m_pOCX->GetSafeHwnd(), WM_IA_DRAW_CALLBACK_EX, (WPARAM)IVS_EVENT_IA_PUSH_DRAW_CALLBACK, (LPARAM)pCallBackData);
	SendMessage(CIVS_OCXPlayerCtrl::m_pOCX->GetSafeHwnd(), WM_IA_DRAW_CALLBACK_EX, (WPARAM)IVS_EVENT_IA_PUSH_DRAW_CALLBACK, (LPARAM)pCallBackData);
}//lint !e818
...
IVS_INT32 CVideoPane::StopDrawGraphics(ULONG ulGraphicGroup)
{
	...
	m_ulGroupDrawMaxNum	   = 0;
	eSDK_MEMSET(&m_CurrentGraphic, 0, sizeof(IA_GRAPHICS));
	m_pAnalysisToolBar->SetToolbarEnableStatus((ULONGLONG)(0x01 | 0x02), (ULONGLONG)FALSE);
...
// 		m_CurrentGraphic.ulGraphicGroup = m_ulCurrentGraphicGroup;
		...
		case IA_GRAPHICS_TYPE_POLYGON:
			if (!m_bIsDrawing)// 还没有开始画
			{
				...
				(void)Draw_PolygonSetPoint((DRAW_HANDLE)m_ulDrawHandle, CurrentPoint, m_CurrentGraphic.ulGraphicGroup);
// 			    CHECK_DRAW_START_SUCCESS(lRet);
            }
			break;
...
IVS_INT32 CVideoPane::GetTime(IVS_INT32 iSessionID, IVS_ULONG ulPlayHandle, IVS_LOCAL_CAPTURE_PARAM &LocalCaptureParam)const
{
	...
	_tzset();
	struct tm tm;
	eSDK_MEMSET(&tm, 0, sizeof(tm));
	TM stTM; // 临时时间字符串数组
	eSDK_MEMSET(&stTM, 0, sizeof(TM));
	time_t t_Time = 0;
	...
	case VIDEO_PLAY_STATE_REALTIME:
		time_t tmBeg;
		eSDK_MEMSET(&tmBeg, 0, sizeof(tmBeg));
		time( &tmBeg );
...
// 		gmtime_s(&tm, &tmBeg);     // 转换为绝对时间结构体
	...
	case VIDEO_PLAY_STATE_VOD:
		...
		eSDK_MEMCPY(stTM.Year, sizeof(stTM.Year), &stPlaybackTime.cPlayBackTime[0], 4);
		tm.tm_year = atoi(stTM.Year) - 1900;
		eSDK_MEMCPY(stTM.Month, sizeof(stTM.Month), &stPlaybackTime.cPlayBackTime[4], 2);
		tm.tm_mon = atoi(stTM.Month) - 1;
		eSDK_MEMCPY(stTM.Day, sizeof(stTM.Day), &stPlaybackTime.cPlayBackTime[6], 2);
		tm.tm_mday = atoi(stTM.Day);
		eSDK_MEMCPY(stTM.Hour, sizeof(stTM.Hour), &stPlaybackTime.cPlayBackTime[8], 2);
		tm.tm_hour = atoi(stTM.Hour) - tz;
		eSDK_MEMCPY(stTM.Min, sizeof(stTM.Min), &stPlaybackTime.cPlayBackTime[10], 2);
		tm.tm_min = atoi(stTM.Min);
		eSDK_MEMCPY(stTM.Sec, sizeof(stTM.Sec), &stPlaybackTime.cPlayBackTime[12], 2);
...
IVS_INT32 CVideoPane::ReachTimeInterval(IVS_INT32 iSessionID, IVS_ULONG ulPlayHandle, const IVS_LOCAL_CAPTURE_PARAM &LocalCaptureParam, BOOL &bReachTimeInterval)
{
	...
	eSDK_MEMSET(&tmCurrent, 0, sizeof(tmCurrent));
	time_t tmLast;    // 上次抓拍时间
	eSDK_MEMSET(&tmLast, 0, sizeof(tmLast));

	tm stCurrentTime; // 当前时间int结构体
	eSDK_MEMSET(&stCurrentTime, 0, sizeof(tm));
	tm stLastTime;    // 上次抓拍时间int结构体
	eSDK_MEMSET(&stLastTime, 0, sizeof(tm));

	TM stTempTimeArr; // 临时时间字符串数组
	eSDK_MEMSET(&stTempTimeArr, 0, sizeof(TM));
	...
	case VIDEO_PLAY_STATE_VOD:
		...
		eSDK_MEMCPY(stTempTimeArr.Year, sizeof(stTempTimeArr.Year), &stPlaybackTime.cPlayBackTime[0], 4);
		stCurrentTime.tm_year = atoi(stTempTimeArr.Year) - 1900;
		eSDK_MEMCPY(stTempTimeArr.Month, sizeof(stTempTimeArr.Month), &stPlaybackTime.cPlayBackTime[4], 2);
		stCurrentTime.tm_mon = atoi(stTempTimeArr.Month) - 1;
		eSDK_MEMCPY(stTempTimeArr.Day, sizeof(stTempTimeArr.Day), &stPlaybackTime.cPlayBackTime[6], 2);
		stCurrentTime.tm_mday = atoi(stTempTimeArr.Day);
		eSDK_MEMCPY(stTempTimeArr.Hour, sizeof(stTempTimeArr.Hour), &stPlaybackTime.cPlayBackTime[8], 2);
		stCurrentTime.tm_hour = atoi(stTempTimeArr.Hour) - tz;
		eSDK_MEMCPY(stTempTimeArr.Min, sizeof(stTempTimeArr.Min), &stPlaybackTime.cPlayBackTime[10], 2);
		stCurrentTime.tm_min = atoi(stTempTimeArr.Min);
		eSDK_MEMCPY(stTempTimeArr.Sec, sizeof(stTempTimeArr.Sec), &stPlaybackTime.cPlayBackTime[12], 2);
	...
	// 获取上次抓拍时间time_t
	eSDK_MEMSET(&stTempTimeArr, 0, sizeof(TM));
	eSDK_MEMCPY(stTempTimeArr.Year, sizeof(stTempTimeArr.Year), &LocalCaptureParam.cCurrentTime[0], 4);
	stLastTime.tm_year = atoi(stTempTimeArr.Year) - 1900;
	eSDK_MEMCPY(stTempTimeArr.Month, sizeof(stTempTimeArr.Month), &LocalCaptureParam.cCurrentTime[5], 2);
	stLastTime.tm_mon = atoi(stTempTimeArr.Month) - 1;
	eSDK_MEMCPY(stTempTimeArr.Day, sizeof(stTempTimeArr.Day), &LocalCaptureParam.cCurrentTime[8], 2);
	stLastTime.tm_mday = atoi(stTempTimeArr.Day);
	eSDK_MEMCPY(stTempTimeArr.Hour, sizeof(stTempTimeArr.Hour), &LocalCaptureParam.cCurrentTime[11], 2);
	stLastTime.tm_hour = atoi(stTempTimeArr.Hour);
	eSDK_MEMCPY(stTempTimeArr.Min, sizeof(stTempTimeArr.Min), &LocalCaptureParam.cCurrentTime[14], 2);
	stLastTime.tm_min = atoi(stTempTimeArr.Min);
	eSDK_MEMCPY(stTempTimeArr.Sec, sizeof(stTempTimeArr.Sec), &LocalCaptureParam.cCurrentTime[17], 2);
	stLastTime.tm_sec = atoi(stTempTimeArr.Sec);
	char cDST[4] = {0};
	eSDK_MEMCPY(cDST, sizeof(cDST), &LocalCaptureParam.cCurrentTime[20], 3);
	if (!strcmp("DST", cDST))// DST时间需要减去一个小时
	{
		stLastTime.tm_hour--;
	}
...
afx_msg LRESULT CVideoPane::OnSyncPlayBackMsg(WPARAM wParam,LPARAM lParam)
{
	...
	CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
	CHECK_POINTER(pOCXPlayerCtrl, IVS_OPERATE_MEMORY_ERROR);
	...
	if (bStarted)
	{
		...
		pOCXPlayerCtrl->ResumeMagnifyStatus(this);
	...
	{
		// 关闭其他资源
		pOCXPlayerCtrl->StopPlayResource(this);
...
void CVideoPane::UpdataMagnifyWindow(ULONG ulGraphicGroup)
{
	...
	CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
	CHECK_POINTER_VOID(pOCXPlayerCtrl);
	IVS_SDK_UpdateWindowPartial(pOCXPlayerCtrl->GetSDKSessionID(), m_ulPlayHandle, pMagnifyPane->GetVideoWnd(), &IArectPercent);
...
IVS_INT32 CVideoPane::GetMediaSize(SIZE& size)
{
	...
	CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
	CHECK_POINTER(pOCXPlayerCtrl, IVS_OPERATE_MEMORY_ERROR);
	IVS_INT32 iRet = IVS_SDK_GetMediaInfo(pOCXPlayerCtrl->GetSDKSessionID(), m_ulPlayHandle, &strMediaInfo);
	if (IVS_SUCCEED != iRet)
	{
...
IVS_INT32 CVideoPane::ChangeSrcGraphicRect(int iRate,ULONG ulGraphicGroup)
{
	// 按中心去扩展
	SIZE sz = {0};
	IVS_INT32 iRet = GetMediaSize(sz);
...
// 	pMagnifyPane->DeleteZoomTitleBar();
	...
	CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
	CHECK_POINTER(pOCXPlayerCtrl, IVS_OPERATE_MEMORY_ERROR);
	IVS_SDK_DelPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), pMagnifyPane->GetMagnifyPlayHandle(), pMagnifyPane->GetVideoWnd());
	if (BOTTOMBAR_ALARM_OPEN != pMagnifyPane->GetRealtimeAlarmState())
	{
		// 恢复被覆盖之前的窗口
		IVS_SDK_AddPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), pMagnifyPane->GetPlayHandle(), pMagnifyPane->GetVideoWnd());
	}
...
LRESULT CVideoPane::OnInstantReplayStart(WPARAM wParam, LPARAM pParam)
{
    ...
    if (m_bIsInstanstPlayBack)
    {
		CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
		CHECK_POINTER(pOCXPlayerCtrl, IVS_OPERATE_MEMORY_ERROR);
        IVS_UINT32 ulRet = (IVS_UINT32)IVS_SDK_StopPlatformPlayBack(pOCXPlayerCtrl->GetSDKSessionID(),m_ulInstanstPlayBackHandle);
        if (IVS_SUCCEED == ulRet)
        {
			CHECK_POINTER(m_pPlayTimerBar, IVS_OPERATE_MEMORY_ERROR);
...
LRESULT CVideoPane::OnInstantReplayMove(WPARAM wParam,LPARAM pParam)
{
    ...
    if (m_bIsInstanstPlayBack)
    {
        if (!m_bIsInstansPlayPause)
        {
			CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
			CHECK_POINTER(pOCXPlayerCtrl, IVS_OPERATE_MEMORY_ERROR);
            iRet = IVS_SDK_SetPlayBackTime(pOCXPlayerCtrl->GetSDKSessionID(),m_ulInstanstPlayBackHandle,ulTime);
            if (IVS_SUCCEED == iRet)
            {
				CHECK_POINTER(m_pPlayTimerBar, IVS_OPERATE_MEMORY_ERROR);
...
void CVideoPane::ResumeRealPlay()
{
	...
	CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
	CHECK_POINTER_VOID(pOCXPlayerCtrl);
	IVS_SDK_DelPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), m_ulInstanstPlayBackHandle, GetVideoWnd());
	if (m_bIsInstanstPlayBack)
	{
		IVS_UINT32 ulRet = (IVS_UINT32)IVS_SDK_StopPlatformPlayBack(pOCXPlayerCtrl->GetSDKSessionID(),m_ulInstanstPlayBackHandle);
		if (IVS_SUCCEED != ulRet)
	...
	IVS_SDK_AddPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), m_ulInstanstRealPlayHandle, GetVideoWnd());
	//隐藏即时回放面板
	if (m_pPlayTimerBar != nullptr)
	{
		(void)m_pPlayTimerBar->StopRecordPlay();
...
void CVideoPane::ReSetToolBarStatus()
{
	CVideoPaneMgr *pVideoPaneMgr = GetVideoPaneMgr();
	CHECK_POINTER_VOID(pVideoPaneMgr);
	SetToolBarBtnVisableStatus(static_cast<ULONGLONG>(pVideoPaneMgr->GetToolBarVisableStatus()));
}
...
LONG CVideoPane::StartRecordPlay()
{
	...
	if (!m_bIsInstansPlayPause)
	{
		...
		eSDK_MEMSET(&stTimerMsg,0,sizeof(ST_TIMER_MSG));
		if(!m_pPlayTimerBar->IsCanPlayBack(stTimerMsg))
		{
            ...
            {
			    IVS_SDK_DelPlayWnd(pOcx->GetSDKSessionID(), GetPlayHandle(), GetVideoWnd());
                return -1;
            }
		...
		ULONG ulHandle = 0;
		m_ulInstanstRealPlayHandle = GetPlayHandle();
		iRet = IVS_SDK_DelPlayWnd(pOcx->GetSDKSessionID(), GetPlayHandle(), GetVideoWnd());
		if (IVS_SUCCEED != iRet)
		{
			BP_RUN_LOG_ERR(iRet, "IVS SDK DelPlayWnd failed", "SessionID = %d, PlayHandle = %d", pOcx->GetSDKSessionID(), GetPlayHandle());
		}
		iRet = IVS_SDK_StartPlatformPlayBack(pOcx->GetSDKSessionID(), m_szCameraID, &stPlaybackParam, GetVideoWnd(), &ulHandle);
		if (IVS_SUCCEED == iRet)
		{
			(void)CGlobalPlayMgr::Instance().RegPlayBackHandle(ulHandle, this, m_szCameraID);
	...
	{
		iRet = IVS_SDK_PlayBackResume(pOcx->GetSDKSessionID(),m_ulInstanstPlayBackHandle);
		if (IVS_SUCCEED == iRet)
		{
			m_bIsInstansPlayPause = FALSE;
		}	
...
LONG CVideoPane::StopRecordPlay()
{
	if (m_bIsInstanstPlayBack)
	{
		...
		if (IVS_SUCCEED == ulRet)
		{
			CHECK_POINTER(m_pPlayTimerBar, IVS_OPERATE_MEMORY_ERROR);
			(void)m_pPlayTimerBar->StopRecordPlay();
			CHECK_POINTER(m_pRealTimeToolBar, IVS_OPERATE_MEMORY_ERROR);
			m_pRealTimeToolBar->SetRecordPlayStop(TRUE);
			m_bIsInstansPlayPause = FALSE;
			//ResetBarPosition();
			Invalidate(TRUE);
...
LONG CVideoPane::PauseRecordPlay()
{
	...
	if (!m_bIsInstansPlayPause)
	{
		ulRet = (IVS_UINT32)IVS_SDK_PlayBackPause(pOCXPlayerCtrl->GetSDKSessionID(),m_ulInstanstPlayBackHandle);
		if (IVS_SUCCEED == ulRet)
		{
			CHECK_POINTER(m_pPlayTimerBar, IVS_OPERATE_MEMORY_ERROR);
			(void)m_pPlayTimerBar->StopRecordPlay();
...
void CVideoPane::MagnifyMouseDown( const CPoint &point )
{
	...
	{ 
		...
		else
		{
			...
			eSDK_MEMSET(pMagnifyGraphics,0x0,sizeof(IA_GRAPHICS));
...
void CVideoPane::MagnifyMouseUp( const CPoint &point )
{
	...
	if (GRAPHICS_STATE_DRAWING == m_ulGraphicState)// 画图状态
	{
		...
		CTranVideoRealtimeBar *pTranVideoRealtimeBar = pMagnifyPane->GetVideoRealtimeBar();
		if (NULL != pTranVideoRealtimeBar)
		{
			if (pMagnifyPane->GetShowInstantStatus())
			{
				::SendMessage(pTranVideoRealtimeBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_RETURN, BN_CLICKED), (LPARAM)pMagnifyPane->GetVideoRealtimeBar()->GetReturnBtnHwnd());//lint !e613 !e835 !e845
			}
			if (BOTTOMBAR_PTZ_OPEN == pMagnifyPane->GetRealtimePtzState())
			{
				::SendMessage(pTranVideoRealtimeBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_PTZ, BN_CLICKED), (LPARAM)pMagnifyPane->GetVideoRealtimeBar()->GetPTZBtnHwnd());//lint !e613 !e835 !e845
			}
		}
		...
		if (BOTTOMBAR_ALARM_OPEN != pMagnifyPane->GetRealtimeAlarmState())
		{
			IVS_SDK_DelPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), pMagnifyPane->GetPlayHandle(), pMagnifyPane->GetVideoWnd());
		}
		...
		IVS_SDK_AddPlayWindowPartial(pOCXPlayerCtrl->GetSDKSessionID(), m_ulPlayHandle, pMagnifyPane->GetVideoWnd(), &IArectPercent);
		...
		if (0 != iRate)
		{
			CTranZoomBar *pTranZoomBar = pMagnifyPane->m_pZoomBar;
			if (NULL != pTranZoomBar)
			{
				pTranZoomBar->SetMagnifyRate(100*100/iRate);
			}
		}
		...
		CTranZoomTitleBar *pTranZoomTitleBar = pMagnifyPane->m_pZoomTitleBar;
		if (NULL != pTranZoomTitleBar)
		{
			pTranZoomTitleBar->SetZoomTitleName(strZoomName);
		}
...
void CVideoPane::FinishMagnifyByGraphicPoint(const CPoint &point)
{
	...
	if (GRAPHICS_STATE_DRAWING == m_ulGraphicState)// 画图状态
	{
		...
		if (NULL != pTranVideoRealtimeBar)
		{
			if (pMagnifyPane->GetShowInstantStatus())
			{
				::SendMessage(pTranVideoRealtimeBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_RETURN, BN_CLICKED), (LPARAM)pTranVideoRealtimeBar->GetReturnBtnHwnd());//lint !e613 !e835 !e845
			}
			if (BOTTOMBAR_PTZ_OPEN == pMagnifyPane->GetRealtimePtzState())
			{
				::SendMessage(pTranVideoRealtimeBar->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_PTZ, BN_CLICKED), (LPARAM)pTranVideoRealtimeBar->GetPTZBtnHwnd());//lint !e613 !e835 !e845
			}
		}
		...
		if (BOTTOMBAR_ALARM_OPEN != pMagnifyPane->GetRealtimeAlarmState())
		{
			(void)IVS_SDK_DelPlayWnd(pOCXPlayerCtrl->GetSDKSessionID(), pMagnifyPane->GetPlayHandle(), pMagnifyPane->GetVideoWnd());
		}
		...
		pMagnifyPane->SetPlayState(GetPlayState());
		pMagnifyPane->SetMagnifyPlayHandle(m_ulPlayHandle);
		(void)IVS_SDK_AddPlayWindowPartial(pOCXPlayerCtrl->GetSDKSessionID(), m_ulPlayHandle, pMagnifyPane->GetVideoWnd(), &IArectPercent);
	...
	else if (GRAPHICS_STATE_EDITING == m_ulGraphicState)// 编辑状态
	{
		...
		if (iRate < 12)
		{
			...
			if (NULL != pTranZoomBar)
			{
				pTranZoomBar->SetMagnifyRate(800);
			}
		}
		else
		{
...
{   
	...
	CIVS_OCXPlayerCtrl *pOCXPlayerCtrl = GetOCXPlayerCtrl();
	CHECK_POINTER_VOID(pOCXPlayerCtrl);
    IVS_INT32 iRet = IVS_SDK_GetPlayBackTime(pOCXPlayerCtrl->GetSDKSessionID(), m_ulInstanstPlayBackHandle, &stPlaybackTime);
    if (IVS_SUCCEED == iRet)
    {
...
void CVideoPane::SetSkin()
{
	...
	if (m_pRealTimeToolBar!=NULL)
	{
		Gdiplus::Image* pImage = SkinMgr.GetImageBarBKG();
		if (NULL == pImage)
		{
			return;
		}
...
void CVideoPane::StopRealStream()
{
	...
	{
		(void)IVS_SDK_StopRealPlay(Sid, m_ulPlayHandle);
	}
	else
	{
		(void)IVS_SDK_DelPlayWnd(Sid, m_ulPlayHandle, GetVideoWnd());
	}
	m_ulPlayHandle = 0;
}
...
int CVideoPane::StopRealStreamAsync()
{
	...
	if (m_hWorkThread)
	{
		...
		{
			(void)WaitForSingleObject(m_hWorkThread, INFINITE);
			(void)CloseHandle(m_hWorkThread);
		    m_hWorkThread = NULL;
		}
		else
		{
	            ...
	            BOOL bLayoutChange, ULONG ulStreamOpenMode, unsigned long transid)
{
	...
	REALPLAY_CMD cmd;
	eSDK_STRNCPY(cmd.CamID, sizeof(cmd.CamID), pCameraID, sizeof(cmd.CamID) - 1);
	eSDK_MEMCPY(&cmd.Param, sizeof(cmd.Param), &realplayParam, sizeof(cmd.Param));
	cmd.transid = transid;
	cmd.bStop   = false;
...
unsigned int __stdcall CVideoPane::BegRealPlayProc(LPVOID lpParameter)
{
	...
	while (true) //lint !e716
	{
		...
		eSDK_MEMSET(&realplayParam, 0, sizeof(realplayParam));
		...
		REALPLAY_CMD &cmd = pVideoPanel->m_CmdList.front();
 		eSDK_MEMCPY(&realplayParam, sizeof(realplayParam), &cmd.Param, sizeof(realplayParam));
		eSDK_STRNCPY(CameraID, sizeof(CameraID), cmd.CamID, sizeof(CameraID) - 1);
		bool bCmdStop  = cmd.bStop;
		IVS_ULONG transid   = cmd.transid;
		IVS_UINT32 uCmdSeq   = pVideoPanel->m_uCmdSeq;
...
IVS_INT32 CVideoPane::IsEnableDrawCallBackEx(IVS_BOOL bEnable)
{
	IVS_DEBUG_TRACE("bEnable: %d", bEnable);
	if (bEnable)
	{
		return IVS_SDK_SetDrawCallBackEx(m_iSessionID, m_ulPlayHandle, GetVideoWnd(), DrawCallBackEx, this);
	}
	else
	{
		return IVS_SDK_SetDrawCallBackEx(m_iSessionID, m_ulPlayHandle, GetVideoWnd(), NULL, NULL);
	}
}
