...
IVS_INT32 COCXXmlProcess::GetUnifiedFormatQueryParseXML(IVS_VOID* &pReqBuffer,CXml& xmlReq,IVS_INT32 iFiedNum)
{
	...
	GET_ELEM_VALUE_NUM("FromIndex", szElemValue, iFromTemp, xmlReq);	//lint !e838
	GET_ELEM_VALUE_NUM("ToIndex", szElemValue, iToTemp, xmlReq);	//lint !e838
	// sdk在这要对值进行判断
	...
	if (xmlReq.FindElem("QueryCond"))
	{
		...
		{        
			...
			do 
			{
				...
				xmlReq.IntoElem();
				eSDK_MEMSET(&stQueryField, 0, sizeof(IVS_QUERY_FIELD));

				// 对于字段的值，不知道具体的大小 现在目前先定IVS_DESCRIBE_LEN
				// 查询字段
				eSDK_MEMSET(cTemp, 0, IVS_DESCRIBE_LEN+1);
				...
				// 查询类型
				eSDK_MEMSET(cTemp, 0, IVS_DESCRIBE_LEN);
...
IVS_INT32 COCXXmlProcess::AlloceQueryStructMemory( IVS_VOID* &pReqBuffer, IVS_INT32 iFieldNum )
{
	...
	eSDK_MEMSET(pReqBuffer, 0, iFieldBufferSize);
	return IVS_SUCCEED;
}
