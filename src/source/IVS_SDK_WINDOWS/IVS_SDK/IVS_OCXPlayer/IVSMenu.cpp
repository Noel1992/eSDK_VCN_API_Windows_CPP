...
BOOL  CALLBACK EnumWindowsProcB(HWND hwnd, LPARAM lparam)
{
	if(hwnd!=NULL)
	{
		...
		{
			CIVSMenu *pMenu = dynamic_cast<CIVSMenu*>(CWnd::FromHandle(hwnd));
...
CIVSMenu::CIVSMenu()
	...
	,m_crGradFill(RGB(0,0,0))	//lint !e835
	,m_hCursor(nullptr)
...
BOOL CIVSMenu::CreateGroupMenu(const CWnd *pParent, const CRect &rcClient, UINT /*nID*/, BOOL bShowHead,int nModeIcon)
{
	...
	if(m_bShowHead)
	{
		if (!m_MenuData.btnHeader.Create(m_MenuData.strTitle, WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON, CRect(0,0,0,0), this, IDC_MENUHEADER))	//lint !e835
		{
			return FALSE;
		} else {
	...
	for(POSITION pos = m_MenuData.BtnList.GetHeadPosition(); pos!=NULL; )
	{
		...
		if (pBtnInfo)
		{
			(void)pBtnInfo->btn.Create(pBtnInfo->strTitle, WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON, CRect(0,0,0,0), this, pBtnInfo->nBtnID);	//lint !e835
...
void CIVSMenu::ShowMenuPos(int x, int y, int nMode)
{
	...
	case RIGHTTOP:
		...
		break;
	default:
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "Show Menu Pos failed", "Mode = %d is invalid", nMode);
		break;
	}
}
