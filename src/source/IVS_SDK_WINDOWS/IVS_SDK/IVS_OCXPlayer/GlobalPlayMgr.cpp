    ...
    : m_ref(0),m_bRealStream(bRealStream),m_uStreamType(0),m_ulHandle(handle)
{
    ...
    eSDK_MEMSET(m_CamID, 0, sizeof(m_CamID));
}
...
void CGlobalPlayMgr::PostDownLoadEvent(CIVS_OCXPlayerCtrl *pOcxCtrl, long EventID, unsigned long ulHandle, int Code)const
{
    ...
    if (IVS_SUCCEED == COCXRecordXMLProcess::RecordDownLoadBuildXml(ulHandle, Code, xml)) //lint !e732
    {
        unsigned int uiLength = 0;
		char * pEvent = NULL;
        const char* pszXML = xml.GetXMLStream(uiLength);
		if (NULL != pszXML && 0 != uiLength)
		{
			try
			{
				pEvent = new char[uiLength + 1];
			}
			catch (...)
			{
				BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Post DownLoad Event", "new memory failed");
				return;
			}
			eSDK_MEMSET(pEvent, 0x0, uiLength + 1);
		}
		
		if (!CToolsHelp::Memcpy(pEvent, uiLength + 1, pszXML, uiLength))
		{
			BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR,"Post DownLoad Event","Memcpy Failed.");
			delete[] pEvent;
			pEvent = NULL;
			return;//lint !e438
		}

		BP_DBG_LOG("PostDownLoadEvent EventID[%d] xml[%s] OCX Address[%x]", EventID, pEvent, pOcxCtrl);
		PostMessage(pOcxCtrl->GetSafeHwnd(), WM_EVENT_REMOTE_DOWNLOAD_CALLBACK, (WPARAM)EventID, (LPARAM)pEvent);
    }
}
...
#define BUF_SIZE 128
    ...
    for (; iter != end; iter++)
    {
        ...
        {
            eSDK_MEMSET(Line, 0, sizeof(Line));
