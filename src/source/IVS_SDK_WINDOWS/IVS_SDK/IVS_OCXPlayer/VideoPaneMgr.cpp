	...
	, m_ulSkinType(1)
{
    ...
    eSDK_MEMSET(m_strToolbarStatus, 0, 33);
...
void CVideoPaneMgr::SetColor(ENUMDEF::COLOR_MODE ulColor)
{
    ...
    if (COLOR_GRAY == ulColor)
    {
        ...
        m_TextColor = RGB(0,0,0);	//lint !e835
...
void CVideoPaneMgr::ChangeSize( ULONG ulIsPlaying )
{
    ...
    {
		BP_RUN_LOG_INF("Change Size", "Window NO. = %d", m_ulShowWndNum);
        return;
    }
...
void CVideoPaneMgr::Minify(void)
{
	...
	if(NULL != pOcxCtrl)
	{
		pOcxCtrl->PostMagnifyOrMinifyEventMsg(0, m_ulActivePaneID, MINIFY);
	}
}
...
void CVideoPaneMgr::Magnify(CRect& rect)
{
	...
	if(NULL != pOcxCtrl)
	{
		pOcxCtrl->PostMagnifyOrMinifyEventMsg(0, m_ulActivePaneID, MAGNIFY);
	}
}
...
void CVideoPaneMgr::ExchangePane(ULONG ulMouseUpPaneID)
{
    ...
    // 把两个记录读出来
    CVideoPane *firstPane  = firstIter->second;  //lint !e64
    CVideoPane *secondPane = secondIter->second; //lint !e64
...
BOOL CVideoPaneMgr::OnMouseWheel(UINT nFlags, short zDelta, const CPoint& pt)
{
	...
	{
		(void)pActiveVideoPane->OnMouseWheel(nFlags, zDelta, pt);
		return TRUE;
	}
...
void CVideoPaneMgr::CancleAllMagnifyWnd()
{
	...
	for (; iter != end; ++iter)
	{
		...
		if (MAGNIFY_OPEN == pVideoPane->GetRealtimeMagnifyState())
		{
			::PostMessage(pVideoPane->GetVideoRealtimeBar()->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_MAGNIFY, BN_CLICKED), (LPARAM)pVideoPane->GetVideoRealtimeBar()->GetMagnifyBtnHwnd());//lint !e835 !e845
		}
	}
}
...
void CVideoPaneMgr::SetActivatePane( ULONG PaneID )
{
    ...
    if (PaneID != m_ulActivePaneID)
    {
		...
		if(NULL != pOcxCtrl)
		{
			pOcxCtrl->PostActiveWndChangeEventMsg(0, m_ulActivePaneID);
		}
    }
...
void CVideoPaneMgr::ResetFullScreen(void)
{
    ...
    if (FULL_SCREEN == m_ulIsFullScreen)
    {
		...
		if(NULL != pOcxCtrl)
		{
			pOcxCtrl->PostExitFullScreenEventMsg("EVENT_EXIT_FULL_SCREEN", IVS_SUCCEED, GetActivePaneID());
		}
...
//       m_pOcxCtrl->ShowWindow(SW_HIDE);
       ...
       if(NULL != pOcxCtrl)
	   {
			pOcxCtrl->PostEnterFullScreenEventMsg("EVENT_ENTER_FULL_SCREEN", IVS_SUCCEED, GetActivePaneID());
	   }
...
LONG CVideoPaneMgr::SetLanguage(LPCTSTR pLanguage)
{
    ...
    eSDK_MEMSET(Buffer,0x0,BUFSIZE);
...
{
    return static_cast<ULONG>(IVS_SDK_SetMicVolume(static_cast<IVS_UINT32>(ulVolumeValue)));
}//lint !e715
...
{
	IVS_DEBUG_TRACE("");
    if (m_ulShowWndNum < m_ulOperatePaneID)
	{
		BP_RUN_LOG_INF("Set VideoPane Position", "Show window = %d is less then operate paneID = %d", m_ulShowWndNum, m_ulOperatePaneID);
		return;
	}
...
ULONG CVideoPaneMgr::StartVoiceTalkback(const TCHAR* pVoiceCode, ULONG ulDecoderType, ULONG ulStreamType)
{
	...
	if(NULL != pOcxCtrl)
	{
		CIVS_OCXPlayerCtrl *pRealPlayCtrl = pOcxCtrl->GetOcxPlayerCtrl(WND_TYPE_REALTIME);
...
LONG CVideoPaneMgr::SetCustomLayout( CRect rect, RECTLIST &videoRects,const IVS_UINT32 uX,const IVS_UINT32 uY )
{
   ...
   // RefreshAllAlarmWnd(ulLayoutType);
    ...
    //Rect 区域验证
    FunctionObjectSumRect _SumRectFO =  for_each(videoRects.begin(),videoRects.end(), FunctionObjectSumRect());	//lint !e864
