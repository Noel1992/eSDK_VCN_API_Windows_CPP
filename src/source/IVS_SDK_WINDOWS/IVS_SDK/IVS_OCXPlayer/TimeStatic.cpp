...
{
	m_pVideoPane = NULL;
	eSDK_MEMSET(&m_stCurrentTime,0x0,sizeof(ST_TIME));
	eSDK_MEMSET(&m_stStartTime,0x0,sizeof(ST_TIME));
	eSDK_MEMSET(&m_stEndTime,0x0,sizeof(ST_TIME));
	eSDK_MEMSET(&m_stSaveCurrentTime,0x0,sizeof(ST_TIME));
    eSDK_MEMSET(&m_stStartPlayTime, 0x0, sizeof(ST_TIME));
	...
	m_dScondGap = 0.0;
    eSDK_MEMSET(&m_Layer, 0x0, sizeof(ST_LAYERTIME));
}
...
void CTimeStatic::Render(Gdiplus::Graphics& g)
{
	...
	//		ST_TIME stOldTime;
	//		ST_TIME stNewTime;
	//		eSDK_MEMSET(&stOldTime,0,sizeof(ST_TIME));
	//		eSDK_MEMSET(&stNewTime,0,sizeof(ST_TIME));
	...
	//		eSDK_MEMSET(pArea,0,sizeof(ST_AREAINFO));
...
void CTimeStatic::MovePaneBig(const CPoint& point)
{
	...
	//更新刻度
	ST_TIME stAdd;
	eSDK_MEMSET(&stAdd,0,sizeof(ST_TIME));
	int iMoveStep = 0;
	int iRate = 0;
...
void CTimeStatic::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_bIsMouseFlag)
	{
		...
		ST_TIMER_MSG stMsg;
		eSDK_MEMSET(&stMsg,0,sizeof(ST_TIMER_MSG));
...
ST_TIME CTimeStatic::PointToTime(const CPoint &point) const
{
	...
	ST_TIME time;
	eSDK_MEMSET(&time,0,sizeof(ST_TIME));
	...
	while(pos!=NULL)
	{
		...
		if (NULL != pAreaInfo)
		{
			if ((m_iCurrentPointPos>=pAreaInfo->dLeft)&&(m_iCurrentPointPos<=pAreaInfo->dRight))
			{
				...
				eSDK_MEMSET(&stLayerTime,0,sizeof(ST_LAYERTIME));
...
void CTimeStatic::AddLayeTimeRange(int iType,const ST_LAYERTIME &stLayerTime)
{
	...
	case TYPE_YELLO:
		{//lint !e831
			...
			eSDK_MEMSET(pYello,0,sizeof(ST_LAYERTIME));
	...
	case TYPE_GREEN:
		{//lint !e831
			...
			eSDK_MEMSET(pGreen,0,sizeof(ST_LAYERTIME));
...
void CTimeStatic::InitTimeRange()
{
	...
	eSDK_MEMSET(&m_stStartTime,0,sizeof(ST_TIME));
	...
	//结束时间为当前时间
	eSDK_MEMSET(&m_stEndTime,0,sizeof(ST_TIME));
...
void CTimeStatic::AddOneScond()
{
    ...
    if (IsCurrentPlayEnd())
    {
        // 查找是否下一段时间还有录像
        ST_TIME NextTime;
        eSDK_MEMSET(&NextTime, 0, sizeof(ST_TIME));
...
void CTimeStatic::StartRecordPlayTimer()
{
    ...
    while(pos!=NULL)
    {
        ...
        if (NULL != pLayer)
        {
            ...
            {
                eSDK_MEMCPY(&m_Layer, sizeof(m_Layer), pLayer, sizeof(ST_LAYERTIME));
                break;
            }
        }
    }
...
BOOL CTimeStatic::IsPlayEnd()
{
	...
	ST_TIMER_MSG stTest;
	eSDK_MEMSET(&stTest,0,sizeof(ST_TIMER_MSG));
	CHECK_POINTER(m_pPlayTimerBar,FALSE);
