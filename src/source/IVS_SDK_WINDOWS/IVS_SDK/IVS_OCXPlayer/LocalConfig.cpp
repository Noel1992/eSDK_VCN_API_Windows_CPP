...
std::string CLocalConfig::CheckLocalConfigFileExist(const char* pFileName) const
{
	if (NULL == pFileName)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "CheckLocalConfigFileExist failed", "FileName is NULL");
		return "";
	}
	// 获取appdat目录
	IVS_CHAR szPath[1024] = {0};
	IVS_INT32 iRet = CToolsHelp::GetOcxPathFromCLSID(IVS_OCXPLAYER_CLSID, szPath, MAX_PATH);
	if (IVS_SUCCEED != iRet)
	{
		BP_RUN_LOG_ERR(iRet, "Check LocalConfig File Exist failed", "get OCX path failed");
		return "";
	}
	BP_RUN_LOG_INF("get OCX path from CLSID success", "path = %s", szPath);

	// get default config path
	std::string strDefaultConfigPath = szPath;
	strDefaultConfigPath = strDefaultConfigPath.substr(0, strDefaultConfigPath.rfind('\\'));
	strDefaultConfigPath+= LOCAL_CONFING_PATH;
	// append file name
	strDefaultConfigPath += pFileName;
	// 检查文件是否存在，不存在就返回空
	if(_access(strDefaultConfigPath.c_str(),0) != 0)
	{
		strDefaultConfigPath = "";
	}
	return strDefaultConfigPath; //lint !e1036
}
...
{
	char buf[1024] = {0};
	IVS_INT32 nRet = (int)GetModuleFileName(NULL, buf, sizeof(buf) - 1);
	if(0 == nRet)
	{
		return -1;
	}
