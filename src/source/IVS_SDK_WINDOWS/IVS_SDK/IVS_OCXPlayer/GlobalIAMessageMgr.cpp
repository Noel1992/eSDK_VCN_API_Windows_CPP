...
{
	eSDK_MEMSET(m_CameraID, 0, sizeof(m_CameraID));
}
						   ...
						   CVideoPane *pVideoPane)
{
    ...
    eSDK_MEMSET(&m_ReqParam, 0, sizeof(IA_REQ_PARAM));
	(void)CToolsHelp::Memcpy(&m_ReqParam, sizeof(IA_REQ_PARAM), 
							 &ReqParam, sizeof(IA_REQ_PARAM));
	eSDK_MEMSET(m_cTransID, 0, sizeof(m_cTransID));
...
CGlobalIAMessageMgr::CGlobalIAMessageMgr()
{
	...
	m_ThreadHandle = NULL;
	eSDK_MEMSET(&m_IAReqParam, 0, sizeof(m_IAReqParam));
}
											...
											IVS_ULONG ulRuleType)
{
	...
	eSDK_MEMSET(&IAReqParam, 0, sizeof(IAReqParam));
	IVS_INT32 iRet = GetReqParam(pVideoPane, ulRuleType, IAReqParam);
	if (IVS_SUCCEED != iRet)
	{
													...
													IA_REQ_PARAM &IAReqParam) 
{
	...
	// 发送请求轨迹XML
	IVS_INT32 iResult = SendMessageReqXML(iSessionID, IAReqParam, NULL);
...
// 发送请求轨迹XML消息
IVS_INT32 CGlobalIAMessageMgr::SendMessageReqXML(IVS_INT32 iSessionID, IA_REQ_PARAM &IAReqParam, const IVS_CHAR *pTransID)const
{
	return IVS_SUCCEED;
	//CXml xml;
...
IVS_INT32 CGlobalIAMessageMgr::SendMessageStopXML(IVS_INT32 iSessionID, 
													...
													const IVS_CHAR *CameraID, const IVS_CHAR *pTransID)const
{
	return IVS_SUCCEED;
//	if (NULL == CameraID) 
//	{
...
void CGlobalIAMessageMgr::CalculateStopTime(IA_REQ_PARAM &IAReqParam)
{
	...
	tm stStartInt;
	eSDK_MEMSET(&stStartInt, 0, sizeof(tm));
	TM stStartArr;
	eSDK_MEMSET(&stStartArr, 0, sizeof(TM));

	eSDK_MEMCPY(stStartArr.Year, sizeof(stStartArr.Year), &IAReqParam.StartTime[0], 4);
	stStartInt.tm_year = atoi(stStartArr.Year) - 1900;
	eSDK_MEMCPY(stStartArr.Month, sizeof(stStartArr.Month), &IAReqParam.StartTime[4], 2);
	stStartInt.tm_mon = atoi(stStartArr.Month) - 1;
	eSDK_MEMCPY(stStartArr.Day, sizeof(stStartArr.Day), &IAReqParam.StartTime[6], 2);
	stStartInt.tm_mday = atoi(stStartArr.Day);
	eSDK_MEMCPY(stStartArr.Hour, sizeof(stStartArr.Hour), &IAReqParam.StartTime[8], 2);
	stStartInt.tm_hour = atoi(stStartArr.Hour);
	eSDK_MEMCPY(stStartArr.Min, sizeof(stStartArr.Min), &IAReqParam.StartTime[10], 2);
	stStartInt.tm_min = atoi(stStartArr.Min);
	eSDK_MEMCPY(stStartArr.Sec, sizeof(stStartArr.Sec), &IAReqParam.StartTime[12], 2);
