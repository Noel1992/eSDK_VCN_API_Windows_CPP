	...
	for (; m_paneList.end() != it; it++)
	{
		...
		for (IVS_UINT32 i=1; i <= ulNum; i++)
		{
			if (i == vPaneID.ulChildWndID)
			{
				...
				eSDK_MEMCPY(stTVWallParam.cDecoderCode, sizeof(stTVWallParam.cDecoderCode), stPane.szDecoderID, IVS_TVWALL_DECODER_CODE_LEN);
...
IVS_INT32 COCXTVWallXMLProcess::AddTVWall(IVS_INT32& iNewID, const CString& strTVWall, CXml& xml)const
{
	...
	while(xml.FindElemValue(TVWALL_INFO_ROOT))//FindElem会导致死循环
	{
		...
		{ 
			...
			IVS_INT32 iTemp = 0;
			GET_ELEM_VALUE_NUM("ID",pValue, iTemp, xml);	//lint !e838
...

	{
		if(xml.FindElem(TVWALL_LIST_ROOT))
		{
			...
			(void)xml.AddElem("Layout");
			(void)xml.AddChildElem("HorizontalCellCount");
			(void)xml.IntoElem();
			FIND_XML_ELEM_EX(addXml, "Content/TVWall/Layout/HorizontalCellCount", IVS_XML_INVALID);
			(void)xml.SetElemValue(addXml.GetElemValueEx("Content/TVWall/Layout/HorizontalCellCount"));
			(void)xml.AddElem("VerticalCellCount");
			FIND_XML_ELEM_EX(addXml, "Content/TVWall/Layout/VerticalCellCount", IVS_XML_INVALID);
			(void)xml.SetElemValue(addXml.GetElemValueEx("Content/TVWall/Layout/VerticalCellCount"));
			(void)xml.AddElem("PaneNum");
			FIND_XML_ELEM_EX(addXml, "Content/TVWall/Layout/PaneNum", IVS_XML_INVALID);
			(void)xml.SetElemValue(addXml.GetElemValueEx("Content/TVWall/Layout/PaneNum"));
			(void)xml.AddElem("PaneList");
			int i=0;
			(void)addXml.FindElemEx("Content/TVWall/Layout/PaneList");
			while(addXml.FindElemValue("Pane"))
			{
				(void)addXml.IntoElem();
				if(0==i)
				{
...
IVS_INT32 COCXTVWallXMLProcess::ModifyTVWallInfo(const CString& strResult, CXml& xml)const
{
	...
	while(xml.FindElemValue(TVWALL_INFO_ROOT))//FindElem会导致死循环
	{
		...
		{//找到节点，修改信息
			...
			const IVS_CHAR *pTemp = modXml.GetElemValue();
			if(NULL!=pTemp)
			{
...
IVS_INT32 COCXTVWallXMLProcess::ModifyTVWallLayout(const CString& strResult, CXml& xml)const
{
	...
	const IVS_CHAR *pTVWallID = modXml.GetElemValue();
	CHECK_POINTER(pTVWallID, IVS_XML_INVALID);
    ...
    const IVS_CHAR *pTVWallName = TempXml.GetElemValue();
    ...
    FIND_XML_ELEM_AND_INTO(xml, "Content", IVS_XML_INVALID);
    (void)xml.AddChildElem("TVWallList");
    (void)xml.IntoElem();
    ...
    (void)xml.AddElem("Name");
    (void)xml.SetElemValue(pTVWallName);

    (void)xml.AddElem("Dsc");//lint !e830
    (void)xml.SetElemValue(modXml.GetElemValueEx("Content/TVWall/Dsc"));

    (void)xml.AddElem("Layout");
    (void)xml.AddChildElem("HorizontalCellCount");
    (void)xml.IntoElem();
    FIND_XML_ELEM_EX(modXml, "Content/TVWall/Layout/HorizontalCellCount", IVS_XML_INVALID);
    (void)xml.SetElemValue(modXml.GetElemValueEx("Content/TVWall/Layout/HorizontalCellCount"));
    (void)xml.AddElem("VerticalCellCount");
    FIND_XML_ELEM_EX(modXml, "Content/TVWall/Layout/VerticalCellCount", IVS_XML_INVALID);
    (void)xml.SetElemValue(modXml.GetElemValueEx("Content/TVWall/Layout/VerticalCellCount"));
    (void)xml.AddElem("PaneNum");
    FIND_XML_ELEM_EX(modXml, "Content/TVWall/Layout/PaneNum", IVS_XML_INVALID);
    (void)xml.SetElemValue(modXml.GetElemValueEx("Content/TVWall/Layout/PaneNum"));
    (void)xml.AddElem("PaneList");
    int i=0;
    (void)modXml.FindElemEx("Content/TVWall/Layout/PaneList");
    while(modXml.FindElemValue("Pane"))
    {
        (void)modXml.IntoElem();
        if(0==i)
        {
            (void)xml.AddChildElem("Pane");
            (void)xml.IntoElem();
...
IVS_INT32 COCXTVWallXMLProcess::GetTVWallByID(IVS_UINT32 uiTVWallID, CString& strResult, CXml& xml)const
{
	...
	try
	{
		...
		if(xml.FindElem("Layout"))
		{
			...
			{
				(void)xml.IntoElem();
				(void)pTvwallXml->AddElem("PaneList");
				while(xml.FindElemValue("Pane"))
				{
					...
					{
						(void)xml.IntoElem();
						(void)pTvwallXml->AddElem("ChannelList");
...
IVS_INT32 COCXTVWallXMLProcess::PlatformPlaybackParamParseXML(const IVS_CHAR* pPlaybackParam, IVS_PLAYBACK_PARAM& PlaybackParam)const
{
	...
	GET_ELEM_VALUE_CHAR("NVRCode", pElemValue, PlaybackParam.cNVRCode, IVS_NVR_CODE_LEN, xml);
    return IVS_SUCCEED;
}
...
IVS_INT32 COCXTVWallXMLProcess::GetTVWallPaneList(IVS_ULONG ulTVWallID, TVWALL_PANE_LIST& paneList, IVS_UINT32& uiHonCount, IVS_UINT32& uiVerCount)const
{
	...
	IVS_INT32 iRet = IVS_FAIL;
	const IVS_CHAR* pElemValue = NULL;
	GET_ELEM_VALUE_NUM("ResultCode", pElemValue, iRet, xml);	//lint !e838
	if(IVS_SUCCEED!=iRet)
	{
		return iRet;
	}
...
IVS_INT32 COCXTVWallXMLProcess::ParesTVWallLayoutXML2List(CXml& xml, TVWALL_PANE_LIST& paneList, IVS_UINT32& uiHorCount, IVS_UINT32& uiVerCount)const
{
	...
	GET_ELEM_VALUE_NUM_FOR_UINT("HorizontalCellCount", pElemValue, uiHorCount, xml);	//lint !e838 //水平窗格数量
	GET_ELEM_VALUE_NUM_FOR_UINT("VerticalCellCount", pElemValue, uiVerCount, xml);	//垂直窗格数量
	...
	while(xml.FindElemValue("Pane"))//FindElem会导致死循环
	{
		...
		GET_ELEM_VALUE_NUM_FOR_UINT("PointX", pElemValue, uiX, xml);	//lint !e838
		GET_ELEM_VALUE_NUM_FOR_UINT("PointY", pElemValue, uiY, xml);	//lint !e838
		GET_ELEM_VALUE_NUM_FOR_UINT("PointHeight", pElemValue, uiHeight, xml);	//lint !e838
		GET_ELEM_VALUE_NUM_FOR_UINT("PointWidth", pElemValue, uiWidth, xml);	//lint !e838
...
std::string COCXTVWallXMLProcess::MakeSureTVWallDirExist()const
{
	...
	eSDK_MEMSET(szPath,0x0,MAX_PATH);
...
IVS_INT32 COCXTVWallXMLProcess::GetNotifyStartRealPlayTVWallRetXML(ULONG ulTVWallID, ULONG ulWndID, IVS_INT32 iErrCode, ULONG ulTransID, char* pXmlBuf, UINT bufLen)const
{
    ...
    {
        eSDK_MEMCPY(pXmlBuf, bufLen, pXml, uiLen);
        return IVS_SUCCEED;
    }
