...
void COCXPlayBackXMLProcess::GetPlayInfoEventMsgXML(CString &xmlToCU, ULONG ulWinID,IVS_MEDIA_INFO &mediaInfo, CPoint& XY,IVS_INT32 resultcode)
{
	...
	if (IVS_SUCCEED == resultcode)
	{
		...
		case VIDEO_DEC_MJPEG:
			xmlToCU +="MJPEG";
			break;
		case VIDEO_DEC_MPEG4:
			xmlToCU +="MPEG4";
			break;
		case VIDEO_DEC_MPEG2:
			xmlToCU +="MPEG2";
			break;
		case VIDEO_DEC_SVAC:
			xmlToCU += "SVAC";
...
IVS_INT32 COCXPlayBackXMLProcess::GetPlayBackTime(const IVS_PLAYBACK_TIME& stPlaybackTime, IVS_INT32 iResultCode, CXml& xmlTime)
{
    ...
    eSDK_MEMSET(cPlayBackTime, 0, IVS_TIME_LEN+1);
    ...
    eSDK_MEMSET(cPlayBackStartTime, 0, IVS_TIME_LEN+1);
    ...
    eSDK_MEMSET(cPlayBackEndTime, 0, IVS_TIME_LEN+1);
...
IVS_INT32 COCXPlayBackXMLProcess::GetPlaybackParam(const char* pPlayback, IVS_PLAYBACK_PARAM& stPlaybackParam)
{
    ...
    if(xml.FindElem("Content"))
    {
        ...
        {				
			...
			GET_ELEM_VALUE_CHAR("NVRCode", szElemValue, stPlaybackParam.cNVRCode, IVS_NVR_CODE_LEN, xml);
        }
        else
        {
...
static time_t MakeTimeFromStr(const char *date)
{
    ...
    struct tm _tm;
	eSDK_MEMSET(&_tm, 0, sizeof(_tm));

	char tmp[8] = {0};
	eSDK_MEMCPY(tmp, sizeof(tmp), date + 4, 2);
	_tm.tm_mon = atoi(tmp) - 1;

	eSDK_MEMCPY(tmp, sizeof(tmp), date + 6, 2);
	_tm.tm_mday =  atoi(tmp);

	eSDK_MEMCPY(tmp, sizeof(tmp), date + 8, 2);
	_tm.tm_hour =  atoi(tmp) + tz;

	eSDK_MEMCPY(tmp, sizeof(tmp), date + 10, 2);
	_tm.tm_min  =  atoi(tmp);

	eSDK_MEMCPY(tmp, sizeof(tmp), date + 12, 2);
	_tm.tm_sec = atoi(tmp);

	eSDK_MEMCPY(tmp, sizeof(tmp), date, 4);
...
IVS_INT32 COCXPlayBackXMLProcess::ParsePlayBackSync(const char *szReqXml, SyncPlayInfo &syncInfo)
{
	...
	do 
	{
		...
		eSDK_MEMSET(playInfo.CameraCode, 0, sizeof(playInfo.CameraCode));
		...
		if (xml.FindElem("CameraCode")&& 
			NULL != (value = xml.GetElemValue()))
		{
			eSDK_STRNCPY(playInfo.CameraCode, sizeof(playInfo.CameraCode), value, sizeof(playInfo.CameraCode) - 1);
		}

		if (xml.FindElem("NVRCode")&& 
			NULL != (value = xml.GetElemValue()))
		{
			eSDK_MEMCPY(playInfo.NVRCode, sizeof(playInfo.NVRCode), value, sizeof(playInfo.NVRCode) - 1);
		}
