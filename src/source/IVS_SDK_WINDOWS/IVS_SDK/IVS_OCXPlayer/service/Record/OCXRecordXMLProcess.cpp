...
IVS_INT32 COCXRecordXMLProcess::GetRecordListGetXML(const IVS_CHAR* pCameraCode,IVS_RECORD_INFO_LIST *pRecordInfoList,IVS_INT32 iRetCord,CXml &xml)
{
	...
	if(pRecordInfoList->uiTotal>0)
	{
		...
		for (int iIndex = 0;iIndex<iRealNum;iIndex++)
		{
			...
			eSDK_MEMSET(cRecordFileName,0,IVS_FILE_NAME_LEN+1);
			...
			eSDK_MEMSET(cNvrCode,0,IVS_NVR_CODE_LEN+1);
			...
			eSDK_MEMSET(cMBUDomain,0,IVS_DOMAIN_CODE_LEN+1);
			...
			eSDK_MEMSET(cAlarmType,0,IVS_ALARM_TYPE_LEN+1);
			...
			eSDK_MEMSET(cStartTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cEndTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cBookInfoName,0,IVS_BOOKMARK_NAME_LEN+1);
			...
			eSDK_MEMSET(cBookInfoTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cUserDomain,0,IVS_DOMAIN_CODE_LEN+1);
			...
			eSDK_MEMSET(cBookInfoCreatorName,0,IVS_NAME_LEN+1);
			...
			eSDK_MEMSET(cLockOperatorName,0,IVS_NAME_LEN+1);
			...
			eSDK_MEMSET(cLockDesc,0,IVS_RECORD_LOCK_DESC_LEN+1);
			...
			eSDK_MEMSET(cLockTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cLockStartTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cLockEndTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cPresetName,0,IVS_RECORD_PTZ_PRESET_NAME_LEN+1);
...
IVS_INT32 COCXRecordXMLProcess::GetRecordBookmarkListGetXML(const IVS_CHAR* pCameraCode,IVS_RECORD_BOOKMARK_INFO_LIST *pRecordBookmarkInfoList,IVS_INT32 iRetCode,CXml &xml)
{
	...
	if(pRecordBookmarkInfoList->uiTotal>0)
	{
		...
		for (int iIndex = 0;iIndex<iRealNum;iIndex++)
		{
			...
			eSDK_MEMSET(cBookName,0,IVS_BOOKMARK_NAME_LEN+1);
			...
			eSDK_MEMSET(cBookTime,0,IVS_TIME_LEN+1);
			...
			eSDK_MEMSET(cBookCreatorName,0,IVS_NAME_LEN+1);
...
IVS_INT32 COCXRecordXMLProcess::RecordDownLoadParseXml(IVS_DOWNLOAD_PARAM &DLParam, LPCTSTR pXmlStr)
{
	...
	{
		(void)CToolsHelp::Memcpy(DLParam.cNVRCode, sizeof(DLParam.cNVRCode), value, strlen(value));  
	}
	return IVS_SUCCEED;
}
