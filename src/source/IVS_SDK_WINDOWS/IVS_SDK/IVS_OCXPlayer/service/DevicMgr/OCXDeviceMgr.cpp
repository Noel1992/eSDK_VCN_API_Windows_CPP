...
{ 
    ...
    eSDK_MEMSET(pReqData, 0, iReqBufferSize);

    IVS_CHAR chDomainCode[IVS_DOMAIN_CODE_LEN + 1] ;  
    eSDK_MEMSET(chDomainCode, 0, IVS_DOMAIN_CODE_LEN + 1);
    ...
    eSDK_MEMSET(pRspData, 0, iRspBufferSize);    
...
IVS_INT32 COCXDeviceMgr::QueryDeviceList( IVS_INT32 iSessionID, IVS_ULONG ulDeviceType, CXml& reqXml, CXml& xmlRsp )
{
    ...
    GET_ELEM_VALUE_CHAR("DomainCode", pTemp, chDomainCode, IVS_DOMAIN_CODE_LEN, reqXml);	//lint !e838
	...
	CXml& xmlRsp)
{
	...
	IVS_INT32 iRet = IVS_SDK_GetDeviceList(iSessionID,(IVS_UINT32)ulDeviceType,&stRange,pRspBuffer,uiRspBufferSize);
	if (IVS_SUCCEED != iRet)
	{        
...
// 获取指定域设备列表(无查询条件设置)
IVS_INT32 COCXDeviceMgr::GetDomainDeviceList(IVS_INT32 iSessionID, 
	...
	CXml& xmlRsp)
{
	...
	if (NULL == pDomainCode)
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "Get Domain DeviceList", "DomainCode is NULL");
	...
	stRange.uiToIndex = ulToIndex;
	stRange.uiFromIndex = ulFromIndex;
	IVS_INT32 iRet = IVS_SDK_GetDomainDeviceList(iSessionID, pDomainCode, (IVS_UINT32)ulDeviceType,&stRange,pRspBuffer,uiRspBufferSize);
	if (IVS_SUCCEED != iRet)
	{        
		BP_RUN_LOG_ERR(iRet, "SDK_GetDeviceList fail", "NA");
		IVS_DELETE(pRspBuffer, MUILI);
...
IVS_VOID* COCXDeviceMgr::GetListObject( IVS_INT32 lDeviceType , IVS_UINT32 uiNum, IVS_UINT32 &uiSizeBuff)
{
	...
	case DEVICE_TYPE_CAMERA_EX:
		{
			uiSizeBuff = sizeof(IVS_CAMERA_BRIEF_INFO_LIST_EX) + sizeof(IVS_CAMERA_BRIEF_INFO_EX) * (uiNum - 1);
			break;
		}	
	case DEVICE_TYPE_SHADOW_CAMERA:
	case DEVICE_TYPE_CAMERA_AND_SHADOW:
		{
			uiSizeBuff = sizeof(IVS_SHADOW_CAMERA_BRIEF_INFO_LIST) + sizeof(IVS_SHADOW_CAMERA_BRIEF_INFO) * (uiNum - 1);  
			break;
		}	
    ...
    if (NULL != pBuff)
    {
        eSDK_MEMSET(pBuff, 0, uiSizeBuff);
    }
...
IVS_VOID COCXDeviceMgr::GetDomainRoute(IVS_INT32 iSessionID, CString &strResult)
{
	...
	eSDK_MEMSET(pReqRoute, 0, uiBufferSize);
...
IVS_VOID COCXDeviceMgr::GetDeviceInfo( IVS_INT32 iSessionID, CXml& xmlReq, CString &strResult )
{
    ...
    //eSDK_MEMSET(szDomainCode, 0, IVS_DEV_CODE_LEN+1);
...
IVS_VOID COCXDeviceMgr::GetNVRList(IVS_INT32 iSessionID, CXml& xmlReq, CString &strResult)
{
    ...
    eSDK_MEMSET(cDomainCode,0,IVS_DOMAIN_LEN + 1);
...
IVS_VOID COCXDeviceMgr::GetChannelList( IVS_INT32 iSessionID, const IVS_CHAR *pDevCode,  CString &strResult)
{
    ...
    eSDK_MEMSET(pBuffer, 0, uiBuffSize);
...
IVS_VOID COCXDeviceMgr::DeleteDevice(IVS_INT32 iSessionID, CString &strResult, LPCTSTR pReqXml)
{
	...
	eSDK_MEMSET(pReqBuff, 0, uiReqBufferSize);
...
IVS_INT32 COCXDeviceMgr::GetCameraEncodeCapabilities(IVS_STREAM_INFO_LIST *pStreamInfoList,IVS_INT32 iRetCord,CXml &xml)
{
	...
	for (int iIndex = 0;iIndex<pStreamInfoList->iRealNum;iIndex++)
	{
		...
		eSDK_MEMSET(cStreamType,0,IVS_FILE_NAME_LEN+1);
		...
		for (int iEncodeModeIndex = 0;iEncodeModeIndex < stStreamInfo.iRealNum;iEncodeModeIndex++)
		{
			...
			eSDK_MEMSET(cEncodeMode,0,IVS_DEV_MODEL_LEN+1);
			...
			for (int iResolutionIndex = 0;iResolutionIndex < stEncodeModeInfo.iRealNum;iResolutionIndex++)
			{
				...
				eSDK_MEMSET(cResolution,0,IVS_FILE_NAME_LEN+1);
				...
				eSDK_MEMSET(cPFrameList,0,IVS_MAX_NAME_LIST_LEN+1);
				...
				eSDK_MEMSET(cNFrameList,0,IVS_MAX_NAME_LIST_LEN+1);
			...
			eSDK_MEMSET(cProfileList,0,IVS_MAX_NAME_LIST_LEN+1);
			...
			eSDK_MEMSET(cBitRateList,0,IVS_MAX_NAME_LIST_LEN+1);
