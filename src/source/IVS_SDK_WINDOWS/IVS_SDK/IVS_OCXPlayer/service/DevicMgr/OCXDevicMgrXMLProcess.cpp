...
IVS_INT32 COCXDeviceXMLProcess::AddDeviceParseXml( CXml &reqXml, IVS_VOID* pReqBuffer, const IVS_UINT32 &uiNum, IVS_CHAR* pDomain)
{
    ...
    GET_ELEM_VALUE_CHAR("DomainCode", pTemp, pDomain, IVS_DOMAIN_CODE_LEN, reqXml);	//lint !e838
    reqXml.OutOfElem();
...
IVS_INT32 COCXDeviceXMLProcess::AddDeviceGetXml( CXml &rspXml, IVS_VOID* pRspData, const IVS_UINT32 &uiLen, const IVS_CHAR *pDomainCode)
{
    ...
    (void)rspXml.SetElemValue("0");
    (void)rspXml.AddElem("DevList");
    for (IVS_UINT32 uiIndex = 0 ; uiIndex < uiLen; uiIndex++)
    {
        ...
        if (0 == uiIndex)
        {
            (void)rspXml.AddChildElem("DevInfo");            
        }
        else
        {
            (void)rspXml.AddElem("DevInfo");
        }
        (void)rspXml.IntoElem();

        (void)rspXml.AddChildElem("Sequence");
        (void)rspXml.IntoElem();
        (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stQueryDeviceInfo.uiSequence).c_str());
        ...
        (void)rspXml.SetElemValue(strBigDevCode.c_str());
        ...
        (void)rspXml.AddElem("Result");
        (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stQueryDeviceInfo.uiResult).c_str());
...
IVS_INT32 COCXDeviceXMLProcess::GetMainDeviceListGetXml( CXml &rspXml, IVS_VOID* pRspData ,IVS_VOID* pDomain)
{
    ...
    (void)rspXml.IntoElem();
    (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)pRspDeviceList->uiTotal).c_str());

    (void)rspXml.AddElem("FromIndex");
    (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)pRspDeviceList->stIndexRange.uiFromIndex).c_str());

    (void)rspXml.AddElem("ToIndex");
    (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)pRspDeviceList->stIndexRange.uiToIndex).c_str());
    ...
    while (i < iNum)
    {
        i == 0 ? (void)rspXml.AddChildElem("DevInfo"): (void)rspXml.AddElem("DevInfo");
        (void)rspXml.IntoElem();
        IVS_DEVICE_BRIEF_INFO &devInfo = pRspDeviceList->stDeviceBriefInfo[i];

        //主设备编码
        (void)rspXml.AddChildElem("DevCode");
        (void)rspXml.IntoElem();
		...
		(void)rspXml.SetElemValue(strBigCode.c_str());   
        //主设备设备名称
        (void)rspXml.AddElem("DevName");
        eSDK_MEMSET(chTemp, 0, uiMaxElemLen);
        ...
        (void)rspXml.SetElemValue(chTemp);
        //主设备协议类型
        (void)rspXml.AddElem("DevProtocolType"); 
		...
		(void)rspXml.SetElemValue(cDevProtocolType);
        //认证类型
        (void)rspXml.AddElem("DevLoginType");
        (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)devInfo.iLoginType).c_str());
        //主设备提供商类型
        ...
        (void)rspXml.SetElemValue(CDevEnumXML::GetDevType((IVS_INT32)devInfo.stDeviceInfo.uiType));
        //主设备型号
		...
		(void)rspXml.SetElemValue(cDevSN);
        //IP类型
        (void)rspXml.AddElem("DevIPType");
        (void)rspXml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)devInfo.stDeviceInfo.stIP.uiIPType).c_str());
        //设备所在IP地址
        ...
        (void)rspXml.SetElemValue(CDevEnumXML::GetCommStatusType((IVS_INT32)devInfo.uiStatus));
        //是否支持集群调度 
        (void)rspXml.AddElem("BlanceSupport");
        (void)rspXml.SetElemValue(CToolsHelp::Int2Str(devInfo.bEnableSchedule).c_str());
...
IVS_INT32 COCXDeviceXMLProcess::GetDeviceInfoParseXML(const IVS_CHAR* pDomainCode,const IVS_CHAR* pDevCode, CXml& xml)
{
    ...
    GET_ELEM_VALUE_CHAR("DomainCode", szElemValue, pDomainCode, IVS_DEV_CODE_LEN, xml);	//lint !e838
...
IVS_INT32 COCXDeviceXMLProcess::GetNVRListRsp(IVS_DEVICE_BRIEF_INFO_LIST *pNVRList,IVS_INT32 iRetCode,CXml& xml)
{
    ...
    if(pNVRList->uiTotal>0)
    {
        ...
        for (IVS_UINT32 iIndex = 0;iIndex<iRealNum;iIndex++)
        {
			...
			eSDK_MEMSET(cDomainCode,0,IVS_DOMAIN_CODE_LEN+1);
			...
			eSDK_MEMSET(cName,0,IVS_DEVICE_NAME_LEN+1);
			...
			eSDK_MEMSET(cModel,0,IVS_DEV_MODEL_LEN+1);
			...
			eSDK_MEMSET(cIP,0,IVS_IP_LEN+1);
...
IVS_INT32 COCXDeviceXMLProcess::DeleteDevicePraseXML(IVS_DEVICE_CODE* pDevCode, IVS_CHAR *pDomainCode, const IVS_UINT32& uiDeviceNum, CXml& xmlReq)
{
	...
	do 
	{
		...
		eSDK_MEMSET(chTemp , 0, MAXELEMLEN + 1);
		eSDK_MEMSET(chDevCode , 0, IVS_DEV_CODE_LEN + 1);
		eSDK_MEMSET(chTemp , 0, IVS_DOMAIN_CODE_LEN+1);
...
IVS_INT32 COCXDeviceXMLProcess::DeleteDeviceGetXml(IVS_RESULT_LIST* pReqList, CXml& xmlRsp, const IVS_CHAR *pDomainCode)
{
    ...
    for (IVS_UINT32 uiIndex = 0; uiIndex < uiNum; uiIndex++)
    {
        ...
        if (0 == uiIndex)
        {
            (void)xmlRsp.AddChildElem("DevInfo");
        }
        else
        {
            (void)xmlRsp.AddElem("DevInfo");
        }
	...
	CXml &xmlRsp)
{
	...
	case DEVICE_TYPE_CAMERA_EX:
		{
			IVS_CAMERA_BRIEF_INFO_LIST_EX *pRspDeviceList = (IVS_CAMERA_BRIEF_INFO_LIST_EX*)pRspData;
			iRet = GetCamerasExGetXml(pRspDeviceList,xmlRsp); 
		}	
		break;
	case DEVICE_TYPE_SHADOW_CAMERA:
	case DEVICE_TYPE_CAMERA_AND_SHADOW:
		{
			IVS_SHADOW_CAMERA_BRIEF_INFO_LIST *pRspDeviceList = (IVS_SHADOW_CAMERA_BRIEF_INFO_LIST*)pRspData;  
			iRet = GetshadowCamerasGetXml(pRspDeviceList,xmlRsp); 
		}
		break;
	default:
		{
	...
	CXml &rspXml)
{
   ...
   if (pRspDeviceList->stIndexRange.uiFromIndex > pRspDeviceList->uiTotal)
   {
	...
	while (i < iNum)
	{
		i == 0 ? (void)rspXml.AddChildElem("AlarmDevInfo"):(void)rspXml.AddElem("AlarmDevInfo");
		(void)rspXml.IntoElem();
	...
	CXml &rspXml)
{
	...
	while (i < iNum)
	{
		i == 0 ? (void)rspXml.AddChildElem("VoiceChanInfo"):(void)rspXml.AddElem("VoiceChanInfo");
		(void)rspXml.IntoElem();
	...
	CXml &rspXml)
{
	...
	while (i < iNum)
	{
		i == 0 ? (void)rspXml.AddChildElem("VideoInChanInfo"):(void)rspXml.AddElem("VideoInChanInfo");
		(void)rspXml.IntoElem();
...
IVS_INT32 COCXDeviceXMLProcess::GetDomainRouteGetXML(IVS_DOMAIN_ROUTE_LIST *pList, CXml &xmlRsp )
{
    ...
    for (IVS_UINT32 uiIndex = 0; uiIndex < uiLen; uiIndex++)
    {
        if (0 == uiIndex)
        {
            (void)xmlRsp.AddChildElem("DomainRoute");
        }
        else
        {
            (void)xmlRsp.AddElem("DomainRoute");
        }
        (void)xmlRsp.IntoElem();
        IVS_DOMAIN_ROUTE &stRoute = pList->stDomainRoute[uiIndex];

        (void)xmlRsp.AddChildElem("DomainCode");
        (void)xmlRsp.IntoElem();
        eSDK_MEMSET(chTemp, 0, uiMaxElemLen);
        ...
        (void)xmlRsp.SetElemValue(chTemp);

        (void)xmlRsp.AddElem("DomainName");
        eSDK_MEMSET(chTemp, 0, uiMaxElemLen);
        ...
        (void)xmlRsp.AddElem("DomainPort");  
        (void)xmlRsp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stRoute.uiPort).c_str());

        (void)xmlRsp.AddElem("DomainType");
        (void)xmlRsp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stRoute.uiDomainType).c_str());

        (void)xmlRsp.AddElem("IsProxy");
        (void)xmlRsp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stRoute.bIsAgent).c_str());

        (void)xmlRsp.AddElem("SuperDomain");
        eSDK_MEMSET(chTemp, 0, uiMaxElemLen);
		...
		/*
		(void)xmlRsp.AddElem("IsLocalDomain");
		(void)xmlRsp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stRoute.bIsLocalDomain).c_str());

		//是否支持转发
		xmlRsp.AddElem("IsProgressionTransmit"); 
		xmlRsp.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)stRoute.bProgressionTransmit).c_str());
		*/
        xmlRsp.OutOfElem();
    }
...
IVS_INT32 COCXDeviceXMLProcess::SetDevPWDParseXML(IVS_CHAR* pDevCode, IVS_CHAR* pDevUserName, IVS_CHAR* pDevPWD, IVS_CHAR* pDevRegPWD, CXml &xml)
{
	...
	(void)xml.IntoElem();
	const IVS_CHAR* pTemp = NULL;
	GET_ELEM_VALUE_CHAR("DevCode", pTemp, pDevCode, IVS_DEV_CODE_LEN, xml);	//lint !e838
...
IVS_INT32 COCXDeviceXMLProcess::GetCamerasExGetXml( IVS_CAMERA_BRIEF_INFO_LIST_EX* pRspDeviceList,  CXml &rspXml)
{
	CHECK_POINTER(pRspDeviceList, IVS_OPERATE_MEMORY_ERROR); 
	IVS_DEBUG_TRACE("");
	...
	while (i < iNum)
	{
		i == 0 ? (void)rspXml.AddChildElem("VideoInChanInfo"):(void)rspXml.AddElem("VideoInChanInfo");
		(void)rspXml.IntoElem();
		IVS_CAMERA_BRIEF_INFO_EX &devInfo = pRspDeviceList->stCameraBriefInfo[i];
		//设备组编码
		(void)rspXml.AddChildElem("GroupID");
		(void)rspXml.IntoElem();
		IVS_CHAR cGroupID[IVS_DEVICE_GROUP_LEN + 1] = {0};
		...
		(void)rspXml.AddElem("Longitude");
		IVS_CHAR cLongitude[IVS_LONGITUDE_LATITUDE_LEN] = {0};
		if (!CToolsHelp::Memcpy(cLongitude, IVS_LONGITUDE_LATITUDE_LEN, devInfo.cLongitude, IVS_LONGITUDE_LATITUDE_LEN))
		{
			BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Memcpy(cLongitude, IVS_LONGITUDE_LATITUDE_LEN, devInfo.cLongitude, IVS_LONGITUDE_LATITUDE_LEN) fail", "NA");
			return IVS_ALLOC_MEMORY_ERROR;
		}
		(void)rspXml.SetElemValue(cLongitude);

		(void)rspXml.AddElem("Latitude");
		IVS_CHAR cLatitude[IVS_LONGITUDE_LATITUDE_LEN] = {0};
		if (!CToolsHelp::Memcpy(cLatitude, IVS_LONGITUDE_LATITUDE_LEN, devInfo.cLatitude, IVS_LONGITUDE_LATITUDE_LEN))
		{
			BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Memcpy(cLatitude, IVS_LONGITUDE_LATITUDE_LEN, devInfo.cLatitude, IVS_LONGITUDE_LATITUDE_LEN) fail", "NA");
			return IVS_ALLOC_MEMORY_ERROR;
		}
		(void)rspXml.SetElemValue(cLatitude);

		(void)rspXml.AddElem("Height");
		IVS_CHAR cHeight[FLOAT_TO_LONG_COEFF] = {0};
		(void)CToolsHelp::Snprintf(cHeight, FLOAT_TO_LONG_COEFF, "%f", devInfo.fHeight);
		(void)rspXml.SetElemValue(cHeight);
...
IVS_INT32 COCXDeviceXMLProcess::GetshadowCamerasGetXml( IVS_SHADOW_CAMERA_BRIEF_INFO_LIST* pRspDeviceList,  CXml &rspXml)
{
	CHECK_POINTER(pRspDeviceList, IVS_OPERATE_MEMORY_ERROR); 
	IVS_DEBUG_TRACE("");
	...
	while (i < iNum)
	{
		i == 0 ? (void)rspXml.AddChildElem("VideoInChanInfo"):(void)rspXml.AddElem("VideoInChanInfo");
		(void)rspXml.IntoElem();
		IVS_SHADOW_CAMERA_BRIEF_INFO &devInfo = pRspDeviceList->stCameraBriefInfo[i];
		//设备组编码
		(void)rspXml.AddChildElem("GroupID");
		(void)rspXml.IntoElem();
		IVS_CHAR cGroupID[IVS_DEVICE_GROUP_LEN + 1] = {0};
		...
		(void)rspXml.AddElem("IsShadowDev");
		(void)rspXml.SetElemValue(CToolsHelp::Int2Str(devInfo.bIsShadowDev).c_str());

		(void)rspXml.AddElem("OrigDevCode");
		IVS_CHAR cOrigDevCode[IVS_DEV_CODE_LEN] = {0};
		if (!CToolsHelp::Memcpy(cOrigDevCode, IVS_DEV_CODE_LEN, devInfo.cOrigDevCode, IVS_DEV_CODE_LEN))
		{
			BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Memcpy(cOrigDevCode, IVS_DEV_CODE_LEN, devInfo.cOrigDevCode, IVS_DEV_CODE_LEN) fail", "NA");
			return IVS_ALLOC_MEMORY_ERROR;
		}
		(void)rspXml.SetElemValue(cOrigDevCode);

		(void)rspXml.AddElem("OriDevDomainCode");
		IVS_CHAR cOriDevDomainCode[IVS_DOMAIN_CODE_LEN] = {0};
		if (!CToolsHelp::Memcpy(cOriDevDomainCode, IVS_DOMAIN_CODE_LEN, devInfo.cOriDevDomainCode, IVS_DOMAIN_CODE_LEN))
		{
			BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Memcpy(cOriDevDomainCode, IVS_DOMAIN_CODE_LEN, devInfo.cOriDevDomainCode, IVS_DOMAIN_CODE_LEN) fail", "NA");
			return IVS_ALLOC_MEMORY_ERROR;
		}
		(void)rspXml.SetElemValue(cOriDevDomainCode);
