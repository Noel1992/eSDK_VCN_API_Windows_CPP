...
COCXDeviceParaConfigXMLProcess::REQUEST_NAME COCXDeviceParaConfigXMLProcess::m_reqNames[] =
{
	...
	//add by zwx211831, Date:20140912,告警联动录像时设置录像图像质量。目前没有get的方法，暂时使用旧的get方法，但是没有对外开放。有相关需求时再改动。
	{ CONFIG_CAMERA_STREAM_EVENTEX_CFG,     reinterpret_cast<FnSetDeviceParaParseXML>(&COCXDeviceParaConfigXMLProcess::SetCameraStreamCfgEventExParseXML),      reinterpret_cast<FnGetDeviceParaGetXML>(&COCXDeviceParaConfigXMLProcess::GetCameraStreamCfgGetXML)},
	//add by zwx211831, Date:20140915, 摄像机多行OSD信息查询与设置。
	{ CONFIG_CAMERA_MULTI_OSD_CFG,     reinterpret_cast<FnSetDeviceParaParseXML>(&COCXDeviceParaConfigXMLProcess::SetCameraMutlOSDCfgParseXML),      reinterpret_cast<FnGetDeviceParaGetXML>(&COCXDeviceParaConfigXMLProcess::GetCameraMutlOSDCfgGetXML)}
}; 
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetMainDevConfigParseXml(IVS_VOID*& pBuffer, CXml &xml)const
{
    ...
    //主设备编码
    GET_ELEM_VALUE_CHAR("DevCode", szElemValue, chTemp, maxLen, xml);	//lint !e838
    ...
    //设备所属NVR编码
    GET_ELEM_VALUE_CHAR("NVRCode", szElemValue, pDevCfg->cNvrCode, IVS_NVR_CODE_LEN, xml);
    //设备所属TAS编码
    GET_ELEM_VALUE_CHAR("DCGCode", szElemValue, pDevCfg->cTasCode, IVS_DOMAIN_CODE_LEN, xml);
    //设备登录用户名
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::GetCameraStreamCfgGetXML(IVS_VOID* &pBuffer, const IVS_CHAR* pDevCode, CXml &xml)const
{
	...
	(void)xml.IntoElem();
	IVS_UINT32 i = 0;
	if(pCameraStrCfg->uiStreamInfoNum > IVS_MAX_STREAM_NUM)
	{
		pCameraStrCfg->uiStreamInfoNum = IVS_MAX_STREAM_NUM;
	}
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::GetCameraCfgGetXML(IVS_VOID* &pBuffer, const IVS_CHAR* pDevCode, CXml &xml)const
{
	...
	eSDK_MEMSET(cCode,0,IVS_DEV_CODE_LEN + 1);
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	//是否支持组播IsBroadcast
	GET_ELEM_VALUE_NUM("IsBroadcast", szElemValue, iTemp, xml);	//lint !e838
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraPTZCfgParseXml(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	//其他云台参数结构体默认值
	eSDK_MEMSET(pCameraStrCfg->cReserve,0,32);
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetAlarmInCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	GET_ELEM_VALUE_NUM("AutoClearAlarmTime", pTemp, iTemp, xml);	//lint !e838
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetAlarmOutCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	GET_ELEM_VALUE_NUM("HoldTime", pTemp, iTemp, xml);	//lint !e838
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::GetALarmOutCfgGetXML(IVS_VOID* &pBuffer, const IVS_CHAR* pDevCode, CXml &xml)const
{
	...
	eSDK_MEMSET(cAlarmOutCfg, 0x0, IVS_ALARM_OUT_NAME_LEN + 1);
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraOsgCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	(void)xml.IntoElem();
	{
		...
		GET_ELEM_VALUE_CHAR("OSDTimeEnable", pTemp, cOSDTimeEnable, iMaxLen, xml);	//lint !e838
		pCameraOsgCfg->stOSDTime.bEnableOSDTime = CDevEnumXML::GetCommStatusBool(cOSDTimeEnable);
		GET_ELEM_VALUE_NUM("TimeFormatType", pTemp, iTemp, xml);	//lint !e838
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetMotionDetecParseXML(IVS_VOID*& pBuffer, CXml& xml)const
{
	...
	eSDK_MEMSET(szTemp, 0x0, ENABLE_lENGTH);
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::GetDevTimeCfgGetXML(IVS_VOID* &pBuffer, const IVS_CHAR* pDevCode, CXml &xml)const
{
	...
	(void)xml.IntoElem();
	{
		...
		eSDK_MEMSET(cDevTime, 0x0, IVS_TIME_LEN + 1);
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraRecordStreamCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	GET_ELEM_VALUE_NUM_FOR_UINT("RecordStreamID", pTemp, uiStreamType, xml);	//lint !e838
...
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraFECCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	...
	GET_ELEM_VALUE_CHAR("ReliableTransmission", pTemp, cReliableTransmission, STR_LEN, xml);	//lint !e838
...
// 告警联动录像时设置录像图像质量设置XML
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraStreamCfgEventExParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	IVS_DEBUG_TRACE("");
	CHECK_POINTER(pBuffer, IVS_OPERATE_MEMORY_ERROR);
	
	IVS_CAMERA_STREAM_EVENTEX_CFG* pCameraStrEventExCfg = reinterpret_cast<IVS_CAMERA_STREAM_EVENTEX_CFG *>(pBuffer);
	if (!xml.FindElemEx("Content/VideoInChanInfo"))
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "Set Camera Stream Cfg EventEx ParseXML", "Can't FindElemEx VideoInChanInfo");
		return IVS_XML_INVALID;
	}
	...
	IVS_INT32 iTemp = 0;
	GET_ELEM_VALUE_NUM("StreamNum", pTemp, iTemp, xml);	//lint !e838
	pCameraStrEventExCfg->uiStreamInfoNum = static_cast<IVS_UINT32>(iTemp);

	if (!xml.FindElem("StreamList"))
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "Set Camera Stream Cfg EventEx ParseXML", "Can't FindElem StreamList");
		return IVS_XML_INVALID;
	}
	(void)xml.IntoElem();
	...
	if (!xml.FindElem("StreamInfo"))
	{
		BP_RUN_LOG_ERR(IVS_XML_INVALID, "Set Camera Stream Cfg EventEx ParseXML", "Can't FindElem StreamInfo");
		return IVS_XML_INVALID;
	}

	IVS_UINT32 uiStreamNum = 0;
	const IVS_INT32 maxLen = 120;
	do 
	{
		uiStreamNum++;
		if (IVS_MAX_STREAM_NUM < uiStreamNum)
		{
			pCameraStrEventExCfg->uiStreamInfoNum = IVS_MAX_STREAM_NUM;
			BP_RUN_LOG_WAR("Set Camera Stream Cfg EventEx ParseXML", "Stream EventEx Info is more than MaxNum, Ignore unnecessary");
			break;
		}

		IVS_STREAM_INFO_EVENTEX stStreamInfoEx = {0};
		(void)xml.IntoElem();

		GET_ELEM_VALUE_NUM("StreamType", pTemp, iTemp, xml);
		stStreamInfoEx.uiStreamType = static_cast<IVS_UINT32>(iTemp);

		IVS_CHAR cEncodeMode[maxLen] = {0};
		GET_ELEM_VALUE_CHAR("EncodeMode", pTemp, cEncodeMode, maxLen, xml);
		stStreamInfoEx.uiEncodeType = static_cast<IVS_UINT32>(CDevEnumXML::GetEnCodeModeEnum(cEncodeMode));

		GET_ELEM_VALUE_NUM("PicQuality", pTemp, iTemp, xml);
		stStreamInfoEx.uiPicQuality = static_cast<IVS_UINT32>(iTemp);

		GET_ELEM_VALUE_CHAR("Resolution", pTemp, stStreamInfoEx.cResolution, IVS_RESOLUTION_LEN, xml);

		GET_ELEM_VALUE_NUM("BitRate", pTemp, iTemp, xml);
		stStreamInfoEx.uiBitRate = static_cast<IVS_UINT32>(iTemp);

		IVS_CHAR cBitRateType[maxLen] = {0};
		GET_ELEM_VALUE_CHAR("BitRateType", pTemp, cBitRateType, maxLen, xml);
		stStreamInfoEx.uiBitRateType = static_cast<IVS_UINT32>(CDevEnumXML::GetBitRateTypeEnum(cBitRateType));

		GET_ELEM_VALUE_NUM("FrameRate", pTemp, iTemp, xml);
		stStreamInfoEx.uiFrameRate = static_cast<IVS_UINT32>(iTemp);

		GET_ELEM_VALUE_NUM("EventRecordAdapt", pTemp, iTemp, xml);
		stStreamInfoEx.uiEventRecordAdapt = static_cast<IVS_UINT32>(iTemp);

		GET_ELEM_VALUE_NUM("EventFrameRate", pTemp, iTemp, xml);
		stStreamInfoEx.uiEventFrameRate = static_cast<IVS_UINT32>(iTemp);

		GET_ELEM_VALUE_NUM("EventBitRate", pTemp, iTemp, xml);
		stStreamInfoEx.uiEventBitRate = static_cast<IVS_UINT32>(iTemp);

		IVS_CHAR cProfile[maxLen] = {0};
		GET_ELEM_VALUE_CHAR("Profile", pTemp, cProfile, maxLen, xml);
		if (1 == stStreamInfoEx.uiEncodeType)
		{
			stStreamInfoEx.uiProfile = static_cast<IVS_UINT32>(CDevEnumXML::GetH264ProfileEnum(cProfile));
		}
		if (2 == stStreamInfoEx.uiEncodeType)
		{
			stStreamInfoEx.uiProfile = static_cast<IVS_UINT32>(CDevEnumXML::GetMjpegProfileEnum(cProfile));
		}
		
		GET_ELEM_VALUE_NUM("IframeInterval", pTemp, iTemp, xml);
		stStreamInfoEx.uiFrameInterval = static_cast<IVS_UINT32>(iTemp);

		GET_ELEM_VALUE_NUM("TransProtocol", pTemp, iTemp, xml);
		stStreamInfoEx.uiTransProtocol = static_cast<IVS_UINT32>(iTemp);

		GET_ELEM_VALUE_NUM("PKTProtocol", pTemp, iTemp, xml);
		stStreamInfoEx.uiPktProtocol = static_cast<IVS_UINT32>(iTemp);

		pCameraStrEventExCfg->stStreamInfo[uiStreamNum - 1] = stStreamInfoEx;
		xml.OutOfElem();
...
// 设置摄像机多行OSD信息设置XML
IVS_INT32 COCXDeviceParaConfigXMLProcess::SetCameraMutlOSDCfgParseXML(IVS_VOID*& pBuffer, CXml &xml)const
{
	IVS_DEBUG_TRACE("");
	CHECK_POINTER(pBuffer, IVS_OPERATE_MEMORY_ERROR);

	IVS_CAMERA_MULTI_OSD_CFG* pMultOsgCfg = reinterpret_cast<IVS_CAMERA_MULTI_OSD_CFG*>(pBuffer);

	if (!xml.FindElemEx("Content/OSDParaInfo/OSDTimeInfoList"))
	{
		BP_RUN_LOG_ERR(IVS_PARA_INVALID, "Set Camera MutlOSDCfg ParseXML", "FindElemEx OSDTimeInfoList fail");
		return IVS_PARA_INVALID;
	}
	...
	do 
	{
		...
		stOSDTime.bEnableOSDTime = CDevEnumXML::GetCommStatusBool(cOSDTimeEnable);
		GET_ELEM_VALUE_NUM("TimeFormatType", pTemp, iTemp, xml);
		stOSDTime.uiTimeFormat = static_cast<IVS_UINT32>(iTemp);
		GET_ELEM_VALUE_FLOAT("TimeX", pTemp, stOSDTime.fTimeX, xml);
		GET_ELEM_VALUE_FLOAT("TimeY", pTemp, stOSDTime.fTimeY, xml);

		pMultOsgCfg->stCameraOSDCfg[uiOSDTimeNum].stOSDTime = stOSDTime;	//lint !e661 zwx211831程序内部开辟足够大内存，确保不会内存越界，可以忽略
		uiOSDTimeNum++;
		xml.OutOfElem();
	...
	do 
	{
		...
		IVS_CHAR cOSDNameEnable[iMaxLen] = {0};
		GET_ELEM_VALUE_CHAR("OSDNameEnable", pTemp, cOSDNameEnable, iMaxLen, xml);
		stOSDName.bEnableOSDName = CDevEnumXML::GetCommStatusBool(cOSDNameEnable);
		GET_ELEM_VALUE_CHAR("OSDNameText", pTemp, stOSDName.cOSDNameText, IVS_MAX_OSD_TEXT_LEN, xml);
		GET_ELEM_VALUE_FLOAT("TextX", pTemp, stOSDName.rectText.left, xml);
		GET_ELEM_VALUE_FLOAT("TextY", pTemp, stOSDName.rectText.right, xml);
		GET_ELEM_VALUE_NUM("SwitchEnable", pTemp, stOSDName.bEnableSwitch, xml);
		GET_ELEM_VALUE_NUM("SwitchInterval", pTemp, iTemp, xml);
		stOSDName.uiSwitchInterval = static_cast<IVS_UINT32>(iTemp);
		...
		pMultOsgCfg->stCameraOSDCfg[uiOSDTimeNum].stOSDName = stOSDName;	//lint !e661 zwx211831程序内部开辟足够大内存，确保不会内存越界，可以忽略
		uiOSDNameNum++;
		xml.OutOfElem();

	} while (xml.NextElem());

	pMultOsgCfg->uiOSDTimeNum = uiOSDTimeNum;
	pMultOsgCfg->uiOSDNameNum = uiOSDNameNum;
	pMultOsgCfg->uiTotal = (uiOSDTimeNum > uiOSDNameNum) ? uiOSDTimeNum : uiOSDNameNum;
...
// 获取摄像机多行OSD信息参数XML
IVS_INT32 COCXDeviceParaConfigXMLProcess::GetCameraMutlOSDCfgGetXML(IVS_VOID* &pBuffer, const IVS_CHAR* pDevCode, CXml &xml)const
{
	CHECK_POINTER(pBuffer, IVS_OPERATE_MEMORY_ERROR);
	CHECK_POINTER(pDevCode, IVS_OPERATE_MEMORY_ERROR);
	IVS_DEBUG_TRACE("");
	...
	for(IVS_UINT32 uiTimes = 0; uiTimes < uiOSDTimeNum; uiTimes++)
	{
		...
		(void)xml.AddChildElem("OSDTimeEnable");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(COCXXmlProcess::TransIntToOnOff(static_cast<IVS_UINT32>(pOSDTime.bEnableOSDTime)).c_str());
	...
	for(IVS_UINT32 uiTimes = 0; uiTimes < uiOSDNameNum; uiTimes++)
	{
		...
		(void)xml.AddChildElem("OSDNameEnable");
		(void)xml.IntoElem();
		(void)xml.SetElemValue(COCXXmlProcess::TransIntToOnOff(static_cast<IVS_UINT32>(pOSDName.bEnableOSDName)).c_str());
