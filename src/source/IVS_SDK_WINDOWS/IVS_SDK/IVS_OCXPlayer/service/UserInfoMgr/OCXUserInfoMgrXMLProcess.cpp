...
IVS_INT32 COCXUserInfoMgrXMLProcess::MoveUserGroupPraseXML(IVS_CHAR* pCameraCode, IVS_UINT32& uiTargetDevGroupID, IVS_USER_GROUP_LIST* &pUserGroupList, CXml& xml)
{
    ...
    while (xml.FindElemValue("GroupInfo"))
    {
        ...
        eSDK_MEMSET(&groupInfo, 0, sizeof(IVS_USER_GROUP));
        ...
        eSDK_STRNCPY(groupInfo.cName, sizeof(groupInfo.cName), pszGroupName, IVS_NAME_LEN - 1);
    ...
    eSDK_MEMSET(pBuffer, 0, uiBufferSize);
...
IVS_VOID COCXUserInfoMgrXMLProcess::AddRoleGetXML(IVS_UINT32 &uiRoleID, IVS_INT32 iRet, CXml &xml)
{
    ...
    (void)xml.AddChildElem("RoleID");
    (void)xml.IntoElem();
    (void)xml.SetElemValue(CToolsHelp::Int2Str((IVS_INT32)uiRoleID).c_str());
    xml.OutOfElem();
...
IVS_INT32 COCXUserInfoMgrXMLProcess::ShutdownStreamGetXml(const IVS_SHUTDOWN_NOTIFY_INFO *pShutdownNotifyInfo, 
	CXml &xml)
{
	IVS_DEBUG_TRACE("");
	...
	(void)xml.AddChildElem("CameraCode");
	(void)xml.IntoElem();
	IVS_CHAR cDevTmp[IVS_DEV_CODE_LEN + 1] = {0};
	if (!CToolsHelp::Strncpy(cDevTmp, IVS_DEV_CODE_LEN + 1, pShutdownNotifyInfo->cCameraCode, IVS_DEV_CODE_LEN))
	{
		BP_RUN_LOG_ERR(IVS_ALLOC_MEMORY_ERROR, "Shutdown Stream GetXml", "Strncpy CameraCode failed");
		return IVS_ALLOC_MEMORY_ERROR;
	}
	(void)xml.SetElemValue(cDevTmp);

	(void)xml.AddElem("StreamType");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(pShutdownNotifyInfo->uiStreamType).c_str());

	(void)xml.AddElem("Duration");
	(void)xml.SetElemValue(CToolsHelp::Int2Str(pShutdownNotifyInfo->uiDuration).c_str());
