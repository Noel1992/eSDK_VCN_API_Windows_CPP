...
IVS_INT32 CFavorite::AddFavoriteDevice(const IVS_CHAR* pDeviceFavoriteInfo, std::string& strDeviceGUID)
{
    ...
    {
        (void)xmlConfig.AddElem("DeviceFavoriteInfo");
    }
    else
    {//第一次新增
        (void)xmlConfig.AddChildElem("DeviceFavoriteInfo");
        (void)xmlConfig.IntoElem();
    }
    (void)xmlConfig.AddChildElem("DeviceFavoriteID");
    (void)xmlConfig.IntoElem();
    (void)xmlConfig.SetElemValue(pszDeviceFavoriteID);
    (void)xmlConfig.AddElem("ParentFolderID");
    (void)xmlConfig.SetElemValue(szParentFolderID);
    (void)xmlConfig.AddElem("DeviceCode");
    (void)xmlConfig.SetElemValue(szDeviceCode);
    (void)xmlConfig.AddElem("FolderID");
    (void)xmlConfig.SetElemValue("");
    (void)xmlConfig.AddElem("FolderName");
    (void)xmlConfig.SetElemValue("");
    xmlConfig.OutOfElem();
    xmlConfig.OutOfElem();
    xmlConfig.OutOfElem();
...
IVS_INT32 CFavorite::AddFavoriteDeviceParseXML(const IVS_CHAR* pDeviceFavoriteInfo, IVS_CHAR* pszParentFolderID, IVS_CHAR* pszDeviceCode)
{
    ...
    GET_ELEM_VALUE_CHAR("ParentFolderID", szElemValue, pszParentFolderID, IVS_FAVORITE_ID_LEN, xml);//lint !e534 !e838
    GET_ELEM_VALUE_CHAR("DeviceCode", szElemValue, pszDeviceCode, IVS_DEV_CODE_LEN, xml);//lint !e534 !e838
    if (NULL == szElemValue)
    {
        return IVS_FAIL;
    }
...
IVS_INT32 CFavorite::MoveFavoriteDeviceParseXML(const IVS_CHAR* pDeviceFavoriteInfo, IVS_CHAR* pszParentFolderID, IVS_CHAR* pszDeviceFavoriteID)
{
    ...
    const char* szElemValue = NULL;
    GET_ELEM_VALUE_CHAR("ParentFolderID", szElemValue, pszParentFolderID, IVS_FAVORITE_ID_LEN, xml);//lint !e534 !e838
    GET_ELEM_VALUE_CHAR("DeviceFavoriteID", szElemValue, pszDeviceFavoriteID, IVS_FAVORITE_ID_LEN, xml);//lint !e534 !e838
    if (NULL == szElemValue)
    {
        return IVS_FAIL;
    }
...
IVS_INT32 CFavorite::GetFavoriteDevice(CXml& xmlRsp)
{
    ...
    //构造查询响应报文
    IVS_INT32 uiPos = sizeof("<?xml version=\"1.0\" encoding=\"UTF-8\" ?><Content><ResultCode>0</ResultCode><DeviceFavoriteList>") - 1;
    ...
    eSDK_MEMSET(pszXmlRsp, 0, uiLen + 1);
...
IVS_INT32 CFavorite::DelFavoriteFolderParseXML(const IVS_CHAR* pDeviceFavoriteInfo, IVS_CHAR* pszFolderID)
{
    ...
    (void)xml.IntoElem();
    const char* szElemValue = NULL;
    GET_ELEM_VALUE_CHAR("FolderID", szElemValue, pszFolderID, IVS_FAVORITE_ID_LEN, xml);//lint !e534 !e838
    if (NULL == szElemValue)
    {
        return IVS_FAIL;
    }
...
//         return IVS_FAIL;
//     }
    (void)xmlConfig.AddChildElem("FolderID");
    (void)xmlConfig.IntoElem();
    (void)xmlConfig.SetElemValue(pszFolderID);
    (void)xmlConfig.AddElem("ParentFolderID");
    (void)xmlConfig.SetElemValue(szParentFolderID);
    (void)xmlConfig.AddElem("FolderName");
    (void)xmlConfig.SetElemValue(szFolderName);
    (void)xmlConfig.AddElem("DeviceCode");
    (void)xmlConfig.SetElemValue("");
    (void)xmlConfig.AddElem("DeviceFavoriteID");
    (void)xmlConfig.SetElemValue("");
...
IVS_INT32 CFavorite::AddFavoriteFolderParseXML(const IVS_CHAR* pDeviceFavoriteInfo, IVS_CHAR* pszParentFolderID, IVS_CHAR* pszFolderName)
{
    ...
    const char* szElemValue = NULL;
    GET_ELEM_VALUE_CHAR("ParentFolderID", szElemValue, pszParentFolderID, IVS_FAVORITE_ID_LEN, xml);//lint !e534 !e838
    GET_ELEM_VALUE_CHAR("FolderName", szElemValue, pszFolderName, IVS_FOLDER_NAME_LEN, xml);//lint !e534 !e838
    if (NULL == szElemValue)
    {
        return IVS_FAIL;
    }
...
void CFavorite::BulidGetScenarioInfoListResult(IVS_INT32 iResultCode, const std::string& strScenarioInfoList, std::string& strResult)
{
	...
	TiXmlText* pTiXmlText =  new TiXmlText(CToolsHelp::Int2Str(iResultCode).c_str());
	CHECK_POINTER_VOID(pTiXmlText);
	pResultCodeElem->LinkEndChild(pTiXmlText);
	// 插入到content后面
...
IVS_INT32 CFavorite::AddCameraSwitch(const IVS_CHAR* pSwitchInfo, const std::string& strSwitchID)
{
	...
	if (NULL == pCameraSwitchInfoElem)
	{
		...
		return IVS_PARA_INVALID;//lint !e438
	}
...
IVS_INT32 CFavorite::GetCameraSwitchID(const IVS_CHAR* pSwitchPlanInfo, std::string &pCameraSwitchID)
{
	...
	const char* szElemValue = NULL;
	eSDK_MEMSET(cCameraSwitchID, 0, IVS_DEV_CODE_LEN+1);
	GET_ELEM_VALUE_CHAR("CameraSwitchID",szElemValue, cCameraSwitchID,IVS_DEV_CODE_LEN,xml);	//lint !e838
