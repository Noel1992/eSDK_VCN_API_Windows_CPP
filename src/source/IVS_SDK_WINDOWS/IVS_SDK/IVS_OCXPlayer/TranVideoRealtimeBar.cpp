...
} //lint !e1740 !e1579 m_pVideoPane 由外部传入 无须释放
...
void CTranVideoRealtimeBar::OnBnClickedButtonRealtimeMagnify()
{
	...
	if (MAGNIFY_OPEN == pVideoPane->GetRealtimeMagnifyState())
	{
		...
		// 恢复所有窗口的局部放大按钮
		pVideoPaneMgr->SetALLMagnifyBtnStatus(pVideoPane, 1);//lint !e747
		return;
	}
	...
	// 除本窗口外。禁用其他窗口的局部放大按钮
	pVideoPaneMgr->SetALLMagnifyBtnStatus(pVideoPane,0);//lint !e747

	// 开启画图
	(void)pCtrl->IVS_OCX_EnableDraw(pVideoPane->GetPaneID());
	// 设置局部放大状态
	(void)pVideoPane->SetRealtimeMagnifyState(MAGNIFY_OPEN);
	pVideoPaneMgr->SetMagnifyLayOut(pVideoPaneMgr->GetWindLayOut());
	// 设置按钮按下状态
...
void CTranVideoRealtimeBar::SetToolbarEnableStatus( ULONGLONG dwToolbarBtn, ULONGLONG lButtonStatus )
{
	...
	for (IVS_UINT32 _iCount = 0; _iCount < TOOLBARBTNNUM && _iCount < s; _iCount++)
	{
...
void CTranVideoRealtimeBar::OnBnClickedBtnToolRealtimeInstantBackPlay()
{
	...
	if (BOTTOMBAR_PTZ_OPEN == pVideoPane->GetRealtimePtzState())
	{
		...
		::SendMessage(this->GetSafeHwnd(), WM_COMMAND, MAKEWPARAM(IDC_BTN_TOOL_REALTIME_PTZ, BN_CLICKED), (LPARAM)this->GetPTZBtnHwnd());	//lint !e835 !e845
	}
	...
	if (TYPE_WND_TURNCTUISE == pVideoPane->GetTypeWnd())
	{
		...
		pVideoPane->SetPrveTypeWnd(TYPE_WND_TURNCTUISE);
	}
...
void CTranVideoRealtimeBar::OnBnClickedBtnToolRealtimePtz()
{
	...
	if (BOTTOMBAR_PTZ_OPEN == pVideoPane->GetRealtimePtzState())
	{
		...
		{
			pVideoPane->SetTypeWnd(TYPE_WND_TURNCTUISE);
	...
	const std::string &strButtonName)
{
	...
	eSDK_MEMSET(pcImageList,0x0,256*5);
...
void CTranVideoRealtimeBar::ChangeSoundCtrlValue( ULONG ulType, ULONG ulValue )
{
 ...
 // TODO 加类型，换句柄
	...
	{
		DWORD ulVolumeValue = (ULONG)(((float)0xFFFF / 100) * (float)ulValue);
