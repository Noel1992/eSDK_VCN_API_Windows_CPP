...
void CTranslucentButton::PreSubclassWindow()
{
	...
	if (NULL == m_ToolTip.GetSafeHwnd())
	{
		...
		eSDK_STRNCPY(logFont.lfFaceName, sizeof(logFont.lfFaceName), strFontFamily.c_str(), sizeof(logFont.lfFaceName) - 1);
...
bool CTranslucentButton::LoadImageList(const LPCTSTR szImageList[TWS_BUTTON_NUM])
{
	...
	WCHAR* wstr = NULL;
    eSDK_MEMSET(wsFile,0x0,MAX_PATH);
    ...
    {
        eSDK_MEMSET(wsFile,0x0,sizeof(wsFile));
    ...
    {
        eSDK_MEMSET(wsFile,0x0,strlen(wsFile));
