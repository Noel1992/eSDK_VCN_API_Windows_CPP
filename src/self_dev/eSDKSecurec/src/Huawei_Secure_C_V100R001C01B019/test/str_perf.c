...
#if defined(__LINUX__) || defined(__UNIX__) || defined(__GNUC__)   || defined(__CYGWIN__)  || defined(__hpux)  
...
{	\
    ...
    if(is_secure)	\
    {	\
		funcName##_s(destBuf, len, srcBuf);	\
    }	\
    else	\
    {	\
        funcName(destBuf, srcBuf);	\
...
\
		...
		printf("chunk: %5dM, avg: %6ldms  -- %6ldms %.2f\n",bufSize[j]* sizeof(bufType) / M_SIZE, avg, avg_s, (float)avg_s / avg );	\
	}	\
	free(dest_mem); \
	free(src_mem); 	\
	printf("\n");	\
...
static clock_t sprintfPerfTest(int is_secure, int loopCnt) 
{
	...
	if (is_secure) {	
		for(i = 0 ; i < loopCnt; ++i ) {	
			...
			sprintf_s(destBuf, FMT_STR_LEN + 20, "%d %s", i, srcBuf);	
...
MAKE_CHUNK_TEST_FUNC(strcpy, char*)
MAKE_CHUNK_TEST_FUNC(wcscpy, wchar_t*)

MAKE_CHUNK_TEST_FUNC(strcat, char*)
MAKE_CHUNK_TEST_FUNC(wcscat, wchar_t*)

MAKE_CHUNK_PERF_TEST_FUNC(strcpy, char)
MAKE_CHUNK_PERF_TEST_FUNC(wcscpy, wchar_t)

MAKE_CHUNK_PERF_TEST_FUNC(strcat, char)
MAKE_CHUNK_PERF_TEST_FUNC(wcscat, wchar_t)
