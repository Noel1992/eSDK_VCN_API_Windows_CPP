                                ...
                                int* pmalloc_FloatStrFlag)
{
    ...
    if (nFloatStrUsed == (*pnFloatStrSz))
    {
        ...
        {
            if (NULL == ((*pFloatStr ) = (_TCHAR*)calloc((*pnFloatStrSz), 2 * sizeof(_TCHAR))))/*lint !e586 */
            {
                return 0;
            }
            (*pmalloc_FloatStrFlag) = 1;
            (void)memcpy_s((*pFloatStr), (*pnFloatStrSz) * 2 * sizeof(_TCHAR), floatstring, (*pnFloatStrSz) * sizeof(_TCHAR));/*lint !e40 */
            (*pnFloatStrSz) *= 2;
        }
        else
        {
            ...
            tmpPointer = malloc(nextSize);/*lint !e586 */
            if (tmpPointer == NULL)
            {
                return 0;
            }
...
{/*lint !e10*/
    ...
    memset_s(&arglistsave, sizeof(arglistsave), 0, sizeof(arglistsave));
...
{
    /* coverity[returned_null] */
    return _IS_DIGIT(chr) ? chr : ((chr & ~(_T('a') - _T('A'))) - _T('A')) + 10 + _T('0');/*lint !e40 !e10 */
}
...
#endif
{
    ...
    do
    {
        ...
        if ((str->_flag & FILE_STREAM_FLAG) > 0 && str->_cnt == 0 )
        {
            ...
            if (!str->_base )
            {
                str->_base = (char*)malloc(BUFFERED_BLOK_SIZE);/*lint !e586*/
                if (!str->_base )
                {
                    ch = _TEOF;
                    break;
...
#else
    /* coverity[returned_null] */
    while (isspace((_TUCHAR)ch));/*lint !e40 !e26 !e10 */
#endif
    return ch;/*lint !e40 */
}
