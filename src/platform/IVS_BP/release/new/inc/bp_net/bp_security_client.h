...
private:
    ...
    unsigned char           m_szRnc[EXCHANGE_KEY_LENGTH + 1];     // 保存Rnc
    unsigned char           m_szRns[EXCHANGE_KEY_LENGTH + 1];     // 保存Rns
    std::string             m_strTransID;                      //事物ID
    std::string             m_strVersion;                     //秘钥交换版本号
    bool                    m_bNewVersion;                    // 是否是新版本获取公钥
    int                     m_ServerVersion;                  // 密钥交换服务端版本号
