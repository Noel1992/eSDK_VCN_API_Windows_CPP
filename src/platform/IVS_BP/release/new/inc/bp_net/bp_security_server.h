...
public:
    ...
    bool DecryptString(
	 ...
	 /*****************************************************
     * 功能: AES加密cbc模式，先用Base64解码, 再用AES解密出字符串数据
     * pszInBuff:   字符串类型的Base64编码的密文
     * pszOutBuff:  输出缓冲
     * iOutBuffLen: 输出缓冲的大小, 成功的话, 会保存'\0'
     * 返回值:       是否成功
	...
	bool EncryptCbcString(
		const std::string & strTransID, 
		const char * pszInBuff, const char* pszIvBuff,
    	char * pszOutBuff, int iOutBuffLen);

    bool DecryptCbcString(
		const std::string & strTransID, 
		const char * pszInBuff, const char* pszIvBuff,
        char * pszOutBuff, int iOutBuffLen);

    bool EncodeCbcSaltString(
		const std::string & strTransID, 
		const char * pszInBuff,
    	char * pszOutBuff, int iOutBuffLen);
	
    bool DncodeCbcSaltString(
		const std::string & strTransID, 
		const char * pszInBuff,
        char * pszOutBuff, int iOutBuffLen);
...
private:
    ...
    std::map<
             ...
             std::pair<
                       ...
                       time_t
                      >
             >                               m_mapTransID2PublicKeyCTime;  // TransID到交换密钥及其创建时间的一一映射


    ACE_Thread_Mutex                         m_aPublicKeyMapThreadMutex;   // 保存交换密钥的map对应的线程互斥量
    ...
    typedef std::map<
                     ...
                     std::pair<
                               ...
                               time_t
                               >
                    >::iterator              TransID2PublicKeyCTimeMapIterator;

    std::string m_strRnc; 
     
    std::string m_strReqTransId; 
