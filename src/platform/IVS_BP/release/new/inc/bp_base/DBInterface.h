...
public:
    ...
    *********************************************************************/
	...
	/**********************************************************************
    * name           : Init_kmc( DBExceptionCallBack pDBCallBackFunc = NULL,std::string InitFile)
    * description    : 数据库连接管理类的初始化
    * input          : pDBCallBackFunc 数据库异常回调函数
    * return         : BP_OK - 成功初始化 BP_FAIL -初始化失败
    * remark         : 业务开始时，需要调用，建议在mainctrl的init函数中调用。
    *********************************************************************/
    int Init_kmc ( std::string InitFile, DBExceptionCallBack pDBCallBackFunc = NULL);
...
private:
    ...
    BP::CBPKeyConfig m_BPDBKeyConfig;										//密钥配置文件
