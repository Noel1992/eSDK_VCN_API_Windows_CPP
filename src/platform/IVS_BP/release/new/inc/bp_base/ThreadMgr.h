...
#if defined(ARM_HIS_100)	
        pthread_mutex_t g_AllRedAppleLockMutex;    //lint !e708    定义时初始化避免使用时未初始化导致错误
    #else
...
#define MUTEX_THREAD_REACTION(TIMEOUT_SEC, RETURN)               \
	 ...
	 if (0 != pthread_mutex_lock(&g_AllRedAppleLockMutex))       \
       {                                                         \
           RETURN;                                               \
       }                                                                           
#else    
...
#define MUTEX_THREAD_RELEASE()       \
	(void)pthread_mutex_unlock(&g_AllRedAppleLockMutex);
#else
