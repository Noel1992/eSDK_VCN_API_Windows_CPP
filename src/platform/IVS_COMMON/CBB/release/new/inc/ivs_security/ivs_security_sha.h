...
const int SHA1_HEX_LENGTH = 2 * SHA_DIGEST_LENGTH + 1;

/****************************************************************
 * SHA1是一种摘要算法, OpenSSL中已经有相应实现, 这里对其进行简易封装
 * pszData  : 输入数据
 * iDataLen : 输入数据长度
 * pszDigest: 保存得到的摘要数据
 * 返回值    : 摘要数据的长度, 
 *            如果输入数据有效, 返回固定值: SHA_DIGEST_LENGTH
 *            否则为0
 ****************************************************************/
extern "C" int EXPORT_DLL IVS_Security_SHA1(
    const unsigned char * pszData, int iDataLen, 
    unsigned char * pszDigest);

/****************************************************************
 * 先进行SHA1摘要处理, 再进行十六进制转换处理成可打印的数据
 * pszData     : 输入数据
 * iDataLen    : 输入数据长度
 ...
 *               否则为0
 ****************************************************************/
extern "C" int EXPORT_DLL IVS_Security_SHA1_Hex(
    const unsigned char * pszData, int iDataLen, 
    char * pszDigest);
 ...
 *            否则为0
 ****************************************************************/
extern "C" int EXPORT_DLL IVS_Security_PBKDF2_SHA256(
    const unsigned char * pszData, int iDataLen, 
    const unsigned char *pszSalt, int iSaltLen,
    char * pszDigest);
 ...
 *               否则为0
 ****************************************************************/
extern "C" int EXPORT_DLL IVS_Security_PBKDF2_SHA256_Hex(const unsigned char * pszData, int iDataLen, 
    const unsigned char *pszSalt, int iSaltLen,
    char * pszDigestHex);
