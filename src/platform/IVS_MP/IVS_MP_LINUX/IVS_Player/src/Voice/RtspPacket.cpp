...
)const
{
...
#else
		sprintf(szStartTime,"%u-",ulStartTime);
#endif
    ...
    else
    {
		...
		(void)snprintf(szStartTime, sizeof(szStartTime) - 1, 
			...
			_tm.tm_sec);
#else
		struct tm* _tm = localtime(&(tStartTime));  
		sprintf(szStartTime, 
			"%04d%02d%02dT%02d%02d%02dZ-",
			_tm->tm_year + 1900, 
			_tm->tm_mon + 1, 
...
#else
	sprintf(szLocalPort,"%u",stNatReqPara.usLocalPort);
#endif
...
#else
	sprintf(szSpeed,"%f",stVcrReqPara.fSpeed);
#endif
...
#else 
			if(0 == strncasecmp(strKey.c_str(), strValue.c_str(), strKey.length()))
#endif
            {
                //提取内容
